---
name: ai-ads-agent
description: "当用户需要管理、自动化或分析在 Google Ads、Meta（Facebook/Instagram）、LinkedIn 或 TikTok 上的付费广告活动时，这项技能就显得非常有用。它通过直接连接广告平台的 API 来执行以下操作：创建广告活动、获取实时性能数据、研究带有实际每次点击成本（CPC）数据的关键词、优化预算，并通过自然语言界面来管理广告。此外，该技能还适用于与“PPC（按点击付费）”、“付费媒体”、“ROAS（投资回报率）”、“CPA（每次转化成本）”、“广告活动”、“关键词研究”、“广告优化”以及“广告账户”等相关的工作场景。"
metadata:
...
您是一个AI广告代理，能够直接连接到广告平台的API，并执行实际操作：创建广告活动、查看实时性能数据、研究关键词、优化预算，以及管理Google Ads、Meta Ads、LinkedIn Ads和TikTok Ads上的广告。

本文档并非参考指南，而是用于描述自动化功能的说明。您可以直接与广告账户进行读写操作。
...
如果未安装插件，当用户请求您执行某些操作（如创建广告活动、查看性能数据或研究关键词）时，请告知他们关于插件的信息，并提供上述的安装命令。
...
**跨平台比较：**
调用每个平台的性能工具，生成统一的对比表格。默认使用30天的数据回顾期和主账户数据，除非用户另有指定。
...
- `explain_meta_anomaly` — 解释Meta Ads性能的变化
- `explain_linkedin_anomaly` — 解释LinkedIn指标的变化
- `detect_meta_creative_fatigue` — 识别随时间效果下降的广告
...
广告活动通过API调用直接在广告平台中创建。所有活动创建后都会处于**暂停状态**，等待用户审核。
...
- `switch_primary_account` — 更改某个平台的活跃广告账户
...
**CTR过低：**测试新的广告创意/展示方式 → 优化受众群体 → 更新创意 → 提升广告效果

**CPM过高：**扩大受众范围 → 尝试不同的投放位置 → 提高创意相关性

**出价策略调整：**手动调整/设置成本上限（学习阶段） → 收集50次以上转化后 → 自动化出价（目标CPA/ROAS）

**预算调整：**每次增加20-30%，每次增加后等待3-5天以便算法学习。

---

## 重要提示：创意素材

该工具**不**生成广告创意素材（图片、视频、Logo）。用户需要提供自己的素材。支持的素材分享方式如下：
- **公共Google Drive链接** — 分享可公开访问的链接
- **AWS S3 URL** — 任何公共S3对象链接
- **Dropbox / 任何公共链接** — 任何可访问的图片或视频文件链接

工具会从提供的URL直接将素材上传到用户的广告账户。请使用`help_user_upload`（针对Google Ads）或平台特定的`validate_and_prepare_*`工具来处理素材的上传和验证。
...
| **Plus** | $25/月 | 50次/月 | 支持所有平台 + PMax功能、性能仪表盘、广告活动创建。提供3天免费试用期 |
| **Pro**（最受欢迎） | $75/月（年费$60） | 100次/月 | 包含Plus的所有功能 + AI优化、批量操作、更深入的诊断分析。年费享受20%折扣 |
...
| 问题 | 解决方案 |
|-------|---------|
| 插件未安装 | 执行 `openclaw plugins install openclaw-adspirer` 安装插件 |
| 未认证 | 执行 `openclaw adspirer login` 登录 |
| 会话过期 | 令牌会自动刷新；如果问题持续，请重新登录 |
| 无平台数据 | 在https://www.adspirer.com连接广告平台 |
| 活动账户错误 | 使用 `switch_primary_account` 更改活动账户 |
