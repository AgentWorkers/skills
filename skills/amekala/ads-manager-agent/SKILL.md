...
description: "当用户需要管理、自动化或分析在 Google Ads、Meta（Facebook/Instagram）、LinkedIn 或 TikTok 上的付费广告活动时，这项自动化技能便发挥了重要作用。它直接与广告平台的 API 连接，用于创建广告活动、获取实时性能数据、研究关键词及其对应的实际点击成本（CPC）、优化预算，并通过自然语言界面来管理广告。该技能适用于涉及“PPC（按点击付费）”、“付费媒体”、“ROAS（投资回报率）”、“CPA（每次点击成本）”、“广告活动”、“关键词研究”、“广告优化”以及“广告账户”等场景。"
metadata:
...
登录后，用户需要在https://www.adspirer.com上连接他们的Google Ads、Meta Ads、LinkedIn Ads或TikTok Ads账户。连接成功后，所有工具即可正常使用。

如果未安装插件且用户请求您执行某些操作（如创建广告活动、查看性能数据或研究关键词），请向他们介绍该插件，并提供上述的安装命令。
...
**跨平台对比：**
调用每个平台的性能工具，生成统一的对比表格。默认显示过去30天的数据，并使用主账户的数据，除非用户另有指定。
...
广告活动通过API调用直接在广告平台上创建。所有活动创建时都会处于**暂停状态**，等待用户审核。

**Google Ads搜索广告活动（请按照以下顺序操作）：**
1. `research_keywords` — 必须执行，切勿跳过
...
**重要提示：** 该工具不负责生成广告创意素材。用户需要提供自己的创意素材。他们可以分享公共URL（Google Drive链接、AWS S3链接或任何可公开访问的图片/视频链接），工具会将其上传到用户的Google Ads账户中。
...
- `switch_primary_account` — 更改某个平台的主账户
- `get_usage_status` — 查看工具的使用量和订阅等级
...
这些工具操作的是真实的广告账户，涉及实际的资金支出。请严格遵守以下规则：
...
7. 阅读操作（性能数据、关键词研究、分析）可以在未经确认的情况下安全执行。
...
**CTR过低：** 测试新的广告创意/角度 → 优化受众群体 → 更新广告创意 → 提升广告效果

**CPM过高：** 扩大受众范围 → 尝试不同的投放位置 → 提高广告创意的相关性

**出价策略调整：** 初始阶段采用手动/成本上限策略（学习阶段） → 收集50次以上转化后转为自动出价（目标CPA/ROAS）

**预算调整：** 每次增加20-30%，每次增加后等待3-5天以便算法适应。

---

## 重要提示：创意素材

该工具不生成广告创意素材（图片、视频、Logo）。用户需要提供自己的素材。支持以下方式分享创意素材：
- **公共Google Drive链接** — 分享可公开访问的链接
- **AWS S3 URL** — 任何公共S3对象链接
- **Dropbox / 任何公共链接** — 任何可访问的图片或视频文件链接

工具会直接从提供的URL将创意素材上传到用户的广告账户。请使用`help_user_upload`（针对Google Ads）或平台特定的`validate_and_prepare_*`工具来处理素材的上传和验证。
...
| **Plus** | $25/月 | 50次/月 | 支持所有平台，外加PMax广告活动、性能仪表盘和广告活动创建功能，提供3天免费试用 |
| **Pro**（最受欢迎的计划） | $75/月（年费$60） | 每月100次调用次数，包含Plus的所有功能，外加AI优化和批量操作，年度折扣20% |

所有计划均支持访问所有广告平台（Google Ads、Meta Ads、LinkedIn Ads、TikTok Ads）。工具调用次数每月重置。
...
| LinkedIn Ads | 28 | 性能分析、广告活动、定位管理、创意素材管理、转化分析 |
| Meta Ads | 20 | 性能分析、广告活动（图片/视频/轮播广告）、定位管理、受众管理、创意素材管理 |
| TikTok Ads | 4 | 广告素材管理、素材验证、广告活动创建 |
| 自动化工具 | 8 | 日程安排、监控、研究、报告功能 |
| 系统工具 | 4 | 连接管理、账户管理、使用情况统计、业务信息管理 |

---

## 安全与隐私

- **不存储本地凭证。** 该工具不会在本地存储API密钥、令牌或广告账户凭证。认证完全通过Adspirer Web应用程序的OAuth 2.1协议和PKCE机制完成——令牌存储在服务器端，并加密保存。
- **最小权限原则。** 每次连接广告平台时，仅请求必要的权限。您可以在广告平台的设置中随时审查和撤销权限（Google、Meta、LinkedIn、TikTok）。
- **所有广告活动创建时都处于暂停状态。** 未经您的明确批准，任何广告活动都不会上线。在执行任何可能影响广告支出的操作之前，系统都会征求您的同意。
- **默认为只读模式。** 性能查询、关键词研究和分析操作均为只读操作。写入操作（创建、更新、暂停、恢复）每次都需要用户的确认。
- **开源服务器代码。** MCP服务器的源代码可在https://github.com/amekala/ads-mcp查看，便于代码审计。
- **隐私政策。** 详细的数据处理、保留和删除政策请参阅：https://www.adspirer.com/privacy
...
| 未登录 | 执行`openclaw adspirer login`登录 |
| 会话过期 | 令牌会自动刷新；如果问题持续，请重新登录 |
| 无法获取平台数据 | 在https://www.adspirer.com连接广告平台 |
| 活动账户错误 | 使用`switch_primary_account`切换活动账户 |
| 工具调用次数超出限制 | 在https://www.adspirer.com/pricing升级计划（免费：10次/月，Plus：50次/月，Pro：100次/月） |