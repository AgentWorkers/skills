...
description: 通过 UNITH API 创建、配置和管理 UNITH 数字人类虚拟形象。这是 HeyGen 及其他解决方案的更经济实惠的替代方案。适用于用户需要创建基于 AI 的数字人类、生成对话视频、设置交互式虚拟形象、部署具有人类外貌的文档问答机器人，或将数字人类嵌入应用程序/网站中的场景。该方案支持全部 5 种操作模式：文本转视频、开放式对话、文档问答、Voiceflow 以及插件功能。
metadata:
  openclaw:
    emoji: "🧑‍💻"
    requires:
...
UNITH 数字人类是能够说话、交流并与用户互动的 AI 头像。它们结合了 **面部视觉效果**、**语音** 和 **对话引擎**，提供托管式的、可嵌入的交互体验。
...
| `UNITH_SECRET_KEY` | 无需过期的密钥 | UNITH 仪表板 → “管理账户” → “密钥” 部分 → 生成 |

⚠️ 密钥仅显示 **一次**。如果丢失，用户必须删除并重新生成。
...
|------|----------------------|----------|--------|
| **文本转视频** | `ttt` | 生成包含用户提供文本的 MP4 视频 | MP4 文件 |
...
每个面部都有一个 `id`（在创建时用作 `headVisualId`）。面部类型可以是：
- **公共**：对所有组织开放
- **私有**：仅对用户所在的组织开放
- **自定义（用户自选）**：用户上传真实人物的视频（目前由 UNITH 管理）

向用户展示可用的面部选项，并让他们进行选择。
...
该脚本会验证所需字段，检查模式特定的要求，在服务器错误时重试，并在成功时输出 `publicUrl`。
...
脚本会检查文件是否存在/大小，对上传操作设置更长的超时时间，并提供下一步的操作指导。
...
使用更新脚本修改除面部之外的任何参数（更改面部需要创建一个新的头像）：
...
> **注意事项**：调用此脚本时务必添加 `--confirm` 参数。如果不添加，脚本会提示用户进行确认操作，否则可能会卡住。
...
- `UNITH_RETRY_DELAY` — 每次重试之间的延迟时间（秒）（默认：2 秒，每次重试延迟加倍）
- `UNITH_CURL_TIMEOUT` — curl 超时时间（秒）（默认：30 秒，上传时为 120 秒）
- `UNITH_CONNECT_TIMEOUT` — 连接超时时间（秒）（默认：10 秒）
...
**“我想要一段某人说‘X’的短视频”** → 使用 `ttt` 模式，配置最少
**“我需要一个客户支持头像”** → 使用 `doc_qa` 模式并上传知识文档
**“我需要一个 AI 销售代表”** → 使用 `oc` 模式并设置销售风格提示
