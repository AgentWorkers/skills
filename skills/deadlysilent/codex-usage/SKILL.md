---
name: codex-usage
description: 用于查询 Codex 配置文件状态及使用情况的 Telegram 命令（采用斜杠分隔的格式）。当用户发送 `/codex_usage`、`/codex_usage default`、`/codex_usage all` 或 `/codex_usage <profile>` 时，或请求查询 OpenAI-Codex 的配置文件使用情况、限制信息或运行健康状况时，可使用此命令。
---
运行 `scripts/codex_usage.py` 以生成从本地认证配置文件中提取的 Codex 配置报告。

## 安全默认设置
- `/codex_usage` 是只读的。
- 对于任何配置更改操作，必须获得明确确认，并建议先使用 `--dry-run` 选项进行测试。
...
完成检查后，再发送最终的使用结果（不要跳过进度通知）。

### 用户交互界面
- 如果支持内联按钮：显示选择器按钮（默认选项：/ 所有配置文件 | 发现的配置文件）。
- 如果不支持内联按钮：显示文本菜单（选项：/default | all | <profile>）。
- 对于同一用户，禁止在 20 秒内重复发送请求，以防止意外的大量请求。

## 命令
- `/codex_usage` **必须** 首先显示选择器按钮（默认选项：/ 所有配置文件 | 发现的配置文件）。
- `/codex_usage default`
- `/codex_usage all`
- `/codex_usage <profile>`
- `/codex_usage delete <profile>`（执行更改操作前必须获得明确确认）
...
- 该脚本不允许执行 `sudo` 操作、SSH 连接或系统服务相关的配置更改。
- 网络请求仅限于受信任的 Codex 使用端点主机允许列表（当前为 `chatgpt.com`，通过 HTTPS 协议）。
- 严禁打印完整的令牌信息；请将回调/令牌数据视为敏感信息。

## 其他说明
- 该脚本默认从 `~/.openclaw/agents/main/agent/auth-profiles.json` 文件中读取 OAuth 凭据（可通过 `--auth-path` 参数进行覆盖）。
...
- 如果端点返回 `401` 错误代码，脚本会报告 “auth_not_accepted_by_usage_endpoint” 并继续显示本地配置文件的运行状态信息。
- `401` 错误通常表示端点不接受当前的 OAuth/会话令牌格式（而非 Codex CLI 未安装）。
- 脚本会自动添加 Codex 使用端点所需的请求头信息：`Authorization`、`ChatGPT-Account-Id`（如果存在）以及 `User-Agent: CodexBar`。
- 脚本会报告本地配置文件的运行状态（有效期、最后一次使用时间、错误/速率限制统计信息），以及远程端点的使用情况（每周 5 小时内的使用次数、剩余使用时间）。
- 报告格式符合用户友好原则（如 `reset_in`、`reset_at` 等字段，显示时间信息时使用本地时区）。
- 支持重试、超时机制以及用于诊断的调试信息（如尝试次数、耗时、状态等）。
- 报告内容包含以下字段，以简化命令行输出的格式：
  - `Profile: %name%`
  - `Usable: ✅/❌ | Limited: ✅/❌`
