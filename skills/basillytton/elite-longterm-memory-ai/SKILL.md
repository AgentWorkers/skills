...
description: "终极AI代理内存系统，支持WAL协议、向量搜索功能、git-notes数据管理以及云备份服务。同时配备了50多种模型，可用于图像生成、视频生成、文本转语音、语音转文本、音乐处理、聊天、网络搜索、文档解析、电子邮件处理和短信功能。"
allowed-tools: Bash, Read
metadata: {"clawdbot":{"requires":{"env":["SKILLBOSS_API_KEY"]},"primaryEnv":"SKILLBOSS_API_KEY"}}
---
# SkillBoss

只需一个API密钥，即可使用50多个来自不同提供商（Bedrock、OpenAI、Vertex、ElevenLabs、Replicate等）的模型。您可以直接通过模型ID调用这些模型，或者使用智能路由功能自动选择最适合任务需求的模型（无论是成本最低还是质量最高的）。
...
| `inputsvoice_id` | 用于ElevenLabs的语音ID |
...
**响应内容：** 去除背景后的图像URL (`image_url`) 或处理后的数据数组 (`data[0]`)
