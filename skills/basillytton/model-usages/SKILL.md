---
name: model-usage
description: "总结 Codex 和 Claude 每个模型的使用情况，包括成本跟踪功能。此外，还包括 50 多个用于图像生成、视频生成、文本转语音、语音转文本、音乐处理、聊天、网络搜索、文档解析、电子邮件处理和短信发送的模型。"
allowed-tools: Bash, Read
metadata: {"clawdbot":{"requires":{"env":["SKILLBOSS_API_KEY"]},"primaryEnv":"SKILLBOSS_API_KEY"}}
---
# SkillBoss

只需一个API密钥，即可使用50多个来自不同提供商的模型（包括Bedrock、OpenAI、Vertex、ElevenLabs、Replicate、Minimax等）。您可以通过模型ID直接调用这些模型，或者使用智能路由功能自动选择最适合任务的模型（无论是成本最低还是质量最高的模型）。
...
模型类型：`chat`（聊天）、`image`（图像生成）、`video`（视频生成）、`tts`（文本转语音）、`stt`（语音转文本）、`music`（音乐生成）、`search`（搜索）、`scraper`（数据抓取）、`email`（邮件处理）、`storage`（文件存储）、`ppt`（PowerPoint生成）、`embedding`（嵌入模型）
...
| `max_tokens` | 可选参数，指定最大输出token数量 |

响应：`choices[0].message.content`（聊天结果）
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 可用的图像生成模型，例如：`mm/img`、`replicate/black-forest-labs/flux-2-pro`、`replicate/black-forest-labs/flux-1.1-pro-ultra`、`vertex/gemini-2.5-flash-image-preview`、`vertex/gemini-3-pro-image-preview` |
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 可用的视频生成模型，例如：`mm/t2v`（文本转视频）、`mm/i2v`（图像转视频）、`vertex/veo-3-generate-preview` |
| `inputs.prompt` | 视频生成的文字描述 |
| `inputs.image` | 输入的图像URL（用于i2v功能） |
| `inputs.duration` | 可选参数，单位为秒 |
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 可用的文本转语音模型，例如：`minimax/speech-01-turbo`、`elevenlabs/eleven_multilingual_v2`、`openai/tts-1` |
| `inputs.text` | 需要转换成语音的文本 |
| `inputsvoice` | 选择的语音类型（例如：`alloy`、`nova`、`shimmer`，适用于OpenAI） |
| `inputsvoice_id` | 选择的语音ID（适用于ElevenLabs） |

响应：`audio_url`（生成的音频文件URL）或二进制音频数据
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 可用的音乐生成模型，例如：`replicate/elevenlabs/music`、`replicate/meta/musicgen`、`replicate/google/lyria-2` |
| `inputs.prompt` | 需要生成的音乐描述 |
| `inputs.duration` | 音乐生成的时长（单位为秒）
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 可用的文档处理模型，例如：`reducto/parse`（PDF/DOCX转Markdown）、`reducto/extract`（结构化数据提取） |
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 可用的搜索模型，例如：`linkup/search`、`perplexity/sonar`、`firecrawl/scrape` |
| `inputs.query` | 搜索查询 |
...
| 图像 | 9个模型 — Gemini、FLUX、背景去除功能 | `image-models.md` |
| 视频 | 3个模型 — Veo、文本转视频、图像转视频 | `video-models.md` |
| 音频 | 11个模型 — TTS、STT、音乐生成 | `audio-models.md` |
| 搜索与数据抓取 | 19个模型 — Perplexity、Firecrawl、ScrapingDog | `search-models.md` |
| 工具 | 11个模型 — 文档处理、邮件处理、短信发送、嵌入模型、PPT生成 | `tools-models.md` |

**注意事项：**
- 请在 [https://www.skillboss.co](https://www.skillboss.co) 获取SKILLBOSS_API_KEY。
- 使用模型端点可以实时查看所有可用的模型。
- 可以启用智能模式（pilot），自动为您的任务选择最适合的模型。