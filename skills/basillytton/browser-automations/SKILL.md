...
description: "通过 CLI 命令，使用自然语言自动化网页浏览器的交互操作。同时提供 50 多种模型，用于图像生成、视频生成、文本转语音、语音转文本、音乐处理、聊天、网络搜索、文档解析、电子邮件发送以及短信发送等功能。"
allowed-tools: Bash, Read
metadata: {"clawdbot":{"requires":{"env":["SKILLBOSS_API_KEY"]},"primaryEnv":"SKILLBOSS_API_KEY"}}
---
# SkillBoss

只需一个API密钥，即可使用50多个来自不同提供商的模型（包括Bedrock、OpenAI、Vertex、ElevenLabs、Replicate、Minimax等）。您可以直接通过模型ID调用任意模型，或者使用智能路由功能自动选择最适合任务需求的模型（无论是成本最低还是质量最高的模型）。
...
模型类型：`chat`（聊天）、`image`（图像生成）、`video`（视频生成）、`tts`（文本转语音）、`stt`（语音转文本）、`music`（音乐生成）、`search`（搜索）、`scraper`（数据抓取）、`email`（电子邮件处理）、`storage`（文件存储）、`ppt`（PowerPoint生成）、`embedding`（嵌入模型）
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 可用的聊天模型，例如：`bedrock/claude-4-5-sonnet`、`bedrock/claude-4-6-opus`、`openai/gpt-5`、`vertex/gemini-2.5-flash`、`deepseek/deepseek-chat` |
| `messages` | 包含`role`和`content`的对象数组 |
| `system` | 可选的系统提示语 |
| `temperature` | 可选参数，范围为0.0–1.0 |
| `max_tokens` | 可选参数，指定最大输出字符数 |

**响应内容：** `choices[0].message.content`（返回的聊天内容）
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 用于图像生成的模型，例如：`mm/img`、`replicate/black-forest-labs/flux-2-pro`、`replicate/black-forest-labs/flux-1.1-pro-ultra`、`vertex/gemini-2.5-flash-image-preview`、`vertex/gemini-3-pro-image-preview` |
| `inputs.prompt` | 图像的文字描述 |
| `inputs.size` | 可选参数，例如：“1024*768” |
| `inputs.aspect_ratio` | 可选参数，例如：“16:9” |

**响应内容：** `image_url`（图像URL）、`data[0]`（图像数据）或`generated_images[0]`（生成的图像数组）
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 用于视频生成的模型，例如：`mm/t2v`（文本转视频）、`mm/i2v`（图像转视频）、`vertex/veo-3-generate-preview` |
| `inputs.prompt` | 视频生成的文字描述 |
| `inputs.image` | 输入的图像URL（仅适用于i2v功能） |
| `inputs.duration` | 可选参数，以秒为单位指定视频时长 |

**响应内容：** `video_url`（生成的视频URL）
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 用于文本转语音的模型，例如：`minimax/speech-01-turbo`、`elevenlabs/eleven_multilingual_v2`、`openai/tts-1` |
| `inputs.text` | 需要转换成语音的文本 |
| `inputsvoice` | 选择的语音类型（例如：`alloy`、`nova`、`shimmer`，适用于OpenAI） |
| `inputsvoice_id` | 选择的语音ID（适用于ElevenLabs） |

**响应内容：** `audio_url`（音频文件URL）或二进制音频数据
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 用于音乐生成的模型，例如：`replicate/elevenlabs/music`、`replicate/meta/musicgen`、`replicate/google/lyria-2` |
| `inputs.prompt` | 需要生成的音乐描述 |
| `inputs.duration` | 音乐生成的时长（以秒为单位） |

**响应内容：** `audio_url`（生成的音频文件URL）
...
**响应内容：** 去除图像背景后的图像URL或处理后的图像数据（`data[0]`）
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 用于处理文档的模型，例如：`reducto/parse`（将PDF/DOCX格式转换为Markdown格式）、`reducto/extract`（提取文档结构化数据） |
...
| 参数 | 描述 |
|-----------|-------------|
| `model` | 用于网页搜索的模型，例如：`linkup/search`、`perplexity/sonar`、`firecrawl/scrape` |
| `inputs.query` | 搜索查询内容 |
...
| 图像 | 9个模型 — Gemini、FLUX、图像质量提升、背景去除 | `image-models.md` |
| 视频 | 3个模型 — Veo、文本转视频、图像转视频 | `video-models.md` |
| 音频 | 11个模型 — TTS、语音转文本、音乐生成 | `audio-models.md` |
| 搜索与数据抓取 | 19个模型 — Perplexity、Firecrawl、ScrapingDog、CEO访谈数据 | `search-models.md` |
| 工具 | 11个模型 — 文档处理、电子邮件处理、短信发送、嵌入模型、演示文稿生成 | `tools-models.md` |

**注意事项：**
- 请在 [https://www.skillboss.co] 获取SKILLBOSS_API_KEY。
- 使用模型端点可以实时查看所有可用的模型。
- 可以启用智能模式（pilot），自动为任务选择最佳模型。