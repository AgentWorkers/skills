...
> **注意**：请参阅 [Coinbase AWAL 文档](https://docs.cdp.coinbase.com/agentic-wallet/welcome) 以获取完整的设置说明。您需要使用 USDC 为您的 AWAL 钱包进行身份验证和充值。

配置完成后，所有 Base 网络的支付脚本将自动使用 Coinbase Agentic 钱包，而不再使用私钥。
...
- **测试时**：请使用临时钱包，并确保其中包含足够的 USDC（1-5 美元即可）。
- **API 密钥**：创建 API 端点后，请妥善保管返回的 API 密钥。
- **代码审核**：所有脚本均可在 `scripts/` 目录中找到并接受审核。
...
1. **获取 API 密钥**：运行 `create_endpoint.py` 时会产生该密钥。
2. **验证请求头**：您的服务器必须检查每个请求中是否包含以下头部信息：
   ```http
...
2. **设置价格**：每次调用 0.01 美元。
3. **用户调用 1,000 次 API**：您将获得 10 美元的收益，同时消耗 1,000 信用额度。
4. **剩余信用额度**：19,000 信用额度 + 10 美元的收益。
5. **信用额度不足？** 使用 `topup_endpoint.py` 进行充值。

### 信用额度耗尽时会发生什么？

您的 API 端点将 **停止服务** 并返回错误。用户需要重新充值才能继续使用。
...
- **收益**：用户的支付会直接进入您的钱包，然后从信用额度中扣除 1 信用额度。
...
您的 API 端点每次请求会消耗 1 信用额度。当信用额度耗尽时，服务将停止运行。请使用 `topup_endpoint.py` 进行充值。
...
- 使用 `VersionedTransaction` 和 `MessageV0` 协议：
  - 中间机构负责支付 gas 费用（来自 `extra.feePayer`）。
...
| `AWAL_PACKAGE` | AWAL 模式（可选） | AWAL CLI 的 NPM 包版本（默认：`awal@1.0.0`） |
| `AWAL_BIN` | AWAL 模式（可选） | 已预安装的 AWAL 可执行文件路径/名称 |
...
⚠️ **Solana 网络的支付成功率约为 75%，这可能受到中间机构费用支付机制的影响。`pay_solana.py` 中包含重试逻辑。**Base（EVM）网络的支付方式 100% 可靠**，建议在生产环境中使用。