...
description: 通过智能模型路由、心跳信号优化、预算跟踪以及多供应商备选方案来减少 OpenClaw 的令牌使用量和 API 成本。当令牌成本较高、API 速率限制被触发，或需要大规模托管多个代理时，建议使用这些方法。该方案提供现成的脚本用于任务分类、使用情况监控以及优化心跳信号调度。所有操作仅涉及本地文件分析，无需进行网络请求或代码执行。详情请参阅 SECURITY.md。
---
# 令牌优化器
...
**新增功能：** 强制使用经济型模型来处理简单的通信内容（如“hi”或“thanks”等）！
...
**模型使用建议：** 心跳请求应始终使用经济型模型（Quick）——请参考更新后的 `HEARTBEAT.template.md` 以获取模型覆盖说明。

### 4. 定时任务优化（新增功能！）

**问题：** 即使是常规任务，定时任务也经常默认使用高级型模型（Standard/Deep）。

**解决方案：** 90% 的定时任务应指定使用经济型模型。

**参考文档：** `assets/cronjob-model-guide.md`，其中包含详细指南和示例。

**快速参考：**

| 任务类型 | 模型层级 | 示例 |
|-----------|------------|---------|
| 监控/警报 | Quick | 检查服务器健康状况、磁盘空间 |
| 数据解析 | Quick | 解析 CSV/JSON/日志 |
| 提醒 | Quick | 每日站会、备份提醒 |
| 简单报告 | Quick | 状态摘要 |
| 内容生成 | Standard | 博文摘要（质量要求较高） |
| 高级分析 | Standard | 周度洞察 |
| 复杂推理 | 避免使用高级型模型 | 仅在绝对必要时使用 |
...
**节省效果：** 对于 10 个每日定时任务，使用经济型模型代替高级型模型可每月为每个代理节省 $17.70**。
...
监控使用情况，并在接近预算限制时发出警报。现在支持来自提供者的 **实时定价**。
...
**动态定价：**
- 定价数据存储在技能目录下的 `pricing.json` 文件中。
- 运行 `token_tracker.py refresh-pricing` 以从 OpenRouter 获取最新价格（需要 `OPENROUTER_API_KEY`）。
- `suggest` 命令使用加载的定价数据进行准确的成本比较。
- 如果 `pricing.json` 丢失，将使用内置的默认值（可能已过时）。
...
| Anthropic | Sonnet 4.5 | $3.00 | 平衡型默认模型 |
| Anthropic | Opus 4 | $15.00 | 高级推理模型 |
| OpenRouter | Gemini 2.5 Flash | $0.075 | 大量操作 |
...
2. 实施灵活的路由策略（80% 使用 Gemini 模型，15% 使用 Quick 模型，5% 使用 Standard 模型）
...
- **解决方法：** 确保已安装 Python 3.7 及更高版本。脚本仅使用标准库。
...
**问题：** 路由建议的模型层级错误**
- **解决方法：** 在 `model.router.py` 中自定义 `ROUTING_RULES` 以适应你的使用模式。
...
- 全部使用标准型模型（如 Sonnet）：100,000 × $3/令牌 = 每天 $0.30 = 每月 $9

| 方案 | 上下文使用 | 模型层级 | 日成本 | 每月成本 | 节省成本 |
|----------|---------|------------|-----------|---------|---------|
| 基线（无优化） | 50,000 | Standard | $0.30 | $9.00 | 0% |
| 仅优化上下文 | 10,000 | Standard | $0.18 | $5.40 | 40% |
| 仅优化模型路由 | 50,000 | 混合使用 | $0.18 | $5.40 | 40% |
| **同时使用这两种方案** | 10,000 | 混合使用 | $0.09 | $2.70 | 70% |
| 使用高级模型 + Gemini | 10,000 | Quick（Gemini） | $0.03 | $0.90 | 90% |

**注意：** 实际成本取决于 `config.py` 中配置的模型。对于当前设置，Gemini Flash 模型是免费的；Minimax 模型的价格为每百万令牌约 $0.60。

**关键见解：** 仅优化上下文使用即可节省的令牌数量就超过了优化模型路由带来的节省！

**xCloud 托管场景**（100 位客户，每位客户每天 50,000 个令牌）：
- 基线方案（全部使用标准型模型）：每月 $450
- 使用令牌优化器后：每月 $135
...
### 脚本（共 4 个）
- **`context_optimizer.py`** — 上下文加载优化和延迟加载（新增功能！）
- **`model.router.py`** — 任务分类、模型推荐和通信策略管理（升级功能！）
- **`heartbeat_optimizer.py`** — 间隔管理和检查调度
- **`token_tracker.py** — 预算监控和警报

### 参考资料
- `PROVIDERS.md` — 替代 AI 提供者、定价和路由策略

### 资源文件（共 3 个）
- **`HEARTBEAT.template.md`** — 带有经济型模型强制使用的优化心跳模板（升级功能！）
- **`cronjob-model-guide.md** — 定时任务中模型选择的完整指南（新增功能！）
