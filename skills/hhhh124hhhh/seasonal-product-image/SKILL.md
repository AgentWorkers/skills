...
该模板提供了一套高质量、由人工精心设计的提示语，适用于人工智能（AI）生成的电子商务图片。  

**注意**: 该提示语经过人工设计和测试，专为电子商务和内容创作的实际应用场景进行了优化。  

## 使用方法  

### 基本用法  

将此提示语模板与您的 AI 视频/图片生成工具结合使用：  
```text
...
请将提示语中的以下变量替换为您的具体值：  
- `{product_name}`：产品名称  
- `{brand_colors}`：品牌配色方案（例如：“蓝色、白色、灰色”）  
- `{background_style}`：首选背景风格（极简风格、渐变效果、特定场景）  
- `{target_audience}`：目标受众（例如：“25-35岁的年轻人”）  
- `{cta_text}`：行动号召语（例如：“立即购买 - 享受8折优惠”）  
- `{setting}`：场景设置（家庭、办公室、户外、咖啡馆）  
- `{feature}`：需要突出的产品特性  
- `{season}`：季节（春季、夏季、秋季、冬季）  
- `{holiday_colors}`：节日配色方案  
- `{competitor_product}`：竞争对手产品名称（用于对比）  
...
电子商务（E-commerce）、图片（Image）、季节性（Seasonal）、节日（Holiday）  

## 使用要求  

该模板兼容以下支持文本转视频或文本转图片功能的 AI 工具：  
- 视频生成工具：Sora2、Google Veo、Runway、Pika Labs、Kling  
- 图片生成工具：Midjourney、DALL-E 3、Stable Diffusion、Stable Diffusion XL  

## 使用建议：  
1. **自定义变量**：将所有 `{variable}` 占位符替换为您的实际值。  
2. **多次测试与优化**：生成多个版本并选择最佳结果。  
3. **调整参数**：根据需要调整分辨率、时长或风格。  
4. **组合使用**：结合其他模板以创建更丰富的内容。  
...
## 质量保证：  
- 该模板经过人工设计和策划，专为电子商务场景优化。  
- 已在多种 AI 视频/图片生成工具上进行测试。  
- 内容清晰且效果显著。  

## 意见反馈：  

如果您觉得该模板有用，请留下评论或分享您使用后的生成结果！  

*该模板属于“人工提示语集合”系列，专为 AI 内容生成提供高质量、精心策划的提示语。*