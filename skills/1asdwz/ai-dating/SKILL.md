---
name: ai-dating
description: "该技能支持约会和匹配工作流程。当用户请求交友、寻找伴侣、进行匹配服务或更新个人资料/偏好设置时，可使用该技能。该技能会执行 `dating-cli` 命令来完成个人资料设置、任务创建/更新、匹配结果检查以及联系信息的展示和审核等操作。"
license: MIT
metadata:
  author: 1asdwz
  version: "1.1.5"
...
该功能通过 `dating-cli` 支持约会和匹配工作流程。它帮助用户创建个人资料、定义伴侣偏好、查看匹配结果、显示联系方式以及提交聊天后的评价。
...
3. 用户描述伴侣偏好（例如性别、身高、收入、城市、性格、爱好等），并请求系统进行匹配。
...
5. 解析用户的伴侣偏好并创建匹配任务（完整参数示例）。用户无需填写所有字段，只需提供他们已有的信息即可。
```bash
...
```
如果在 `task create` 中省略了 `--*-embedding-min-score`，后端会将其默认设置为 `0.1`。

6. 如果存在未完成的 `taskId` 且用户没有明确请求创建新任务，则更新现有任务（完整参数示例）。
```bash
...
8. 执行 `check` 命令以查看匹配结果（完整参数示例，支持分页）。  
每页返回 10 位候选者。需要时可以使用 `--page` 参数获取后续页面。
`check` 命令返回的候选者信息中包含 `photoUrls`（用户上传的图片 URL 数组），这些信息可用于展示和选择候选者。
如果返回结果为 `NO_RESULT_RETRY_now`，则根据需要再次调用 `check` 命令。
如果返回结果为 `MATCH_FOUND`，则继续进行联系方式的展示。
建议用户通过 `dating-cli profile update --email ...` 命令注册电子邮件地址，以便接收定期的匹配推荐通知。

9. 从匹配结果中选择最佳候选者并显示其联系方式（完整参数示例）。
```bash
