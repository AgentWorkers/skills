...
description: >
  **Universal Memory Management System v1.8.2** 专为 OpenClaw 代理设计，提供以下功能：  
  ...
  4. **新功能 v1.8.2**：循环保护机制及用户优先级设置，能够自动检测并处理陷入死循环的任务  
  5. **基于树的分类结构**（三级层次结构）  
  ...
  10. **事件历史记录与时间线功能**  
  **适用场景**：当代理需要具备智能内存管理功能并支持状态监控时。  
  **目标**：  
  - 将令牌消耗量降低 70-90%  
  - 提高搜索结果的关联性（相关度）20%  
  - 增强系统的上下文感知能力（contextual awareness）。  
  **v1.8.2 版本新增的循环保护机制**：通过心跳信号限制、超时检测及自动恢复功能，有效防止任务陷入无限循环。
---
# Universal Memory Management v1.8.2

## 🌳 基于树的分类结构

**核心创新**：采用三层层次化分类机制，并支持自动关键词匹配功能。
...
|--------|----------|----------|
| **树搜索** | 精确匹配 | 知道确切的类别时使用 |
| **选择规则** | 几何邻居算法 | 当 C_ab = 1 时使用 |
| **语义搜索（v1.5）** | TF-IDF + 上下文分析 | 实现智能推理 |
...
代理状态系统为智能的心跳检测提供了上下文感知功能：

| 状态 | 描述 | 心跳检测行为 |
|-------|-------------|-------------------|
| **IDLE** | 代理处于空闲状态 | 进行全面检查（@提及、回复、投票） |
| **DOING** | 代理正在处理任务 | 仅进行关键检查（@提及、回复，不参与投票） |
| **WAITING** | 代理正在等待条件满足 | 进行快速检查（仅检查 @提及） |
...
v1.8.2 版本引入了全面的保护机制，防止任务陷入无限循环或系统资源耗尽：
...
| 情况 | 处理方式 | 触发条件 |
|-----------|--------|---------|
| **关键任务停滞** | 将系统降级为高级模式 | 30 分钟无进展 |
| **关键任务超时** | 将系统降级为高级模式 | 超过 30 分钟无进展 |
| **高级任务停滞** | 将系统降级为普通模式 | 15 分钟无进展 |
| **高级任务截止时间超过两倍** | 自动锁定系统 | 超过 90 分钟无进展 |
| **普通任务截止时间超过两倍** | 自动锁定系统 | 超过 120 分钟无进展 |

#### 带有循环保护的 heartbeat 输出机制
...
v1.8.2 版本引入了 **用户优先级窗口**，确保系统的心跳检测不会干扰用户的活跃对话：

### 工作原理

1. **检测**：记录用户最后一次交互的时间戳。
2. **优先级窗口**：定义一个优先窗口（默认为 30 分钟），在此期间用户的操作具有最高优先级。
3. **跳过检测**：如果系统心跳检测发生在优先窗口内，则自动跳过。
4. **安全阀**：允许最多跳过指定次数的检测（默认为 3 次），之后强制进行检测以确保系统正常运行。
...
系统支持对敏感数据（API 密钥、密码、令牌）进行工业级加密：
