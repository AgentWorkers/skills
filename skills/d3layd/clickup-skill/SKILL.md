...
2. 高级分析与报告功能（任务数量统计、任务分配者明细、状态/优先级分析、每日站会报告），支持子任务的自动包含和分页显示；  
3. 客户文件夹组织功能，附带项目跟踪功能；  
4. 对工作区、文件夹、任务及自定义字段提供完整的 CRUD（创建、读取、更新、删除）操作；  
5. 时间跟踪与计时器管理功能，支持账单生成；  
6. 文档创建与页面管理（基于 API v3）；  
7. 任务依赖关系管理、任务链接及关联关系映射；  
8. 销售流程跟踪功能，可查看潜在客户/项目状态；  
9. 客户续费与定期账单管理功能。  
专为需要在复杂工作区层级中管理多个客户的机构设计。"
env:
...
**专为代理机构工作流程设计的企业级ClickUp集成解决方案。**支持管理多个客户、项目和工作空间，具备高级报告功能、自动子任务处理以及完善的文件夹组织能力。
...
| **🏢 多工作空间** | 可在Elevated、Atrium、Winch Life等多个工作空间之间无缝切换 |
| **👥 客户管理** | 结构化的文件夹：📋 客户概览、📁 已完成的工作、正在进行的项目 |
| **📈 销售流程** | 跟踪提案、谈判和项目生命周期 |
...
该功能支持在多个ClickUp工作空间之间进行操作：

1. 使用`get_teams`列出可用的工作空间
2. 在操作中通过`team_id`引用工作空间
3. 每个工作空间都有独立的空间、文件夹和列表
...
3. **父任务与子任务** — 父任务的`parent`属性为`null`，子任务的`parent`属性为`task_id` |
4. **速率限制** — 每分钟100次请求；我们的分页功能会遵守此限制
...
#### 任务操作
- `get_task task_id="xxx"` - 获取任务详情（包括依赖关系、关联任务）
...
- `add_dependency task_id="xxx" waiting_on="yyy"` - 另一个任务被此任务阻塞/等待
...
- `task_counts team_id="xxx" [filters...]` - 统计明细：总数、父任务、子任务、未分配任务
- `assignee_breakdown team_id="xxx" [filters...] - 按分配者分配的工作量
...
| `429 Too Many Requests` | 超过请求限制 | 等待片刻后重试（每分钟100次请求限制） |
...
1. **存储ID**：工作空间/空间/文件夹/列表ID很少更改。将它们存储在`TOOLS.md`中以便快速参考。
2. **自定义任务ID**：如果使用自定义ID，请在任务操作中始终设置`custom_task_ids=true`和`team_id`。
3. **控制请求频率**：分散批量操作以避免429错误。
4. **时间跟踪**：所有持续时间/时间戳值应使用毫秒为单位。
