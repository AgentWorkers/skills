---
name: clickup
description: "企业级 ClickUp 项目管理系统，具备高级报告功能、多工作区支持以及客户/项目跟踪能力。核心功能包括：  
(1) 多工作区任务管理，支持自动切换工作区；  
(2) 高级分析与报告功能（任务数量统计、任务分配情况、状态/优先级分析、每日站会报告），支持子任务的自动包含和分页显示；  
(3) 客户文件夹组织功能，附带项目跟踪功能；  
(4) 对工作区、文件夹、任务及自定义字段提供完整的 CRUD（创建、读取、更新、删除）操作；  
(5) 时间跟踪与计时器管理功能，支持计费；  
(6) 文档创建与页面管理（基于 API v3）；  
(7) 任务依赖关系管理、任务链接及关联关系映射；  
(8) 销售流程跟踪功能，包括潜在客户/项目状态管理；  
(9) 客户续费与定期计费管理。  
专为需要管理多个客户、且工作区层级复杂的机构设计。"
env:
...
**专为代理工作流程设计的企业级ClickUp集成工具。** 支持管理多个客户、项目和 workspace，具备高级报告功能、自动处理子任务以及完善的文件夹组织能力。
...
3. **父任务与子任务** — 父任务的`parent`字段为`null`，子任务的`parent`字段为`task_id` |
...
#### 列表操作
- `get_lists folder_id="xxx"` - 列出文件夹中的列表
...
- `task_counts team_id="xxx" [filters...]` - 任务统计：总数、父任务、子任务、未分配任务
- `assignee_breakdown team_id="xxx" [filters...] - 按分配者划分的工作量
- `status_breakdown team_id="xxx" [filters...] - 按状态划分的任务
- `priority_breakdown team_id="xxx" [filters...] - 按优先级划分的任务
...
| `429 Too Many Requests` | 超过速率限制 | 等待片刻后重试（每分钟100次请求限制） |
