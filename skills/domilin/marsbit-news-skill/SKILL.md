...
description: 通过 `marsbit-co` 中的托管 MCP 路由获取 MarsBit 的新闻和动态数据。该服务支持最新新闻的查询、频道查找、关键词搜索、详细信息查询以及相关新闻和动态的更新。
...
当用户请求 MarsBit 新闻或即时信息时，需通过 `curl` 直接调用 MCP 工具。
...
- `text` 是一个 JSON 字符串，在回答用户之前需要对其进行解析；
- 如果 `success` 的值为 `false`，则显示错误信息，并询问用户是否希望使用不同的参数重新尝试。
...
- 获取最新的即时信息 -> `get_latest_flash`
- 按关键词搜索即时信息 -> `search_flash`

## 后端架构

该技能依赖于当前的 `marsbit-co` 托管 MCP 实现（`/api/mcp`），其内部使用了以下组件：
- `src/lib/utils.ts` 中的 `fetcher(..., { marsBit: true })`
- 新闻 API：`/info/news/channels`, `/info/news/shownews`, `/info/news/getbyid`, `/info/news/v2/relatednews`
- 即时信息 API：`/info/lives/showlives`
- 搜索 API：`/info/assist/querySimilarityInfo`（通过 `src/lib/db-marsbit/agent`）
