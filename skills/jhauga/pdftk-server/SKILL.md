...
当需要从命令行执行以下操作时，可以使用 pdftk：合并 PDF 文件、分割 PDF 文件、旋转页面、加密/解密 PDF 文件、填写 PDF 表单、添加水印、叠加印章、提取元数据、将文档拆分为多个页面、修复损坏的 PDF 文件、附加或提取文件，以及进行其他 PDF 操作。'
---
# PDFtk Server
...
| 错误 | 解决方案 |
|-------|----------|
| 未找到 `pdftk` 命令 | 验证安装是否成功；检查 `pdftk` 是否在系统的 PATH 环境变量中 |
