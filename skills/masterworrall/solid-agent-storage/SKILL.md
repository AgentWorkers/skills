...
description: 使用 Solid 协议为您的 AI 代理分配持久性身份（WebID）和个人数据存储空间（Pod）。
version: 0.3.7
author: Interition
license: Apache-2.0
metadata:
...
- 当您需要在多次对话中记住某些信息（如笔记、偏好设置或学习到的内容）时
...
在使用任何命令之前，请先设置 `INTERITION_PASSPHRASE` 环境变量。该密码用于加密存储在您设备上的 Solid 服务器凭据（位于 `~/.interition/agents/` 目录下），从而保护这些凭据的安全性。请使用强密码并严格保密。

### 运行自己的 Solid 服务器（推荐）

为了获得完全的控制权，您可以运行自己开发的 [Community Solid Server](https://github.com/CommunitySolidServer/CommunitySolidServer)（CSS）。CSS 是由 [Open Data Institute](https://www.theodi.org/) 维护的开源软件。

请将 `SOLID_SERVER_URL` 设置为您的服务器地址：

```bash
export SOLID_SERVER_URL="http://localhost:3000"
```

有关更完善的 Docker 配置示例，请参阅 [源代码仓库](https://github.com/masterworrall/agent-interition)。

### 使用共享的 Solid 服务器（用于多代理协作）

如果您的代理需要通过 Solid 协议与其他代理进行协作，它们必须使用同一个 Solid 服务器。目前有两个公开的 Solid 服务器可供使用：
- **https://solidcommunity.net** — 由 [Open Data Institute](https://www.theodi.org/) 运营
- **https://crawlout.io** — 由提供本技能的 [Interition](https://interition.net) 运营
...
如果未设置 `SOLID_SERVER_URL`，则技能会默认使用 `https://crawlout.io` 作为服务器地址。
...
3. 代理使用这些凭据获取用于读写数据的 **限时令牌**。
4. CSS 会对每个请求实施访问控制（WAC），只有经过授权的代理才能访问受保护的资源。

`INTERITION_PASSPHRASE` 并非服务器的直接凭据。它用于初始化存储在您设备上的服务器生成凭据的加密过程（使用 AES-256-GCM 算法），并赋予这些凭据 `0600` 权限，从而提高其在 OpenClaw 环境中的安全性。服务器本身永远不会看到您的密码。
...
有关使用 Solid 服务器及整个 Solid 生态系统的更多信息，请访问 [solidproject.org](https://solidproject.org)。完整的协议规范请参阅 [Solid Protocol (W3C)](https://solidproject.org/TR/protocol)。
...
- `status: "ok"` — 代理的所有配置已完全删除，本地文件也已清除
- `status: "partial"` — 本地文件已删除，但 CSS 清理失败（会显示相应的警告）

如果代理是在添加电子邮件/密码存储功能之前被配置的，CSS 将跳过清理操作，并会显示相应的警告信息。
...
| 目录路径 | 用途 |
|------|---------|
| `/{name}/memory/` | 代理的私有内存空间（包含笔记、学习内容、偏好设置） |
...
所有管理命令的输出均为 JSON 格式。发生错误时，标准错误输出（stderr）会显示相应的错误信息：
```json
