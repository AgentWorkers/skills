...
description: 使用 Solid 协议为你的 AI 代理分配一个永久性的身份标识（WebID）以及个人数据存储空间（Pod）。
version: 0.2.0
author: Interition
license: Apache-2.0
...
该技能为您提供了一个名为 **Solid Pod** 的个人数据存储空间，该空间拥有一个 **WebID**（您在网络上的身份标识）。您可以在此存储数据、读取数据，并与其他代理共享特定资源。

## 何时使用此技能

- 当您需要在对话中记住某些信息（如笔记、偏好设置或学到的知识点）时
- 当您需要存储结构化数据（采用 RDF/Turtle 格式）时
...
2. `SOLID_SERVER_URL` 已设置为服务器的地址（默认值：`http://localhost:3000`）。**请确保指向您能够控制且信任的服务器**——该技能会与服务器交换认证信息。
3. `INTERITION_PASSPHRASE` 已设置（用于加密存储的认证信息）。请使用强密码并妥善保管。

## 工作原理

该技能提供了三个用于 CSS 特定操作的管理脚本（配置、解除配置、状态检查），以及一个用于身份验证的辅助工具（token helper）。所有标准的 Solid 操作（读取、写入、删除、共享）均通过 `curl` 和 **Bearer token** 来完成——您的 Solid Pod 实际上就是一个符合 W3C Solid 标准的服务器。
...
彻底删除代理的 WebID 和 Solid Pod：从 CSS 服务器中删除其相关数据（包括 Pod、客户端认证信息、WebID 链接及密码登录信息），同时也会删除本地的认证文件。
...
- `status: "ok"` — 代理的身份和存储空间已完全清除，本地文件也被删除
- `status: "partial"` — 本地文件被删除，但 CSS 服务器的清理操作失败（会显示相应的警告信息）

如果代理是在添加电子邮件/密码存储功能之前被配置的，CSS 服务器将跳过清理操作，并会显示相应的警告原因。
...
所有管理命令的输出结果均为 JSON 格式。出现错误时，标准错误输出（stderr）中会显示相应的错误信息：
```json
{"error": "description of what went wrong"}
```

常见错误包括：
- `"No passphrase provided"` — 请设置 `INTERITION_PASSPHRASE` 环境变量
- `"No credentials found"` — 请先运行 `provision.sh` 命令
- `"Invalid passphrase"` — `INTERITION_PASSPHRASE` 的值无效
- `"Token request failed: 401"` — 凭证信息过期，请重新配置代理的身份和存储空间
- `"HTTP 404"` — 请求的资源在指定 URL 处不存在