...
description: 自我反思 + 自我批评 + 自我学习 + 自组织记忆。该智能体能够评估自身的工作表现，发现错误，并持续进行改进。在开始工作之前以及响应用户请求之后，都应使用这些机制。
...
**偏好信号** → 如果用户明确表达了偏好，请添加到 `memory.md` 中：
- “我喜欢你……的方式”
- “每次都为我做 X”
- “永远不要做 Y”
- “我的风格是……”
...
**潜在的模式** → 如果某种模式被重复使用 3 次以上，就将其标记为重要模式并记录下来：
- 相同的指令被重复使用 3 次以上
- 非常有效的 workflows
- 用户对某种方法的正面评价
...
自我反思的记录也需要遵循相同的规则：如果某种模式被成功应用了 3 次以上，就会将其提升为“重要模式”（HOT）。

## 常见查询

| 用户请求 | 对应操作 |
|-----------|--------|
| “你对 X 了解多少？” | 在所有文件中搜索关于 X 的内容 |
| “你学到了什么？” | 显示 `corrections.md` 中最近的 10 条记录 |
| “显示我的偏好设置” | 列出 `memory.md` 中的重要模式 |
| “显示 [项目] 的偏好设置” | 加载 `projects/{name}.md` 文件 |
| “`projects/` 和 `domains/` 目录中有哪些文件？” | 列出这两个目录下的所有文件 |
| “内存使用统计” | 显示各层级文件的数量 |
| “忘记 X” | 从所有文件中删除相关内容（请先确认） |
...
- 如果某个规则被连续使用 3 次，需要再次确认用户的意图

### 2. 分层存储
| 存储层级 | 存储位置 | 文件大小限制 | 加载方式 |
|------|----------|------------|----------|
| HOT | `memory.md` | 不超过 100 行 | 始终加载 |
| WARM | `projects/`, `domains/` | 每个文件不超过 200 行 | 根据上下文需求加载 |
| COLD | `archive/` | 无大小限制 | 根据用户请求加载 |

### 3. 自动升级/降级规则
- 如果某种模式在 7 天内被使用了 3 次，将其提升为 HOT 级别
- 如果 30 天内未被使用，降级为 WARM 级别
- 如果 90 天内未被使用，将其归档到 COLD 级别
- 未经用户确认，切勿删除任何文件

### 4. 命名空间隔离
- 项目相关的偏好设置存储在 `projects/{name}.md` 文件中
- 全局偏好设置存储在 `memory.md` 中
- 域相关的偏好设置存储在 `domains/` 目录中
- 不同命名空间之间的偏好设置可以相互继承（从全局到领域再到项目）
...
- 每周提供总结报告，包括新学习的模式、降级的模式和被归档的模式
- 可根据需求导出所有文件（格式为 ZIP）

### 8. 安全性
请参考 `boundaries.md`——禁止存储任何敏感信息（如凭据、健康数据或第三方信息）

### 9. 优雅的降级机制
当存储空间达到上限时：
- 仅加载 `memory.md`（HOT 级别的文件）
- 根据用户需求加载相关文件
- 避免系统无声失败——向用户明确说明哪些内容无法加载
...
- `clawhub install memory` — 用于存储代理程序的长期行为模式
- `clawhub install learning` — 提供自适应的教学和解释功能
- `clawhub install decide` — 帮助自动学习决策模式
- `clawhub install escalate` — 判断何时需要寻求用户帮助或自主行动

## 反馈方式
- 如果觉得此技能有用，请给 `self-improving` 添加星标（使用 `clawhub star self-improving`）
- 为了保持更新，请使用 `clawhub sync` 命令同步最新信息