...
description: 使用高效的模式来分析、转换和清洗 DataFrame，支持过滤、分组、合并以及数据透视（pivoting）操作。
metadata: {"clawdbot":{"emoji":"🐼","requires":{"bins":["python3"]},"os":["linux","darwin","win32"]}}
---
## 设置

首次使用时，请创建 `~/pandas/` 目录，并阅读 `setup.md` 以完成初始化配置。用户偏好设置存储在 `~/pandas/memory.md` 文件中，用户可以随时查看或编辑该文件。

## 使用场景

当用户需要在 Python 中处理表格数据时，可以使用该工具。该工具能够执行 DataFrame 操作、数据清洗、数据聚合、数据合并、数据透视（pivoting）以及数据导出等功能。
...
- 在删除缺失值之前，务必记录缺失值存在的原因

### 4. 对于重复出现的字符串，使用分类数据类型（Categorical data type）
...
- **设置警告（SettingWithCopyWarning）**：使用 `.loc[]` 进行数据赋值：`df.loc[mask, 'col'] = value`
- **循环效率低下**：将 `iterrows()` 替换为向量化操作或 `.apply()` 方法
- **内存溢出**：在读取 CSV 文件时指定数据类型：`pd.read_csv(f, dtype={'id': 'int32')`
- **数据丢失**：在合并数据前后检查数据量：`print(f"合并前：{len(df1)}, 合并后：{len(result)})`
- **索引混淆**：在 `groupby()` 后使用 `reset_index()` 以获得清晰的 DataFrame 结构
- **链式索引操作**：`df['a']['b']` 可能会无声地失败；应使用 `df.loc[:, ['a', 'b']]` 进行操作

## 安全性与隐私

**数据存储**：
- 用户偏好设置存储在 `~/pandas/memory.md` 中
- 所有 DataFrame 操作都在本地执行
- 无数据会被发送到外部

**该工具不执行以下操作：**
- 无数据上传至任何服务
- 无权限访问 `~/pandas/` 目录之外的文件
- 无权限在未经明确指示的情况下修改源数据文件
...
- 如果该工具对你有帮助，请使用 `clawhub star pandas` 给予评分
- 保持工具更新：`clawhub sync`