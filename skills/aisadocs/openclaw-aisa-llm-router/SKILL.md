---
name: llm-router
description: "统一大语言模型（Unified LLM）网关——一个用于70多种AI模型的API。只需一个API密钥，即可同时调用GPT、Claude、Gemini、Qwen、Deepseek、Grok等模型。"
...
**仅需一个API密钥，即可使用70多种模型。兼容OpenAI。**

您可以使用一个API密钥替代原有的100多个API密钥，通过统一的、兼容OpenAI的接口访问GPT-4、Claude-3、Gemini、Qwen、Deepseek、Grok等模型。
...
> **注意**：模型可用性可能随时变化。请访问[marketplace.aisa.one/pricing](https://marketplace.aisa.one/pricing)查看当前可用的模型列表及价格信息。
...
| 参数 | 类型 | 是否必填 | 说明 |
|-----------|------|----------|-------------|
| `model` | 字符串 | 是 | 模型标识符（例如：`gpt-4.1`, `claude-3-sonnet`） |
| `messages` | 数组 | 是 | 对话消息 |
| `temperature` | 数字 | 否 | 随机性系数（0-2，默认值：1） |
| `max_tokens` | 整数 | 否 | 最大响应字符数 |
| `stream` | 布尔值 | 否 | 是否启用流式响应（默认值：false） |
| `top_p` | 数字 | 否 | 核心采样率（0-1） |
| `frequency Penalty` | 数字 | 否 | 频率惩罚系数（-2至2） |
| `presence Penalty` | 数字 | 否 | 在场感惩罚系数（-2至2） |
| `stop` | 字符串/数组 | 否 | 停止序列 |
...
| 模型家族 | 大致费用 |
|--------------|------------------|
| GPT-4.1 / GPT-4o | 约0.01美元/1000个令牌 |
| Claude-3-Sonnet | 约0.01美元/1000个令牌 |
| Gemini-2.0-Flash | 约0.001美元/1000个令牌 |
| Qwen-Max | 约0.005美元/1000个令牌 |
| DeepSeek-V3 | 约0.002美元/1000个令牌 |
