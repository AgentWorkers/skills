...
description: 完整的性能工程系统：包括性能分析、优化、负载测试、容量规划以及性能管理文化。适用于诊断应用程序运行缓慢的问题、优化代码/查询/基础设施、在发布前进行负载测试、规划系统容量，以及将性能管理机制集成到持续集成/持续交付（CI/CD）流程中。该系统支持 Node.js、Python、Go、Java、数据库、API 和前端开发语言。
metadata:
  openclaw:
...
> 从“系统运行缓慢”到“问题原因及解决方案”——一套完整的性能问题测量、诊断、优化和预防方法论。
...
在开发之前就设定预算，而不是在收到投诉之后才制定：

| 指标            | Web应用 | API    | 移动应用 | 批量作业     |
|------------------|--------|--------|-----------|
| P50响应时间       | <200毫秒 | <100毫秒 | <300毫秒     |
| P95响应时间       | <500毫秒 | <250毫秒 | <800毫秒     |
| P99响应时间       | <1秒    | <500毫秒 | <1.5秒     |
| 错误率          | <0.1%    | <0.01%    | <0.5%      |
| 交互响应时间      | <3秒    | 无        | <2秒        |
| 每次请求的内存消耗    | <50MB    | <20MB    | <100MB     |
| 每次请求的CPU消耗    | <100毫秒 | <50毫秒 | <200毫秒     |
| 吞吐量          | >100请求/秒 | >500请求/秒 | 无         |
| 其他指标        | 根据实际需求 | 根据实际需求 | 根据实际需求 |

## 第2阶段：性能测量与分析

### 核心原则
**未经测量，勿进行优化；无假设，勿进行测量。**
...
| 问题             | 不良的O()时间复杂度 | 优化方法 | 良好的O()时间复杂度 |
|------------------|--------------|-----------|-------------------|
| 查找未排序数组       | O(n)         | 排序后使用二分查找，或使用Set/Map | O(log n) 或 O(1)         |
| 嵌套循环匹配       | O(n²)        | 使用哈希表查找       | O(n)                |
| 重复字符串拼接       | O(n²)        | 使用StringBuilder或数组连接 | O(n)                |
| 对已排序数据进行排序     | O(n log n)     | 先检查数据是否已排序   | O(n)                |
| 查找重复元素       | O(n²)        | 使用集合进行检测     | O(n)                |
| 频繁获取最小/最大值     | O(n)         | 使用堆/优先队列     | O(log n)                |
...
| 任务            | 推荐工具        | 替代工具            |
|------------------|------------------|---------------------------|
| HTTP基准测试       | k6             | wrk, ab, hey             |
| Node.js CPU性能分析   | clinic flame       | 0x, --prof           |
| Python CPU性能分析   | py-spy           | Scalene, cProfile        |
| Go语言CPU性能分析   | pprof           | go tool trace         |
| Java语言CPU性能分析   | async-profiler      | JFR, VisualVM         |
| 内存性能分析       | 根据语言选择相应工具    | （详见第2阶段）         |
| 命令行界面基准测试    | hyperfine          | time                |
| 代码打包分析       | webpack-bundle-analyzer | source-map-explorer     |
| Web页面性能分析    | Lighthouse         | WebPageTest           |
| 数据库查询分析     | EXPLAIN ANALYZE       | pgMustard, pganalyze       |
| 分布式追踪       | Jaeger, Zipkin        | OpenTelemetry         |
| 应用性能监控工具     | Datadog, New Relic       | Grafana + Prometheus     |
| 持续性能分析工具     | Pyroscope         | Parca                |
