...
- **未安装** → 转到第二步（全新安装）
- **已安装，但版本较旧** → 转到第三步（升级）
- **已安装，且版本为 v0.9.2 或更高** → 告知用户插件已是最新的，无需执行后续步骤
...
`openclaw plugins install` 命令不支持就地升级（即不支持在现有插件基础上进行升级），会提示“插件已存在”的错误。升级步骤如下：
...
- `~/.openclaw/vtai-agent-credentials.json`（VTAI API 认证信息）

### 3b. 清除旧的插件安装记录（保留用户配置）

删除旧插件目录后，`openclaw.json` 中的 `plugins.installs` 部分仍会引用旧插件的路径，从而导致验证错误。只需删除旧的插件安装记录，用户配置（如 API 密钥等）会保留下来：
