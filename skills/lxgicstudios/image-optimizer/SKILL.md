...
description: 获取由人工智能驱动的图像优化建议。当图像导致网站加载速度变慢时，请使用此功能。
---

# 图像优化工具

你的图片每个文件大小约为5MB，之前没人注意到这个问题，直到Lighthouse工具发现了这个问题。这个工具会扫描你的图片，并告诉你具体需要优化哪些地方。
...
- **延迟加载屏幕外的图片**：推迟加载那些不在用户视野范围内的图片
- **进行强力压缩**：通常80%的压缩率就足够了
...
无需安装，只需使用`npx`命令即可运行。建议使用Node.js 18及以上版本。需要设置`OPENAI_API_KEY`环境变量。
...
该工具会扫描你的图片文件，收集图片的尺寸、分辨率和格式等元数据，然后将这些信息发送给GPT-4o-mini模型，后者会提供优先级的优化建议。
