...
“为什么我们选择了 Redis 而不是 Memcached？”六个月后，没人会记得这个原因了。这个工具会根据你的决策背景生成架构决策记录（Architecture Decision Records），记录下所做的决策、考虑过的替代方案以及背后的理由。未来的你会感谢现在的自己。
...
- **在做出决策时立即编写 ADR 文档**——而不是六个月后才想起来原因
- **包含所有替代方案**——你还考虑过哪些方案？为什么没有选择它们？
- **诚实地记录权衡因素**——每个决策都有缺点，务必将其记录下来
- **为 ADR 文档编号**——例如：001-database-choice.md、002-auth-provider.md 等

## 适用场景

- 在做出重大技术决策时
- 新团队成员入职时，当他们询问“我们为什么选择这个方案”时
- 为架构评审做准备
- 建立技术选型的知识库
...
该工具会根据你的决策内容，研究该技术选择常见的替代方案及权衡因素，然后按照标准格式（标题、状态、背景、决策、后果）生成结构化的 ADR 文档。
