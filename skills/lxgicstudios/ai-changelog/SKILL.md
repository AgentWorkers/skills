...
手动编写变更日志非常繁琐。这个工具会读取你指定的两个 Git 参考点（refs）之间的提交历史记录，并生成格式规范的变更日志条目。再也不用复制粘贴提交信息了。
...
- 读取任意两个 Git 参考点（标签、分支、提交）之间的提交历史记录
- 将提交信息分类为“新增内容”、“修改内容”、“修复内容”和“删除内容”
- 将杂乱无章的提交信息整理成易于阅读的条目
...
- **为你的发布版本添加标签**——这会让生成变更日志变得容易得多
- **编写规范的提交信息**——提交的信息质量直接影响生成的变更日志质量
- **审核生成的变更日志**——人工智能可能会错误地分类某些更改
...
该工具会读取你指定的两个 Git 参考点之间的提交历史记录，收集所有提交信息，并将这些信息发送给 GPT-4o-mini。人工智能会解析这些提交信息，理解每个更改的具体内容，并将其按照规范的格式（如 Keep a Changelog 格式）进行整理。
