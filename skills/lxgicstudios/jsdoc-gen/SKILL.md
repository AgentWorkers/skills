---
name: jsdoc-gen
description: 在代码中添加 JSDoc 或 TSDoc 注释。当文档缺失时，请使用这些注释来提供必要的说明。
---
...
- **先预览**：先运行工具（不使用 `--write` 选项），查看具体变化
- **重点关注导出的函数**：内部辅助函数不需要文档
- **添加示例**：尤其是对于复杂的函数
- **审查生成的文档**：人工智能可能无法完全理解某些细微差别
...
该工具会读取您的文件，识别出导出的函数、类和类型，然后生成相应的文档注释。人工智能会解析函数签名，并推断每个参数的用途。
