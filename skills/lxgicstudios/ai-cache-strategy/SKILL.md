...
你知道应该对某些数据进行缓存，但不确定该缓存哪些数据、将数据缓存到哪里以及缓存多长时间。使用这个工具可以针对你的 API 路由获取具体的缓存建议。
...
该工具会读取你的 API 路由文件，理解代码中的数据访问模式，并据此推荐合适的缓存策略。AI 会考虑数据更新频率、个性化需求以及常见的访问模式等因素。
