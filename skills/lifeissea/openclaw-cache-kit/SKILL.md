...
description: 自动为 OpenClaw 应用 Claude 提示缓存优化策略。通过设置较长的缓存保留时间（59 分钟）、实施缓存过期机制（cache-ttl pruning），以及采用 CRON.md 文件分离策略，可将 API 使用成本降低高达 89%。
version: 1.1.1
author: raon
...
在默认设置下，这些大型文件会在每次请求时被发送到服务器，且不会被缓存，从而导致不必要的费用。
...
**核心原则**：
- `CRON.md` 文件中仅应包含任务调度和必要的指令。
- 具体的运营规则应配置在 `SOUL.md` 和 `AGENTS.md` 中。
