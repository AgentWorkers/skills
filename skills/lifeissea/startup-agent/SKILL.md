...
name: raon-os
version: 0.7.10
description: "这是一款由人工智能驱动的初创企业辅助工具，专为韩国创业者设计。它能够评估商业计划书，帮助创业者匹配政府提供的资金支持计划（如TIPS/DeepTech/Global TIPS），并与3,972家获得TIPS资助的初创企业建立联系，同时提供投资者的推荐意见。此外，该工具还支持与Kakao i OpenBuilder的集成。其核心功能包括：智能的问答系统（Agentic RAG，支持HyDE、Multi-Query、CRAG等多种查询方式）、结构化数据提取技术，以及财务匹配功能（Track B）。"
metadata:
  openclaw:
    env:
      - name: GEMINI_API_KEY
...
- **Python 3.9+**（macOS已内置，无需另行安装）
- **Node.js 18+**（用于执行 `npx @yeomyeonggeori/raon-os` 命令）
- **LLM API密钥**（以下选项之一，按优先级排序）：

| 环境变量 | 说明 | 备注 |
|----------|------|------|
| `OPENROUTER_API_KEY` | **优先级1** — 支持所有OpenClaw模型的API | 推荐 |
| `GEMINI_API_KEY` | **优先级2** — Google Gemini及嵌入式应用 | |
| `ANTHROPIC_API_KEY` | **优先级3** — Claude | |
| `OPENAI_API_KEY` | **优先级4** — GPT及嵌入式应用 | |
| Ollama本地版 | 自动检测（无密钥时使用） | 可选 — 使用 `raon.sh install-model` 安装 |

- **向量搜索**：如果拥有 `GEMINI_API_KEY` 或 `OPENAI_API_KEY`，则自动启用该功能；否则将使用BM25关键词搜索。
...
所有API密钥均存储在 `~/.openclaw/.env` 文件中（优先使用环境变量设置）：
...
Raon OS是一个专为创业团队设计的AI助手，可协助您将商业想法转化为实际项目。
...
**支持项目包括**：TIPS、K-Startup Grand Challenge、创业成长技术开发计划、预备创业包、初期创业包等
...
**注意**：如果用户直接上传PDF文件，可能会导致令牌使用量超出限制。**必须先提取文本内容再进行评估**。

**处理流程**：
- 如果PDF文件通过OpenClaw上传：
  1. PDF文件会被保存在 `media/inbound/` 目录中。
  2. **切勿将PDF文件直接嵌入到请求中**（否则可能导致令牌消耗过快）。
  3. 使用 `exec` 工具执行 `evaluate.py --file <文件路径>` 进行评估。
  4. 将评估结果反馈给用户。
...
支持启动本地REST API服务器，便于与Web聊天工具或外部服务集成：
...
当前版本基于本地分析（基于LLM的RAG技术）运行。
如需集成K-Startup AI API，请配置相应的环境变量：
...
如果未配置API，系统将回退到本地LLM + RAG处理流程。

## 评估标准参考

有关政府扶持项目的评审标准，请参考 `references/` 目录下的文件：
- `references/tips-criteria.md` — TIPS项目评审标准
