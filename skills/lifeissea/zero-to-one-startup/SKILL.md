...
name: Raon OS
version: 0.7.10
description: "这是一款由人工智能驱动的初创企业辅助工具，专为韩国创业者设计。它能够评估商业计划书，帮助创业者匹配政府提供的资金支持计划（如TIPS/DeepTech/Global TIPS），并与3,972家获得TIPS资助的初创企业建立联系，提供投资者推荐服务，并支持与Kakao i OpenBuilder的集成。该工具具备以下功能：智能问答系统（Agentic RAG，包括HyDE、Multi-Query、CRAG），结构化数据提取技术，以及财务数据匹配功能（Track B）。"
metadata:
  openclaw:
    env:
      - name: GEMINI_API_KEY
...
- **向量搜索**：如果拥有 `GEMINI_API_KEY` 或 `OPENAI_API_KEY`，则自动启用向量搜索功能；否则使用 BM25 关键词搜索。
...
Raon OS 是一个专为创业者设计的 AI 助手，可帮助您将商业想法转化为实际项目。
...
**支持项目**：TIPS、K-Startup Grand Challenge、创业成长技术开发计划、预备创业包、初期创业包等
...
用户直接上传 PDF 文件时，文件内容会直接进入处理流程，可能导致token使用量超出限制。
**必须先提取文本内容再进行评估。**

**注意事项：**
- 如果 PDF 文件中包含附件，请确保文件被保存在 `media/inbound/` 目录中。
- **切勿将 PDF 文件直接嵌入到处理流程中**，否则可能导致 token 使用量激增。
- 使用 `exec` 命令执行 `evaluate.py --file <文件路径>` 进行评估，并将结果反馈给用户。
...
可启动本地 REST API 服务器，以便与 Web 聊天工具或其他外部服务集成：
...
当前版本基于本地 LLM（RAG）进行评估。
若要集成 K-Startup AI API，请配置相应的环境变量：
