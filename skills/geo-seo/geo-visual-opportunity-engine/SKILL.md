...
GEO Visual Opportunity Engine 是一款基于人工智能的电子商务自动化工具，它使用 Nano Banana 2（Google Gemini）生成产品图片，并自动将产品发布到 Shopify 和 WooCommerce 平台。

## 主要功能

- **产品数据生成**：自动生成产品标题、描述、SKU（库存单位）、价格等信息。
- **AI 图像生成**：自动调用 Nano Banana 2 生成产品图片。
- **多平台支持**：同时支持将产品发布到 Shopify 和 WooCommerce。
- **多种图片风格**：为每个产品提供三种风格的图片（白色背景、生活场景图和封面图片）。
- **GEO 机会分析**：识别具有高优先级的视觉内容生成需求。
...
- **`run_complete_workflow(product_input, country='us', language='en', generate_images=True, publish_to_shopify=False, publish_to_woocommerce=False, output_dir='output')**：执行完整的工作流程：
  ...
  - 生成 AI 图片。
  - 将产品发布到电子商务平台。
- **`run_geo_analysis(brand, product, core_keyword, country, language, competitors, platform_focus, generate_images)**：执行 GEO 机会分析并生成图片。
...
- `WOOCOMMERCE_CONSUMER_SECRET`：WooCommerce API 消费者秘钥。

## 图片风格

- **white_info**：纯白色背景的产品信息图。
- **lifestyle**：包含真实人物互动的、具有真实感的图片。
- **hero**：高质量的商业摄影风格的封面图片。
