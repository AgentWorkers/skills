...
description: >
  **使用说明：**  
  当通过 Resend API 发送交易性电子邮件（欢迎邮件、订单确认邮件、密码重置邮件、收据邮件）、通知邮件或批量邮件时，请使用本指南。
inputs:
    - name: RESEND_API_KEY
...
| `attachments` | 数组 | 文件附件（总大小最多 40MB） |
| `tags` | 数组 | 用于跟踪的键值对（请参阅 [Tags](#tags)） |
| `headers` | 对象 | 自定义头部信息 |

*参数名称因 SDK 而异（例如，在 Node.js 中为 `replyTo`，在 Python 中为 `reply_to`）。
...
由于整个批次在任何验证错误时都会失败，因此在发送前请验证所有邮件：
...
| 实践 | 原因 |
|---------|---------|
| **有效的 SPF、DKIM、DMARC 记录** | 验证邮件真实性，防止欺骗 |
| **链接与发送域名匹配** | 如果从 `@acme.com` 发送邮件，链接应指向 `https://acme.com` - 不匹配的域名会触发垃圾邮件过滤器 |
| **包含纯文本版本** | 同时使用 `html` 和 `text` 参数，以提高可访问性和送达率（Resend 会在未提供纯文本版本时自动生成） |
| **避免使用“no-reply”地址** | 使用真实地址（例如 `support@`） - 提高信任度 |
...
| 实践 | 原因 |
|---------|---------|
| **使用子域名** | 交易性邮件从 `notifications.acme.com` 发送，营销邮件从 `mail.acme.com` 发送 - 保护邮件服务声誉 |
| **关闭交易性邮件的跟踪功能** | 对于密码重置、确认邮件等敏感邮件，关闭跟踪功能 |

## 跟踪（打开和点击）

跟踪功能在 Resend 仪表板的 **域名级别** 进行配置，而不是针对每封邮件单独配置。

| 设置 | 工作原理 | 建议 |
|---------|--------------|----------------|
| **打开跟踪** | 插入 1x1 的透明像素 | 对于交易性邮件请关闭此功能 - 可能影响送达率 |
| **点击跟踪** | 通过重定向方式修改链接 | 对于敏感邮件（如密码重置、安全提醒）请关闭此功能 |
...
| `email.bounced` | 从邮件列表中移除用户，通知用户 |
| `email.complained` | 用户投诉垃圾邮件 |
| `email.opened` / `email.clicked` | 跟踪用户互动（仅限营销邮件） |

**请验证每个请求的 Webhook 签名。** 如果不验证，任何人都可以向您的端点发送伪造的事件。
...
**限制：** 标签名称和值只能包含 ASCII 字母、数字、下划线或破折号。每个标签的最大长度为 256 个字符。

## 模板

请使用预构建的模板，而不是在每次请求时发送 HTML 内容。
...
**注意：** 变量名称区分大小写，必须与模板定义完全匹配。无论使用 `USER_NAME`、`userName` 还是 `user_name`，只要发送请求时使用的大小写与模板定义一致即可。

| 信息 | 详情 |
|------|--------|
| **保留的名称** | `FIRST_NAME`、`LAST_NAME`、`EMAIL`、`UNSUBSCRIBE_URL`、`contact`、`this` |
| **默认值** | 可选 - 如果未设置且在发送时变量缺失，发送将失败（返回 422 错误） |
| **不能与以下参数结合使用** | `html`、`text` 或 `react` |
| **subject`/`from` | 模板中的默认值可以在发送时进行覆盖 |

模板在使用前必须 **发布**。草稿模板无法发送。
...
- **导致您的域名被列入黑名单** - 提供商会将高退回率的域名列入黑名单 |
...
| **Bounced** | `bounced@resend.dev` | 模拟硬退回 |
| **Complained** | `complained@resend.dev` | 模拟垃圾邮件投诉 |
| **您自己的电子邮件地址** | 您的实际地址 | 进行真实的发送测试 |

**开发环境：** 使用 `resend.dev` 测试地址来模拟不同场景，而不会影响您的声誉。**

**测试环境：** 向您控制的真实地址发送邮件（团队成员、您拥有的测试账户）。

## 域名预热

新域名必须逐步增加发送量以建立声誉。

**原因：** 新域名突然大量发送邮件会触发垃圾邮件过滤器。ISP 期望发送量逐渐增加。
...
|--------|--------|-------------------|
| **退回率** | < 4% | 减慢发送速度，清理邮件列表 |
| **垃圾邮件投诉率** | < 0.08% | 减慢发送速度，审查内容 |

**请勿使用第三方预热服务。** 专注于向真实且活跃的收件人发送相关内容。

## 邮件抑制列表

Resend 会自动管理不应接收邮件的地址列表。
...
**发生的情况：** Resend 不会尝试向被抑制的地址发送邮件。此时会触发 `email.suppressed` Webhook 事件。

**这很重要：** 继续向被退回/投诉的地址发送邮件会损害您的声誉。抑制列表会自动保护您。

**管理方式：** 在 Resend 仪表板的“Suppressions” 部分查看和管理被抑制的地址。

## 常见错误

| 错误 | 修复方法 |
|---------|---------|
| 重试时未使用幂等性键 | 必须包含幂等性键 - 防止重试时邮件重复发送 |
| 对带附件的邮件使用批量发送 | 批量发送不支持附件 - 请使用单次发送 |
| 发送前未验证邮件 | 先验证所有邮件 - 一封无效邮件会导致整个批次失败 |
| 重试 400/422 错误 | 这些是验证错误 - 修复请求，不要重试 |
| 使用相同的幂等性键但数据不同 | 会返回 409 错误 - 每封邮件使用唯一的键 |
| 对交易性邮件启用跟踪功能 | 对于密码重置、确认邮件等敏感邮件，请关闭打开/点击跟踪功能 - 会影响送达率 |
| 使用“no-reply”发件人地址 | 使用真实的地址（如 `support@`） - 提高信任度 |
| 未验证 Webhook 签名 | 必须验证签名 - 未验证的请求可能被视为伪造 |

**测试时使用虚假地址（如 test@gmail.com）** | 使用 `delivered@resend.dev` - 虚假地址会导致邮件被退回，损害您的声誉 |

**模板变量名称的注意事项：** 变量名称区分大小写，必须与模板定义完全匹配。无论使用 `USER_NAME`、`userName` 还是 `user_name`，只要发送请求时使用的大小写与模板定义一致即可。 |

| 注意事项：** `from` 地址必须使用经过验证的域名 |
| 如果发件人地址无法接收回复，请将 `reply_to` 参数设置为有效的地址 |
| 将 API 密钥存储在 `RESEND_API_KEY` 环境变量中 |
| Node.js SDK 支持用于 React 邮件组件的 `react` 参数 |
| Resend 在响应中返回 `error`、`data`、`headers` |
| 成功时，数据返回 `{ id: "email-id" }`（单次发送）或 ID 数组（批量发送） |
| 对于大规模邮件列表的营销活动，请使用 Resend Broadcasts 功能 |