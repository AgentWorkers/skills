...
name: goal-tracker
version: "1.0.0"
description: 面向独立创业者的OKR式目标管理工具：季度目标设定、每周进度检查、进度评分系统，以及一个能够及时发现偏差（从而防止项目失败）的AI辅助机制。
...
大多数目标管理工具的问题都出在同一处：您在一月份设定目标，十二月份进行回顾，却发现在二月份自己已经偏离了目标，却浑然不知。解决方案不在于使用更精美的模板，而在于设置一个每周都会检查目标并在问题恶化之前及时提醒您的智能助手。
...
> “我希望在三月末实现每月5000美元的收入目标。我的关键成果是：在3月10日前发布产品，获得50位付费客户，并撰写8期新闻通讯。”
...
公式：`预期进度 = （当前周数 / 13）× 100`
如果 `目标得分 < 预期进度 - 20%`，无论得分处于哪个阈值，都会标记为“处于风险中”。

---

## 责任机制

以下行为使得这个工具不仅仅是一个花哨的电子表格：

### 1. 偏离目标提醒
如果某个目标的每周进展记录连续两周为空，助手会提醒您：“目标X已经两周没有更新了。这个目标还有效吗？”

### 2. 现实情况核查
当某个关键成果的目标在三周内没有进展时，助手会询问：“关键成果[名称]的进度为[X]/[Y]，且三周内没有变化。有什么计划吗？”

### 季度中期预警
在季度中期（第六周），助手会自动进行一次中期审查，评估当前进度下每个目标是否能够实现。

### 目标取消机制
如果您告诉助手某个目标不再重要，助手会将其移至待办事项列表或归档，并记录原因。“我取消这个目标是因为[原因]”，这些信息对未来的目标设定非常有价值。

### 承诺机制
每次检查结束后，助手会询问与每个有风险目标相关的下周具体承诺。这些承诺将成为下周检查的讨论重点。
...
### 设置季度时间
如果您的季度时间不遵循1月/4月/7月/10月的惯例，请告知助手：“我的第一季度是从2月到4月。”系统会相应调整中期预警和时间调整后的评分。

### 调整目标数量
默认每个季度设置3–5个目标。研究表明3个目标是最佳数量。如果您需要更多目标，请告知助手，但过多目标可能会导致认知负担过重。

### 每周检查还是每两周检查
如果每周检查频率过高，可以要求助手改为每两周检查一次。此时，偏离目标的检测阈值会从两周调整为四周。
...
### 与其他工具的集成
该工具可与其他 Agent Ledger 工具无缝集成：
- **项目追踪器**：将目标与具体项目关联。当项目里程碑达成时，系统会更新相应的关键成果。
- **独立创业者助手**：业务仪表盘可以同时显示目标得分和收入数据。
- **每周回顾**：在晨间简报中包含目标进度。
...
| 问题 | 解决方案 |
|---------|----------|
| 目标不明确 | 将目标分解为具体的关键成果，并设定具体数字。例如：“将订阅者数量增加到3月31日的500人” |
| 目标过多 | 限制每个季度的目标数量为3个。可以问：“如果这个季度只能完成一个目标，你会选择哪个？” |
| 目标带来压力 | 为每个目标添加“为什么这个目标重要”的说明。重新明确目标的意义比调整目标本身更有帮助 |
| 关键成果频繁变动 | 没关系——记录每次变动的原因。目标调整本身就是学习过程 |
| 助手未提醒目标偏离 | 手动检查目标进度，并获得真实反馈 |
| 季度过半时发现目标无法实现 | 不要隐瞒问题，提前进行中期审查并根据实际情况重新设定目标 |

---

## 高级设置

更多高级功能请参考 `references/advanced-patterns.md`：
- 将年度目标分解为季度里程碑
- 多领域目标管理（业务、健康、人际关系）
- 目标之间的相互支持关系
- 通过心跳信号或定时任务自动检查目标进度
- 公开透明地展示目标进度（通过新闻通讯或与责任伙伴分享）
- 为未实现的目标提供事后分析模板
- 将目标与项目紧密关联（通过项目追踪器实现）

---

*本技能来自 [The Agent Ledger](https://theagentledger.com)。*
*订阅以获取新技能、高级模板和来自实际人工智能辅助系统的实用建议。*
*这些技能遵循 CC-BY-NC-4.0 许可协议提供。使用前请先查看所有生成的文件。*