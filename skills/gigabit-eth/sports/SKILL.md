...
> **仅限开发网络使用 - 请勿使用主网络钱包**
>
> 该服务仅在**Solana开发网络**上运行，并使用**模拟的USDC代币**。这些并非真实资金。
>
> **安全要求：**
>
> - ✅ **仅使用一次性/开发网络密钥对** — 绝对不要使用主网络钱包
> - ✅ **将私钥保存在`~/.config/optionns/`中**，并设置权限为600（该技能会自动配置）
> - ✅ **在信任之前，请独立验证API端点**（`https://api.optionns.com`）
> - ✅ **建议在隔离环境中运行**以实现自主操作
> - ❌ **严禁将服务指向主网络**或使用真实资金/密钥
>
> **本地存储的内容：**
>
...
>
> 该服务通过`https://api.optionns.com`（远程服务）和Solana开发网络RPC进行通信。在验证来源之前，请将其视为不可信任的网络端点。在允许使用凭据进行自主操作之前，请查看`scripts/signer.py`和`scripts/optionns.sh`。

**安全实现：**该服务在本地生成密钥对（永远不会发送到API），使用`solders`在客户端签署交易，并仅将已签署的交易传输到Solana RPC。私钥始终保留在您的机器上。**用户有责任确保仅配置开发网络的RPC端点** — 签署器将根据提供的RPC URL执行交易。
...
**关键创新：**代理可以同时观看12场以上的比赛，计算100多个微市场的收益，并在<2秒内完成交易 — 这是人类无法做到的。**
...
| `python3`       | ≥3.8    | 交易签署和策略引擎           |

### 可选设置工具

**仅适用于`register`和`faucet`命令** — 请自行准备密钥对以跳过这些工具：

| 二进制文件 | 版本 | 用途                                 |
| --------------- | ------- | --------------------------------------- |
| `solana-keygen` | ≥1.14   | 生成注册所需的密钥对          |
...
| `SOLANA_RPC_URL`     | `https://api.devnet.solana.com`                   | Solana RPC端点（使用Helius获取最新块哈希：`https://devnet.helius-rpc.com/?api-key=YOUR_KEY`） |
...
该服务在`~/.config/optionns/`目录下创建文件（权限设置为600）：
...
> **⚠️ 仅限开发网络使用：**该服务仅在Solana开发网络上运行，并使用模拟的USDC。请勿使用主网络钱包或真实资金。
...
您的私钥始终保留在您的机器上。Optionns API仅构建未签署的交易 — 您的代理会使用自己的密钥对在本地签署这些交易。
...
这会安装`solders`用于本地交易签署，以及`httpx`用于策略引擎。

> **💡 建议：** 获取免费的Helius RPC密钥以获得更稳定的交易体验**
>
> 默认的Solana开发网络RPC存在速率限制，经常返回过时的块哈希，导致交易失败。为了获得最佳体验，请获取**免费的**Helius API密钥：
>
...
>    ```
>
> 签署器包含针对过时块哈希的自动重试逻辑，但使用专用的RPC可以更快、更可靠地提交交易。
...
> [!注意]
> **如果在运行命令时出现“权限被拒绝”的错误**，可能是安装过程中未正确保留了可执行文件的权限。请使用以下命令修复：
> 
> ```bash
...
**根本原因：** 来自免费Solana开发网络RPC的过时块哈希（ATA并未丢失！）  
...
> **如果您之前已经成功进行过交易，请** **不要** 手动运行`spl-token create-account`命令。您的ATA已经存在 — 问题在于块哈希过时。

---

## 流动性管理（链上操作）

将USDC直接存入保险库合约，并从期权溢价中赚取收益。所有交易均通过Solana在链上完成。
...
2. `signer.py`获取最新的块哈希并构建交易
3. 代理使用本地密钥对签署交易并提交到Solana RPC
4. 链上结算在约2-4秒内完成

**重要性说明：**您的API密钥永远不会访问您的私钥。您完全掌控着自己的资金。API仅提供指令 — 您负责构建、签署和提交交易。

> **关键点：ATA处理**  
> API会在需要时自动包含ATA（相关Token账户）的创建指令。**绝对不要** 手动运行`spl-token create-account`命令。  
>   
> 如果在交易过程中遇到`AccountNotInitialized`错误：
> 1. **首先**，检查是否是由于块哈希过时导致的 — 免费的Solana开发网络RPC经常返回过时的块哈希
> 2. **解决方案**：使用像Helius这样的优质RPC服务（免费 tier）：设置`SOLANA_RPC_URL=https://devnet.helius-rpc.com/?api-key=YOUR_KEY`
...
**小贴士：** 使用`--upcoming`可以提前查看今晚的比赛安排，然后在比赛开始时监控最佳的交易机会。
...
1. **扫描**所有正在进行的比赛（NFL、NBA、CBB、NHL、MLB、CFB、SOCCER）
2. **使用Kelly准则**计算收益机会
3. **通过API自动进行交易**
...
- **历史数据：** 团队在类似情况下的表现
- **市场效率：** 存在价格异常的微市场
- **时间衰减：** 时间窗口越短，波动性越大，机会越大

### 资金管理

- **Kelly准则：** 确定最佳投注金额（f\* = (bp-q)/b）
- **半Kelly公式：** 为了安全起见采用保守的投注金额
- **每笔交易的最大风险为5%**
