---
name: next-browser
description: 使用 Nextbrowser 的云 API 来启动云浏览器，以便 Openclaw 执行自主的浏览器任务。主要用途是创建带有个人资料（持久登录信息/cookies）的浏览器会话，Openclaw 可以通过这些会话来管理社交媒体和其他在线账户。次要用途是在住宅代理环境下运行任务子代理，实现快速的自主浏览器自动化操作，以及具备浏览器隐身和验证码破解功能。详细文档请访问 docs.nextbrowser.com。
---
# Nextbrowser
...
**文档链接：**  
- [云 API 文档](https://docs.nextbrowser.com/getting-started)
...
**重要提示：** Nextbrowser 的 API 密钥可能具有不同的格式和前缀。  
**请不要自行验证密钥格式**——只需使用用户提供的密钥即可。如果密钥无效，API 会返回身份验证错误，此时再要求用户验证密钥即可。  
...
配置文件可以跨浏览器会话保存 Cookie 和登录状态。创建一个配置文件后，您可以在浏览器中登录并重复使用该配置。  
...
这些 API 端点提供可用于代理和浏览器配置的地理位置数据。在创建配置文件或根据特定网络条件运行任务之前，可以使用这些信息动态获取支持的国家、地区、城市和互联网服务提供商（ISP）信息。  
...
Nextbrowser 可以像一个子代理一样执行自动化浏览器任务。您只需提供任务指令，它就会完成相应的操作。  
...
curl -X POST "https://app.nextbrowser.com/api/v1/chat/tasks" \
  ...
  -d '{
    "task_description": "'\
    访问 Reddit.com 账户，检查账户是否已登录（如未登录，则使用存储的凭证）；  
    在关于“AI Agents”的主题下找到 10 条相关帖子，给其中 8 条点赞，并发布 3 条具有讽刺幽默风格的评论；  
    确保评论能够成功发布（如不确定是否合适，请请求审批）。完成任务后，应至少查看 10 条相关帖子、给 8 条帖子点赞并发布 3 条评论。"  
    ",  
    "mode": "fast",  
...
执行 `/api/v1/chat/tasks` 请求后，响应中会包含聊天会话的 ID，可用于停止该会话：  
```bash  
...
**可选参数：**  
- `from_step`（整数，>= 1）：指定要查询的步骤索引。如果未提供或参数无效，默认值为 1。该参数用于仅查询新步骤的进度。  
...
```json  
{
  ...
  "payload": {  
    "status": "finished",  
    "output": "任务已完成：共查看 10 条相关帖子，给 8 条帖子点赞并发布 3 条评论。"  
    "isSuccess": true,  
    "steps": [  
      {  
...
有关所有端点的详细信息（包括会话、文件、技能和技能市场等相关内容），请参阅 [references/api.md](references/api.md)。