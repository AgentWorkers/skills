---
name: agenthub
description: 调用32个真实世界的API，包括航班、酒店、天气、加密货币价格、去中心化金融（DeFi）收益、股票报价、网络搜索、地理编码、IP信誉、区块链数据、代码执行以及电子邮件服务。每次调用均按USDC计费，通过x402接口进行支付。该功能需要Node.js 20.0及以上版本，以及一个装有USDC的Base钱包。
...
---
# AgentHub — 提供32个AI API服务，按每次调用计费（使用USDC）

您可以使用**AgentHub**这个网关来调用各种实际数据API。支付过程通过x402协议自动处理，使用的是Base Mainnet上的USDC（无需支付手续费）。
...
该操作会将SDK安装到技能对应的文件夹中，无需全局安装，也不会引发权限问题，适用于任何操作系统。

SDK包链接：[npmjs.com/package/rwagenthub-sdk](https://www.npmjs.com/package/rwagenthub-sdk)

### 2. 配置您的钱包私钥

> ⚠️ 请使用专门用于存储USDC的钱包，切勿使用您的主钱包。

```bash
openclaw config set env.MCP_WALLET_PRIVATE_KEY "0xYOUR_PRIVATE_KEY_HERE"
```

**安全提示：** 私钥永远不会离开您的设备。SDK仅使用私钥在本地生成EIP-3009格式的授权信息（无需支付手续费），然后仅将授权信息（而非私钥本身）发送到支付网络。私钥仅用于在Base Mainnet上处理USDC微支付。

### 3. 向Base Mainnet钱包中充值USDC

- 最低建议金额：1美元（足够进行100多次每次0.01美元的调用）
- 可通过[Coinbase](https://coinbase.com)或其他交易平台购买USDC并转入Base Mainnet钱包。

---

## 在每次调用API之前——验证SDK是否已安装

在进行任何API调用之前，请先运行以下检查。如果SDK未安装，系统会自动进行安装：
...
对于每个API调用，需要创建一个临时脚本并通过Node.js来执行：
...
请将`API_NAME`和`{ ...inputs... }`替换为下表中的API名称及参数。
...
| 错误类型 | 原因 | 解决方案 |
|---|---|---|
| `payment failed` | Base Mainnet上的USDC余额不足 | 请补充钱包余额 |
| `AgentHub: API error — unknown_api` | API名称错误 | 请核对API名称是否正确 |
| `AgentHub: API error — invalid_inputs` | 参数缺失或错误 | 请检查相关API的输入参数 |
