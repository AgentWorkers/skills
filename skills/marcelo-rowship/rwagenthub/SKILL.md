---
name: agenthub
description: 调用32个真实世界的API，包括航班、酒店、天气、加密货币价格、去中心化金融（DeFi）收益、股票报价、网络搜索、地理编码、IP信誉、区块链数据、代码执行以及电子邮件服务。每次调用均按USDC计费，通过x402协议在Base平台上完成支付。该功能需要Node.js 20.0及以上版本，以及一个装有USDC的Base钱包。
...
您可以使用**AgentHub**这个网关来调用各种实际数据API。支付过程通过x402协议自动处理，使用的是Base Mainnet上的无gas费用（即无需支付额外手续费）的USDC。
...
该操作会将SDK安装到技能（skill）所在的文件夹中，无需全局安装，也不会引发权限问题，适用于任何操作系统。

SDK包链接：[npmjs.com/package/rwagenthub-sdk](https://www.npmjs.com/package/rwagenthub-sdk)

### 2. 配置您的钱包私钥

> ⚠️ 请使用专门用于此目的的钱包，并仅存放所需的USDC。切勿使用您的主钱包。

```bash
openclaw config set env.MCP_WALLET_PRIVATE_KEY "0xYOUR_PRIVATE_KEY_HERE"
```

**安全提示：** 私钥永远不会离开您的设备。SDK会使用该私钥在本地生成EIP-3009格式的授权请求（无需支付gas费用），只有授权请求（而非私钥本身）会被发送到支付网络。私钥仅用于在Base Mainnet上进行USDC微支付。

### 3. 向Base Mainnet上的钱包充值USDC

- 最低推荐金额：1美元USDC（足够进行100多次调用，每次调用费用为0.01美元）
...
请将`API_NAME`以及`{ ...inputs... }`替换为下表中的API名称及相应的参数。
...
| `AgentHub: API error — invalid_inputs` | 参数缺失或错误 | 请检查相关API的参数是否正确 |
