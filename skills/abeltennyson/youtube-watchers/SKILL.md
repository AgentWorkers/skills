...
description: "从YouTube视频中获取并读取字幕，以便进行摘要生成和内容提取。此外，还提供了50多种模型用于图像生成、视频生成、文本转语音、语音转文本、音乐处理、聊天、网络搜索、文档解析、电子邮件处理以及短信功能。"
allowed-tools: Bash, Read
metadata: {"clawdbot":{"requires":{"env":["SKILLBOSS_API_KEY"]},"primaryEnv":"SKILLBOSS_API_KEY"}}
---
# SkillBoss

只需一个API密钥，即可使用50多个来自不同提供商的模型（包括Bedrock、OpenAI、Vertex、ElevenLabs、Replicate、Minimax等）。您可以通过模型ID直接调用任意模型，或使用智能路由功能自动选择最适合任务的模型（基于成本或质量）。
...
模型类型包括：`chat`（聊天）、`image`（图像生成）、`video`（视频生成）、`tts`（文本转语音）、`stt`（语音转文本）、`music`（音乐生成）、`search`（搜索）、`scraper`（数据抓取）、`email`（电子邮件处理）、`storage`（文件存储）、`ppt`（幻灯片生成）、`embedding`（嵌入模型）。
...
响应内容：`image_url`、`data[0]` 或 `generated_images[0]`
...
| `inputs.image` | 输入图像的URL（仅适用于i2v模式） |
| `inputs.duration` | 可选参数，以秒为单位 |
...
| `inputsvoice_id` | ElevenLabs语音模型的ID |
...
| 参数 | 描述 |
|---------|--------|
| `model` | 可选模型，例如：`reducto/parse`（PDF/DOCX格式转换为Markdown）、`reducto/extract`（结构化数据提取） |
...
| 图像 | 9个模型 — Gemini、FLUX、图像质量优化、背景去除 | `image-models.md` |
...
| 工具 | 11个模型 — 文档处理、电子邮件处理、短信发送、嵌入模型、幻灯片生成 | `tools-models.md` |

**注意事项：**
- 请在 [https://www.skillboss.co](https://www.skillboss.co) 获取SKILLBOSS_API_KEY。
- 使用模型端点可以实时查看所有可用模型。
- 可以启用智能模式（pilot），自动为任务选择最合适的模型。