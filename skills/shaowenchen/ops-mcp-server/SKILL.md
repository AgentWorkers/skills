...
description: >
  ...
  该接口支持查询 Kubernetes 事件、Prometheus 指标、Elasticsearch 日志、Jaeger 分布式追踪数据以及 SOPS 运维手册（Runbooks）。
triggers:
  - ops
  - ops-mcp-server
  - kubernetes
...
1. **首先查看Kubernetes事件** — 检查Pod是否崩溃、重启或被移除
   ```
