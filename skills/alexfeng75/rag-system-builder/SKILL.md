...
description: 构建并部署本地RAG（Retrieval-Augmented Generation，检索增强生成）系统，该系统支持离线文档处理、模型嵌入以及向量存储功能。
---
# RAG系统构建技能

构建完整的本地RAG（Retrieval with Attention and Generation）系统，该系统支持离线运行，具备文档导入、语义搜索以及基于AI的问答功能。

## 🎯 本技能的功能

本技能将指导您构建一个完整的RAG系统，该系统能够：
- **从多种格式（TXT、PDF、DOCX、MD、HTML、JSON、XML）导入文档**
- **使用句子转换器生成文档嵌入（无需外部API）**
- **利用FAISS库将生成的嵌入向量存储在本地，以实现快速的相似性搜索**
- **通过命令行界面（CLI）和网页界面提供问答功能**
- **完全离线运行**——无需任何外部API调用
