...
这是一个基于 MiniMax Voice API 的简单文本转语音（TTS）功能，可以将任何文本转换为音质自然的语音，并支持自定义语音选择。

## 使用方法
### [步骤 1] 准备工作

**重要提示：** 在使用此功能之前，请务必检查环境配置：
...
**必填参数：**
- `text`：需要转换为语音的文本
- `-o OUTPUT`：输出音频文件的路径（必填）

**可选参数：**
- `-v VOICE_ID`：要使用的语音（默认值：male-qn-qingse）

### 使用细节
- **文本长度限制：** 每次请求最多支持 10,000 个字符的文本。
- 如果文本过长，可以分成多个请求处理，之后再合并音频文件。

**暂停插入：** 使用 `<#x#>` 标记暂停位置（x 表示暂停时长，单位为秒）。
- 例如：`"Hello<#1.5#>world"` 表示单词之间有 1.5 秒的暂停。
...
|| `reference/getting-started.md` | 环境配置 |
|| `reference/audio-guide.md` | 音频处理相关知识 |
|| `reference/voice-guide.md` | 语音克隆与定制方法 |
|| `reference/troubleshooting.md | 常见问题及解决方法 |

## 常见问题与解决方法
1. **API 密钥未设置：** 运行 `export MINIMAX_VOICE_API_KEY="your-key"` 命令进行设置。
2. **FFmpeg 未安装：** 在 macOS 上使用 `brew install ffmpeg`，在 Ubuntu 上使用 `sudo apt install ffmpeg` 安装。
3. **找不到所需语音：** 使用 `python mmvoice.py list-voices` 命令查看可用的语音列表。
...
更多解决方案请参阅 `reference/troubleshooting.md`。