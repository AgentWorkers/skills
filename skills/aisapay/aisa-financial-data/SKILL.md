...
**参数：**
- `ticker`：股票代码（必填）
- `interval`：时间间隔（秒、分钟、天、周、月、年）（必填）
- `interval_multiplier`：时间间隔的倍数，例如 5 表示 5 分钟的间隔（必填）
- `start_date`：开始日期（YYYY-MM-DD 格式）（必填）
- `end_date`：结束日期（YYYY-MM-DD 格式）（必填）
...
**注意：** 加密货币的代码格式为 `SYMBOL-USD`（例如：`BTC-USD`、`ETH-USD`）。
...
| 端点 | 方法 | 描述 |
|----------|--------|-------------|
| `/financial/prices` | GET | 历史股票价格（需要指定时间间隔） |
...
| `/financial/sec/items` | GET | SEC 文件内容 |
...
| 财务报表 | 约 0.002 美元 |
| 分析师预测 | 约 0.002 美元 |
| 美国证券交易委员会（SEC）文件 | 约 0.001 美元 |
| 加密货币实时价格 | 约 0.0005 美元 |
| 加密货币历史数据 | 约 0.001 美元 |

每个 API 请求的响应中都会包含 `usage.cost` 和 `usage.credits_remaining` 字段。
