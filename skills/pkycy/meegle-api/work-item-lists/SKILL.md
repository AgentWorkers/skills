...
description: Meegle OpenAPI 用于列出、搜索和查询工作项。支持单空格过滤、跨空格过滤、复杂搜索、全文搜索，以及关联项的查询。使用前提：需要具备 token 和 domain 的权限——详情请参见 skill meegle-api-users。
metadata:
  openclaw: {}
...
用于列出、筛选、搜索和查询 Meegle 空间中的工作项。这些 API 仅支持读取操作。

**前提条件：** 首先获取域名和访问令牌；请参阅相关技能文档 **meegle-api-users**。

---

## 获取单个工作项列表

根据指定空间（`project_key`）内的条件查询工作项列表。排除已终止的工作项。此 API 仅支持读取操作，适用于数据检索、分析和汇总。

### 使用场景

- 当需要按名称、负责人、状态、类型、日期等条件筛选工作项时。
...
- **user_keys**：最多支持 10 个用户键，用于筛选创建者、观察者或角色成员。可通过双击用户头像获取这些用户键。
- **work_item_ids / work_item_type_keys**：用于缩小搜索范围，仅返回特定工作项或类型。
- **created_at / updated_at**：使用 13 位毫秒级时间戳；省略 `end` 表示“直到现在”。
- **search_id**：如果提供该参数，将覆盖所有其他筛选条件。
...
在多个空间中查询符合条件的工作项列表。适用于全局搜索、跨项目统计和分析。此 API 仅支持读取操作。
...
- 当需要结合多个字段条件（例如状态、优先级和负责人）进行搜索时。
...
- **search_group**：包含基础筛选信息（`conjunction`（AND/OR）、`search_params`（字段条件数组）以及可选的嵌套 `search_groups` 的对象，用于实现复杂逻辑。
- **search_params**：每个条目包含 `param_key`（字段键）、`operator`（如 `HAS ANY OF`、`=`、`!=`、`IN`）和 `value`（值数组）。
- **fields**：使用正数值指定需要返回的字段，或使用 `-` 前缀来排除某些字段；请勿混合使用不同模式。
...
在多个 Meegle 空间中进行全文搜索。支持按标题、描述及相关字段搜索工作项或视图。返回排名靠前的结果（最多 200 条）。

### 使用场景

- 当需要跨空间按关键词（如标题、描述等）搜索时。
...
- **project_keys**：需要搜索的空间列表。
- **query_type**：`workitem` 或 `view`——分别表示搜索工作项或视图。
- **query_sub_type**：当 `query_type=workitem` 时，指定具体的工作项类型（如故事、任务、漏洞）。
...
检索与单个 Meegle 空间中的指定工作项相关联的工作项。通过配置的关联字段（`relation_key`）来查询相关项。

### 使用场景

- 当需要获取关联的工作项（例如故事中的子任务）时。
...
- **relation_work_item_type_key**：需要返回的相关工作项类型（例如任务、漏洞）。
...
在指定的 Meegle 空间中使用全文搜索功能，支持复杂的筛选条件以及按需选择字段。支持基于游标的深度分页。

### 使用场景

- 当需要在单个空间中使用复杂的筛选条件（如 `SearchGroup`）并进行字段选择时。
- 当处理大量结果时，使用基于游标的分页功能（`search_after`）。
- 当仅需要某些字段时，通过设置 `page_size` 来减少数据传输量。
...
- **search_group**：与 `search_work_items_complex` 中的 `SearchGroup` 结构相同，包括连接词（AND/OR）和字段条件数组 `search_params` 以及嵌套的 `search_groups`。
