...
检索指定Meegle空间和类型下的一个或多个工作项的详细信息。返回包括系统字段、自定义字段、工作流状态和节点/状态在内的完整详情页面数据。

### 使用场景

- 当需要根据ID获取特定工作项的完整详情时。
...
更新指定空间和类型下的一个或多个可编辑的工作项字段。只能修改元数据和权限允许的字段。模板和计算字段不支持修改。
...
删除指定空间和类型下的工作项实例。此操作会永久删除工作项，并需要具备相应的权限。
...
批量更新指定空间和类型下的多个工作项实例的单一字段。支持APPEND、UPDATE和REPLACE三种模式，并对字段类型有限制。

### 使用场景

- 当需要同时更新多个工作项的同一字段时（例如批量更改状态、批量分配负责人）。
...
更新、添加或删除Meegle工作项中的复合字段组。支持对复合字段组的数据进行添加、更新或删除操作。
...
查询指定Meegle工作项资源库实例的详细信息，包括字段数据。仅支持启用了资源库的工作项；非资源实例和值为空的字段不会被返回。
...
更新指定Meegle工作项资源库实例的字段。仅支持资源字段；非资源字段或角色将被忽略。
...
根据复杂的过滤条件搜索并列出Meegle中的工作项资源库实例。支持通过`search_after`进行分页和按需选择字段。仅当工作项类型启用了资源库时才可用。
...
在指定的项目空间和类型下创建一个工作项资源库实例。工作项类型必须在高级配置中启用了“资源库”功能。
...
使用资源库工作项作为来源创建工作项实例。在启用了工作项资源库功能后，可以使用此接口从资源条目创建相应的工作项实例。
...
- **field_value_pairs**：在此处发送其他字段（描述、优先级、负责人等）；对于选项类型的字段，请使用选项ID，而不是显示标签。
...
- 请确保遵循上述API规范和使用场景，以保持代码的准确性和一致性。
- 对于代码示例、命令和URL，请保持原样不变。
- 技术术语（如OpenClaw、ClawHub、API、CLI等）保持英文形式。
- 仅翻译代码块中的注释，如果它们具有解释性。