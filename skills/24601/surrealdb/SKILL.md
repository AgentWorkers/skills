---
name: surrealdb
description: "**SurrealDB 3 专家级架构师与开发人员技能**  
精通 SurrealQL 查询语言，具备多模型数据建模能力（文档型、图模型、向量模型、时间序列模型、地理空间模型），擅长数据库架构设计、安全性配置、部署优化以及性能调优。能够实现与多种编程语言（JavaScript、Python、Go、Rust）的 SDK 集成。此外，还熟悉 Surrealism WASM 扩展技术以及完整的 SurrealDB 生态系统（包括 Surrealist、Surreal-Sync、SurrealFS 等组件）。这些技能使其成为开发 30 多种 AI 代理系统的理想人选。"
license: MIT
metadata:
  version: "1.2.0"
  author: "24601"
...
| `uv run {baseDir}/scripts/schema.py introspect` | 输出正在运行的 SurrealDB 实例的完整架构信息 |
| `uv run {baseDir}/scripts/schema.py tables` | 列出所有表格及其字段数量和索引 |
| `uv run {baseDir}/scripts/onboard.py --agent` | 代理集成的 JSON 功能列表 |

## 先决条件

- **surreal CLI** -- 在 macOS 上使用 `brew install surrealdb/tap/surreal` 进行安装；更多信息请参阅 [安装文档](https://surrealdb.com/docs/surrealdb/installation) |
- **Python 3.10+** -- 用于技能脚本的运行 |
- **uv** -- 在 macOS 上使用 `brew install uv` 或 `pip install uv` 进行安装；更多信息请参阅 [uv 文档](https://docs.astral.sh/uv/getting-started/installation/) |
...
> **身份验证警告**：以下示例仅适用于 **本地开发**，切勿在生产环境或共享实例中使用默认凭据。请为非本地环境创建具有有限权限的用户账户。
...
内置的向量相似性搜索功能，支持 HNSW 算法和暴力索引。可以定义向量字段，并使用可配置的距离度量（余弦、欧几里得、曼哈顿、明可夫斯基）创建索引；通过 `vector::similarity::*` 函数进行查询；支持在 SurrealQL 中构建 RAG（Retrieval with Aggregation）管道和语义搜索。

详情请参阅：`rules/vector-search.md`

### 安全性与权限控制

支持基于 `DEFINE TABLE ... PERMISSIONS` 的行级安全性；支持命名空间/数据库/记录级别的访问控制；支持 JWT/令牌认证；支持 `DEFINE USER` 用于系统用户认证；`$auth`/`$session` 运行时变量用于权限判断。

详情请参阅：`rules/security.md`

### 部署与运维

支持单二进制文件部署、Docker、Kubernetes（Helm 图表）；支持多种存储引擎（内存、RocksDB、SurrealKV、TiKV）；支持备份/恢复、监控及生产环境优化。

详情请参阅：`rules/deployment.md`

### 性能调优

提供索引策略（唯一索引、搜索索引、向量 HNSW 索引、MTree 索引）；支持 `EXPLAIN` 用于查询优化；支持连接池管理、存储引擎选择及资源限制设置。

详情请参阅：`rules/performance.md`

### SDK 集成

提供官方 SDK（JavaScript/TypeScript、Python、Go、Rust、Java、.NET、C、PHP、Dart）；支持 HTTP/WebSocket 连接协议、认证流程、实时查询订阅及类型化记录处理。

详情请参阅：`rules/sdks.md`

### Surrealism WASM 扩展

SurrealDB 3 新功能：支持使用 Rust 编写的自定义函数和逻辑，并将其编译为 WASM 在数据库中运行；支持定义、部署和管理这些扩展模块。
...
`doctor` 脚本用于验证以下内容：surreal CLI 是否已安装并添加到 PATH 中；服务器是否可访问；认证是否成功；命名空间和数据库是否存在；版本是否兼容；存储引擎的状态是否正常。
...
| 规则文件 | 所涵盖内容 |
|-----------|----------|
| `rules/surrealql.md` | SurrealQL 语法、语句、函数、操作符和常用用法 |
| `rules/data-modeling.md` | 数据库架构设计、记录 ID、字段类型、关系定义和规范化 |
| `rules/graph-queries.md` | 图谱遍历操作符、路径表达式和递归查询 |
| `rules/vector-search.md` | 向量字段、HNSW/暴力索引、相似性函数和 RAG 模式 |
| `rules/security.md` | 权限控制、访问认证和 JWT 认证机制 |
...
| `rules/surrealism.md` | WASM 扩展功能及自定义函数的编写 |
| `rules/surrealist.md` | SurrealDB 的 IDE 和 GUI 工具使用方法 |
| `rules/surreal-sync.md` | 数据迁移工具和迁移工作流程 |
| `rules/surrealfs.md` | AI 代理文件系统和元数据管理 |

## 工作流程示例

> **所有工作流程示例仅适用于本地开发**。在生产环境中，请使用具有有限权限的用户账户（`DEFINE USER`）。
...
将所有跟踪仓库的当前 HEAD SHA 和版本标签与 `SOURCES.json` 中的基准值进行比较，以便规划增量式更新。
...
这意味着 `2>/dev/null` 会隐藏文本输出，而将 `stdout` 重定向到 `/dev/null` 可生成干净的 JSON 数据，以便后续处理。