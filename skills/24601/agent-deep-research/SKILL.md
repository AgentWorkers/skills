---
name: deep-research
description: 通过 Gemini Interactions API 进行异步深度研究（无需依赖 Gemini CLI）。支持对本地文件进行基于 RAG（Retrieval, Attention, Generation）的查询（使用 `--context` 参数），可预览查询成本（使用 `--dry-run` 参数），输出结构化的 JSON 数据，并支持自适应轮询机制。该技能适用于 30 多种 AI 工具，包括 Claude Code、Amp、Codex 和 Gemini CLI。
license: MIT
metadata:
  version: "1.3.0"
  author: "24601"
...
该技能利用 Google Gemini 的深度研究功能进行深入研究。用户可以将文档上传到文件搜索存储库中，以获得基于 RAG（Retrieval, Augmentation, and Generation）技术的答案。同时，该技能支持通过持久的工作区状态来管理研究会话。
...
| `uv run {baseDir}/scripts/research.py start "问题" --context ./路径 --dry-run` | 估算研究成本 |
...
| 变量 | 描述 |
|----------|-------------|
| `GEMINI_DEEP_RESEARCH_API_KEY` | 专门用于此技能的 API 密钥（最高优先级） |
| `GOOGLE_API_KEY` | 标准的 Google AI API 密钥 |
| `GEMINI_API_KEY` | Gemini 专用的 API 密钥 |

可选的模型配置：

| 变量 | 描述 | 默认值 |
|----------|-------------|---------|
| `GEMINI_DEEP_RESEARCH_MODEL` | 用于文件搜索查询的模型 | `models/gemini-flash-latest` |
| `GEMINI_MODEL` | 备用模型名称 | `models/gemini-flash-latest` |
| `GEMINI_DEEP_RESEARCH_AGENT` | 深度研究代理的标识符 | `deep-research-pro-preview-12-2025` |
...
| `--output FILE` | 等待研究完成后将结果保存到单个文件中 |
...
| `--context-extensions EXT` | 按文件扩展名过滤上传的上下文（例如 `py,md` 或 `.py .md`） |
...
这是推荐的人工智能代理集成方式——代理接收一个简短的 JSON 数据包，而完整报告则会被写入磁盘。

## 自适应轮询

当使用 `--output` 或 `--output-dir` 时，脚本会持续轮询 Gemini API 直到研究完成。默认情况下，脚本会使用**自适应轮询**机制，该机制会根据之前的研究完成时间进行调整：

- 完成时间记录在 `.gemini-research.json` 文件的 `researchHistory` 部分（最近 50 条记录，区分基于 RAG 的研究和非基于 RAG 的研究）。
- 当存在 3 个或更多匹配的数据点时，轮询间隔会根据历史数据进行调整：
  - 在任何研究完成之前：轮询间隔较慢（30 秒）
  - 在可能的完成时间范围内（25%–75%）：轮询间隔较快（5 秒）
  - 在完成时间较晚的情况下（超过平均完成时间的 75%）：轮询间隔适中（15–30 秒）
  - 对于耗时异常长的研究（超过平均完成时间的 1.5 倍）：轮询间隔较慢（60 秒）
- 所有轮询间隔都被限制在 [2 秒, 120 秒] 之间，作为安全措施。

当历史数据不足（<3 个数据点）或使用了 `--no-adaptive-poll` 选项时，将使用固定的轮询间隔：前 30 秒为 5 秒，30 秒到 2 分钟为 10 秒，2 分钟到 10 分钟为 30 秒，10 分钟以上为 60 秒。
...
向标准输出打印一个 JSON 成本估算，其中包含上下文上传成本、研究查询成本以及总成本。这些估算基于启发式方法（Gemini API 不返回令牌计数或计费数据），并且会明确标注。
...
使用 `--force` 可以跳过确认提示。当标准输入不是终端（例如由 AI 代理调用时）时，会自动跳过确认提示。
...
成功上传后，系统会保存哈希缓存，因此后续的 `--smart-sync` 命令会正确地跳过未更改的文件，即使第一次上传时没有使用 `--smart-sync` 选项。

### MIME 类型支持

Gemini 文件搜索 API 支持 36 种文件扩展名。常见的编程文件（JS、TS、JSON、CSS、YAML 等）会通过回退机制自动以 `text/plain` 格式上传。二进制文件会被拒绝。详细列表请参见 `references/file_search_guide.md`。
...
在任何状态相关命令后添加 `--json` 选项，可以将结构化 JSON 数据输出到标准输出：
...
使用 `-y` 可以跳过确认提示。当标准输入不是终端（例如由 AI 代理调用时）时，会自动跳过确认提示。

## 非交互式模式

当标准输入不是终端时，所有确认提示（如 `store.py delete`、`state.py clear`）都会被自动跳过。这样 AI 代理和持续集成管道就可以在不需要等待交互式提示的情况下调用这些命令。
...
这意味着 `2>/dev/null` 可以隐藏人类可读的输出，而将标准输出重定向到 `/dev/null` 可以得到干净的 JSON 数据。