---
name: openocr-skills
description: 使用 OpenOCR 从图像、文档和扫描的 PDF 文件中提取文本——支持文本检测、识别、通用的 VLM（Visual Language Model）识别以及带有布局分析的文档解析功能。
author: openocr
version: "0.1.4"
...
该技能利用 **OpenOCR**（一个精确且高效的通用 OCR 系统）实现智能文本提取、文档解析和通用识别功能。它提供了一个统一的接口，支持文本检测、文本识别、端到端 OCR、基于 VLM 的通用识别（文本/公式/表格）以及带有布局分析的文档解析，支持中文、英文等多种语言。

## 使用方法

1. 提供图片、扫描的文档或 PDF 文件。
2. （可选）指定任务类型（检测/识别/OCR/通用识别/文档解析）。
3. 系统将提取文本、公式、表格或完整的文档结构。
...
1. **选择合适的任务类型**：使用 `ocr` 进行一般文本识别，使用 `unirec` 进行公式/表格识别，使用 `doc` 进行完整文档解析。
2. **使用移动模式提高速度**：`mode='mobile'` 速度更快；仅在需要高精度时使用 `mode='server'`。
3. **使用 ONNX 后端**：默认的 ONNX 后端可在 CPU 上运行，无需额外依赖。
4. **设置合适的阈值**：根据实际需求调整 `drop_score`（OCR）和 `layout_threshold`（文档解析）。
5. **启用布局检测**：对于包含混合内容（文本 + 公式 + 表格）的文档，务必启用 `use_layout_detection`。
6. **批量处理**：使用 `rec_batch_num` 控制识别批次大小以优化吞吐量。
...
- 文本识别精度受图片质量影响。
- 非常小或严重旋转的文本可能会降低识别精度。
- `server` 模式需要 PyTorch，且速度比 `mobile` 模式慢。
- UniRec 和 Doc 任务使用 0.1B 大小的 VLM 模型，更大的模型可能获得更好的识别效果。
- PDF 处理会将页面转换为图片，处理大型 PDF 文件时可能会占用大量内存。
- 复杂的手写文本识别精度会有所波动。
- 建议使用 GPU 以获得最佳性能，尤其是在处理文档和公式识别任务时。
...
- [UniRec 文章](https://github.com/Topdu/OpenOCR#unirec)
