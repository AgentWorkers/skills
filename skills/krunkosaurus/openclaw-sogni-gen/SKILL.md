...
name: sogni-gen
version: "1.5.4"
description: 使用 Sogni AI 的去中心化网络生成图像和视频。你可以要求代理根据提示或参考图像来“绘制”、“生成”图像，或“制作视频/动画”。
...
| `--ref <path>` | 视频或面部转移的参考图像 | 必需 |
| `--ref-end <path>` | i2v 插值的结束帧 | - |
| `--ref-audio <path>` | s2v 的参考音频 | - |
| `--ref-video <path>` | 动画/视频转换的参考视频 | - |
| `--controlnet-name <name>` | v2v 的 ControlNet 类型 | canny | pose | detailer | - |
| `--controlnet-strength <n>` | ControlNet 强度（0.0-1.0） | 0.8 |
| `--sam2-coordinates <coords>` | SAM2 点击坐标（用于 animate-replace） | x,y 或 x1,y1;x2,y2 | - |
...
| `--extract-last-frame <video> <image>` | 从视频中提取最后一帧（使用安全的 ffmpeg 包装器） | - |
| `--concat-videos <out> <clips...>` | 合并视频片段（使用安全的 ffmpeg 包装器） | - |
| `--list-media [type]` | 列出最近的传入媒体（图片 | 音频 | 全部） | images |
...
使用参考图像编辑图片（Qwen 模型支持最多 3 张参考图像）：
...
当没有使用 `-m` 选项提供参考图像时，默认使用 `qwen_image_edit_2511_fp8_lightning` 模型。

## 面部转移（Photobooth）

使用 InstantID ControlNet 从面部照片生成风格化的肖像。当用户请求“photobooth”或希望将自己的面部转移到某个风格中时，使用 `--photobooth` 并指定面部图像作为 `--ref`。
...
`--angles-360-video` 会在连续角度之间生成 i2v 视频片段（包括从最后到第一个角度），并使用 ffmpeg 将它们合并成无缝循环。
...
3. **始终使用第一帧/最后一帧拼接**：`--angles-360-video` 标志会自动处理这一点，通过在连续角度之间生成 i2v 视频片段来实现无缝循环。
...
“资金不足。请在 https://app.sogni.ai 上领取每日 50 个免费 Spark 积分。”
...
ControlNet 类型：`canny`（边缘检测）、`pose`（人体姿态）、`depth`（深度图）、`detailer`（细节增强）。
...
- **认证错误**：检查 `~/.config/sogni/credentials` 中的凭据信息
- **i2v 尺寸问题**：视频尺寸有限制（最小 480px，最大 1536px，必须是 16 的倍数）。对于 i2v，客户端封装工具会调整参考图像的大小（`fit: inside`），并使用调整后的尺寸作为最终视频尺寸。由于存在舍入，请求的尺寸可能会导致最终尺寸无效（例如：请求 `1024x1536`，但实际尺寸可能变为 `1024x1535`）。
- **自动调整**：如果使用本地 `--ref`，脚本会自动调整请求的尺寸以避免非 16 的参考尺寸。
- **如果希望强制忽略自动调整**：可以设置 `--strict-size`，此时脚本会显示建议的 `--width/--height`。
