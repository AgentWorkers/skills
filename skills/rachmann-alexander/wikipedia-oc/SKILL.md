...
所有搜索操作 **必须** 在英文版维基百科（语言代码为 `"en"`）中进行，以确保结果的一致性和覆盖范围的最大化。
...
这样可以确保所有搜索查询和页面检索都在英文版维基百科中执行。



## 搜索页面

使用 `wikipedia.search()` 函数来查找候选页面。
...
处理搜索结果的工作流程：
1. 执行 `wikipedia.search(query, results=3)` 以减少无关信息的干扰。
2. 选择最相关的页面标题。
3. 使用选定的标题来获取完整的页面内容。
...
如果查询具有歧义，维基百科可能会抛出 `DisambiguationError` 异常。
...
内容的准确性取决于维基百科本身的质量。
页面摘要可能会遗漏一些重要的细节；因此，在需要时才应获取完整的内容。