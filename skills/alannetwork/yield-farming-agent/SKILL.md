...
description: 基于 BNB Chain 的自主收益 farming（ farming 指通过投资虚拟货币来获取收益的过程）代理，具备确定性执行能力、智能合约集成功能以及自动化决策机制。
---
# YieldVault Agent

这是一个专为 BNB Chain 设计的自动化收益 farming（收益收割）代理工具，具备确定性执行能力、智能合约集成以及自动化决策功能。

## 主要特性

- **确定性决策引擎**：相同的输入总是产生相同的输出（可审计）  
- **智能合约集成**：能够与 BNB 测试网/主网上的 YieldVault 合约进行交互  
- **自动调度器**：每小时自动执行收益收割决策，无需人工干预  
- **交易执行器**：自动执行存款（DEPOSIT）、取款（WITHDRAW）、收益收割（HARVEST）、复利（COMPOUND）和资金再平衡（REBALANCE）操作  
- **Telegram 通知**：实时推送执行结果、年化收益率（APR）变化以及错误信息  
- **风险管理**：仅选择风险评分（risk_score）≤ 0.5 的资金池进行操作  
- **收益优化**：计算净年化收益率（Net APR = APR - 费用 - 风险惩罚）
...
系统会自动通过 Telegram 发送以下通知：
- 执行开始（包括资金池 ID、执行的操作类型和金额）
- 年化收益率变化（变化幅度 > 1%）
- 错误信息（附带错误等级）
- 收益收割周期完成（包含统计摘要）
...
1. **读取** 当前资金池的状态（年化收益率 APR、总价值 TVL、用户余额）  
2. **计算** 净年化收益率（Net APR = APR - 费用 - 风险评分 × 0.10）  
3. **筛选** 风险评分 ≤ 0.5 的资金池  
4. **选择** 净年化收益率最高的资金池  
5. **决定** 执行的操作：
   - 如果待收割的收益 ≥ 25 美元，则执行收益收割（HARVEST）  
   - 如果净年化收益率变化 ≥ 2%，则执行复利操作（COMPOUND）  
   - 如果有其他资金池的收益比当前资金池高 ≥ 2%，则执行资金再平衡操作（REBALANCE）  
   - 如果资金池已达到最优状态，则不执行任何操作（NOOP）  
6. **执行** 交易（包含重试机制）  
