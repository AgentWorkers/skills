...
name: youtube
description: >
  **YouTube 数据 API 与托管型 OAuth 的集成**  
  该功能支持搜索视频、管理播放列表、访问频道数据以及与评论进行交互。当用户需要与 YouTube 进行交互时，可以使用此技能。对于其他第三方应用程序，请使用 `api-gateway` 技能（https://clawhub.ai/byungkyu/api-gateway）。
...
您可以使用托管的 OAuth 认证来访问 YouTube Data API v3。该 API 允许您搜索视频、管理播放列表、获取频道信息，以及与评论和订阅相关的内容进行交互。
...
如果您有多个 YouTube 连接，可以使用 `Maton-Connection` 标头来指定使用哪个连接：
...
查询参数：
- `part` - 必需参数：`snippet`（返回视频、频道或播放列表的详细信息）
- `q` - 搜索查询
- `type` - 过滤类型：`video`（视频）、`channel`（频道）、`playlist`（播放列表）
- `maxResults` - 每页显示的结果数量（1-50，默认为 5）
- `order` - 排序方式：`date`（时间）、`rating`（评分）、`relevance`（相关性）、`title`（标题）、`viewCount`（观看次数）
...
- 播放列表 ID 以 `PL`（用户创建的）或 `UU`（用户上传的）开头
- 使用 `pageToken` 进行分页查询
- `part` 参数是必需的，它决定了返回的数据类型
- 不同端点的使用量费用不同：搜索操作费用较高（100 单位），读取操作费用较低（1 单位）
- 某些写入操作需要频道验证
...
| 403 | 禁止操作——超出配额或权限不足 |
| 404 | 视频、频道或播放列表未找到 |
| 429 | 每账户每秒的请求次数受限（10 次） |
| 4xx/5xx | 来自 YouTube API 的传递错误 |

### 故障排除：API 密钥无效

**当收到 “Invalid API key” 错误时，请务必按照以下步骤操作，再判断是否存在问题：**
