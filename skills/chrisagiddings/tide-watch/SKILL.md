...
**建议选择**：  
- **仅使用指令**：如果您只需要容量警告和恢复提示功能。  
- **使用CLI工具**：如果您需要手动管理会话、存档或查看仪表盘数据。  
...
**总结：** v1.0.0版本中的`editResumePrompt`命令存在shell注入漏洞，攻击者可通过`--session`参数执行任意命令。该问题已在v1.0.1版本中通过将`execSync`替换为`spawnSync`得到修复。  

**如果您安装了v1.0.0版本，请** **立即更新至v1.0.1**。  
...
**使用CLI工具模式（如需）：**  
1. ⚠️ **确认使用版本1.0.1或更高版本**（`tide-watch --version`）。  
2. ⚠️ **安装前检查代码：**  
   - 查看`lib/capacity.js`和`lib/resumption.js`文件。  
   - 确认`package.json`中没有安装钩子。  
...
- `tide-watch resume-prompt edit --session <id>` – 打开编辑器（v1.0.1版本已修复漏洞）。  
...
- 移动文件：`~/.openclaw/agents/main/sessions/archive/`（存档会话）。  
...
- **依赖项：** 无。  
- **二进制文件：** 无。  
- **安装方式：** 将模板文件复制到工作区配置文件中。  

**模式2（使用CLI工具）：**  
- **Node.js**：版本14.0.0或更高。  
- **npm**：任意最新版本。  
- **依赖项：**  
  - 开发环境：`jest@^30.2.0`（仅用于测试）。  
  - 运行时环境：无依赖项。  
...
**推荐：**  
**大多数用户应选择仅使用指令模式**。这种方式无需安装任何代码即可实现自动容量监控。仅在使用手动管理功能时才需要安装CLI工具。  

## 功能说明：**  
该工具会监控您的OpenClaw会话状态，并在会话容量达到阈值时发出警告：  
- 🟡 **75%** – 提醒您尽快结束当前任务。  
- 🟠 **85%** – 建议完成当前任务并重置会话。  
- 🔴 **90%** – 会话即将锁定，建议立即保存数据。  
- 🚨 **95%** – 严重警告！立即保存会话数据！  
...
### 第3步：配置设置（可选）  
默认设置适用于大多数用户，但您也可以在`AGENTS.md`中进行自定义：  
...
- **频道特定设置**（高级选项）：  
  - 为不同频道（Discord、Webchat、私信）设置不同配置。  
...
## 工作原理：**  
### 自动监控（心跳检测）：**  
当您在`HEARTBEAT.md`中启用Tide Watch后，我会：  
1. **解析配置文件**（`AGENTS.md`），设置检查频率和警告阈值。  
2. **按计划检查会话容量**（默认：每小时一次）。  
3. **计算会话使用率**（`tokens_used / tokens_max * 100`）。  
4. **与阈值进行比较**。  
5. **根据阈值发出警告**。  
6. **自动备份（如果启用）**：  
  - 检查是否达到备份阈值。  
...
8. **清理旧备份**：  
  - 删除超过保留时间的备份文件（默认：7天）。  
9. **恢复静默模式**：  
  - 如果会话容量低于所有阈值，返回`HEARTBEAT_OK`状态，不产生任何输出或中断。  
...
### 主要特点：**  
- **兼容性高**：适用于任何提供会话服务的平台（Anthropic、OpenAI、DeepSeek等）。  
- **非侵入式**：仅在达到阈值时发出警告。  
- **可配置**：可根据需求调整阈值、检查频率和操作方式。  
- **状态记录**：记录每次警告的阈值和会话重置情况。  

**使用理由：**  
**问题**：会话使用量会逐渐增加，可能导致会话锁定并停止响应。**  
**解决方案**：通过主动监控提前发现容量问题，让您有时间保存数据、切换频道或重置会话。  

**实际案例**：  
在Discord #navi-code-yatta中，某次会话使用率达到97%并锁定（2026年2月23日），导致数据丢失。  
...
- **技术支持**：https://clawhub.ai/chrisagiddings/tide-watch  
