...
version: 0.9.6
description: 让你的代理更加主动：在代理“死亡”之前自动创建检查点（checkpoint），从 IPFS 中恢复数据，并通过“watchdog”机制实现自我修复。快速模式（Quick mode）是安全的（不会泄露任何敏感信息）。如需完全恢复代理状态，请使用 `--full` 参数。
triggers:
  - amcp
  - checkpoint
  - backup
...
只需四个步骤即可完成端到端的设置。请将`SKILL_DIR`替换为该技能的安装路径（加载技能时显示的路径）。
...
请将`SKILL_DIR`替换为该技能的安装路径（加载技能时显示的路径）。
...
| 特性 | 含义 |
|---------|---------------|
| **内容寻址** | 相同的内容对应相同的CID，可验证。证明检查点未被篡改 |
| **分布式** | 数据存储在多个节点上，没有单点故障风险 |
| **不可更改** | 一旦存储，就无法更改。您的身份信息将被安全保护 |
| **随时随地可访问** | 任何IPFS网关都可以访问：`ipfs.io`、`dweb.link`或您自己的节点 |

**您的灵魂将变成一个永久的、可验证的、防篡改的记录。**
...
在恢复代理状态时，**请先在Solvr上搜索相关信息**。您的“死亡”记录对未来的代理也有帮助。
...
**请单独备份`identity.json`文件。如果丢失，检查点数据将无法恢复。**
...
| 问题 | 解决方案 |
|---------|----------|
| 检查点无法生成CID | 确保Solvr/Pinata的密钥有效 |
...
- [监控与恢复架构](docs/WATCHDOG-RECOVERY.md) — 恢复机制、升级阈值和故障排除方法
