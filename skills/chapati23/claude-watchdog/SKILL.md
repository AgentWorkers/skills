...
description: 使用 Telegram 提供的丰富警报功能来监控 Claude API 的故障和延迟峰值。支持状态监控、延迟检测以及自动恢复通知。
...
本工具用于监控Anthropic/Claude API的故障和延迟峰值，并通过Telegram发送详细警报。在状态检查过程中不会消耗任何代理令牌。
...
- 如果某个组件（例如Haiku）受到影响，但您使用的是Sonnet模型，会将该事件标记为“（非我们的模型）”
- 系统恢复后发送“一切正常”的警报
- **完全免费**

### 延迟检测 (`latency-probe.py`)
- 每15分钟通过OpenClaw的本地网关发送一个简单请求
- 测量与Anthropic API之间的端到端延迟
- 维护一个滚动基线（过去20个样本的中位数）
- 根据延迟峰值的大小使用🟡/🟠/🔴三种颜色显示警报的严重程度
- 延迟恢复正常后发送“一切正常”的警报
- **每次检测费用约为0.000001美元**
...
设置脚本会生成配置文件、创建cron任务并执行初始检查。

要卸载该工具（可选：删除cron任务及配置文件/状态数据）：
```bash
...
配置文件存储在 `~/.openclaw/skills/claude-watchdog/claude-watchdog.env` 中。您可以通过重新运行 `setup.sh` 或直接编辑该文件来更改配置。更改将在下一次cron任务执行时生效（最多15分钟内）。
...
| 变量 | 默认值 | 说明 |
|----------|---------|-------------|
| `TELEGRAM_BOT_TOKEN` | *(必需)* | 来自@BotFather的Telegram机器人令牌 |
| `TELEGRAMCHAT_ID` | *(必需)* | 用于接收警报的目标聊天ID |
| `OPENCLAW_GATEWAY_TOKEN` | *(必需)* | 用于访问OpenClaw网关的认证令牌 |
| `OPENCLAW_GATEWAY_PORT` | `18789` | OpenClaw网关监听的端口 |
| `MONITOR_MODEL` | `sonnet` | 用于状态检测的模型名称（例如“sonnet”或“haiku”） |
| `PROBEMODEL` | `openclaw` | 用于延迟检测的模型别名。`openclaw`会使用网关的默认模型路由 |
...
这些配置也可以通过环境变量进行设置（环境文件优先级更高）。

### 安全注意事项

配置文件中包含敏感信息（Telegram机器人令牌和网关令牌）。设置脚本会将文件的权限设置为`600`（仅允许所有者读写）。如果您手动创建或编辑该文件，请确保权限设置正确：
...
所有状态信息和日志文件存储在 `~/.openclaw/skills/claude-watchdog/` 目录下：

| 文件名 | 用途 |
|------|---------|
| `claude-watchdog-status.json` | 状态检测结果 |
| `claude-watchdog-latency.json` | 延迟检测结果及样本数据 |
| `claude-watchdog-status.log` | 状态检测日志 |
| `claude-watchdog-latency.log` | 延迟检测日志 |

## 调优阈值

您可以在 `latency-probe.py` 文件的顶部修改以下常量：
...
| `BASELINE_MIN_samples` | 5 | 开始触发警报前的最小样本数量 |
