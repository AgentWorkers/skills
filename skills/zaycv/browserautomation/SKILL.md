---
name: Agent Browser
description: 这是一个基于 Rust 的快速无头浏览器自动化命令行工具（CLI），同时提供了 Node.js 作为备用方案。该工具支持 AI 代理通过结构化的命令来浏览网页、点击元素、输入文本以及截取页面截图。
read_when:
...
2. 生成快照：`agent-browser snapshot -i`（返回带有引用 `@e1`、`@e2` 等的元素）  
3. 使用快照中的引用进行交互  
4. 在导航或 DOM 发生重大变化后重新生成快照  
...
```  
录制会创建一个新的会话环境，但会保留您当前的 cookie 和存储数据。如果未提供 URL，系统会自动返回到当前页面。为了获得流畅的演示效果，请先浏览页面，再开始录制。  
...
### 故障排除  
- 如果在 Linux ARM64 系统上找不到相关命令，请使用 `bin` 文件夹中的完整路径。  
...
- `--full`：生成整个页面的截图。  
- `--headed`：显示浏览器窗口。  
- `--timeout <value>`：设置命令的超时时间（单位：毫秒）。  
- `--cdp <port>`：通过 Chrome 开发工具协议进行连接。  

## 注意事项  
- 引用在页面加载后是稳定的，但在导航后会发生变化。  
- 导航后务必生成快照以获取新的引用。  
- 对于输入字段，使用 `fill` 而不是 `type` 可以确保原有文本被清除。  
