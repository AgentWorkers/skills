---
name: video-analyzer
description: 从 YouTube、X/Twitter 和 TikTok 下载视频，对其进行转录和分析，并利用本地 Whisper 处理技术进行处理。该工具非常适合提取视频的精华内容（TL;DR）、时间戳以及有用的信息。适用于需要转录视频、总结 YouTube 视频内容、从播客或演讲中提取关键点、分析视频中的对话、获取长视频的时间戳等情况；同时也适用于用户分享 YouTube、X/Twitter 或 TikTok 视频链接并希望了解视频内容时使用。
...
当用户请求您对视频或音频进行总结、转录或下载时，请使用随工具提供的 Python 脚本：
...
### Local Whisper 的质量设置
如果脚本需要使用 `Whisper` 作为备用方案（例如处理来自 X 或 Twitter 的视频），默认使用 `normal` 质量设置：
- `normal`：快速处理（30 分钟的视频大约需要 1 分钟）——**默认设置**
- `max`：最高质量（30 分钟的视频大约需要 5 分钟）——在需要高精度时请使用 `--quality max` 参数。

### 多语言支持
所有 `Whisper` 模型默认支持多语言。该工具可以转录多种语言的视频（如意大利语、西班牙语、日语等）。

**重要提示：**请始终用用户的语言回复他们，而不是视频的语言。例如，如果用户使用意大利语交流，但发送的是英语视频，那么请用意大利语提供摘要。

### 查找特定片段
转录结果中包含精确的时间戳（格式为 `[05:53] text...`）。如果用户询问“他们在什么时候提到了 X？”，请在转录文本中使用 `grep` 命令查找并返回相应的时间戳。