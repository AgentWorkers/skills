...
HZL（https://hzl-tasks.com）是一个以本地使用为主的任务管理工具，代理们可以通过它来：

- 将多步骤的工作分解为项目和小任务；
- 设置检查点以保存工作进度，确保工作在会话结束后仍能继续；
- 通过项目池将任务分配给合适的代理；
- 在多个代理之间协调工作，处理任务之间的依赖关系。

本技能将指导代理如何使用 `hzl` 命令行工具（CLI）。

## 何时使用 HZL

**OpenClaw 没有内置的任务跟踪功能。** 与 Claude Code（具有 TodoWrite 功能）或 Codex（具有 update_plan 功能）不同，OpenClaw 依赖内存和 Markdown 文件来记录工作进度。HZL 可以填补这一空白。

**适合使用 HZL 的场景：**
- 需要按顺序执行的多步骤项目；
- 需要跨会话持续进行的工作，或者涉及多个代理的工作；
- 需要“从上次停止的地方继续执行”的工作；
...
**经验法则：** 如果你需要制定多步骤的计划，或者担心无法在当前会话内完成任务，那么请使用 HZL。
...
| `hzl task prune ... --yes` | **永久删除** 已完成或存档的任务及历史记录。 |

**除非用户明确要求删除数据，否则切勿运行这些命令。** 删除操作无法撤销。
...
- **子任务（Subtask）**：任务的细分部分（`--parent <id>`）。最多支持一级嵌套。`hzl task claim --next` 命令不会返回父任务。
- **检查点（Checkpoint）**：用于会话恢复的进度快照。
- **租约（Lease）**：一种有限期的任务分配机制，有助于在多代理环境中检测任务是否卡住。
...
**任务池分配规则：** 在创建任务时不需要指定 `--agent` 参数。任何符合条件的代理都可以通过 `--next` 命令领取任务。
...
**代理任务分配：** 如果在创建任务时指定了 `--agent` 参数，只有指定的代理（或没有任务的代理）才能通过 `--next` 命令领取任务。没有代理分配的任务对所有代理都可见。
...
当需要将任务分配给特定代理时，在创建任务时使用 `--agent` 参数；如果任务可以由任意代理领取，则省略该参数。
...
必须指定 `--project` 参数，代理需要根据自己被分配到的项目来执行任务。可以使用 `--any-project` 来扫描所有项目（例如协调代理）。

这个命令可以同时处理租约到期后的任务恢复和新任务的领取。如果返回了任务，则继续执行该任务；如果没有返回任务，则表示任务队列为空。此时会应用代理任务分配规则：分配给其他代理的任务会被跳过。
...
在交接任务时，必须指定 `--agent` 和 `--project` 参数。省略 `--agent` 会导致任务被分配到任务池；此时需要指定 `--project` 来指定任务所属的池。
...
在重要的里程碑或暂停之前设置检查点。一个好的检查点应该能够回答：“如果当前会话结束，其他代理能否从这里继续执行任务？”
...
HZL 会在任务状态发生重要变化时发送通知——目前仅支持 `on_done` 事件。其他生命周期事件（如任务卡住、阻塞或进度变化）需要手动轮询。这是有意设计的：只有当有重要事件发生时，系统才会发送通知；其他情况需要代理或协调者手动轮询。

钩子的配置在安装过程中完成（详见文档）。作为代理，你需要了解以下操作细节：
- **只有 `on_done` 事件会触发通知。** 当任务完成时，HZL 会触发 Webhook。对于任务卡住、状态过期或进度变化等情况，需要使用 `hzl task stuck --stale` 或 `hzl task list` 命令进行轮询。
- **通知不会立即发送。** `hzl hook drain` 命令会按照 cron 计划（通常每 2–5 分钟执行一次）来发送通知。你的任务完成信息会立即被记录，但通知会在下一次轮询时才发送到目标地址。
- **通知中包含上下文信息。** 每条通知都会包含代理信息、项目信息和完整事件详情。HZL 会统一将通知发送到同一个地址，无论任务是由哪个代理完成的。
- **如果通知失败，请使用 `hzl hook drain --json` 命令查看失败原因和错误详情。**
...
**任务完成的标准：** “我完成了 [任务]” 应该描述一个具体的成果。
- ✓ “安装了车库运动传感器”（具体任务）
- ✗ “完成了家庭自动化系统”（这是一个开放性的任务描述，实际上无法完成）
...
可以通过 `http://<your-box>:3456` 访问（通过 Tailscale 进行访问）。在 macOS 上，可以使用 `hzl serve --background` 命令启动控制面板。
...
这些功能属于任务管理系统的上层逻辑，不属于 HZL 的职责范围。

---

## 其他注意事项

- 使用 `exec` 工具来运行 `hzl` 命令。
- 查阅 `TOOLS.md` 文件，了解你的身份信息、需要监控的项目以及与你的角色相关的命令。
