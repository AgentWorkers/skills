...
HZL（https://hzl-tasks.com）是一个以本地使用为主的任务记录系统，代理们可以通过它来：

- 将多步骤的工作分解为项目及任务；
- 设置检查点以保存工作进度，确保工作在会话结束后仍能被继续；
- 通过项目池将任务分配给合适的代理；
...
**OpenClaw 没有内置的任务跟踪功能。** 与 Claude Code（具有 TodoWrite 功能）或 Codex（具有 update_plan 功能）不同，OpenClaw 依赖内存和 Markdown 文件来跟踪工作进度。HZL 正是在这方面填补了这一空白。

**适合使用 HZL 的场景：**
- 需要按顺序执行的多步骤项目；
- 需要跨会话持续进行的工作，或者涉及多个代理的工作；
- 需要“从上次中断的地方继续执行”的工作；
- 需要将工作委托给其他代理，并在代理失败时能够恢复工作进度的情况。
...
| `hzl init --force --yes` | **不提示用户确认即可删除所有数据。** |
| `hzl task prune ... --yes` | **永久删除** 已完成/归档的任务及历史记录。 |
...
- **子任务（Subtask）**：任务的细分部分（使用 `--parent <id>` 标记）。子任务最多只能嵌套一层。`hzl task claim --next` 命令不会返回父任务。  
...
只有当需要指定特定代理时才使用 `--agent` 参数；当任务可以由任意代理处理时，使用 `--project` 参数。
...
该命令可以同时处理租约过期后的任务恢复和新任务的领取。如果返回了任务，就继续执行该任务；如果没有返回任务，说明队列为空。
...
在任务交接时，必须指定 `--agent` 和 `--project` 参数。省略 `--agent` 会导致任务被分配到任务池；此时需要使用 `--project` 来指定任务所属的池。
...
**HZL 默认支持跨项目依赖关系。** 可使用 `hzl dep list --cross-project-only` 命令查看跨项目的依赖关系。

---

## 设置检查点

在重要的里程碑或暂停工作之前设置检查点。一个好的检查点应该能够回答：“如果当前会话突然结束，其他代理能否从这里继续工作？”
...
配置信息位于 `~/.config/hzl/config.json` 文件中（如果文件不存在，请创建该文件）：
...
HZL 使用主机进程模型，没有内置的守护进程。在 OpenClaw 中，可以每 2 分钟运行一次 `hzl hook drain` 命令作为定时任务。如果没有调度器，回调信息只会被排队而不会被执行。
...
**任务完成度的判断标准：** “我完成了[任务]” 应该描述一个具体的成果。  
- ✓ “安装了车库运动传感器”  
- ✗ “完成了家庭自动化系统”（这是一个开放性的任务，通常无法完全完成）  

**何时将任务拆分为多个部分：** 当任务的各个部分能够独立完成或解决不同的问题时。

**补充说明：**  
不要在任务描述中重复详细的技术规格，建议参考官方文档以避免信息不一致。
...
## Web 控制面板（始终在线，适用于 Linux）

**访问地址：`http://<your-box>:3456`（可通过 Tailscale 访问）。** 在 macOS 上，可以使用 `hzl serve --background` 命令。

---

## HZL 不支持的功能

- **任务编排**：HZL 不负责自动创建代理或分配任务；
- **任务自动分解**：HZL 不会自动将任务拆分为更小的部分；
- **智能调度**：HZL 仅使用简单的优先级和 FIFO 排序机制。

这些功能属于任务编排层的职责，不属于 HZL 的功能范围。
...
- `hzl workflow run` 命令需要 HZL v2.0 或更高版本；如果该版本不可用，请使用上述的手动替代方案。