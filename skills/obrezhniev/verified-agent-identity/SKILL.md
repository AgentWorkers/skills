...
description: Billions/Iden3 提供了一套用于代理的身份认证和身份管理工具，支持链接验证、身份证明、签名以及信息验证等功能。
metadata: { "category": "identity" }
---
## 何时使用此技能

本技能用于让AI代理在Billions Network上创建和管理自己的身份，并将这些身份与人类所有者关联起来。

1. 当您需要将代理的身份与所有者关联时。
2. 当您需要签署挑战请求时。
3. 当您需要将人类用户与代理的DID关联时。
4. 当您需要验证签名以确认身份所有权时。
5. 当您需要使用共享的JWT令牌进行身份验证时。
6. 当您需要创建和管理去中心化身份时。

### 安装插件后，运行以下命令来创建一个身份并将其与您的DID关联：
...
所有身份数据都存储在`$HOME/.openclaw/billions`目录中，以便与OpenClaw插件兼容。
...
**副作用**: 生成的挑战信息会存储在`$HOME/.openclaw/billions/challenges.json`目录中。
...
**描述**: 使用DID的私钥签署挑战请求，以证明身份所有权，并将JWS令牌作为直接消息发送给指定的接收者。当您需要证明自己拥有某个特定的DID时，可以使用此脚本。
**参数**:
- `--to` - （必需）消息发送者的标识符，通过`openclaw message send`传递。
- `--challenge` - （必需）需要签署的挑战字符串。
- `--did` - （可选）验证接收者的DID；如果省略，则使用默认DID。
...
- `credentials.json` - 可验证的凭证信息。
- `identities.json` - 身份元数据。
- `profiles.json` - 用户资料信息。
...
3. 使用`node scripts/linkHumanToAgent.js --to <sender> --challenge <challenge_value>`一次性完成挑战的签署和验证URL的生成。
   - `--to`参数表示消息发送者（即调用者的标识符）。
   - 如果调用者提供了具体的挑战内容，请使用该内容；如果没有提供挑战内容，可以使用`{"name": <AGENT_NAME>, "description": <SHORT_DESCRIPTION>}`作为挑战内容。
4. 将结果返回给调用者。
