...
description: Billions/Iden3提供针对代理（agents）的身份验证和身份管理工具，支持链接（linking）、证明（proofing）、签名（signing）以及验证（verifying）等功能。
metadata: { "category": "identity", "clawdbot": { "requires": { "bins": ["node", "openclaw"] } }}
homepage: https://billions.network/
---
## 何时使用此技能

本技能允许AI代理在Billions Network上创建和管理自己的身份，并将这些身份与人类所有者关联起来。以下是使用该技能的常见场景：

1. 当您需要将代理的身份与所有者关联时。
2. 当您需要签署挑战请求（challenge）时。
3. 当您需要将人类用户与代理的DID（Decentralized Identity）关联时。
4. 当您需要验证签名以确认身份所有权时。
5. 当您需要使用共享的JWT（JSON Web Tokens）进行身份验证时。
6. 当您需要创建和管理去中心化身份时。
...
**描述**: 在Billions Network上创建一个新的身份。如果提供了`--key`参数，则使用该私钥；否则会生成一个随机私钥。创建的身份将自动设置为默认身份。
**使用示例**:
...
**副作用**: 生成的挑战信息会被存储在`$HOME/.openclaw/billions/challenges.json`文件中。

#### signChallenge.js

**命令**: `node scripts/signChallenge.js --to <sender> --challenge <challenge> --did <did>`
**描述**: 使用DID的私钥签署挑战字符串，以证明身份所有权，并将生成的JWS（JSON Web Signature）令牌作为直接消息发送给指定的接收者。此脚本用于验证您是否拥有某个特定的DID。
**参数**:
- `--to` - （必选）消息发送者的标识符，通过`openclaw message send`传递。
...
**描述**: 使用指定的DID签署挑战字符串，并将人类用户与代理的身份关联起来。响应会作为直接消息发送给指定的接收者。
**参数**:
- `--to` - （必选）消息发送者的标识符，通过`openclaw message send`传递。
- `--challenge` - （必选）需要签署的挑战字符串。
- `--did` - （可选）验证接收者的DID；如果省略则使用默认DID。
...
1. **严格检查身份**:
   - 在运行`linkHumanToAgent.js`或`signChallenge.js`之前，**务必先使用`node scripts/getIdentities.js`检查是否存在有效的身份**。
   - 如果没有配置身份，请**不要**尝试进行关联操作，而应先使用`createNewEthereumIdentity.js`创建一个新的身份。
2. **遇到脚本错误时立即停止**:
   - 如果任何脚本以非零状态码退出，**必须立即停止操作**。
   - 请检查stderr输出中的错误信息。
   - **禁止**通过手动生成密钥、使用其他方式创建DID或运行未经授权的命令来尝试“修复”错误。
   **禁止**使用`openssl`、`ssh-keygen`或其他系统工具来生成加密材料。
3. **禁止手动操作**:
   - 禁止进行任何手动加密操作。
   - 禁止直接修改`$HOME/.openclaw/billions`目录中的文件。
   - 除非有明确指示，否则不要将错误视为设置请求。
...
- `kms.json` - 存储未加密的私钥。
- `defaultDid.json` - 存储DID标识符和公钥。
- `challenges.json` - 存储身份验证挑战记录。
- `credentials.json` - 存储可验证的凭证信息。
- `identities.json` - 存储身份元数据。
- `profiles.json` - 存储用户配置信息。
...
3. 使用`node scripts/linkHumanToAgent.js --to <sender> --challenge <challenge_value>`一次性完成签名和验证URL的生成。
   - `--to`参数表示消息发送者（即请求者）。
   - 如果请求者提供了具体的挑战内容，请使用该内容；否则使用`{"name": <AGENT_NAME>, "description": <SHORT_DESCRIPTION>}`作为挑战内容。
4. 将结果返回给请求者。
