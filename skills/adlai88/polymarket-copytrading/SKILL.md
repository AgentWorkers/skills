...
description: 使用 Simmer API 获取 Polymarket 顶部交易者的持仓位置信息，并对多个钱包的持仓数据进行大小加权聚合。
...
> ⚠️ **谨慎开始：** 先使用较小的金额（例如`--max-usd 5-10`）并进行`--dry-run`测试，了解脚本的运行情况后再逐步增加金额。

## 工作原理

脚本每个周期会执行以下操作：
1. 通过Simmer API获取所有目标钱包的持仓信息
2. 使用按规模加权的算法合并这些持仓数据（钱包规模越大，影响力越大）
3. 检测持仓冲突（例如一个钱包持有多头头寸，另一个钱包没有），并跳过这些市场
...
**最佳使用场景：**
- 关注2-5个钱包以实现投资多样化
- 确保所关注的钱包在投资策略上较为一致（避免将高风险和低风险的钱包混合）
- 确保所关注的钱包在Polymarket上有可交易的品种
...
- **“检测到持仓冲突”**
  - 两个钱包在同一市场持有相反方向的持仓（例如一个持有多头，另一个持有空头）
  - 这些市场会被自动跳过

- **“余额不足”**
  - USDC余额不足以完成所有交易
  - 降低`SIMMER_COPYTRADING_TOP_N`或`SIMMER_COPYTRADING_MAX_USD`参数

- **“无法导入某些市场”**
  - 有些市场可能无法导入数据（可能是私密市场等）
  - 这些市场会被自动跳过