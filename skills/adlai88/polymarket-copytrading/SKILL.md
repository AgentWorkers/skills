...
description: 使用 Simmer API 获取 Polymarket 上顶级交易者的镜像交易位置信息；对来自多个钱包的交易数据进行大小加权聚合处理。
metadata:
...
> **这是一个模板。** 默认逻辑会按照资金规模进行加权分配来复制大型交易者的持仓——您可以根据自己的钱包选择标准、持仓筛选条件或再平衡规则对其进行调整。该工具会处理所有底层逻辑（如钱包获取、冲突检测和交易执行），而策略的制定则由用户自行完成。

## 何时使用此工具

当用户需要以下操作时，请使用此工具：
- 在Polymarket上复制大型交易者的持仓
- 使用$SIM进行模拟交易（无需真实资金）以测试交易策略
- 查看某个钱包的持仓情况
- 关注特定的交易者
- 查看他们的复制交易持仓
...
2. 使用资金规模加权的方式合并这些持仓（资金越多的钱包影响越大）
3. 检测持仓冲突（如果一个钱包持有多头持仓，另一个钱包也持有多头持仓，则跳过该市场）
...
→ 将您的持仓与大型交易者的持仓进行对比，并卖出他们已平仓的资产
...
该工具在以下情况下效果最佳：
- 关注2-5个钱包以实现投资多元化
- 这些钱包的交易策略较为一致（避免将高风险交易者与保守型交易者混合）
- 这些钱包交易的品种在Polymarket上均有交易

## 故障排除

**“订单金额太少” / “低于最低要求（5份资产）**
- Polymarket要求每笔订单至少需要5份资产
- 增加`--max-usd`的值或减少`--top-n`的值，以集中关注较少数量的持仓

**“未指定钱包”**
- 在消息中提供钱包地址，例如：“copytrade 0x1234...”  
- 或为自动扫描设置`SIMMER_COPYTRADING_WALLETS`环境变量

**“代理账户中没有USDC余额”**
- 确保您的Polymarket钱包中有USDC，或使用`--venue sim`进行$SIM模拟交易  
- 检查钱包是否已连接到simmer.markets/dashboard

**“检测到持仓冲突”**
- 如果两个钱包在某个市场的持仓方向相反（一个持有多头，另一个也持有多头），则跳过该市场  
- 市场的净持仓比例小于10%的交易会被跳过
...
- Polymarket使用的是**USDC.e**（桥接后的USDC，合约地址为`0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174`）——并非原生USDC  
