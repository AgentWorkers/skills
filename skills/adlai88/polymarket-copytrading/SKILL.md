...
description: 通过 Simmer API 获取 Polymarket 顶级交易者的持仓位置信息，并对多个钱包的持仓数据进行大小加权汇总。
...
使用 Simmer SDK 模仿 Polymarket 上成功交易者的持仓策略。

> **这是一个模板。** 默认逻辑会按照资产规模进行加权分配来模仿大型交易者的持仓策略——您可以根据自己的钱包选择标准、持仓筛选条件或再平衡规则对其进行调整。该工具负责处理所有底层逻辑（如获取钱包信息、检测冲突、执行交易等），而您只需提供策略参数即可。

## 适用场景

当用户希望执行以下操作时，可以使用此工具：
- 在 Polymarket 上复制大型交易者的持仓策略
- 查看某个钱包的持仓情况
- 关注特定的交易者
- 查看他们的复制交易持仓情况
...
| 设置 | 环境变量 | 默认值 |
|---------|---------------------|---------|
| 目标钱包地址 | `SIMMER_COPYTRADING_WALLETS` | （未指定） |
...
- 账户余额 500 美元及以上：显示前 50 个持仓

**Polymarket 的限制：**
- 每笔订单至少需要持有 5 枚代币
- SDK 要求每个持仓的最低价值为 1.00 美元（以过滤小额交易）

> ⚠️ **建议谨慎开始：** 先使用较小的金额（例如 `--max-usd 5-10`）并进行 **干运行**（dry-run），了解工具的运行情况后再逐步增加投资额。

## 工作原理

脚本每个周期会执行以下操作：
1. 通过 Simmer API 从所有目标钱包中获取持仓信息
2. 使用加权算法合并这些持仓数据（资产规模较大的钱包影响更大）
3. 检测持仓冲突（例如一个钱包持有多头仓位，另一个钱包没有）
4. 筛选出最具投资信心的持仓（即“置信度”最高的持仓）
5. 自动从 Polymarket 获取缺失的市场数据
6. 计算所需的再平衡交易量
7. 通过 Simmer SDK 执行交易（同时遵守平台的交易限额）
8. 将结果反馈给用户
...
每次运行后，会向用户发送以下信息：
- 当前配置（目标钱包地址、前 N 个持仓、每个钱包的最大持仓数量）
- 获取到的钱包数量及总持仓数量
- 因冲突被跳过的市场
- 执行的交易（或未执行的交易及其原因）
- 当前的投资组合持仓情况
...
**用户：“进行全套再平衡以匹配大型交易者的持仓”**
→ 运行：`python copytrading_trader.py --rebalance`
→ 包括买入和卖出操作，以匹配大型交易者的持仓策略

## 如何寻找值得关注的交易者钱包

常见方法：
1. **排行榜跟踪**：查看 Polymarket 的排行榜，选择表现稳定的交易者
2. **关注社交媒体上的盈利交易者**：关注在社交媒体上活跃的盈利交易者
3. **特定策略**：关注那些在特定市场（如天气、政治或加密货币相关领域）有交易习惯的交易者
...
- **“订单金额太少” / “低于最低要求（5 美元）**：Polymarket 要求每笔订单至少持有 5 枚代币
- 增加 `--max-usd` 的值或减少 `--top-n` 的值，以集中投资于少数几个持仓

- **未指定钱包地址**：在命令中提供钱包地址，例如：“copytrading_trader.py --wallets 0x1234...”  
- 或为自动扫描设置环境变量 `SIMMER_COPYTRADING_WALLETS`

- **“代理账户中没有 USDC”**：确保您的 Polymarket 账户中有足够的 USDC
- 检查钱包是否已连接到 simmer.markets dashboard

- **“检测到持仓冲突”**：某些钱包在该市场持有相反方向的仓位（一个持有多头，另一个持有空头）
- 账户在该市场的净持仓比例低于 10% 的市场会被自动跳过

- **“余额不足”**：账户中的 USDC 不足以完成所有交易
- 减少 `SIMMER_COPYTRADING_TOP_N` 或 `SIMMER_COPYTRADING_MAX_USD` 的值

- **“无法导入某些市场”**：某些市场可能无法导入数据（可能是私有的等原因）
- 这些市场会被自动跳过