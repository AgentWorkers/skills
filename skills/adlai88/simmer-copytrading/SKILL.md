...
description: 使用 Simmer API 获取 Polymarket 顶级交易者的持仓位置信息，并对多个钱包中的持仓数据进行大小加权聚合处理。
...
**前N大持仓的自动计算规则（未指定时）：**
- 账户余额 < 50美元：显示前5大持仓
- 账户余额 50-200美元：显示前10大持仓
- 账户余额 200-500美元：显示前25大持仓
- 账户余额 500美元以上：显示前50大持仓

**Polymarket的限制：**
- 每笔订单至少需要5股
- SDK要求最低持仓价值为1.00美元（以过滤小额持仓）

> ⚠️ **谨慎开始：** 先使用较小的金额（`--max-usd 5-10`）并进行`--dry-run`测试，了解技能的运行情况后再逐步增加金额。

## 工作原理

脚本每个周期会执行以下操作：
1. 通过Simmer API获取所有目标钱包的持仓信息
2. 使用大小加权的方式合并这些持仓数据（持仓金额越大，影响力越大）
3. 检测持仓冲突（例如，一个钱包持有多头仓位，另一个钱包也持有多头仓位，则跳过该市场）
4. 应用前N大持仓的筛选规则，专注于信心度最高的持仓
5. 从Polymarket自动导入缺失的市场数据
6. 计算所需的再平衡交易量以匹配目标持仓配置
...
→ 将用户的持仓与大型交易者的持仓进行对比，并卖出他们已平仓的资产
...
**最佳使用场景：**
- 关注2-5个钱包以实现投资多样化
- 确保所关注的钱包在投资策略上具有一致性（避免将高风险和低风险的钱包混合）
- 确保所关注的钱包在Polymarket上有可交易的资产

## 常见问题及解决方法**

**“订单金额太少” / “低于最低要求（5股）**
- Polymarket要求每笔订单至少5股
- 增加`--max-usd`参数或减少`--top-n`参数，以专注于更少的持仓
...
**“检测到持仓冲突”**
- 两个钱包在同一市场持有相反方向的持仓（例如，一个持有多头，另一个持有多头）
- 持仓比例低于10%的市场会被自动跳过

**“余额不足”**
- USDC余额不足以完成所有交易
- 减少`SIMMER_COPYTRADING_TOP_N`或`SIMMER_COPYTRADING_MAX_USD`参数

**“无法导入某些市场”**
- 有些市场可能无法导入数据（可能是私有的等）
- 这些市场会被自动跳过