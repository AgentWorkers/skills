...
description: 需要进行 x402 类型的支付才能访问付费 API 和受限制的内容。当某个技能需要从受 x402 保护的端点（例如 Kaito Mindshare API、Simmer Premium 端点或任何 x402 提供商的端点）获取数据时，应使用此功能。该功能会自动使用 USDC 在 Base 平台上处理“402 Payment Required”（需要支付）的响应。
...
使用USDC在Base网络上为x402保护的API进行支付。此技能使代理能够在访问付费网络资源时自动进行加密货币支付。

## 何时使用此技能

在以下情况下使用此技能：
- 需要从x402保护的API（例如Kaito mindshare）获取数据
- 遇到HTTP 402 “Payment Required”（需要支付）的响应
- 需要检查Base钱包的余额（USDC + ETH）
...
1. **设置钱包私钥**
   ...
   ```
   如果未设置`EVM_PRIVATE_KEY`，则使用`WALLET_PRIVATE_KEY`（Simmer/Polymarket用户使用的密钥相同）。您的EVM地址适用于所有链——Polygon用于交易，Base用于x402支付。

2. **在Base网络上用USDC充值**
   - 将USDC发送到您的Base网络钱包地址
   - Base上的x402支付是完全无需gas的——只需USDC，无需ETH
...
| `python x402_cli.py fetch <url>` | 自动支付费用地获取URL数据 |
| `python x402_cli.py fetch <url> --json` | 同上，但仅输出原始JSON |
| `python x402_cli.py fetch <url> --dry-run` | 显示支付信息（不进行支付） |
...
```
Quicknode x402支持55个以上的网络（Ethereum、Polygon、Base、Arbitrum、Solana、Bitcoin等）。10美元可购买100万次RPC信用额度——每次成功调用消耗1个信用额度。

## 支持的x402提供商

| 提供商 | 端点 | 价格 | 描述 |
|----------|----------|-------|-------------|
| Kaito | `/api/payg/mindshare` | 每个数据点0.02美元 | Token mindshare时间序列 |
| Kaito | `/api/payg/sentiment` | 每个数据点0.02美元 | Token情感时间序列 |
| Kaito | `/api/payg/narrative_mindshare` | 每个数据点0.02美元 | 叙事性mindshare时间序列 |
...
| Simmer | `/api/sdk/context/:id` | 每次请求0.005美元 | 市场上下文（绕过速率限制） |
| Simmer | `/api/sdk/briefing` | 每次请求0.005美元 | 投资组合简报（绕过速率限制） |
| Quicknode | `/:network`（55个以上网络） | 每次请求10美元/100万信用额度 | 无需API密钥的RPC访问 |
...
| 最大支付金额 | `X402_MAX_payment_USD` | 10.00美元 | 每次请求的安全上限 |
...
- 确保`.env`文件被包含在`.gitignore`中——切勿将私钥提交到git
...
**“支付选项中未支持的网络”**
- 确保您的Base网络上拥有足够的USDC。虽然某些提供商可能支持其他链，但此技能仅使用Base网络。