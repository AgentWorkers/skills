...
使用实时价格信号在 Polymarket 的 5 分钟快速市场中进行交易。默认使用来自 Binance 的 BTC 动量指标；也支持 ETH 和 SOL。

> **仅适用于 Polymarket。** 所有交易均在 Polymarket 上使用真实的 USDC 执行。使用 `--live` 选项可进行实时交易，否则为模拟交易。

> **这是一个模板。** 默认的信号（Binance 动量指标）可帮助您开始使用该技能——您可以根据自己的信号、数据源或策略进行定制。该技能负责处理所有市场发现、数据导入和交易执行等繁琐工作，您只需提供交易策略即可。

> ⚠️ 快速市场需支付 Polymarket 的 10% 手续费（`is_paid: true`），请在交易策略中考虑这一点。

## 如何查找市场

- 直接通过 Polymarket 的 Gamma API 查询实时快速市场数据（不依赖于 Simmer 的市场列表）
- 每个周期都会在新市场出现时自动发现它们
- 支持 BTC、ETH 和 SOL — 只需更改 `--set asset=ETH` 参数，或指定您想要交易的市场
- 每 5 分钟运行一次，以捕捉每个交易窗口的机会（或每 1 分钟运行一次，以抓住窗口内的交易机会）

**您无需等待 Simmer 中显示市场数据**。FastLoop 会实时在 Polymarket 中查找市场，然后通过 Simmer 进行交易。

## 何时使用此技能

当用户希望：
- 在任何支持的资产上进行 5 分钟或 15 分钟的快速市场交易
- 自动化短期加密货币预测交易
- 使用 CEX 的价格动量指标（或其他自定义信号）作为 Polymarket 的交易信号
- 监控快速市场中的持仓情况
...
3. **确认设置**（或使用默认值）
   ...
   - 最大持仓量：单次交易的金额（默认为 5.00 美元）
   - 交易窗口时长：5 分钟或 15 分钟（默认为 5 分钟）

4. **设置定时任务或循环**（用户负责安排任务执行时间——详见“如何运行循环任务”）
...
该脚本会运行一个周期，具体由用户设置的定时任务或循环机制来触发执行：

**Linux crontab（本地/VPS 环境）：**
```
...
**通过 OpenClaw 的心跳机制运行：** 在 `HEARTBEAT.md` 文件中添加相应配置：
```
...
| `max_position` | 5.0 | `SIMMER_SPRINT_MAX_POSITION` | 单次交易的最大金额 |
| `signal_source` | binance | `SIMMER_SPRINT_SIGNAL` | 价格数据来源（Binance 或 Coingecko） |
...
2. 计算价格动量：`(当前价格 - 5 分钟前的价格) / 5 分钟前的价格`
3. 比较动量方向与 Polymarket 当前的价格走势
4. 在满足以下条件时进行交易：
   - 动量 ≥ `min_momentum_pct`（默认为 0.5%）
   - 价格波动幅度 ≥ `entry_threshold`（默认为 5 分）
   - 交易量大于平均交易量的 1.5 倍（排除价格波动较小的情况）

**示例：** 如果过去 5 分钟内 BTC 价格上涨了 0.8%，但快速市场的价格仅为 0.52 美元，且实际价格与预期价格（约 0.55 美元）相差 3 分，则触发买入信号。

### 自定义信号

**此技能是一个模板**。默认的 Binance 动量指标只是一个起点。您可以根据自己的需求定制信号：
- **多交易所价格对比**：比较 Binance、Kraken、Bitfinex 等交易所的价格差异，以预测价格走势
- **市场情绪**：结合 Twitter 或社交媒体上的信息，因为热门推文可能会影响快速市场
- **技术指标**：使用 RSI、VWAP 或您喜欢的数据源中的订单流分析
- **新闻**：利用新闻事件来预测价格走势
- **链上数据**：分析大型交易者的交易行为、资金流动率或清算情况

要自定义信号，请编辑 `fastloop_trader.py` 文件中的 `get_momentum()` 函数或添加自己的信号处理逻辑。其余部分（市场发现、数据导入、交易规模调整、费用计算等）保持不变。
...
**“未找到活跃的快速市场”**
- 可能是因为快速市场尚未启动（非交易时段或周末）
- 请直接在 Polymarket 上查看是否有活跃的 BTC 快速市场

**“剩余时间少于 60 秒时没有快速市场”**
- 当前交易窗口即将结束，下一个市场尚未开放
- 如果您希望更接近交易结束时间进行交易，请减小 `min_time_remaining` 的值

**“数据导入失败：超出请求限制”**
- 免费账户每天只能导入 10 次数据。高级账户每天可导入 50 次
- 快速市场交易需要高级账户才能频繁进行

**“无法获取价格数据”**
- 可能是因为 Binance API 出现故障或请求次数达到限制
- 可尝试使用 `--set signal_source=coingecko` 作为备用数据源

**“交易失败：市场流动性不足”**
- 快速市场的交易量较小，可以尝试减小交易规模
