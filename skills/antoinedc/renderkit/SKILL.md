...
name: renderkit
version: "1.4.0"
description: 使用 RenderKit API 将结构化数据渲染成美观的托管网页，并创建用于数据收集的托管表单。该 API 可用于生成可视化页面、调查问卷、回复确认表单、反馈表单或任何其他结构化数据相关的应用。
metadata:
  openclaw:
    requires:
      env:
...
所有命令均使用 `curl` 来调用 RenderKit API。请选择正确的端点：

- **仅读取页面**（结果、摘要、对比信息、行程安排）→ `POST /v1/render`
- **数据收集**（表单、调查问卷、回复确认、注册、反馈）→ `POST /v1/forms`
...
返回一个可供受访者使用的 `url`。您也可以直接指定表单字段的内容，而无需使用提示语。
...
- **切勿使用 `/v1/render` 来伪造表单**——该接口会生成一个无法收集回复的静态页面。
- 请在数据中直接包含页面的 URL，因为这些 URL 会自动添加图片、评分和元数据。
- 可选参数：`"theme": { "mode": "dark", "palette": ["#color1", "#color2"] }` 用于设置页面主题。
- 更新操作（PATCH 请求）是免费的，且不会占用您的 API 使用额度。
- 如果您之前已经生成了相关页面，后续修改建议使用 `PATCH` 而不是 `POST` 请求。
