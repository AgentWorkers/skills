...
description: >
  ...
  - 处理 Excel 文件（.xlsx 格式）；
  - 在 SQLite 数据库中跟踪文件中的更改；
  - 将文件内容同步到 Google Drive；
  - 在更新文件时保留原有的公式。
  当用户需要上传 Excel 文件、查询数据、更新单元格同时保留公式，或与 Google Drive 同步时，可以使用此功能。
...
这是一个完整的 Excel 文件管理流程，支持本地处理、Google Drive 备份、公式保留以及 SQLite 数据跟踪功能。

## 特点

- ✅ **公式保留** — 更新单元格时不会丢失公式（由 openpyxl 提供支持）
- ✅ **Google Drive 同步** — 使用 rclone 自动将文件备份到 Google Drive
- ✅ **SQLite 数据跟踪** — 在本地数据库中记录所有文件、公式和元数据
- ✅ **多文件支持** — 可同时处理多个 Excel 文件
- ✅ **数据分析** — 查询数据、读取公式并分析文件结构
- ✅ **批量操作** — 可一次性为整列创建公式
...
1. 安装 Python 3.8 及以上版本，并确保已安装 openpyxl
2. 安装 rclone（用于与 Google Drive 集成）
3. 拥有 Google Drive 账户
...
2. **跟踪器 CLI**（`~/.openclaw/tools/openclaw-excel/excel-tracker`）
...
详细安装步骤请参阅 [安装指南](https://github.com/your-repo/excel-workflow)。

## 命令说明

### process 命令

分析文件内容，将其上传到 Google Drive 并保存到跟踪数据库中：
...
**用户：**“将 iPhone 的数量更改为 20”

**代理工作流程：**
1. 查询文件结构：`excel-workflow query "find iPhone row"`
2. 确定 iPhone 所在的行（例如第 2 行）和对应的数量（C 列）
...
1. **Excel CLI**：基于 openpyxl 的工具，用于读写 Excel 文件
   - 使用 `data_only=False` 选项来保留公式
   - 命令：`read`、`update`、`info`、`show-formulas`、`get-cell`

2. **跟踪器**：使用 SQLite 数据库存储文件元数据
   - 数据库文件路径：`~/.openclaw/excel_tracker.db`
   - 记录文件名、工作表、公式、文件路径和修改时间戳等信息

3. **工作流程协调器**：整合 Excel CLI、rclone 和跟踪器
   - 负责协调文件分析、上传、跟踪和更新等操作
   - 将进度信息输出到标准错误流（stderr），并将结果以 JSON 格式输出到标准输出流（stdout）

### 文件存储位置

- **跟踪数据库：** `~/.openclaw/excel_tracker.db`
- **Google Drive 文件夹：** `gdrive:Excel/`
- **本地文件：** 用户指定的路径（Telegram 下载的文件保存在 `~/.openclaw/media/`）
...
```
- **问题：“openpyxl 未安装”**：```bash
# Install in virtual environment
cd ~/.openclaw/tools/openclaw-excel
python3 -m venv venv
./venv/bin/pip install openpyxl
```
- **问题：“无法访问 Google Drive”**：```bash
# Reconfigure rclone
rclone config
# Delete old gdrive remote and create new one
```
- **问题：公式被错误地转换为数值**：**这种情况不应该发生！** 如果发生，请检查：
  - 确保 Excel CLI 在读取文件时使用了 `data_only=False` 选项
  - 确保公式字符串以 `=` 开头
  - 查看 openpyxl 的版本（使用 `pip show openpyxl`，版本应大于或等于 3.1.0）

## 最佳实践

- **对于用户：**
  - 为文件起具有描述性的名称（例如 `sales_2026_Q1.xlsx`）
  - 提出具体问题（例如 “1 月份的总收入是多少？”）
  - 保持文件结构的一致性，以便 AI 更容易理解文件内容

- **对于开发者：**
  - 在处理文件之前始终检查文件是否存在
  - 使用 `try/except` 语句优雅地处理错误
  - 在标准错误流（stderr）中显示处理进度，在标准输出流（stdout）中显示结果
  - 在更新单元格之前验证公式内容的正确性

## API 参考

详细 API 文档请参阅 [Excel CLI 文档](./excel_cli.md) 和 [跟踪器文档](./tracker.md)。
...
- [使用 rclone 的 Google Drive 功能](https://rclone.org/drive/)

## 技术支持

- 问题反馈：[your-repo/excel-workflow/issues]
- 文档资料：[your-repo/excel-workflow/docs]
- 社区资源：[OpenClaw 社区]

## 许可证

本工具采用 MIT 许可证，详细内容请参阅 `LICENSE.txt` 文件。
