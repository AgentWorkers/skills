...
> 1. **切勿直接使用 `gog` 命令** - 必须始终使用包装脚本（`create_event.sh`、`email_read.sh` 等）。直接使用 `gog` 会绕过跟踪机制，导致事件重复。这一点没有商量余地。
> 2. **忽略日历通知** - 不要处理来自 `calendar-notification@google.com` 的电子邮件（如 “已接受”、“已拒绝”、“待定” 等）。这些只是对现有邀请的回复，并非新事件。请运行 `process_calendar_replies.sh` 来归档这些邮件。
> 3. **创建事件前务必询问用户** - 未经当前对话中的明确确认，切勿创建日历事件。
> 4. **检查是否已处理** - 在处理任何电子邮件之前，请先查看 `index.json` 文件中的 `processed_emails` 列表。
> 5. **先阅读配置文件** - 在展示事件信息之前，请加载并应用 `ignore_patterns` 和 `auto_create_patterns` 的设置。
> 6. **查看用户偏好设置** - 请阅读 `MEMORY.MD` 文件，了解用户之前的偏好设置。
> 7. **包含所有配置的参与者** - 在创建/更新/删除事件时，务必使用 `--attendees` 标志包含配置中的参与者（如果支持 `--send-updates all` 选项，请一并使用）。
> 8. **先查找已跟踪的事件** - 在进行日历搜索之前，请使用 `lookup_event.sh --email-id` 命令查找已存在的事件（这样更快、更可靠）。
> 9. **跟踪所有创建的事件** - `create_event.sh` 脚本会自动跟踪事件；更新或删除事件时请使用相应的跟踪 ID。
> 10. **显示星期几** - 在展示事件信息时，务必包含星期几，以便用户确认。
...
该功能从电子邮件中提取日历事件和待办事项，展示给用户审核，并创建/更新日历事件，同时支持重复检测和撤销操作。

**首次设置：** 请参阅 [SETUP.md](SETUP.md)，了解配置选项和智能引导流程。
...
**直接模式：** 扫描所有未读邮件，查找事件相关的信息（日期、时间、会议关键词）。

**转发模式：** 仅处理带有转发标记（Fwd:）的邮件。

### 2. 提取事件信息（由代理程序直接完成）

读取电子邮件并提取事件数据，包括以下内容：
- **标题**：事件的描述性名称（最多 80 个字符）
- **日期**：事件发生的日期
- **星期几**：用于验证
- **时间**：开始/结束时间（默认为上午 9:00 - 下午 5:00）
- **是否跨多天**：事件是否持续多天
- **是否重复**：事件是否定期重复
- **可信度**：高/中/低
- **URL**：邮件中包含的所有网址（必填项 - 请务必查找注册链接、信息页面、购票网站等）
- **截止日期**：回复/注册/购票的截止日期（如果存在）
- **截止日期操作**：用户需要执行的操作（例如 “回复”、“购票”、“注册”）
- **截止日期链接**：用于执行操作的直接链接（通常与事件链接相同）

**URL 提取规则：** 请务必在事件描述的开头部分包含最相关的网址。

### 2.1 判断截止日期

扫描邮件中的截止日期相关内容，确定用户需要执行的操作：
...
使用大型语言模型（LLM）进行语义匹配，以识别相似的重复事件（例如：“团队外出会议” 与 “团队外出会议 5-6pm”）。

### 5. 创建或更新日历事件

**建议使用 `create_event.sh` 脚本** - 该脚本负责日期解析、事件跟踪和日志记录：
...
电子邮件的处理（标记为已读或归档）由 `create_event.sh` 根据配置设置自动完成。无需手动操作——事件创建完成后，系统会自动处理邮件。
...
当事件有截止日期（如需要回复、注册、购票等）时，需要创建两个日历事件：
- **主事件**：按常规方式创建，并在描述中添加提示信息：
```bash
"$SCRIPTS_DIR/create_event.sh" \
    "$CALENDAR_ID" \
    "Spring Concert" \
...
**截止日期事件的属性：**
- **标题格式**：`DEADLINE: [操作] 于 [事件名称]`
- **日期**：截止日期
- **时间**：上午 9:00（持续 30 分钟）
- **提醒方式**：事件前 1 天发送邮件提醒 + 事件前 1 小时弹窗提醒
- **描述**：需要执行的操作、相关链接以及主事件的链接

### 发送截止日期通知

在创建带有截止日期的事件时，发送通知邮件以提醒用户：
...
| `~/.openclaw/workspace/memory/email-to-calendar/pending_invites.json` | 待处理的邀请列表 |
| `~/.openclaw/workspace/memory/email-to-calendar/activity.json` | 活动日志 |
| `~/.openclaw/workspace/memory/email-to-calendar/changelog.json` | 变更历史记录 |
| `~/.openclaw/workspace/skills/email-to-calendar/scripts/` | 辅助脚本 |
...
- **提取规则**：[references/extraction-patterns.md] - 日期/时间解析规则
