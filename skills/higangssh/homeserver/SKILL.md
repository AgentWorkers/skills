---
name: homeserver
description: 通过 homebutler CLI 进行家庭实验室服务器管理。可以查看系统状态（CPU/内存/磁盘）、管理 Docker 容器、实现基于局域网的唤醒功能（Wake-on-LAN）、扫描开放的端口、发现网络设备、监控资源警报，并通过 SSH 管理多台服务器。当需要查询服务器状态、Docker 容器信息、远程唤醒机器、检查开放端口、网络设备状态或进行多服务器管理时，均可使用该工具。
...
使用 [`homebutler`](https://github.com/Higangssh/homebutler) 命令行工具来管理家庭实验室中的服务器。该工具采用单一二进制文件格式，输出结果为 JSON 格式，非常适合与人工智能系统集成。
...
```
通过 ping 扫描和 ARP 表来发现局域网中的设备。输出内容：IP 地址、MAC 地址、主机名、设备状态。
**注意**：扫描过程可能需要最多 30 秒。部分设备可能因未响应 ping 而无法被检测到。
...
**安装路径优先级**：`/usr/local/bin` → `sudo /usr/local/bin` → `~/.local/bin`（系统会自动将路径添加到 `.profile`、`.bashrc` 或 `.zshrc` 文件中的 PATH 变量中）。
...
```
启动一个实时服务器监控的 Web 仪表盘，包含以下功能：服务器概览、系统状态、Docker 信息、进程列表、警报信息、开放端口列表以及网络唤醒功能。通过下拉菜单可以选择要查看的具体服务器的数据。

### 交互式设置
```bash
homebutler init                      # Setup wizard
```
系统会自动检测本地计算机，并指导用户通过 SSH 密钥发现和连接测试来添加远程服务器。
...
```
启动一个内置的 MCP（Model Context Protocol）服务器，以便与 Claude Desktop、ChatGPT、Cursor 等 MCP 客户端配合使用。所有 `homebutler` 命令（如 `system_status`、`docker_list`、`docker_restart`、`docker_stop`、`docker_logs`、`wake`、`open_ports`、`network_scan`、`alerts`）均通过标准的 MCP 协议提供。该服务器不开放任何网络端口，仅使用标准输入输出（stdio）进行通信。
...
所有命令默认以人类可读的文本形式输出。若需要机器可解析的 JSON 格式输出（推荐用于人工智能或脚本集成），可使用 `--json` 标志。
...
3. `~/.config/homebutler/config.yaml` — XDG 标准配置文件（推荐使用）
...
- `wake` — 包含网络唤醒目标的配置信息（包括 MAC 地址和广播地址）
- `alerts.cpu/memory/disk` — 资源使用阈值的百分比设置
...
1. **始终执行命令，不要猜测** — 使用 `homebutler status` 命令获取实际数据。
2. **以用户可理解的方式解释结果** — 不要直接输出原始 JSON 数据，而是用自然语言进行总结。
3. **在出现警报时提醒用户** — 如果有任何资源状态显示为“警告”或“严重”，请及时提醒用户。
4. **使用 `--all` 命令获取全部服务器信息** — 当用户询问“所有服务器”的情况时，使用 `--all`。
5. **使用 `--server` 命令查询特定服务器** — 当用户指定了服务器名称时，使用 `--server <服务器名称>`。
6. **处理 Docker 相关问题** — 如果服务器上未安装 Docker 或 Docker 守护进程未运行，请向用户说明情况。
7. **进行网络扫描时** — 提醒用户扫描可能需要约 30 秒。
8. **安全注意事项** — 绝不要在群组聊天中直接发送包含主机名/IP 地址的原始 JSON 数据，应进行汇总处理。
9. **在隔离环境中进行部署** — 建议使用 `--local` 选项。
...
- **数据隐私**：`homebutler` 不会向外部发送任何数据，所有操作仅在本地或用户指定的服务器上进行。

## 错误处理

- **SSH 连接失败** — 检查配置文件中的主机名/端口号/用户名是否正确，并确认远程服务器已注册相应的 SSH 密钥。
- **远程服务器上未找到 `homebutler`** — 先运行 `homebutler deploy --server <服务器名称>` 命令。
- **服务器上未安装 Docker** — 告知用户该服务器不支持 Docker。
...
用户：**服务器运行状况如何？**
→ 运行 `homebutler status`，并得到如下总结：“CPU 使用率为 23%，内存使用率为 40%，磁盘使用率为 37%，运行时间为 42 天。一切正常 👍”

用户：**查看所有服务器的状态？**
→ 运行 `homebutler status --all`，并获取所有服务器的详细状态。

用户：**Raspberry Pi 的运行情况如何？**
→ 运行 `homebutler status --server rpi`，获取 Raspberry Pi 的状态信息。

用户：**当前有哪些 Docker 容器在运行？**
→ 运行 `homebutler docker list`，列出所有容器的名称和状态。

用户：**唤醒 NAS 服务器？**
→ 运行 `homebutler wake nas`（如果已配置），或询问 NAS 的 MAC 地址。

用户：**所有服务器是否有任何警报？**
→ 运行 `homebutler alerts --all`，查看所有服务器的警报信息。

用户：**将 `homebutler` 部署到新服务器上？**
→ 运行 `homebutler deploy --server <服务器名称>`，并获取部署结果。