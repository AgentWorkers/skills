...
## 1. 概述
**技能名称：** Hyperbot交易分析  
**描述：** 为Hyperbot平台提供交易数据分析功能，包括智能资金追踪、大户监控、市场数据查询和交易者统计。适用于进行市场分析和决策的加密货币交易者。  
**版本：** 1.0.0  
**MCP服务器URL/端点：** https://mcp.hyperbot.network/mcp  

**sessionId：** 通过SSE在https://mcp.hyperbot.network/mcp/sse获取的会话ID  
...
#### `get_market_stats`  
**功能：** 获取活跃订单统计（多头/空头数量、价值、大户订单比例）和市场中间价  
**参数：**  
- `coin`：币种代码（例如：btc、eth）  
- `whaleThreshold`：大户阈值（以USDT计）  
...
**参数：**  
- `address`：用户钱包地址  
- `days`：统计天数（选项：1天、7天、30天、60天、90天）  
- `scope`：统计范围（默认：perp）  
...
| whale-tracking | 大户行为分析与市场影响评估 | ```分析大户持仓数据和最新动向：1. 判断主力意图；2. 评估市场影响；3. 预测短期趋势；4. 提供交易建议。输出格式：JSON。```  
| market-sentiment | 市场情绪分析 | **基于市场数据（订单簿、活跃订单、大户多空比例）：1. 分析当前市场情绪；2. 识别支撑/阻力水平；3. 判断短期趋势。输出格式：JSON。**  
...
1. 调用工具：`get_whale_events(20)` 获取最新大户动向  
2. 调用工具：`get_whale_directions("BTC")` 查看BTC大户的多空比例  
3. 使用提示：`whale-tracking` 分析大户行为  
4. 生成市场影响评估报告  
...
4. 调用工具：`get_whale_history_ratio("1d, 30)` 获取历史多空比例  
...
- **params.name**：MCP工具名称（对应于@McpTool注释的名称属性）  
- **params.arguments**：工具参数，以键值对形式传递  
