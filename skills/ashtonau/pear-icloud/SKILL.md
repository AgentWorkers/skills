...
description: 通过 Pear 管理 iCloud 日历、提醒事项和联系人。可以处理事件、提醒事项、联系人信息以及每日简报的安排，同时支持基于 CalDAV/CardDAV 协议与 Apple iCloud 的交互。提供了 27 种工具来帮助您更高效地使用 iCloud 的各项功能。
...
Pear通过27个MCP工具提供对iCloud日历、提醒事项和联系人的读写访问功能。所有这些工具的前缀都是`pear_`，并且它们通过CalDAV/CardDAV协议与iCloud进行通信。
...
| `pear_create_event` | 创建事件，可选设置重复频率、闹钟、参与者和位置 |
| `pear_update_event` | 更新现有事件的属性 |
| `pear_delete_event` | 通过文件名删除事件 |
| `pear_find_free_slots` | 查找指定持续时间的空闲时间段 |
| `pear_check_availability` | 检查特定时间段是否可用，返回冲突情况 |

### 提醒事项（4个工具）

| 工具 | 描述 |
|------|-------------|
| `pear_list_reminders` | 列出提醒事项，支持列表过滤，包括完成状态的切换 |
| `pear_create_reminder` | 创建提醒事项，可选设置截止日期、优先级（1=高，5=中，9=低）和备注 |
...
| 工具 | 描述 |
|------|-------------|
| `pear_list_contacts` | 列出所有联系人及其完整的vCard信息（电话、电子邮件、地址、生日） |
| `pear_search_contacts` | 按名称、电子邮件、电话或组织单位搜索联系人 |
| `pear_create_contact` | 创建联系人，支持完整的vCard信息（包括照片） |
| `pear_update_contact` | 更新联系人字段（与现有数据合并） |
...
| `pear_update_contact_photo` | 更新联系人的照片（Base64、数据URI或外部URL）

### 每日简报（1个工具）

| 工具 | 描述 |
|------|-------------|
| `pear_get_daily_briefing` | 一次性获取今天的事件和待办提醒事项，同时提供联系人的详细信息 |

### 安排会议（1个工具）

| 工具 | 描述 |
|------|-------------|
| `pear_find_best_time` | 通过人工智能评分推荐最佳会议时间段，考虑工作时间、时间偏好、工作日偏好、缓冲时间和提醒截止时间 |
...
- **全天事件**：设置`isAllDay: true`并使用仅日期的格式`2025-06-15`
...
3. 对于参与者，可以传递名称——Pear会自动从用户联系人列表中匹配
4. 使用`idempotencyKey`进行重试以避免创建重复事件
5. 闹钟设置使用`{ action: "display", trigger: 15 }`表示15分钟后提醒

### 寻找会议时间

安排会议时，优先使用`pear_find_best_time`而不是`pear_find_free_slots`：
- `pear_find_best_time`会考虑工作时间、偏好设置和现有安排，返回评分最高的会议时间段
- 传递`preferences`进行自定义：`{ timeOfDay: "morning", focusTime: true, workHoursStart: 9, workHoursEnd: 17 }`
- `pear_find_free_slots`更简单——仅返回原始的空闲时间段，不进行评分

### 提醒事项

- 提醒事项通过CalDAV（VTODO协议）访问——基本操作（标题、截止日期、优先级、备注）可以正常使用。现代Apple提醒功能（如子任务、标签、智能列表和基于位置的提醒）无法通过CalDAV实现
- 优先级值：`1` = 高，`5` = 中，`9` = 低，`0` = 无
- 要仅列出未完成的提醒事项，使用`includeCompleted: false`（默认值）
- 如果使用`listName`时，系统会自动创建提醒列表

### 联系人

- `pear_update_contact`会合并字段——不会删除您未指定的数据
- 生日格式：`YYYY-MM-DD`表示完整日期，`--MM-DD`表示年份未知
- 电话/电子邮件可以是单个字符串或数组形式
...
| 操作 | 风险 | 指导建议 |
|--------|------|----------|
| `pear_delete_event` | 永久删除事件 | 在删除前请与用户确认事件标题和日期 |
| `pear_delete_contact` | 永久删除联系人 | 必须先显示联系人姓名确认 |
| `pear_delete_contacts_batch` | 批量删除最多50个联系人 | 需要用户明确确认并显示删除数量 |
| `pear_update_event` | 覆盖事件字段 | 在应用更改前总结修改内容 |
| `pear_complete_reminder` | 将提醒事项标记为已完成 | 可以通过将`completed`设置为`false`来撤销 |
| 批量创建操作 | 一次创建最多50个条目 | 在执行前确认数量并总结操作内容 |

### 数据安全

- **读取操作始终是安全的**——列出、搜索和简报操作没有副作用 |
- **更新操作会合并数据，而非替换**——`pear_update_contact`会保留您未指定的字段 |
- **批量操作有速率限制**——每次操作最多5个条目，间隔200毫秒，无需手动限制 |
