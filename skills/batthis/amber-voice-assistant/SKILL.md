...
description: "适用于 OpenClaw 的具备电话功能的 AI 代理——这是目前最全面的电话技能解决方案。该方案支持生产环境使用，具备低延迟的通话功能（包括来电和去电），支持多种语言，提供实时监控界面，并支持“大脑在循环中”（brain-in-the-loop）的交互模式。"
...
Amber 为任何基于 OpenClaw 的部署方案提供了一款具备电话功能的人工智能语音助手。它内置了一个 **适用于生产环境的 Twilio + OpenAI Realtime 通信桥接层**（`runtime/`），能够处理来电筛选、拨出电话、预约预订以及实时查询 OpenClaw 的信息——所有这些操作都通过自然语言对话完成。
...
- **运行时桥接层** (`runtime/`)：一个完整的 Node.js 服务器，用于将 Twilio 电话呼叫与 OpenAI Realtime 服务连接起来，并实现 OpenClaw 的智能处理。
- **Amber 技能** (`amber-skills/`)：在通话过程中可使用的模块化功能（如日历查询、日志记录和消息转发），同时提供了自定义功能的规范。
- **通话日志仪表盘** (`dashboard/`)：可以查看通话记录、通话内容以及捕获的消息；包含 **手动同步按钮**，可按需拉取新通话记录。
- **设置与验证脚本**：用于预检查的环境配置文件和快速启动工具。
- **架构文档与故障排除指南**：包含通话流程图和常见故障处理方案。
- **安全机制**：针对拨出电话的审批流程、支付处理规则以及用户同意权限的设定。

## 🔌 Amber 技能——设计上具备可扩展性

Amber 配备了一个不断扩展的 **技能库**（Amber Skills），这些模块化功能可以直接集成到实时语音对话中。每个技能都提供了一个结构化的函数，允许你在通话过程中调用它们，从而构建强大的语音工作流程，而无需修改桥接层的代码。

### 📅 日历功能

- 在通话过程中查询操作员的日程安排或预订新事件。
  - **可用性查询**：查询当天、明天、本周或任何特定日期的可用时间。
  - **事件创建**：直接通过电话对话在操作员的日历中预订事件。
  - **默认保护用户隐私**：仅告知来电者操作员是否忙碌；事件标题、名称和地点不会被透露。
  - 基于 `ical-query` 实现，完全本地处理，无网络延迟。

### 📬 日志记录与消息转发

- 允许来电者留下消息，这些消息会自动保存并转发给操作员。
  - 捕获来电者的消息、姓名以及可选的回拨号码。
  - 首先会保存到通话日志中（作为审计记录），然后通过操作员配置的消息渠道发送。
  - 发送前会先获得操作员的确认。
  - 发送目的地由操作员自行配置；来电者无法更改消息的发送路径。

### 自定义技能

Amber 的技能系统支持扩展。每个技能都是一个独立的目录，包含 `SKILL.md`（元数据 + 函数规范）和 `handler.js` 文件。你可以：
- **根据自身需求定制内置技能**。
...
- **⬇ 同步按钮**（绿色）：立即从 `runtime/logs/` 中获取新通话记录并刷新仪表盘。建议在通话结束后立即使用此按钮，无需等待后台同步。
- **↻ 刷新按钮**（蓝色）：仅重新加载现有数据，不会重新处理日志。
- 后台同步脚本 (`node scripts/watch.js`) 每 30 秒自动执行一次同步。

## 选择 Amber 的理由

- **几分钟内即可部署语音助手**：只需执行 `npm install`、配置 `.env`，然后运行 `npm start`。
- 全面支持来电筛选功能：包括问候语、消息接收和日历集成。
- 支持结构化的拨出电话流程（如预约、咨询、跟进等）。
- **`ask_openclaw` 工具**：在通话过程中可以查询 OpenClaw 服务器中的日历、联系人信息及用户偏好设置。
- 通过语音助手与 OpenClaw 服务器进行交互，提升对话的自然感（避免查询时的沉默时刻）。
- 全面可配置：助手名称、操作员信息、组织名称、日历设置、筛选方式等均可通过环境变量进行调整。
- 提供操作员安全保护机制，包括审批流程、故障处理和支付处理。
...
- 指定自己的 OpenClaw 服务器端点。
- 设置自己的通话安全策略（包括审批流程、故障处理和支付处理方式）。
...
3. 按照向导提示操作：
   ...
   - 生成可用的 `.env` 文件。
   - （可选）安装依赖项并构建项目。
4. 配置 Twilio 的 webhook（向导会提供正确的 URL）。
5. 启动服务器：`npm start`
...
4. 将 Twilio 的语音 webhook 配置为 `https://<your-domain>/twilio/inbound`
...
4. 如果预检查通过，执行一次来电和一次拨出电话的测试。
5. 确认无误后才能投入生产环境。

## 安全默认设置

- 拨出电话前必须获得明确批准。
- 如果需要支付或预付款，系统会停止操作并转接给人工操作员。
- 保持问候语简短明了。
- 当 `ask_openclaw` 功能响应缓慢或不可用时，系统会自动采取超时处理措施。

## 工作流程

1. **确认 V1 版本的功能范围**：
   - 仅包含稳定运行的功能：通话流程、桥接层行为、备用方案以及设置步骤。
   - 不包含与特定硬件相关的敏感信息和私有路径。
2. **文档化系统架构和限制**：
   - 阅读 `references/architecture.md`。
   - 保证功能描述符合实际情况（注意网络延迟可能有所不同；内存查询操作仅供参考）。
3. **运行发布前的检查清单**：
...
- **“电话连接成功但助手无响应”**：检查 TTS 模型的配置和提供商的认证信息。
- **`ask_openclaw` 超时**：检查 gateway URL 和 token 的配置，并适当延长超时时间。
- **“Webhook 无法访问”**：检查隧道/域名和 Twilio webhook 的连接是否正常。

## 公开发布的注意事项

- 绝不要发布任何敏感信息（如密钥、token、电话号码、webhook URL 或个人数据）。
- 为拨出电话、支付处理和故障处理制定明确的安全规则。
- 如果对话质量或延迟仍在调整中，应将 V1 版本标记为测试版本。

## 相关资源

- **运行时桥接层**：`runtime/`（完整源代码及使用说明）
