...
description: "这是为 OpenClaw 设计的最全面的电话功能。具备生产级性能，支持低延迟的人工智能通话（包括来电和去电），支持多语言处理，并配备了实时监控界面（live dashboard），实现了智能辅助决策（brain-in-the-loop）功能。"
...
Amber 为任何基于 OpenClaw 的部署系统提供了一款具备电话功能的人工智能语音助手。它内置了一个 **可用于生产环境的 Twilio + OpenAI Realtime 通信桥接层**（`runtime/`），该桥接层支持处理来电筛选、拨出电话、预约预订以及实时查询 OpenClaw 知识库——所有这些操作均通过自然语言对话完成。

**✨ 新功能：** 交互式设置向导 (`npm run setup`) 可实时验证凭据并生成可用的 `.env` 文件——无需手动配置！
...
- **Amber 技能** (`amber-skills/`)：一系列模块化的通话中可使用的功能（如日历查询、日志记录和消息转发），同时提供了自定义功能的开发规范。
- **通话记录仪表板** (`dashboard/`)：可以浏览通话历史记录、通话记录文本以及捕获的消息；包含 **手动同步按钮**，可根据需要拉取新通话数据。
- **设置与验证脚本**：用于预检查的脚本、环境变量模板以及快速启动工具。
- **架构文档与故障排除指南**：包含通话流程图和常见故障处理方案。
- **安全机制**：针对拨出电话的审批流程、支付处理规则以及用户同意权限的设定。

## 🔌 Amber 技能——设计上具备高度扩展性

Amber 配备了一个不断扩展的 **技能库**，这些技能可以直接集成到实时语音对话中。每个技能都定义了一个结构化的函数，允许您在通话过程中调用这些函数，从而构建强大的语音工作流程，而无需修改桥接层的代码。

### 📅 日历功能

在通话过程中，您可以查询操作员的日历以查看其可用性或安排新事件：
- **查询可用性**：查看当天、明天、本周或任何特定日期的可用时间段。
- **事件预订**：直接通过电话对话在操作员的日历中预约事件。
- **隐私保护**：仅向呼叫者显示操作员是否忙碌；事件标题、名称和地点等信息不会被透露。
- **技术支持**：基于 `ical-query` 实现，仅支持本地操作，无网络延迟。

### 📬 日志记录与消息转发

允许呼叫者留下消息，这些消息会自动保存并转发给操作员：
- 捕获呼叫者的消息、姓名以及可选的回拨号码。
- 首先会保存到通话记录中（作为审计追踪），然后通过操作员配置的消息渠道发送。
- 发送前会先征得呼叫者的确认。
- 发送目的地由操作员自行配置；呼叫者无法更改消息的发送路径。

### 自定义技能开发

Amber 的技能系统支持扩展。每个技能都是一个独立的目录，包含 `SKILL.md`（元数据及函数规范）和 `handler.js` 文件。您可以：
- **根据自身需求定制内置技能**。
- **开发新的技能**，以满足特定业务需求（如 CRM 查询、库存检查、自定义通知等）。
- **通过 [ClawHub](https://clawhub.com) 与 OpenClaw 社区分享技能**。

请访问 [`amber-skills/`](amber-skills/) 查看示例和完整规范以开始使用。
...
- **↻ 刷新按钮**（蓝色）：从磁盘重新加载现有数据，不会重新处理通话记录。
- 后台同步脚本 (`node scripts/watch.js`) 在运行时每 30 秒自动执行一次同步。

## 选择 Amber 的理由

- **几分钟内即可部署语音助手**：只需执行 `npm install`、配置 `.env`，然后运行 `npm start` 即可。
- 全面支持来电筛选功能：包括问候语、接收消息以及与日历的集成。
- 支持结构化的拨出电话流程（如预约、咨询、跟进等）。
- **`ask_openclaw` 工具**：在通话过程中，语音助手会查询 OpenClaw 系统中的日历、联系人信息及用户偏好设置。
- 通过语音填充语保持对话的自然流畅（避免查询过程中的沉默环节）。
- 全面可配置：助手名称、操作员信息、组织名称、日历设置、筛选规则等均可通过环境变量进行调整。
- 提供操作员安全机制，包括审批流程、故障升级和支付处理功能。
...
- 选择助手的名称/语音以及问候语文本。
- 提供自己的 Twilio 账号和密钥。
- 配置自己的 OpenAI 项目及其 Webhook 密钥。
- 设置自己的 OpenClaw 服务端点。
- 定义自己的通话安全策略（包括审批流程、故障升级和支付处理规则）。
...
2. 将 `../references/env.example` 复制到 `runtime/.env` 文件中，并填写相应的值。
3. `npm run build && npm start`
4. 将您的 Twilio 语音 Webhook 指向 `https://<your-domain>/twilio/inbound`
...
1. **确认 V1 版本的功能范围**：
   ...
   - 避免包含与特定系统相关的敏感信息和私有路径。
2. **文档化系统架构与限制**：
   - 阅读 `references/architecture.md`。
   - 确保功能描述符合实际情况（注意网络延迟可能有所不同；内存查询功能为尽力而为）。
3. **运行发布前的检查清单**：
...
- 架构与行为说明：`references/architecture.md`
