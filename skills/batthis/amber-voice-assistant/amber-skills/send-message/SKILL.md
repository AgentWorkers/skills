...
name: send-message
version: 1.0.0
description: "为操作员留下消息——该消息会被保存到通话记录中，并通过操作员偏好的消息传递渠道发送给操作员。"
...
该功能允许来电者为客服人员留言。该功能实现了电话助手中常见的“留言”操作模式。
...
3. 确认后，留言会：
   - **首先**被保存到通话记录中（作为审计追踪）；
   - **随后**通过客服人员配置的消息通道发送给客服人员。

## 安全性

- 收件人由客服人员的配置决定，绝不会根据来电者的输入来决定。
- 数据模型中没有任何参数用于指定消息的接收者或发送目的地。
- 在发送留言之前必须进行确认（通过大语言模型（LLM）的功能描述来强制执行）。
- 消息内容会经过处理（限制最大长度，并删除控制字符）。

## 信息传递失败处理

- 如果消息传递失败，通话记录会标记为“delivery_failed”。
- 客服人员的助手可以在定期检查时查看未送达的留言。
- Amber会告诉来电者：“我已经收到您的留言了”，但不会承诺具体的传递渠道。