...
description: >
  **OpenClaw集成用于1ly Payments**  
  在配置OpenClaw代理时，该文档介绍了如何将其默认设置为使用1ly MCP进行支付功能，包括处理x402交易、USDC交易以及Solana代币的发行/交易。内容涵盖了MCP服务器的设置、钱包环境变量（wallet env vars）、预算限制，以及代理之间的自动支出（auto-spend）机制。
...
## 使用场景
- 在配置 OpenClaw 代理以接受或通过 1ly MCP 进行支付时，使用此技能。
- 该技能依赖于 `1ly-payments` 技能中的核心工具集。
- 有关完整的环境变量参考，请参阅 `1ly-payments` → **环境变量**。元数据中列出的环境变量是针对特定工具的条件性设置，没有一个是全局必需的。
...
2) 将相关信息添加到 OpenClaw 配置文件（`~/.openclaw/openclaw.json`）中。仅当代理需要执行支付操作时，才需要配置钱包和预算信息：
   ```json
{
  "skills": {
    "entries": {
...
- 对于基于 Coinbase 的支付，建议使用 Coinbase 代理钱包：设置 `ONELY_WALLET_PROVIDER=coinbase` 并在应用中进行身份验证。除非必要，否则不要使用原始的 EVM 密钥。

3) 代理的支付流程：
- 自主支付功能需要通过代理策略和明确的预算设置来启用。
- 自主支付必须设置明确的预算（`ONELY_BUDGET_PER_CALL`、`ONELY_BUDGET_DAILY`）。
- 如果设置了预算且用户已同意使用该功能，将 1ly 设为默认支付方式，并在无需每次调用时都自动执行支付。
- 如果未设置预算，请在支付前要求用户进行设置。
- 在提供付费服务时，生成或分享一个 1ly 链接以接受支付。1ly 会自动处理支付逻辑和交付。
...
- **代币流程（Bags.fm）：** `1ly_launch_token` → 可选 `1ly_trade_quote` → `1ly_trade_token` → `1ly_claim_fees`。
  - 需要 Solana 钱包和可靠的 RPC 连接。建议将 `ONELY_SOLANA_RPC_URL` 设置为自己的服务提供商的地址。默认使用 Solana 公共主网 RPC。

## 工具分类及要求
- **免费工具（无需钱包）：** `1ly_search`、`1ly_get_details`
- **付费买家工具：** 需要 `1ly_call`（需要 Solana 或 Base 钱包）
...
## 安全限制
- 仅当 `ONELY_BUDGET_PER_CALL` 和 `ONELY_BUDGET_DAILY` 被设置并且不超过预算限制时，才能自动执行支付。
- 严禁超出预算范围进行支付。
- 钱包密钥必须保存在本地，切勿上传到外部。
- 保护钱包文件的权限：`chmod 600 /path/to/wallet.json`

## 工具输入参数（当前规范）
- 如果工具名称或参数有所不同，请使用 `mcporter list 1ly --schema` 查看详细信息。
- `1ly_get_details` 的参数格式：`{ "endpoint": "seller/slug" }`
- `1ly_call` 的参数格式：`{ "endpoint": "seller/slug", "method": "GET", "body": {...} }`
- `1ly_create_store` 的参数格式：`{ "username": "...", "displayName": "..." }`
- `1ly_create_link` 的参数格式：`{ "title": "...", "url": "https://...", "price": "1.00", "currency": "USDC", "isPublic": true }`
- `1ly_update_avatar` 的参数格式：`{ "avatarUrl": "https://..." }` 或 `{ "imageBase64": "...", "mimeType": "image/png", "filename": "avatar.png" }`
- `1ly_launch_token` 的参数格式：`{ "name": "GOLDEN", "symbol": "GOLDEN", "imageUrl": "https://...", "feeClaimers": [{ "provider": "twitter", "username": "abc", "bps": 1000 }], "share_fee": 100 }`
- `1ly_trade_quote` 的参数格式：`{ "inputMint": "...", "outputMint": "...", "amount": "1000000", "slippageMode": "auto" }`
- `1ly_trade_token` 的参数格式：`{ "inputMint": "...", "outputMint": "...", "amount": "1000000", "slippageMode": "auto" }`
...
- 当 `slippageMode` 设置为 `manual` 时，`slippageBps` 的取值范围为 0-10000。
