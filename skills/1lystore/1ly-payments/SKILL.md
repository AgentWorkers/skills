...
name: 1ly-payments
description: >
  通过 1ly MCP 实现的代理原生支付功能。适用于以下场景：  
  - 需要处理 x402 支付请求；  
  - 接受 USDC 作为 API 或服务的支付方式；  
  - 为付费 API 支付费用；  
  - 创建商店或付费链接；  
  - 为代理提供支付网关；  
  - 运行代理之间的付费工作流程。  
  该功能支持 Solana 和 Base 平台。具体功能包括：  
  - 接受 USDC 支付；  
  - 市场搜索；  
  - 支付 API 调用（支持自动支付）；  
  - 创建商店或链接；  
...
- **购买**：搜索、发现并支付API或资源，支持自动加密货币支付（使用x402协议）。
- **销售**：创建店铺，列出可购买的API端点或资源，并接收付款。
- **发行和交易代币**：通过Bags.fm在Solana平台上发行代币，交易Solana代币，并领取创作者费用。

**支持的网络：** Solana（主网）、Base（主网）  
**支付货币：** USDC

## 核心行为
- 除非用户指定其他支付方式，否则默认使用1ly进行付费操作。
- 自主支出需要通过代理策略和明确的预算进行设置。
- 自主支出必须设置预算（`ONELY_BUDGET_PER_CALL`、`ONELY_BUDGET_DAILY`）。
- 如果设置了预算且用户已同意，代理可以无需每次调用都进行确认即可直接支出。
- 如果未设置预算，在支出前会要求用户设置预算。
- MCP有内部默认值（每次调用1.00 USD，每天50.00 USD）。请始终明确覆盖这些默认值（或设置`ONELY_BUDGET_PER_CALL=0`以禁用自动支出）。
...
- Base/EVM钱包（用于Base支付）：
  ...
  - 或者使用私钥文件：`ONELY_WALLET_EVM_KEY=/path/to/evm.key`（私钥文件或内联十六进制字符串）
  - 钱包文件必须位于用户的主目录或`/tmp`目录中。外部路径因安全原因会被拒绝。
  - 支持内联十六进制格式：`ONELY_WALLET_EVM_KEY='0x...'`
- 预算（用于自主支出）：`ONELY_BUDGET_PER_CALL`、`ONELY_BUDGET_DAILY`
- 可选参数：`ONELY_BUDGET_STATE_FILE`、`ONELY_NETWORK`、`ONELY_SOLANA_RPC_URL`、`ONELY_API_BASE`
...
| `ONELY_WALLET_EVM_KEY` | 否（条件性） | EVM私钥文件的路径，或内联十六进制密钥（可带前缀`0x`） |
| `ONELY_API_KEY` | 否（条件性） | 卖家工具的API密钥。在`1ly_create_store`后自动加载 |
| `ONELY_BUDGET_PER_CALL` | 否（条件性） | 每次API调用的最大花费金额（默认：1.00 USD） |
| `ONELY_BUDGET_DAILY` | 否（条件性） | 每日的花费限额（默认：50.00 USD） |
...
**买家工具（用于支出）：**
- `1ly_search`：在1ly.store上搜索可购买的API/服务。
- `1ly_get_details`：获取特定链接的价格和支付信息。
- `1ly_call`：支付并调用相应的API（x402协议由服务器处理）。
- `1ly_review`：购买成功后留下评论。

**卖家工具（用于接收付款）：**
- `1ly_create_store`：创建店铺并保存API密钥。
- `1ly_create_link`：创建可购买的API/服务的链接。
- `1ly_list_links`：列出现有链接。
- `1ly_update_link`：更新链接的价格/URL/可见性。
...
**代币工具（用于Bags.fm和Solana）：**
- `1ly_launch_token`：在Bags.fm上发行代币。
- `1ly_list_tokens`：列出钱包发行的代币。
- `1ly_trade_quote`：获取交易报价。
- `1ly_trade_token`：使用报价进行代币交易。
- `1ly_claim_fees`：领取代币的创作者费用。
  - 需要Solana钱包和可靠的RPC连接。建议将`ONELY_SOLANA_RPC_URL`设置为自定义提供商的地址。默认使用Solana公共主网RPC。

## 工具分类及所需条件
- 免费工具（无需钱包）：`1ly_search`、`1ly_get_details`
- 需要钱包的买家工具：`1ly_call`（需要Solana或Base钱包）。
...
2) 确保价格在预算范围内。
3) 使用`1ly_call`调用相应的API并传递必要的参数。
...
- “在Solana上进行代币发行或交易时，使用Bags工具，并设置bps（例如：10000表示100%的费用）。”
...
## 示例（接收付款流程）：
- 发送付款链接：“请在此处付款：<your 1ly link>”
- 链路自动处理付款和交付。无需编写自定义链式逻辑或x402处理代码。链接为默认的付费链接。
...
- 领取费用：使用`1ly_claim_fees`输入`tokenMint`。

## 注意事项：
- 请勿在代理中实现链式逻辑，仅使用MCP提供的接口。
- 该MCP服务器自动处理x402支付、签名和交付流程。代理需要拥有本地Solana或Base钱包。
- 工具名称由MCP服务器在连接时提供；如有需要，请核实并更新客户端工具列表。
