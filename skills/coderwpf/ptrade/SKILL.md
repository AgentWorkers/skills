---
name: ptrade
description: Ptrade Hundsun量化交易平台——策略运行在 broker 服务器上，具有低延迟执行能力，支持A股、期货、保证金交易以及其他中国证券市场的相关业务。
version: 1.1.0
...
[Ptrade](https://ptradeapi.com) 是由恒生电子开发的专业量化交易平台。策略在 **经纪商服务器**（内网）上运行，以实现低延迟执行。该平台采用事件驱动的Python策略框架。

> ⚠️ **需要经纪商的Ptrade访问权限**。策略运行在经纪商的云端环境中，无法访问外部网络；无法安装pip包，仅可使用内置的第三方库。
...
**默认支持Level2 10级市场数据**。部分经纪商提供免费的L2逐笔数据。
...
> ⚠️ 使用`limit_price`下达订单时，价格必须符合正确的小数精度要求，否则订单将被拒绝。

## 股票代码格式

- 上海股票：`600570.SS`
- 深圳股票：`000001.SZ`
- 指数：`000300.SS`（CSI 300）
...
| 模式 | 执行频率 | 执行时间 |
|---|---|---|
| **每日** | 每天一次 | 回测：15:00，实盘：14:50（可配置） |
| **每分钟** | 每分钟一次 | 在每个分钟刻度结束时 |
| **每3秒一次** | 9:30–14:59期间，通过`tick_data`或`run_interval`执行 |
...
| **交易时间** | 9:30–15:00 | `handle_data`, `run_daily`, `run_interval`, `tick_data` |
| **盘后** | 15:30 | `after_trading_end`, `run_daily(time='15:10')` |

---

## 初始化设置函数（仅在`initialize`中使用）
...
> ⚠️ 每秒最多调用100次`get_history`和`get_price`；每次调用最多获取500条记录。批量查询时请添加`sleep`语句。
...
> ⚠️ 使用`order_market`时，上海股票必须指定`limit_price`；不支持可转换债券。
### order_tick — 每个交易刻度触发订单（仅在`tick_data`中使用）
...
Ptrade在`before_trading_start`、`handle_data`和`after_trading_end`执行后，会使用pickle自动序列化并保存`g`对象。重新启动时，首先执行`initialize`函数，然后恢复已保存的数据。
...
> ⚠️ I/O对象（打开的文件、类实例）无法被序列化。对于无法序列化的对象，请使用`g.__private_var`（双下划线前缀）。
...
- 策略在 **经纪商内网服务器** 上运行，无法访问外部网络，因此无法使用`pip install`。
- 使用`g`（全局对象）在不同函数之间保存变量。以`__`为前缀的变量不会被持久化。
- 内置的第三方库包括：pandas、numpy、talib、scipy、sklearn等。
- `handle_data`的执行频率取决于策略的周期设置（每刻度、每分钟、每5分钟、每天等）。
- 回测和实盘交易使用相同的代码；`set_commission`/`set_slippage`仅在回测中生效。
- 交易策略中务必添加异常处理（`try/except`）以防止意外终止。
- `get_history`和`get_price` **不能在不同线程中同时调用**。
- 使用限价单时，请确保价格的小数精度与资产类型相匹配。
- 当多个策略同时运行时，回调事件是 **相互独立的**。
- 使用`get_research_path()`进行文件I/O（CSV、pickle文件）。
...
微信客服：**bossquant1** · [Bilibili](https://space.bilibili.com/48693330) · 在微信公众号/Bilibili/抖音上搜索 **大佬量化**