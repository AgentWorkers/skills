---
name: garmin-connect
description: Garmin Connect CLI（命令行接口）可用于管理各种功能，包括活动记录、健康数据、身体成分分析、锻炼计划、设备信息、目标设置等。
...
- 无界面模式：`gccli auth login you@example.com --headless`（使用 `--mfa-code <code>` 进行多因素认证）
- 设置默认账户：`export GCCLI_ACCOUNT=you@example.com`
- 验证登录状态：`gccli auth status`

## 输出格式

- 默认输出为易于阅读的表格格式；使用 `--json` 或 `-j` 可以获取 JSON 格式，`--plain` 可以获取 TSV 格式。
- 数据输出到标准输出（stdout），错误信息输出到标准错误输出（stderr）。
- 在程序化解析输出时，始终使用 `--json` 格式。
...
- 获取授权令牌（用于脚本编程）：`gccli auth token`
- 删除登录凭证：`gccli auth remove`
...
- 查看活动期间的天气情况：`gccli activity weather <id>`
- 查看活动中的心率区间：`gccli activity hr-zones <id>`
- 查看活动中的功率区间：`gccli activity power-zones <id>`
...
- 创建新的跑步活动：`gccli activity create --name "Morning Run" --type running --date 2024-06-15T07:30:00 --duration 1800 --distance 5000`
...
- 查看指定日期范围内的每日步数：`gccli health steps daily --start 2024-01-01 --end 2024-01-31`
- 查看指定日期范围内的每周步数：`gccli health steps weekly --weeks 4`
...
- 查看指定日期范围内的每周压力水平：`gccli health stress weekly --weeks 4`
- 查看身体电池电量：`gccli health body-battery [date]`
- 查看身体电池电量的变化范围：`gccli health body-battery range --start 2024-01-01 --end 2024-01-31`
...
- 查看最大摄氧量（VO2max）等健康指标：`gccli health max-metrics [date]`
- 查看乳酸阈值：`gccli health lactate-threshold`
- 查看骑行时的 FTP 阈值：`gccli health cycling-ftp`
...
- 查看指定日期范围内的每周训练强度：`gccli health intensity-minutes weekly --start 2024-01-01 --end 2024-01-31`
- 查看健康事件：`gccli health events [date]`
- 查看身体成分数据：`gccli body composition [date]`
- 查看身体成分的变化范围：`gccli body composition --start 2024-01-01 --end 2024-01-31`
- 查看体重变化记录：`gccli body weigh-ins --start 2024-01-01 --end 2024-01-31`
...
- 从 GPX 文件导入课程数据（默认格式为骑行课程）：`gccli courses import route.gpx`
- 通过名称导入课程数据：`gccli courses import route.gpx --name "Sunday Ride"`
- 按类型导入课程数据：`gccli courses import route.gpx --type gravel_cycling`
- 导入公开课程数据：`gccli courses import route.gpx --privacy 1`
- 将课程数据发送到设备：`gccli courses send <course-id> <device-id>`
- 删除课程数据：`gccli courses delete <id>`（使用 `-f` 可跳过确认）
...
- 记录水分摄入量：`gccli hydration add 500`
- 查看训练计划：`gccli training plans --locale en`
- 查看训练计划的详细信息：`gccli training plan <id>`
- 查看月经周期数据：`gccli wellness menstrual-cycle --start-date 2024-01-01 --end-date 2024-03-31`
- 查看怀孕相关健康信息：`gccli wellness pregnancy-summary`
- 列出所有事件：`gccli events list`
- 根据日期查询事件：`gccli events list --start 2024-06-01 --limit 50`
- 添加新事件：`gccli events add --name "Berlin Marathon" --date 2026-09-27 --type running --race --location "Berlin, Germany" --distance 42.195km --time 09:15 --timezone Europe/Berlin`
- 添加带有目标和训练优先级的事件：`gccli events add --name "Spring 10K" --date 2026-05-10 --type running --distance 10km --goal 40m --training`
- 删除事件：`gccli events delete <id>`（使用 `-f` 可跳过确认）
- 重新加载数据：`gccli reload [date]`

## 注意事项

- 为避免重复输入，建议设置环境变量 `GCCLI_ACCOUNT=you@example.com`。
- 在脚本中使用 `--json` 以获取 JSON 格式输出，使用 `--plain` 以获取 TSV 格式输出。
- 支持的日期格式包括 `today`、`yesterday`、`3d` 和 `YYYY-MM-DD`。
- 授权令牌安全存储在操作系统的密钥链（macOS Keychain、Linux Secret Service）中；如果密钥链不可用，则会使用文件存储。
- 当遇到 401 错误时，令牌会自动刷新；遇到 429 或 5xx 错误时，系统会尝试重试，采用指数级退避策略。
- 如果使用 Garmin China 账户，请设置 `export GCCLI_DOMAIN=garmin.cn`。
- 在删除活动或训练计划前，请先确认操作（或使用 `--force` 快捷键）。
- 支持的数据下载格式包括 FIT、GPX、TCX 和 KML。
- 训练计划的步骤格式为 `type:duration[@target:low-high]`，其中 `type` 可以是 `warmup`、`run`、`recovery` 或 `cooldown`，`target` 可以是配速（分钟：秒）、心率（bpm）、功率（瓦特）或踏频。