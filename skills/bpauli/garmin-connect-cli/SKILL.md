---
name: garmin-connect
description: Garmin Connect CLI（命令行接口）支持管理各种与Garmin设备相关的功能，包括活动记录、健康数据、身体成分分析、锻炼计划、设备信息、个人目标等。
...
使用 `gccli` 可以查询 Garmin Connect 中的健康、健身和活动数据。需要使用 Garmin 的单点登录（SSO）进行身份验证。
...
- 查看身体电量范围：`gccli health body-battery range --start 2024-01-01 --end 2024-01-31`
...
- 删除训练计划：`gccli workouts remove <schedule-id>`（使用 `--force` 可跳过确认）
...
**注意事项：**
- 设置 `GCCLI_ACCOUNT=you@example.com` 可以避免重复输入 `--account` 参数。
- 在脚本编程中，使用 `--json` 获取 JSON 格式输出，使用 `--plain` 获取 TSV 格式输出。
- 支持的日期格式包括 `today`、`yesterday`、`3d` 和 `YYYY-MM-DD`。
- 身份验证令牌安全存储在操作系统的密钥链（macOS Keychain、Linux Secret Service）中；如果令牌无效（401 错误），系统会自动尝试重试（429/5xx 错误时采用指数级重试策略）。
- 对于 Garmin China 账户，请设置 `export GCCLI_DOMAIN=garmin.cn`。
- 在删除活动或训练计划前请先确认；如需强制删除，请使用 `--force` 参数。
- 支持的数据下载格式包括 FIT、GPX、TCX 和 KML。
- 训练计划的步骤格式为 `type:duration[@target:low-high]`，其中：
  - `type` 可选值：warmup（热身）、run（跑步）、recovery（恢复）、cooldown（冷却）
  - `target` 可选值：pace（配速，单位：分钟）、hr（心率，单位：bpm）、power（功率，单位：瓦特）、cadence（踏频）。