...
使用 `gccli` 可以查询 Garmin Connect 中的健康、健身和活动数据。需要使用 Garmin 的 SSO（单点登录）进行身份验证。

**设置（仅需执行一次）：**
- `gccli auth login you@example.com`（会打开浏览器进行 Garmin SSO 登录）
- 无界面模式：`gccli auth login you@example.com --headless`（使用 `--mfa-code <code>` 可以进行多因素认证）
- 设置默认账户：`export GCCLI_ACCOUNT=you@example.com`
- 验证身份：`gccli auth status`

**输出格式：**
- 默认格式为易于阅读的表格；使用 `--json` 或 `-j` 可以获取 JSON 格式，`--plain` 可以获取 TSV 格式。
- 数据输出到标准输出（stdout），错误信息输出到标准错误输出（stderr）。
- 在程序解析输出时，请务必使用 `--json` 格式。
...
- 查看活动的锻炼组：`gccli activity exercise-sets <id>`
...
- 预安排训练计划：`gccli workouts schedule add <id> 2024-06-20`
- 查看预安排的训练计划：`gccli workouts schedule list 2024-06-20`
- 删除预安排的训练计划：`gccli workouts schedule remove <schedule-id>`（使用 `--force` 可跳过确认）
- 删除训练计划：`gccli workouts delete <id>`
- 创建跑步训练计划（包含配速要求）：`gccli workouts create "Easy Run" --type run --step "warmup:5m" --step "run:20m@pace:5:00-5:30" --step "cooldown:5m"`
- 创建有心率目标的训练计划：`gccli workouts create "HR Run" --type run --step "warmup:10m" --step "run:20m@hr:140-160" --step "cooldown:10m"`
- 创建骑行训练计划（包含功率要求）：`gccli workouts create "FTP Intervals" --type bike --step "warmup:10m" --step "run:5m@power:250-280" --step "recovery:3m" --step "run:5m@power:250-280" --step "cooldown:10m"`
- 列出所有课程：`gccli courses list`
- 将课程添加到收藏夹：`gccli courses favorites`
- 查看课程详情：`gccli courses detail <id>`
- 将课程发送到设备：`gccli courses send <course-id> <device-id>`
...
**注意事项：**
- 为了避免重复输入，请将 `GCCLI_ACCOUNT` 设置为 `you@example.com`。
- 在脚本编程中，使用 `--json` 以获取 JSON 格式输出，使用 `--plain` 以获取 TSV 格式输出。
- 支持的日期格式包括 `today`、`yesterday`、`3d` 和 `YYYY-MM-DD`。
- 身份验证令牌安全存储在操作系统的密钥链中（macOS 的 Keychain、Linux 的 Secret Service 或文件）。
- 如果遇到 401 错误，令牌会自动刷新；如果遇到 429/5xx 错误，系统会进行指数级重试。
- 如果使用 Garmin China 账户，请设置 `export GCCLI_DOMAIN=garmin.cn`。
- 在删除活动或训练计划前，请先确认操作（或使用 `--force` 选项）。
...
- 训练计划的步骤格式为 `type:duration[@target:low-high]`，其中：
  ...
  - `target` 可以是 `pace`（配速，单位：分钟/秒）、`hr`（心率，单位：bpm）、`power`（功率，单位：瓦特）或 `cadence`（踏频）。