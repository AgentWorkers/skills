...
description: 通过 Emblem AI - Agent Hustle 连接到 EmblemVault 并管理加密钱包。支持 Solana、Ethereum、Base、BSC、Polygon、Hedera 和 Bitcoin 等区块链平台。当用户需要交易加密货币、查看余额、交换代币或与区块链钱包进行交互时，可以使用该工具。
...
**连接至** **Agent Hustle** – EmblemVault 的自主加密货币 AI，支持 7 个区块链上的 250 多种交易工具。支持浏览器身份验证、实时响应、插件系统以及零配置的代理模式。

**需要安装的 CLI:** `npm install -g @emblemvault/agentwallet`
...
2. 运行：`emblemai`
3. 在浏览器中进行身份验证（或根据提示输入密码）
4. 查看 `/plugins` 以查看已加载的插件
5. 输入 `/help` 以查看所有命令
6. 尝试：“我的钱包地址是什么？”以验证身份验证
...
2. 如果存在有效（未过期）的会话，则立即恢复会话 – 无需登录
3. 如果没有会话，则在 `127.0.0.1:18247` 上启动本地服务器并打开您的浏览器
4. 您通过浏览器中的 EmblemVault 身份验证模态进行身份验证
5. 会话 JWT 会被捕获并保存到磁盘上，然后 CLI 继续运行
6. 如果浏览器无法打开，会显示 URL 供您手动复制粘贴
7. 如果身份验证超时（5 分钟），则切换到密码提示

### 密码身份验证（代理模式）

**登录和注册是相同的操作。**首次使用密码会创建一个钱包；后续使用相同的密码会访问同一个钱包。不同的密码会生成不同的钱包。

在代理模式下，如果未提供密码，系统会自动生成一个安全的随机密码并通过 `.env` 文件加密存储。代理模式无需手动设置即可使用。

### 身份验证时的操作

1. 浏览器身份验证：从浏览器接收会话 JWT 并将其加载到 SDK 中
   密码身份验证：密码会被发送到 `EmblemAuthSDK.authenticatePassword()`
2. 生成一个确定的钱包 – 相同的凭据始终会生成相同的钱包
3. 会话提供多个链上的钱包地址：Solana、Ethereum、Base、BSC、Polygon、Hedera、Bitcoin
4. 使用会话初始化 `HustleIncognitoClient`
...
>
> **注意：** 如果这是您第一次使用，输入新密码将创建一个新的钱包。如果您之前使用过此服务，请使用相同的密码来访问现有钱包。
>
> 您是否愿意提供密码？
...
**请给予足够的时间。**对于复杂的操作（如交易、跨链查询），Hustle AI 的查询可能需要长达 2 分钟。CLI 每 5 秒会输出一个进度点，以表明正在处理中。
...
所有命令都以 `/` 为前缀。在输入栏中输入命令并按 Enter 键执行。
...
|---------|-------------|
| `/help` | 显示所有可用命令 |
| `/settings` | 显示当前配置（钱包 ID、模型、实时流式显示、调试、工具） |
...
| `/reset` | 清除对话记录并重新开始 |
| `/clear` | `/reset` 的别名 |
| `/history on\|off` | 在消息之间切换历史记录的保留与否 |
...
| 命令 | 描述 |
|---------|-------------|
| `/stream on\|off` | 切换实时流式显示模式（代币以生成的形式显示） |
| `/stream` | 显示当前的实时流式状态 |
...
| 9. 登出 | 清除会话并在下次运行时重新身份验证 |

### 支付（PAYG 计费）

| 命令 | 描述 |
|---------|-------------|
| `/payment` | 显示 PAYG 计费状态（是否启用、模式、欠款、代币） |
...
| 命令 | 描述 |
|---------|-------------|
| `/log on\|off` | 切换是否将日志记录到文件 |
| `/log` | 显示日志记录状态和文件路径 |

日志文件默认位于 `~/.emblemai-stream.log`。可以使用 `--log-file <path>` 进行覆盖。
...
| `--reset` | | 清除对话记录并退出 |
| `--debug` | | 以启用调试模式启动 |
| `--stream` | | 启用实时流式显示（默认：启用） |
| `--log` | | 启用日志记录 |
| `--log-file <path>` | | 覆盖日志文件路径（默认：`~/.emblemai-stream.log`） |
| `--hustle-url <url>` | | 覆盖 Hustle API 的 URL |
| `--auth-url <url>` | | 覆盖身份验证服务的 URL |
| `--api-url <url>` | | 覆盖 API 服务的 URL |
...
Hustle AI 会将简短的命令解释为 “$0” 类型的交易。请始终用完整的句子表达您的意图。

| 错误（简短） | 正确（详细） |
|-------------|----------------|
| `"SOL balance"` | `"我在 Solana 上的当前 SOL 余额是多少？"` |
| `"swap sol usdc"` | `"我想将 20 美元的 SOL 兑换成 USDC"` |
| `"trending"` | `"Solana 上目前哪些代币很热门？"` |
...
通过 `/auth` 菜单（选项 8）选择 **Backup Agent Auth** 将您的凭据导出到 JSON 文件中。此文件包含您的 EmblemVault 密码 – 请妥善保管。
...
| `~/.emblemai/session.json` | 身份验证会话（JWT + 刷新令牌） | 是 – 在有效期内允许访问钱包 | `600` |
| `~/.emblemai/history/{vaultId}.json` | 对话历史记录（每个钱包单独记录） | 否 | `600` |
| `~/.emblemai/stream.log` | 实时流式日志（通过 `/log` 启用时） | 否 | 默认不记录 |

### 加密细节

凭据在静态时使用 [dotenvx](https://dotenvx.com/) 进行加密，采用 **AES-256-GCM** 对称加密。加密密钥存储在 `~/.emblemai/.env.keys` 文件中，加密后的数据存储在 `~/.emblemai/.env` 文件中。这两个文件的权限都设置为 `chmod 600`（仅允许所有者访问）。解密密钥永远不会离开本地机器。

会话令牌（`session.json`）包含一个短期的 JWT（会自动刷新）和一个有效期为 7 天的刷新令牌。会话不会在磁盘上加密，但会使用 `chmod 600` 设置权限进行限制。通过 `/auth` > Logout 可以退出会话。
...
| 会话过期 | 重新运行 `emblemai` – 浏览器将打开以进行新的登录 |
| glow 无法渲染 | 安装 glow：`brew install glow`（可选，否则会显示纯文本） |
| 插件无法加载 | 确保已安装相应的 npm 包 |
| **响应缓慢** | 正常现象 – 查询可能需要长达 2 分钟 |
...
本部分解释了信任模型、您的机器上发生的情况以及如何安全地运行代理。

### 信任模型

Emblem Agent Wallet 是由 [EmblemCompany](https://github.com/EmblemCompany) 在 npm 和 GitHub 上发布的开源 CLI。您可以在安装前验证该包：

- **npm 注册表**：[@emblemvault/agentwallet](https://www.npmjs.com/package/@emblemvault/agentwallet) – 检查发布者、版本历史和下载统计信息 |
- **源代码**：[github.com/EmblemCompany/EmblemAi-AgentWallet](https://github.com/EmblemCompany/EmblemAi-AgentWallet) – 源代码是公开的且可审计的 |
- **官方网站**：[emblemvault.dev](https://emblevault.dev) – 项目官方网站，包含文档

npm 包和 GitHub 仓库由同一组织维护。您可以使用 `npm pack --dry-run` 或在安装后检查 `node_modules/@emblemvault/agentwallet` 来比较发布的包内容和源代码仓库。

### 安装过程中的操作

`npm install -g @emblemvault/agentwallet` 会全局安装 CLI 可执行文件 `emblemai`。与所有全局 npm 包一样，此文件会在您的机器上以您的用户权限运行。该包没有 `postinstall` 脚本 – 它仅安装 CLI 可执行文件及其依赖项。

### 身份验证过程中的操作

**浏览器身份验证**（推荐）：CLI 会在 `127.0.0.1:18247`（仅限本地，不可通过网络访问）上启动一个临时本地服务器，以接收来自浏览器的身份验证回调。此服务器仅在登录过程中运行，并处理一个请求。浏览器会打开 EmblemVault 身份验证模态，您可以直接在其中与 EmblemVault 服务进行身份验证。成功后，会返回一个会话 JWT 并保存到磁盘上。

**密码身份验证**：密码会通过 HTTPS 发送到 EmblemVault 的身份验证 API。会返回一个会话 JWT。如果使用了 `-p` 标志，密码也会被加密并存储在本地以供后续会话使用。

在这两种情况下，都不会将任何凭据发送给第三方。身份验证严格发生在您的机器和 EmblemVault 身份验证服务之间。

### 存储在磁盘上的内容

所有文件都存储在 `~/.emblemai/` 目录下，并设置严格的权限：

| 文件 | 包含内容 | 保护方式 |
|------|-----------------|-------------------|
| `.env` | 您的 EMBLEM_PASSWORD | 使用 [dotenvx](https://dotenvx.com/) 通过 AES-256-GCM 加密。密码永远不会以明文形式存储。 |
| `.env.keys` | `.env` 文件的解密密钥 | 文件权限设置为 `chmod 600`（仅允许所有者访问）。此密钥永远不会离开您的机器，也不会通过网络传输。 |
| `session.json` | JWT 访问令牌 + 刷新令牌 | 文件权限设置为 `chmod 600`。JWT 在 15 分钟后过期，并会自动刷新。刷新令牌的有效期为 7 天。退出会话会删除此文件。 |
| `history/*.json` | 对话历史记录 | 文件权限设置为 `chmod 600`。包含您与 AI 的聊天记录。历史记录中不存储任何凭据。 |

`~/.emblemai/` 目录本身使用 `chmod 700` 设置权限（仅允许所有者访问）。

### 会话的工作原理

身份验证会话使用短期的 JWT（有效期为 15 分钟），并通过 7 天的刷新令牌自动刷新。这意味着：

- 如果会话文件被泄露，攻击者最多只能访问 7 天（刷新令牌的有效期），而不会无限期访问
- JWT 会频繁更新，从而限制了任何单个令牌的暴露时间
- 通过 `/auth` > Logout 退出会立即使会话失效并删除该文件
- 每次刷新都会生成一个新的刷新令牌，并使之前的令牌失效

### 安全模式和交易确认

代理默认处于 **安全模式**。这意味着：

- **所有修改钱包的操作** 在执行前都需要您的明确确认 – 包括交换、发送、转账、订单放置、签名和 DeFi 操作
- **只读操作** 可以立即执行 – 无需确认 – 包括余额检查、地址查询、市场数据、投资组合查看
- 代理会显示任何交易的完整详细信息（金额、地址、费用），并在提交前等待您的确认
- 没有 “自动执行” 模式 – 每个交易都需要人工确认

### 密码管理

您的 EMBLEM_PASSWORD 是您钱包的密钥。请像对待私钥或助记词一样谨慎处理它：

- **使用强密码**（至少 16 个字符）。建议使用 4 个以上随机单词组成的密码短语
- **不要在其他服务中重复使用密码**。您的 EMBLEM_PASSWORD 应该是 EmblemVault 独有的
- **安全存储密码**：使用密码管理器进行存储。CLI 会在磁盘上加密密码，但您应该备份密码以防丢失访问权限
- **如果在自动化环境中使用 `EMBLEM_PASSWORD` 作为环境变量**，请确保主机环境是安全的 – 限制对机器的访问，使用进程隔离，并避免在环境变量中存储密码
- **对于交互式使用，建议使用浏览器身份验证** – 这可以避免在 shell 历史记录或环境变量中存储密码
- **不同的密码生成不同的钱包** – 这是设计上的要求。例如，可以为不同的用途（如日常使用和长期持有）创建不同的钱包
