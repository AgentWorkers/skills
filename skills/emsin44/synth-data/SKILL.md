---
name: synth-data
description: 从 Synthdata.co 查询加密货币、商品和股票的波动性预测数据。对比不同资产的表现，并运行蒙特卡洛模拟（Monte Carlo simulations）。
metadata:
  {
    "openclaw":
      {
...
```
使用预测的波动性生成价格区间（预测窗口为24小时）。

### 4. 定时报告
创建一个Cron作业，每天向Slack/Telegram发送预测报告（详见examples/use-cases.md）。
...
- **Polymarket**：利用波动性预测来指导市场方向判断（上涨/下跌）
- **期权交易**：预测波动性高时考虑购买期权
- **投资组合管理**：当整体波动性上升时重新平衡投资组合
- **警报系统**：当预测结果与实际波动性显著不符时发送通知