...
name: skill-auditor
version: 1.0.0
description: ClawHub 技能的安全扫描器：在安装前可检测恶意代码、混淆后的载荷以及社交工程攻击。采用三层分析机制：模式匹配、反混淆以及大语言模型（LLM）意图分析。
author: sypsyp97
---

# 技能审计器 🔍

在安装 ClawHub 的技能之前，对其进行安全威胁审计。
...
| 🟡 中等 | 访问 `~/.openclaw/` | 配置信息泄露 |
| 🟡 中等 | 读取 `$API_KEY` 等信息 | 凭据泄露 |
| 🟡 中等 | 社交工程相关关键词 | 用户被骗 |
| 🟢 低 | 需要 sudo 权限 | 权限提升 |
...
2. **不要盲目信任任何技能** — 包括评级很高或受欢迎的技能 |
3. **检查更新** — 技能更新可能会引入恶意代码 |
4. **报告可疑技能** — 发送到 steipete@gmail.com
