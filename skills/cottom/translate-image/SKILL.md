...
name: translateimage
version: "1.0.0"
description: "将图像中的文本翻译成中文，通过OCR技术提取文本，以及使用TranslateImage AI工具去除文本。"
...
当用户需要翻译图片中的文本、通过OCR提取文本或从图片中删除文本时，请使用此功能。

所有请求都会通过curl直接发送到`https://translateimage.io`上的TranslateImage REST API。
...
所有工具都支持以multipart文件的形式上传图片。请按以下方式处理图片输入：
...
翻译图片中的文本，同时保持原有的视觉布局。返回经过翻译的图片，格式为base64编码的数据URL。

**使用场景：** 用户需要阅读漫画、海报、菜单、产品标签或其他包含外语文本的图片。

**端点：** `POST https://translateimage.io/api/translate`

**表单字段：**
- `image`（文件，必填）—— 需要翻译的图片（格式：JPEG、PNG、WebP、GIF，最大文件大小10MB）
...
**使用场景：** 用户需要从照片、文档扫描件、截图、标志或标签中复制或读取文本。
...
检测图片中的文本区域，并使用AI技术将这些区域替换为背景图像，从而得到一张干净的图片。

**使用场景：** 用户希望去除图片中的文字、水印、嵌入的字幕或注释。
...
结合Gemini AI技术进行高质量的文本提取，并可选择将提取的文本同时翻译成多种语言。

**使用场景：** 当标准OCR无法满足需求时，或者用户需要同时提取和翻译文本时。
...
- 在进行翻译之前，请务必与用户确认目标语言。
- 对于漫画或海报等图像，建议使用`WildWords`或`BadComic`字体以保持真实的视觉效果。
- 对于中文内容，建议使用`MaShanZheng`字体；对于韩文内容，建议使用`RIDIBatang`字体。
- 大于5MB的图片处理时间可能会较长，请提前告知用户。
- AI修复技术对简单背景效果较好；复杂纹理可能导致修复效果不佳。
