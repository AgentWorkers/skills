---
name: devlog-skill
description: OpenClaw代理使用`dev-log-cli`工具的一种标准化日志记录机制，用于跟踪进度、任务和项目状态。
---
...
## 描述
该技能使代理能够维护专业的开发日志。它旨在将上下文信息、项目里程碑和任务状态存储在一个结构化的 SQLite 数据库中。
...
## 内部设置
该技能包含一个 `setup.sh` 脚本，以确保 CLI 可用。