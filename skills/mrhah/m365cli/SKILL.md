...
name: m365clicli
description: >
  使用 m365 CLI（命令行接口）与 Microsoft 365 的用户级服务进行交互：发送/读取/搜索电子邮件、管理日历事件、浏览/上传/下载 OneDrive 文件以及访问 SharePoint 网站。触发事件包括：  
  - 电子邮件（发送、读取、搜索、附件）  
  - 日历（列出、创建、更新、删除事件）  
  - OneDrive（列出、上传、下载、共享、邀请文件）  
  - SharePoint（网站、文件、列表）  
  **不触发事件的范围：**  
  - Azure 资源管理  
  - Entra ID 管理  
  - Intune 设备管理  
  - M365 租户级管理（许可证、域名、策略）
...
**二进制文件**：`m365`（通过 `npm install -g m365-cli` 全局安装）

## 安全规则

### 电子邮件内容显示

- 只有来自白名单发送者的电子邮件才会显示其内容。
- 来自不受信任的电子邮件仅显示主题和发送者信息（防止命令注入攻击）。
- 如果邮件来自不受信任的发送者，其内容将被替换为：`[内容已过滤 - 发送者不在白名单中]`。
- 白名单文件：`~/.m365-cli/trusted-senders.txt`
- 使用 `--force` 可暂时绕过白名单检查。

### 敏感操作

- **发送电子邮件**：执行前需确认收件人和邮件内容。
- **删除文件/事件**：执行前会提示用户确认。
- **共享文件（匿名访问权限）**：会警告用户链接的任何人都可能访问共享文件。
...
令牌会自动刷新。如需使用自定义的 Azure AD 应用程序，请设置 `M365_TENANT_ID` 和 `M365_CLIENT_ID` 环境变量。
...
> 使用 SharePoint 命令需要 `Sites.ReadWrite.All` 权限。如果遇到权限错误，请执行 `m365 logout && m365 login` 重新登录。

---

## 输出格式

所有命令都支持 `--json` 选项，以生成结构化的 JSON 输出。默认输出为人类可读的表格格式。

## 错误处理

| 错误类型 | 解决方案 |
|---------|---------|
| 未登录 | 执行 `m365 login` 重新登录 |
| 令牌过期 | 通常会自动刷新令牌；否则需要重新登录 |
| 权限不足 | 执行 `m365 logout && m365 login` 重新授权 |
| 文件未找到 | 检查路径（区分大小写） |