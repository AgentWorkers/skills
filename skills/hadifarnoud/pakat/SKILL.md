---
name: pakat
description: 与 Pakat 电子邮件营销 API（new.pakat.net）进行交互——需要设置 PAKAT_API_KEY 环境变量。当用户希望通过 Pakat 平台管理邮件列表、订阅者、营销活动、模板、交易邮件、用户分组，或查看营销活动的统计数据和发送日志时，可以使用该 API。该 API 会在提到 Pakat、电子邮件营销活动、邮件列表、订阅者管理或通过 Pakat 发送交易邮件时被触发。
---
# Pakat 邮件营销平台
...
- **HTML 内容必须使用 base64 编码**（`campaign[template][content]`、`email[body]`、`template[content]`）
- **交易邮件的发送时间（`send_at`）采用 UTC 格式，格式为 `Y-m-d H:i:s`
- **营销活动的发送时间（`send_at`）使用客户配置的时区**
...
如需了解完整的接口详情、请求/响应格式以及所有可用字段，请参阅 [references/api_reference.md](references/api_reference.md)。
