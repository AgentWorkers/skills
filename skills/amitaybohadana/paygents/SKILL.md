---
name: paygents
description: 在AI代理的对话中接受加密货币支付。生成MetaMask/Trust Wallet的深度链接，链上验证交易，查询余额，开具收据。无需托管服务，无需后端服务器，也无需API密钥。
metadata:
  openclaw:
    requires:
      bins:
...
2. 代理运行链接生成脚本，获取MetaMask的深链接。
3. 代理将链接发送给用户。
4. 用户点击链接后，MetaMask会自动弹出转账界面，用户只需确认即可。
5. 用户确认“发送”后，代理会在链上验证交易。
...
| `--chain-id` | 否 | 链ID（默认：`8453`（Base链） |
| `--asset` | 否 | 交易资产（默认：`ETH`或`ERC20`） |
| `--token` | 否 | 代币地址（对于USDC，系统会自动检测对应的ERC20合约地址） |
| `--decimals` | 否 | 代币的小数位数（USDC默认为6位，ETH默认为18位） |
...
- `--out <目录>` | 将收据文件保存到指定目录（JSON格式和Markdown格式）
- `--memo` | 交易订单ID或备注信息
- `--merchant` | 商家/收款人名称
...
如果未设置环境变量或配置文件，系统会自动使用公共RPC接口。公共接口（如`eth.llamarpc.com`）可能会显示用户的钱包地址和交易哈希。
