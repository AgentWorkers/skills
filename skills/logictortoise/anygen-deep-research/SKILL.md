...
description: "用户可以在任何需要深入研究或进行全面分析的场合使用此技能，涵盖以下领域：行业分析、竞争格局评估、市场规模估算、趋势分析、技术评估、投资研究、行业概况梳理、尽职调查、基准研究、专利分析、法规分析以及学术调研。此外，在用户发出以下指令时也应触发该技能：帮我调研一下、进行深度分析、进行行业研究、分析市场规模、研究竞争格局、分析技术趋势或编写研究报告。当需要深入研究或全面分析时，请使用此技能。"
metadata:
  clawdbot:
...
使用 AnyGen OpenAPI (`www.anygen.io`) 生成长篇研究报告。报告是在服务器端生成的；此技能会将用户的提示和可选的参考文件发送到 AnyGen API，并获取结果。需要一个 API 密钥 (`ANYGEN_API_KEY`) 来验证服务。

## 使用场景

- 用户需要一份深度研究报告（市场分析、行业分析、竞争分析、战略分析）
- 用户有文件需要作为研究参考资料上传

## 安全性与权限

研究报告由 AnyGen 的 OpenAPI (`www.anygen.io`) 在服务器端生成。`ANYGEN_API_KEY` 通过 `Authorization` 头部或认证后的请求体来验证请求（所有请求都设置了 `allow_redirects=False`）。

**此技能的功能：** 将提示发送到 `www.anygen.io`，在用户同意后上传指定的参考文件，将结果下载到 `~/.openclaw/workspace/`，通过 `sessions_spawn` 在后台监控进度（在 `requires` 中声明），并在 `~/.config/anygen/config.json` 中读写配置。

**此技能不执行以下操作：** 未经明确 `--file` 参数的情况下，不会读取或上传任何文件；不会向除 `www.anygen.io` 以外的任何端点发送凭证；不会访问或扫描本地目录；也不会修改系统配置（仅修改其自身的配置文件）。

**捆绑脚本：** `scripts/anygen.py`、`scripts/auth.py`、`scripts/fileutil.py`（使用 Python 和 `requests`）。这些脚本会将机器可读的标签（例如 `File Token:`、`Task ID:`）打印到标准输出，作为代理工具之间的通信方式。这些标签是非敏感的信息，仅用于会话范围，不属于凭证或 API 密钥。代理不应将原始脚本输出直接传递给用户，以保持对话的自然性（参见“通信风格”部分）。
...
使用自然语言进行交流。切勿向用户透露 `task_id`、`file_token`、`task_xxx`、`tk_xxx`、`anygen.py` 或命令语法。可以使用“你的研究报告”、“正在生成中”、“检查进度”等表达方式。在展示 `reply` 和 `prompt` 时，尽可能保留原始内容——如有需要，可以翻译成用户的语言，但不要重新表述、总结或添加自己的解释。提问时请使用你自己的语气（不要使用“AnyGen 想知道……”这样的表达）。在提示用户输入 API 密钥时，必须使用 Markdown 链接语法：`[获取你的 AnyGen API 密钥](https://www.anygen.io/home?auto_create_openclaw_key=1)`，以确保链接是可点击的。
...
2. **如果之前已经上传过相同的文件**，则直接使用现有的 `file_token`。
3. **读取文件** 并提取与研究相关的关键信息（主题、数据、结构）。
4. **上传文件** 以获取 `file_token`。
5. 在调用 `prepare` 时，将提取的内容包含在 `--message` 参数中（`prepare` 端点使用的是提示文本进行需求分析，而不是上传的文件内容）。仅总结关键点，不要原封不动地粘贴敏感数据。
...
从 `reply` 中向用户展示问题——如有需要，可以将其翻译成用户的语言。根据用户的回答继续进行：
...
- 如果用户表示“直接创建报告”，则直接进入第 3 阶段。

### 第 2 阶段：与用户确认（必选）

当 `status="ready"` 时，将 `reply` 和 `suggested_task_params` 中的提示内容展示给用户，作为研究报告的提纲。`prepare` 返回的提示已经是一个详细且结构良好的提纲——尽可能保留其原始内容。如果内容语言与用户的语言不同，请在保持结构和细节不变的情况下进行翻译。不要重新表述、总结或添加自己的解释。
...
3. 深度研究可能需要 **10–20 分钟** — 在此期间用户可以自由进行其他操作。
...
**重要提示**：调用 `sessions_spawn` 时，必须设置至少 25 分钟的超时时间，以确保后台监控完成。监控操作可能需要最多 20 分钟。
...
**处理完成事件**：后台监控会直接向用户发送完成通知和首次任务的建议（如果适用）。它的最终输出是 `ANNOUNCE_SKIP`，这意味着主会话不应重复或转发任何信息。如果收到 `ANNOUNCE_SKIP`，可以直接忽略它——因为用户已经收到了通知。

#### 备用方案（无后台监控）

告诉用户：“我已经开始生成你的研究报告了。深度研究可能需要 10–20 分钟。你可以在这里查看进度：[任务 URL]。当你准备好让我检查进度时，请告诉我！”

### 第 5 阶段：多轮对话（修改已完成报告）

任务完成后（即第 4 阶段完成），用户可能会请求进行修改，例如：
- “添加关于法规影响的章节”
- “扩展竞争分析”
- “补充更多市场规模的数据”
- “添加 SWOT 分析”

当用户请求修改已经完成的报告时，使用多轮对话 API 而不是创建新任务。

**重要提示**：在整个对话过程中，必须记住第 3 阶段中的 `task_id`。当用户请求修改时，请使用相同的 `task_id`。
...
告诉用户：“我已经发送了你的修改请求。你可以在这里查看进度：[任务 URL]。当你准备好让我检查进度时，请告诉我！”
