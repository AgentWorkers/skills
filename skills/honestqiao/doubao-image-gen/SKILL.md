...
使用 Zhipu 的网页搜索 API 在互联网上进行搜索。

## ⚠️ 安全要求

**使用此功能前，必须设置 `DOUBAO_API_KEY` 环境变量。**

### 安全最佳实践：

1. **切勿将 API 密钥存储在 `.bashrc` 文件中**——这可能导致密钥泄露。
2. **切勿从 shell 配置文件中读取内容**——以防止任意代码的执行。
3. **在运行脚本时直接设置环境变量**。
...
### ✅ 安全措施：
- 不从 `.bashrc` 或 shell 配置文件中读取 API 密钥。
...
### ⚠️ 需要注意的事项：
- **进程列表可见性**：API 密钥会显示在 `ps aux` 中——仅应在受信任的环境中使用该功能。
