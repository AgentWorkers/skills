---
name: adwhiz
description: 通过您的人工智能编程工具，您可以管理 Google 广告和 Meta（Facebook）广告。该工具提供了 99 种功能，支持使用自然语言进行广告账户的审计、创建和优化操作。
metadata:
  openclaw:
...
AdWhiz 是一个托管式的 MCP（Media Buying Center）服务器，它通过安全的、经过身份验证的代理将您的 AI 编码工具连接到 Google Ads API 和 Meta (Facebook) Graph API。该服务器提供了 **99 种工具**，涵盖 7 个类别，使您能够使用简单的英语来审核、创建和管理两个平台上的广告活动。

所有 API 调用都通过您的个人 `ADWHIZ_API_KEY` 进行身份验证，并通过位于 `mcp.adwhiz.ai` 的 AdWhiz 服务器进行路由。AdWhiz 服务器不会存储任何 Google 或 Meta 的凭据——身份验证完全在服务器端完成，您需要先在 https://adwhiz.ai/connect 链接您的广告账户。

## 安全性与权限模型

- **OAuth 2.0**：用户通过 AdWhiz 的 Web UI 使用 Google 和/或 Meta 进行身份验证。刷新令牌（Google）和长期有效的访问令牌（Meta）在存储时会被加密（AES-256-GCM），并且永远不会暴露给代理。
- **API 密钥范围**：每个 `ADWHIZ_API_KEY` 都绑定到单个用户的关联账户。跨用户访问是不允许的。
- **写入安全**：所有 Google Ads 的写入工具默认会将实体设置为 **暂停** 状态。Meta 的写入工具需要明确的状态参数。
- **变更记录**：每次变更都会被记录在 `get_operation_log` 工具中，以便进行全面审计。
- **默认为只读**：99 种工具中的 36 种仅具有读取权限，无法修改您的账户信息。
- **需要用户确认**：写入工具在执行前需要用户的确认。
...
| 工具 | 描述 |
|------|-------------|
| `list_campaigns` | 列出带有状态、类型、预算和出价策略的广告活动 |
...
| 工具 | 描述 |
|------|-------------|
| `create_campaign` | 创建搜索广告、展示广告、Performance Max 或视频广告活动（默认为暂停状态） |
...
| `create_responsive_search_ad` | 创建带有标题和描述的响应式搜索广告（默认为暂停状态） |
...
| `create_callout` | 创建插页广告资产 |
| `create_structured_snippet` | 创建结构化摘要资产 |
...
| `add_audience_to_campaign` | 向广告活动添加目标受众群体 |
...
| `upload_customer_list` | 上传哈希后的 PII 数据到客户匹配用户列表 |
| `apply_recommendation` | 应用 Google Ads 建议 |
| `dismiss_recommendation` | 排除 Google Ads 建议 |
| `add_geo_targeting` | 向广告活动添加地理位置定位 |
| `remove_geo_targeting` | 从广告活动中删除地理位置定位 |
| `add_ad_schedule` | 向广告活动添加日程安排（时间段） |
| `remove_ad_schedule` | 从广告活动中删除日程安排规则 |
| `set_device_bid_adjustment` | 设置设备出价调整（手机、桌面、平板电脑） |
| `set_demographic_targeting` | 设置人口统计目标（年龄、性别、收入） |
| `create_label` | 为实体创建标签 |
| `apply_label` | 将标签应用于广告活动、广告组或广告 |
| `remove_label` | 从广告活动、广告组或广告中删除标签 |
| `create_asset_group` | 为 Performance Max 广告活动创建资产组 |

### 审计相关工具（2 种）——仅限读取分析
| 工具 | 描述 |
| `run_mini_audit` | 快速三指标审计：浪费的支出、最佳/最差的 CPA、预计节省金额 |
...
### Meta 读取相关工具（13 种）——仅限读取
| 工具 | 描述 |
| `meta_list_ad_accounts` | 列出所有关联的 Meta 广告账户 |
| `meta_list_campaigns` | 列出带有状态和目标的广告活动 |
| `meta_get_campaign_insights` | 每个广告活动的指标：支出、点击量、转化率（CPA）、频率（可选细分） |
| `meta_list_ad_sets` | 列出带有定位和预算的广告集 |
| `meta_list_ads` | 列出带有创意详情的广告（标题、正文、图片链接） |
| `meta_get_account_insights` | 账户级别的汇总指标（每日细分） |
| `meta_get_ad_set_insights` | 每个广告集的绩效指标：支出、点击量、CPA、频率 |
| `meta_get_ad_insights` | 每个广告的绩效指标：支出、点击量、转化率 |
| `meta_get_ad_creatives` | 列出广告创意或获取特定广告的创意 |
| `meta_search_interests` | 根据关键词搜索可定向的兴趣受众 |
| `meta_search_geo_locations` | 搜索可用于定向的地理位置 |
| `meta_estimate_audience_size` | 估算定向范围的受众规模 |
| `meta_get_account_pages` | 列出可用于投放广告的 Facebook 页面 |

### Meta 写入相关工具（15 种）——需要用户确认
| 工具 | 描述 |
| `meta_set_campaign_status` | 暂停或激活 Meta 广告活动 |
| `meta_update_campaign_budget` | 更新 Meta 广告活动的每日或生命周期预算 |
| `meta_set_ad_set_status` | 暂停或激活 Meta 广告集 |
| `meta_set_ad_status` | 暂停或激活 Meta 广告 |
| `meta_create_campaign` | 创建新的 Meta 广告活动，设置目标和预算 |
| `meta_create_ad_set` | 创建带有定位和预算的广告集 |
| `meta_create_ad_creative` | 创建带有图片/视频、链接和呼叫动作的广告创意 |
| `meta_create_ad` | 创建将广告集与创意关联的广告 |
| `meta_update_campaign` | 更新广告活动名称、预算、结束时间、支出上限 |
| `meta_update_ad_set` | 更新广告集名称、预算、定位和日程安排 |
| `meta_update_ad` | 更新广告名称、创意或状态 |
| `meta_create_custom_audience` | 从客户列表或网站访问者创建自定义受众 |
| `meta_create_lookalike_audience` | 从源受众创建相似受众 |
| `meta_upload_ad_image` | 从 URL 上传广告创意的图片 |
| `meta_duplicate_campaign` | 复制带有所有广告集和广告的广告活动 |

### Meta 审计相关工具（1 种）——仅限读取分析
| 工具 | 描述 |
| `meta_run_mini_audit` | 快速健康状况审计：根据浪费的支出、CPA 效率、预算利用率和创意使用情况打分（0-100 分）

## MCP 服务器配置

AdWhiz 使用 **HTTP 传输** 与托管的 MCP 服务器连接。在运行时不会下载或执行任何 npm 包。
...
1. 在 https://adwhiz.ai 注册
2. 通过 OAuth 将您的 Google Ads 和/或 Meta Ads 账户关联到 AdWhiz
3. 从仪表板设置页面复制您的 API 密钥

## 示例指令

### Google Ads
- “审核我的 Google Ads 账户并显示浪费最高的 5 个领域”
- “暂停所有 CPA 超过 150 美元的广告活动”
- “将这些否定关键词添加到我的搜索广告活动中：[列表]”
- “创建一个新的搜索广告活动，目标为纽约的律师，预算为每天 100 美元”
- “显示浪费资金的搜索词并建议添加否定关键词”
- “我账户本月的平均质量得分是多少？”

### Meta Ads
- “审核我的 Meta Ads 账户并显示健康状况得分”
- “列出所有活跃的 Meta 广告活动及其支出和转化率”
- “暂停频率最高的 Meta 广告活动”
- “显示我 Meta 账户过去 30 天的每日支出趋势”
- “哪些 Meta 广告集的 CPA 最低？”

## 数据处理

- AdWhiz 仅访问您明确关联的广告账户
- 除了每次 API 请求的持续时间外，不会存储任何广告活动数据
- 变更日志会保留 30 天以供审计
