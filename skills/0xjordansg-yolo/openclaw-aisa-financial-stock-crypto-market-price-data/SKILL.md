...
**参数：**
- `ticker`：股票代码（必填）
- `interval`：时间间隔（可选，单位：秒、分钟、天、周、月、年）
- `interval_multiplier`：时间间隔的倍数（例如，5 表示 5 分钟的间隔）
...
| 端点 | 方法 | 描述 |
|----------|--------|-------------|
| `/financial/prices` | GET | 历史股票价格（需指定时间间隔） |
...
| `/financial/sec/items` | GET | SEC 文件详细内容 |
...
| `/financial/crypto/prices` | GET | 历史价格数据（OHLCV 格式） |
...
所有响应结果都会包含 `usage.cost` 和 `usage.credits_remaining` 字段。
