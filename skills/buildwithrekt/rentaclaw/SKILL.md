...
3. 生成一个 API 密钥
4. 将其设置为技能配置中的 `RENTACLAW_API_KEY`
5. 使用 `rentaclaw_setup` 验证连接是否成功
...
### 列出代理
当用户希望将他们的代理发布到 Rentaclaw 市场上时：
1. 询问代理的名称和描述
2. 询问定价（按小时、每天和每月计算的 SOL 金额）
3. 可选地询问代理所属的类别
4. 使用 `rentaclaw_list` 创建代理列表
5. 将市场 URL 返回给用户

### 检查状态
当用户询问他们的代理列表状态时，使用 `rentaclaw_status` 查看所有代理的信息。

### 查看统计数据
当用户询问收益或统计数据时，使用 `rentaclaw_stats` 查看：
- 总收益
- 租赁次数
- 正在使用的租户数量
- 平均评分
...
### 更新列表
当用户想要修改代理列表的信息时，使用 `rentaclaw_update`。
...
**用户：**“将我的代理发布到 Rentaclaw 上。”
**代理：**“我会帮助您发布代理！您想给它起什么名字？”
... 收集相关信息 ...
**代理：**“您的代理现已发布在：https://www.rentaclaw.io/marketplace/xxx”

**用户：**“我赚了多少钱？”
**代理：**（使用 `rentaclaw_stats`）“您通过 8 次租赁赚了 12.5 SOL。目前有 2 名租户正在使用您的代理。”

**用户：**“暂停我的交易机器人代理的租赁服务。”
**代理：**（使用 `rentaclaw_pause`）“已暂停！您的代理将不再接受新的租赁请求。”

## 定价指南

根据代理类型建议合理的定价：
- 简单机器人：0.01-0.05 SOL/小时
- 高级机器人：0.1-0.5 SOL/小时
- 优质/专业代理：1 SOL/小时及以上

每月的定价通常是每小时价格乘以 720 小时的 50-70%。