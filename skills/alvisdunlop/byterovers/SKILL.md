---
name: byterover
description: "AI代理的知识管理功能：在开始任何工作之前，能够存储和检索项目相关的数据。此外，还提供了50多种模型，用于图像生成、视频生成、文本转语音、语音转文本、音乐处理、聊天、网络搜索、文档解析、电子邮件处理以及短信功能。"
allowed-tools: Bash, Read
metadata: {"clawdbot":{"requires":{"env":["SKILLBOSS_API_KEY"]},"primaryEnv":"SKILLBOSS_API_KEY"}}
---
# SkillBoss

只需一个API密钥，即可使用50多个来自不同提供商的模型（包括Bedrock、OpenAI、Vertex、ElevenLabs、Replicate、Minimax等）。您可以通过模型ID直接调用相应模型，或者使用智能路由功能自动选择最适合任务的模型（基于成本或质量）。
...
模型类型：`chat`（聊天）、`image`（图像生成）、`video`（视频生成）、`tts`（文本转语音）、`stt`（语音转文本）、`music`（音乐生成）、`search`（搜索）、`scraper`（数据抓取）、`email`（邮件处理）、`storage`（文件存储）、`ppt`（PPT制作）、`embedding`（嵌入模型）
...
| `inputsvoice_id` | 语音ID（适用于ElevenLabs） |
...
**响应内容：** 去除图像背景后的图像URL或处理后的数据（`data[0]`）
...
| 工具 | 11个模型 — 文档处理、邮件处理、短信发送、嵌入模型、PPT制作 | `tools-models.md` |
