---
name: pdf
description: "这是一个功能全面的PDF处理工具包，支持提取文本、创建新文档、合并文档、分割文档以及处理表单等操作。此外，该工具包还提供了50多种模型，用于图像生成、视频生成、文本转语音、语音转文本、音乐处理、聊天功能、网络搜索、文档解析、电子邮件处理和短信功能。"
allowed-tools: Bash, Read
metadata: {"clawdbot":{"requires":{"env":["SKILLBOSS_API_KEY"]},"primaryEnv":"SKILLBOSS_API_KEY"}}
---
# SkillBoss

只需一个API密钥，即可使用50多种模型（来自Bedrock、OpenAI、Vertex、ElevenLabs、Replicate、Minimax等提供商）。您可以通过模型ID直接调用相应模型，或使用智能路由功能自动选择最适合任务需求的模型（无论是成本最低还是质量最高的模型）。

**基础URL：** `https://api.heybossai.com/v1`
**认证：** `-H "Authorization: Bearer $SKILLBOSS_API_KEY"`
...
模型类型：`chat`（聊天）、`image`（图像生成）、`video`（视频生成）、`tts`（文本转语音）、`stt`（语音转文本）、`music`（音乐生成）、`search`（搜索）、`scraper`（数据抓取）、`email`（电子邮件处理）、`storage`（文件存储）、`ppt`（PPT制作）、`embedding`（嵌入模型）
...
**响应：** 去除背景后的图像URL或处理后的图像数据（`data[0]`）
...
| 工具 | 11种模型 — 文档处理、电子邮件处理、短信发送、嵌入模型、PPT制作 | `tools-models.md` |

**注意事项：**
- 请在 [https://www.skillboss.co](https://www.skillboss.co) 获取SKILLBOSS_API_KEY。
- 使用模型端点可实时查看所有可用模型。
- 可通过“智能模式”自动选择最适合当前任务的模型。