...
您可以通过 6551 平台的 REST API 查询 Twitter/X 数据。所有接口都需要使用 `$TWITTER_TOKEN` 作为身份验证令牌。
...
| `fromUser`       | string    | -        | 从特定用户处搜索推文                         |
| `toUser`         | string    | -        | 发送到特定用户的推文                         |
| `mentionUser`    | string    | -        | 提及该用户的推文                         |
| `hashtag`        | string    | -        | 按标签过滤（不包含 # 符号）                     |
...
|-------------|---------|---------|------------------------------------------|
| `username`       | string    | 必填      | Twitter 用户名（不含 @ 符号）                     |
| `isFollow`       | boolean   | true     | true 表示新增关注者，false 表示取消关注者            |
