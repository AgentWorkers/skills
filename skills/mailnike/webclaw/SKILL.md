...
description: >
  OpenClaw 的 Web 仪表板：  
  这是一个基于浏览器的用户界面，用于管理已安装的所有技能。该界面采用模式驱动（schema-driven）的渲染方式，支持 JWT 身份验证（JWT auth）、基于角色的访问控制（RBAC）、人工智能聊天功能以及实时数据更新。您可以安装该 Web 仪表板，管理用户账户，配置 SSL/TLS 安全协议（SSL HTTPS），并使用 Web 管理面板进行各项设置。
...
- 使用 JWT 进行身份验证——访问令牌（有效期 15 分钟）+ 刷新令牌（有效期 7 天，设置为 `httpOnly` 的 Cookie）
- 实施基于角色的访问控制（RBAC）——在每个技能操作之前进行权限检查
- 实施速率限制——每分钟最多 5 次身份验证请求，每分钟最多 30 次写入操作，每分钟最多 100 次普通请求（通过 nginx 实现）
...
- **源代码**：从 GitHub 的固定版本标签（`v1.0.10`）克隆应用程序代码
- **依赖项**：在隔离的虚拟环境中安装 Python 和 Node.js 包
- **系统服务**：配置 nginx 反向代理和 systemd 服务（需要使用 `sudo` 权限）
- **SSL**：可选的 Let’s Encrypt 证书（通过 certbot 获取）

安装完成后，所有运行时操作均为本地操作，正常运行无需持续连接互联网。无需提供任何凭据或 API 密钥。所有数据都存储在本地 SQLite 数据库中。
...
**使用自定义域名启用 HTTPS：** 请输入 “Set up SSL for yourdomain.com”

### ERP 公司设置（通过 erpclaw，而非 webclaw）

**重要提示：** 公司设置、演示数据以及所有 ERP 相关操作均由 **erpclaw** 技能处理，而非 webclaw。Webclaw 仅用于 Web 仪表板的管理（用户管理、SSL 配置、会话管理）。

**通过 Telegram/CLI 设置公司：**
1. 首先：执行 `erpclaw initialize-database`（创建表格和共享库——初次安装时必须执行）
2. 然后：执行 `erpclaw setup-company --name "公司名称" --currency USD --fiscal-year-start-month 1`（设置公司信息）
3. 可选：执行 `erpclaw seed-demo-data`（加载示例数据）

**切勿** 直接导入 webclaw 的 Python 模块（例如：`from api.auth import ...`）。Webclaw 的 API 作为服务运行——请使用下面列出的命令或调用 REST API。
...
- 如果 `status` 显示 “ssl=false”，建议再次执行 `setup-ssl` 命令。
- 如果 `status` 显示用户数量为 0，建议用户打开设置页面（`/setup`）。
...
- **代理**：使用 nginx（端口 80/443）将 `/api` 路由到后端，将 `/` 路由到前端
- **数据库**：使用 SQLite 数据库（文件路径：`~/.openclaw/webclaw/webclaw.sqlite`）

### 8 个通用 UI 组件
`DataTable`、`FormView`、`DetailView`、`ChatPanel`、`ChartPanel`、`KanbanBoard`、`CalendarView`、`TreeView`——这些组件会根据技能操作的响应动态渲染。
