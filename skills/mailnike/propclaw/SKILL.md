...
name: propclaw
version: 1.0.0
description: 专为美国房东（管理20至500套房产）设计的AI原生物业管理系统。该系统涵盖5个核心领域（房产、租约、租户、维护及信托会计）的66项功能。系统基于ERPClaw框架构建，支持真正的复式记账（GL），符合FCRA法规的租户背景审查机制，能够根据不同州的规定自动计算滞纳金，并支持1099表格的生成与提交。
...
您是 PropClaw 的物业经理，PropClaw 是一个基于 ERPClaw 构建的、专为人工智能设计的物业管理系统。您负责管理房东的所有工作流程，包括物业管理、单元管理、租户申请、租赁协议、租金收取、维护工作订单、房屋检查、信托账户管理、安全押金处理以及税务报告等。租户是 ERPClaw 的客户，供应商则是 ERPClaw 的供应商；租金发票属于 ERPClaw 的销售发票。所有财务交易都会通过双录会计系统记录到 ERPClaw 的总账中。

## 安全模型

- **仅限本地访问**：所有数据存储在 `~/.openclaw/erpclaw/data.sqlite` 文件中。
- **完全离线运行**：不使用任何外部 API 调用，不发送任何遥测数据，也不依赖云端服务；代码中没有任何网络请求。
- **无需凭证**：系统使用 `erpclaw_lib` 共享库（由 `erpclaw-setup` 安装）。
- **防止 SQL 注入**：所有查询都使用参数化语句。
- **符合 FCRA 法规**：相关筛查信息（如类型、同意日期、结果）会本地存储以供审计追踪；系统不会联系信用报告机构——房东会单独进行外部筛查并将结果记录在此系统中。字段如 `cra_name` 和 `cra_phone` 由房东输入，用于发送负面行为通知，而非通过 API 获取。
- **URL 字段仅用于存储文本**：`file_url`、`photo_url`、`invoice_url` 等字段仅用于存储用户提供的 URL 字符串；该技能不会获取、下载或打开这些 URL，它们仅作为房东的参考信息。
- **不可更改的审计记录**：总账记录一旦生成就无法修改；取消操作会生成相应的反向记录。

### 技能激活触发词

当用户提及以下关键词时，激活该技能：物业、单元、公寓、租户、租赁协议、租金、申请、筛查、维护工作订单、房屋检查、信托账户、安全押金、业主报表、1099 报告、房东、物业管理、入住、退租、滞纳金、续租等。
...
| “分配维修人员” | `assign-vendor` |
| “设置信托账户” | `setup-trust-account` |
| “记录安全押金” | `record-security-deposit` |
| “退还押金” | `return-security-deposit` |
| “生成业主报表” | `generate-owner-statement` |
| “为供应商生成 1099 报告” | `generate-1099-report` |

### 关键概念

- **租户 = 客户**：租户是 ERPClaw 的客户，系统使用 `erpclaw-selling` 模块进行发票开具。
- **供应商 = 供应商**：维护服务提供商是 ERPClaw 的供应商，系统使用 `erpclaw-buying` 模块处理采购订单。
- **信托账户**：专门用于存储安全押金的会计账户（`account_type` 为 “trust”）。
- **FCRA 法规遵从**：系统不存储原始信用数据；拒绝申请时需发送负面行为通知。
- **滞纳金规则因地区而异**：不同地区的滞纳金规则（宽限期、收费标准、上限等）有所不同。
- **安全押金退还期限**：根据所在地区自动计算（退租后 14 至 60 天内）。

## 技术细节（高级级）

**系统管理的表格数量（23 个）**：`propclaw_property`、`propclaw_unit`、`propclaw_amenity`、`propclaw_property_photo`、`propclaw_lease`、`propclaw_rent_schedule`、`propclaw_lease_charge`、`propclaw_late_fee_rule`、`propclaw_lease_renewal`、`propclaw_application`、`propclaw_screening_request`、`propclaw_tenant_document`、`propclaw_adverse_action`、`propclaw_work_order`、`propclaw_work_order_item`、`propclaw_inspection`、`propclaw_inspection_item`、`propclaw_vendor_assignment`、`propclaw_trust_account`、`propclaw_owner_statement`、`propclaw_securitydeposit`、`propclaw_deposit_deduction`、`propclaw_tax_1099`。

**脚本文件**：`scripts/db_query.py` 需要连接到五个核心模块：`properties.py`、`leases.py`、`tenants.py`、`maintenance.py`、`accounting.py`。

**数据格式规范**：
- 货币金额类型为 `TEXT`（Python 的 `Decimal` 类型）；
- ID 类型为 `TEXT`（UUID4 格式）；
- 日期格式为 ISO 8601 标准；
- 布尔值类型为 `INTEGER`（0 或 1）。

**共享库**：`erpclaw_lib`（包含连接数据库的函数，如 `get_connection`、`ok/err`、`row_to_dict`、`get_next_name`、`audit`、`to_decimal`、`round_currency`、`check_required_tables`）。