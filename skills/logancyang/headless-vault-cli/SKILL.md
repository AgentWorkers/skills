...
description: 您可以通过 SSH 隧道在个人电脑上阅读和编辑 Markdown 笔记。当用户需要读取、创建或向他们的笔记库中添加内容时，可以使用此方法。
...
通过 SSH 隧道，您可以从托管在 VPS 上的机器人访问个人计算机上的 Markdown 笔记。

**术语说明**：  
“本地机器”指的是存储您笔记的个人计算机（macOS 或 Linux）。该工具运行在 VPS 上，并通过反向 SSH 隧道连接到您的本地机器。
...
- **Vault 沙箱机制**：`vaultctl` 会验证所有文件路径是否位于 `VAULT_ROOT` 内部，并拒绝路径遍历操作（如使用 `..` 或外部符号链接）。  
...
这确保了即使隧道被攻击，VPS 也只能执行 `vaultctl` 命令。  

## 可用命令  

您只能使用以下命令。请勿尝试未列出的命令（不支持重命名、删除、移动或编辑文件等操作）。  
...
这些变量必须在 VPS 上的运行时环境中设置：  
| 变量 | 是否必填 | 默认值 | 描述 |  
|----------|----------|---------|-------------|  
| `VAULT_SSH_USER` | 是 | — | 用于 SSH 隧道的本地机器用户名 |  
| `VAULT_SSH_PORT` | 否 | `2222` | SSH 隧道端口（localhost 上） |  
...
**路径和标题参数必须使用 `--base64` 进行编码**，以防止 shell 注入攻击：  
```bash
...
**重要提示**：路径和内容都必须使用 `--base64` 进行编码（尤其是包含空格或特殊字符的路径/内容）。  
```bash
...
- 会自动创建父目录。  
- 如果文件已存在，则操作会失败（此时应使用 `append` 向现有文件追加内容）。  
- 文件必须具有 `.md` 扩展名。  
- **注意**：笔记内容中不得使用标题作为内容开头（例如，不要以 “# My Note” 开头）。  

### `append` - 向现有笔记追加内容  

**如果文件不存在，则操作会失败（此时应使用 `create` 创建新文件）：**  
```bash
...
- 无法在文件夹间移动文件  
- 无法编辑文件的具体内容（只能追加内容）  
- 无法创建没有内容的文件夹（文件夹会通过 `create` 命令自动创建）  
...
**重要提示**：如果不确定有哪些笔记，请先运行 `tree` 命令。这可以避免因路径错误或名称重复导致的错误。  
...
```  
**步骤 2**：读取正确的笔记路径（路径必须使用 `--base64` 进行编码）：  
```bash
# echo -n "Projects/Plan.md" | base64 → UHJvamVjdHMvUGxhbi5tZA==
ssh -4 -p 2222 ${VAULT_SSH_USER}@localhost vaultctl read UHJvamVjdHMvUGxhbi5tZA== --base64
```  
...
{"tree": [{"path": "Projects", "type": "dir"}, {"path": "Projects/Plan.md", "type": "file"}]}
```  
**步骤 2**：由于不存在名为 “Meeting Notes” 的笔记，可以安全地创建新笔记（注意不要在内容中使用该标题作为标题）：  
```bash
...
ssh -4 -p 2222 ${VAULT_SSH_USER}@localhost vaultctl tree --depth 2
```  
**回复**：**您有一个名为 “Projects”的文件夹，其中包含 `Plan.md` 文件，以及一个名为 “Ideas.md” 的文件。**  

### 示例 4：处理包含源文件和输出文件的复杂工作流程  
...
- 输出文件 “digest/2025-01-28-digest.md” 不存在，因此需要创建新文件：  
  （如果源文件不存在，请询问用户：“您是指这些文件中的哪一篇？”）  
**步骤 3**：读取源笔记内容（路径必须使用 `--base64` 进行编码）：  
```bash
...
```  
**步骤 4**：浏览源文件并生成摘要内容（这些操作由机器人完成）。  
**步骤 5**：将摘要内容保存到 vault 中（注意不要在文件标题中使用重复的标题）：  
```bash
