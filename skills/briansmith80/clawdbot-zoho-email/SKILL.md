---
name: zoho-email
description: **Zoho Mail 的完整集成：支持 OAuth2、REST API（速度提升 5-10 倍）、HTML 邮件、附件以及批量操作。非常适合用于邮件自动化和工作流程。**
homepage: https://github.com/briansmith80/clawdbot-zoho-email
---

# Zoho 邮件集成

**v2.0** - 完整的 Zoho Mail 集成，支持 OAuth2 认证和 REST API 后端（速度比 IMAP/SMTP 快 5-10 倍）。支持 HTML 邮件、附件、批量操作和高级自动化工作流程。
...
- **优雅的回退机制** - 如果 REST API 不可用，自动切换到 IMAP
...
发送带有富格式的 HTML 邮件，支持多部分/替代格式（HTML 和纯文本版本）：
...
- [ ] **定时发送** - 安排邮件在以后发送
- [ ] **邮件模板** - 可重用的邮件模板，支持变量
- [ ] **Webhook** - 新邮件到达时接收实时通知
- [ ] **高级搜索** - 按大小、是否包含附件、日期范围筛选
- [ ] **Zoho 日历集成** - 从邮件创建事件
...
发现漏洞或想要贡献代码？请在 GitHub 上提交问题或 Pull Request！
...
**创建日期：** 2026-01-29  
**状态：** 已准备好投入生产 ✅  
**要求：** 需要 Python 3.x。对于 REST API 模式：`pip install -r requirements.txt`（包含 `requests` 库）。
...
请查看 `examples/batch-cleanup.py` 以获取完整的自动化清理脚本。