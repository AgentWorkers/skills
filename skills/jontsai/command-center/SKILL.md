...
name: command-center
version: 1.2.0
description: OpenClaw的任务控制面板：提供实时会话监控、大型语言模型（LLM）使用情况跟踪、成本分析以及系统关键指标的查看功能。您可以在这里集中管理所有的AI代理。
metadata:
  openclaw:
    requires:
      node: ">=18"
...
- **LLM 使用情况统计** — 跟踪 Claude、Codex 等模型的使用情况
...
| `GET /api/events` | 实时更新的数据流（SSE 格式）         |
