...
name: command-center
version: 1.4.1
description: OpenClaw的任务控制面板：支持实时会话监控、大型语言模型（LLM）使用情况追踪、成本分析以及系统运行状态监控。您可以在一个界面中查看所有的人工智能代理（AI agents）。
metadata:
  openclaw:
    requires:
      node: ">=18"
...
- **大语言模型使用情况统计** — 跟踪 Claude、Codex 等模型的使用情况
- **系统性能指标** — CPU、内存、磁盘、温度等系统状态
- **定时任务** — 查看和管理已安排的任务
- **对话内容管理** — 自动整理对话内容
- **成本跟踪** — 计算每次会话的成本、预测节省金额
- **隐私设置** — 可隐藏演示中的敏感内容
...
| `GET /api/events`     | 获取实时更新的数据流（SSE 格式）           |
