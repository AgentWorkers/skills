...
name: command-center
version: 1.4.0
description: OpenClaw的任务控制面板：实时会话监控、大型语言模型（LLM）使用情况追踪、成本分析以及系统关键指标。您可以在此处集中查看所有AI代理的状态。
metadata:
  openclaw:
    requires:
      node: ">=18"
...
- **大语言模型使用情况** — 跟踪 Claude、Codex 等模型的使用情况
...
| API 端点            | 功能描述                        |
| ---------------------------- | -------------------------------------- |
| `GET /api/state`     | 获取所有仪表板数据                   |
| `GET /api/events`     | 获取实时更新的 SSE 数据流                |
