...
这是一个用于快速搜索代码库的工具包，支持结构化的 grep（内容搜索）、glob（文件名搜索）以及通过 ripgrep、fd 和 tree CLI 工具进行的目录结构搜索。
...
- `--type <ext>` — 文件类型过滤器（例如：go、py、ts、js 等）（可重复使用）
- `--literal` — 将搜索模式视为普通文本（而非正则表达式）
- `--max <n>` — 最多显示的结果数量（默认：100）
- `--context <n>` — 显示匹配项周围的 N 行上下文（默认：0）
...
- 遵循 `.gitignore` 规则
- 对于 grep 和 glob 操作，结果会按照修改时间（最新文件优先）进行排序