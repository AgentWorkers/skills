...
description: 使用 VirusTotal API，通过 curl 或 Python 工具扫描文件和 URL。检查文件哈希值，上传文件，并管理相关注释。
...
3. **请告知用户**，上传的文件将与安全社区共享，并可能被其他研究人员下载。
...
### 1. 计算文件哈希值（检查文件是否存在所需）
计算文件的 SHA256 哈希值，以确认该文件是否存在于 VirusTotal 中。
```bash
# Linux
...
### 4. 文件评论
**隐私警告**：不要在评论中包含个人身份信息或敏感数据。提供关于文件来源或下载者的相关信息会很有帮助。
