---
name: zhipu-asr
description: 使用 Zhipu AI（BigModel）的 GLM-ASR 模型进行自动语音识别（ASR）。当您需要将音频文件转录为文本时，可以使用该工具。它支持中文音频的转录，并提供上下文提示、自定义关键词以及多种音频格式的支持。
metadata:
  {
    "openclaw":
      {
...
> **注意：** 脚本会使用 ffmpeg 将不支持的音频格式自动转换为 MP3 格式。虽然 API 只接受 WAV 和 MP3 格式，但您可以使用 ffmpeg 支持的任何格式。
...
**参数：**
- `audio_file`（必填）：音频文件的路径（.wav 或 .mp3 格式）
...
**文件大小问题：**
- 将超过 25 MB 的音频文件分割成多个部分
- 降低采样率或位深度
- 对较小的文件使用压缩格式（如 MP3）

**时长问题：**
- 将超过 30 秒的录音文件分割成多个部分
- 分别处理各个片段
- 使用上下文提示来保持转录的连贯性
