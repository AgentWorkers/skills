...
description: >
  **AI代理的日历调度功能**  
  该功能能够解析自然语言中的时间描述，整合Google、Outlook及CalDAV日历中的可用时间信息，检测时间冲突，并支持对重复性事件（RRULE）的详细设置。通过“两阶段提交”（Two-Phase Commit）机制确保操作的原子性（即操作的各个步骤要么全部成功执行，要么全部失败）。适用于会议安排、查询可用时间、处理日期时间表达式以及管理日历事件等场景。
...
本技能介绍了使用 Temporal Cortex MCP 服务器的 AI 代理应掌握的程序化知识，涵盖了从时间理解到无冲突预订的正确工作流程。
...
| 工具 | 使用场景 |
|------|------------|
| `get_temporal_context` | 每次会话开始时调用。返回当前时间、时区、UTC 偏移量、夏令时状态以及星期几。 |
...
| `adjust_timestamp` | 考虑夏令时的时间戳调整。例如，“+1d”表示跨越夏令期的时间调整。 |

### 第 2 层 — 日历操作（需要日历提供者支持）

| 工具 | 使用场景 |
|------|------------|
| `list_events` | 列出指定时间范围内的事件。支持 TOON 格式（可减少约 40% 的输入字符数）。对于多日历环境，使用带有提供者前缀的 ID，例如：“google/primary”、“outlook/work”。 |
| `find_free_slots` | 查找日历中的空闲时间段。可设置 `min_duration_minutes` 以指定最小时间段长度。 |
| `expand_rrule` | 将 RFC 5545 格式的重复规则具体化为可执行的操作。支持夏令时、BYSETPOS、EXDATE 等规则，并使用 `dtstart` 作为本地日期时间（不带时区后缀）。 |
| `check_availability` | 检查特定时间段是否可用。同时会检查事件安排和现有的预订情况。 |

### 第 3 层 — 跨日历的可用性检查

| 工具 | 使用场景 |
|------|------------|
| `get_availability` | 合并多个日历的可用/忙碌状态。需要传递 `calendar_ids` 数组。隐私设置：默认为 “opaque”（隐藏来源日历），也可设置为 “full”。 |

### 第 4 层 — 预订（唯一的写入操作）

| 工具 | 使用场景 |
|------|------------|
| `book_slot` | �原子的地安排时间段。流程包括锁定、验证、写入和释放。这是唯一一个具有写入权限的工具。预订前务必先调用 `check_availability`。 |

## 重要规则

1. **务必先调用 `get_temporal_context`**——切勿直接使用当前时间或时区信息。  
2. **查询前先转换时间**——在将时间表达式传递给日历工具之前，使用 `resolve_datetime` 将其转换为 RFC 3339 格式。  
3. **预订前务必检查**——在调用 `book_slot` 之前，务必先调用 `check_availability`。切勿跳过冲突检查。  
4. **多日历环境使用提供者前缀的 ID**——例如：“google/primary”、“outlook/work”、“caldav/personal”。使用裸 ID（如 “primary”）时，系统会使用默认日历提供者。  
...
| 错误类型 | 处理方式 |
|-------|--------|
| 时间段已占用/检测到冲突 | 使用 `find_free_slots` 提供替代时间段，并向用户展示选项。 |
| “未找到凭证” | 告知用户运行 `npx @temporal-cortex/cortex-mcp auth google`（或 `outlook`/`caldav`）进行认证。详见 [设置脚本](scripts/setup.sh)。 |
| 时区未配置 | 提示用户输入他们的 IANA 时区信息，或运行 `npx @temporal-cortex/cortex-mcp auth google` 进行时区配置。 |
| 锁定失败 | 另有代理正在预订同一时间段。稍后重试或建议其他时间。 |
...
**本地模式**（默认推荐）：
第 1 层工具（时间上下文处理、日期时间转换、时区转换）无需额外配置即可立即使用。日历工具需要一次性完成 OAuth 设置——运行 [设置脚本](scripts/setup.sh) 或使用 `npx @temporal-cortex/cortex-mcp auth google`。

**托管云环境**（适用于开放调度、仪表盘界面、多代理协调）：
（具体配置信息请参考相关文档。）

## 额外参考资料

- [预订安全](references/BOOKING-SAFETY.md)——两阶段提交机制、冲突解决、锁定时长限制  
- [多日历管理](references/MULTI-CALENDAR.md)——提供者前缀的使用、可用性数据合并、隐私设置  
