---
name: bullybuddy
description: **BullyBuddy** — 一个用于管理 Claude Code 会话的 CLI 工具。通过 `/bullybuddy` 命令可以创建、列出、发送输入、终止以及监控多个 Claude Code 会话。该工具会自动从 `~/.bullybuddy conexión.json` 文件中读取认证令牌。
user-invocable: true
...
BullyBuddy负责创建和管理多个Claude Code的CLI会话。它支持两种后端技术：**tmux**（默认选项，会话在服务器重启后仍可保留）或**node-pty**（备用方案）。该工具提供了REST API、WebSocket流传输以及Web仪表板功能。
...
服务器启动时，连接信息会自动保存到`~/.bullybuddy/connection.json`文件中。`/bullybuddy`命令会自动读取该文件，因此无需设置环境变量。
...
- 认证令牌可授予对所有Claude Code会话的**完全控制权**，包括发送任意命令。请将其视为机密信息。
- `/bullybuddy url`命令会输出包含令牌的仪表板URL，请勿公开分享或记录该URL。
- 当使用`--tunnel`选项时，仪表板和API会通过Cloudflare临时URL暴露在互联网上。任何拥有令牌的人都可以远程访问所有会话。
- 创建的会话会以用户的本地权限运行Claude Code；如果启用了`--dangerously-skip-permissions`选项，Claude可以无需确认即可执行任何命令。
...
|--------|----------|-------------|
| `GET` | `/health` | 获取服务器状态 |
| `GET` | `/api/sessions` | 列出所有会话（可按组过滤） |
| `POST` | `/api/sessions` | 创建新会话 |
| `GET` | `/api/sessions/:id` | 查看会话详情及指标 |
| `DELETE` | `/api/sessions/:id` | 终止会话 |
| `POST` | `/api/sessions/:id/input` | 向终端发送输入 |
| `POST` | `/api/sessions/:id/resize` | 调整终端窗口大小 |
| `POST` | `/api/sessions/:id/task` | 设置会话任务元数据 |
| `POST` | `/api/sessions/:id/mute` | 静音通知 |
| `POST` | `/api/sessions/:id/unmute` | 恢复通知 |
| `GET` | `/api/groups` | 查看各组的会话数量 |
| `GET` | `/api/summary` | 获取整体状态统计信息 |
| `GET` | `/api/browse` | 浏览目录（默认禁用） |
| `GET` | `/api/audit` | 查看审计日志 |
| `GET` | `/api/sessions/:id/transcript` | 查看对话记录 |
...
所有字段均为可选。如果提供了`task`参数，该参数会自动作为输入发送给Claude Code。

**注意：**发送输入时，请使用`\r`（回车符）而不是`\n`。
...
| 类型 | 字段 | 描述 |
|------|--------|-------------|
| `subscribe` | `sessionId`, `cols?`, `rows?` | 接收会话输出 |
| `unsubscribe` | `sessionId` | 停止接收输出 |
| `input` | `sessionId`, `data` | 向终端发送按键信息 |
| `resize` | `sessionId`, `cols`, `rows` | 调整终端窗口大小 |
...
| `output` | `sessionId`, `data` | 终端输出数据 |
| `scrollback` | `sessionId`, `data` | 提供滚动功能 |
| `session:created` | `session` | 新会话创建 |
| `session:exited` | `sessionId`, `exitCode` | 会话终止 |
| `session:stateChanged` | `sessionId`, `detailedState` | 会话状态变化 |
| `error` | `message` | 错误信息（例如：无效输入） |

## 状态检测

BullyBuddy通过分析终端的原始输出来实时检测Claude Code的运行状态。

| `detailedState` | 含义 |
|-----------------|---------|
| `starting` | 会话刚创建，Claude正在加载 |
| `working` | Claude正在处理或编辑内容 |
| `permission_needed` | Claude正在等待用户授权 |
| `idle` | Claude已准备好接收输入 |
| `compacting` | Claude正在整理对话记录 |
| `error` | 检测到输出错误 |
...
定期调用`GET /api/summary`接口以检查所有会话的状态。`sessionsNeedingAttention`字段会列出处于`permission_needed`或`error`状态的会话ID。
...
隧道URL会在启动时显示，并保存到`~/.bullybuddy/connection.json`文件中。可以使用`bullybuddy url`或`/bullybuddy url`来获取该URL。
