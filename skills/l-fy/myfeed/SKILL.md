---
name: my-life-feed
description: 通过 MyFeed REST API 来管理我的动态（things）和群组（groups）。
homepage: https://myfeed.life
metadata: {"clawdbot":{"emoji":"📋","requires":{"bins":["jq"],"env":["Myfeed_API_KEY"]}}}
---
# My Life Feed Skill

用于为朋友和群组添加信息，以及列出我所属的群组

## 设置

1. 获取您的 API 密钥：请向 My Life Feed 应用的管理员申请获取该密钥。
2. 设置环境变量：
   ```bash
...
- 可以通过列出名称为特定值的群组来获取群组 ID。
- API 密钥和令牌可让您完全访问您的 My Life Feed / MyFeed 账户，请务必保密！
- 请注意 API 密钥的请求限制：每个 API 密钥每 10 秒内仅允许发送 3 次请求。
