---
name: perplexity
description: 使用 Perplexity API 进行基于网络的 AI 搜索和研究。当用户需要最新信息、包含网络引用的多步骤推理、带有来源参考的详尽研究、涉及当前事件的事实性查询或竞争分析时，可以使用该 API。当用户提到 Perplexity、需要最新信息或要求提供来源引用时，此 API 为默认选择。
---
...
此技能提供了对 Perplexity API 的访问权限，用于基于网络的 AI 搜索和研究。它结合了大型语言模型的强大功能和实时网络搜索能力，能够提供准确、最新的答案，并附带来源引用。
...
- 用户要求提供 **来源引用** 或参考资料
- 需要进行复杂的 **多步骤推理**
- 用户明确提到使用 Perplexity 或希望获得研究风格的答案
- 需要对多个来源进行 **全面分析**
...
- `sonar-research` - 具有深入分析能力的综合研究模型。
- `sonar-research-pro` - 具有全面分析能力和详细报告的高级研究模型。
...
- **低** - 搜索速度更快，来源较少。适合简单查询。
- **中等**（默认） - 平衡性较好。适用于大多数用例。
- **高** - 检索内容最全面。适合研究和详细分析。

## 专业搜索模式

仅适用于 `sonar-pro` 和推理模型。可控制多步骤工具的使用方式：
...
此技能需要 Perplexity API 密钥。有两种配置方式：
...
要永久设置 API 密钥（请将其添加到 `~/.bashrc` 或 `~/.zshrc` 文件中）：
```bash
...
- **深度研究**：每次查询约 0.015–0.03 美元以上
- **专业用户可通过 Perplexity Pro 订阅获得每月 5 美元的信用额度**

请谨慎使用推理/研究模型。对于大多数查询，建议默认使用 `sonar` 模型。
...
### 脚本
- `perplexity_search.sh` - 用于与 Perplexity API 交互的主要脚本
  - 支持所有 Perplexity 模型
  - 从环境或配置文件中获取 API 密钥
  - 提供多种输出格式
...
**注意：** 此技能使用外部 API 调用。请注意使用频率限制和费用。API 密钥切勿提交到版本控制系统中或公开共享。