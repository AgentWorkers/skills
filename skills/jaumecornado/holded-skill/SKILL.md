---
name: holded-skill
description: 通过 `holdedcli` 操作 `Holded ERP`，以安全地读取和更新数据。当用户需要读取、搜索、创建、更新或删除 `Holded` 实体（联系人、发票、产品、CRM、项目、团队、会计记录）时，或者需要从终端调用 `Holded` API 端点时，请使用该工具。
...
- **在创建任何文档之前，务必检查可抵扣规则。** 请参阅下面的“西班牙的会计规则”部分。
- 将任何 `POST`、`PUT`、`PATCH` 或 `DELETE` 操作视为写入操作。
- 将任何 `GET` 操作（或存在的 `HEAD` 操作）视为读取操作。
- 在执行任何操作之前，始终使用 `holded actions describe <action> --json`（在 `holded actions list` 之后）来验证参数是否正确。
- 对于购买收据，必须设置 `docType=purchase` 并在 JSON 正文中包含 `"isReceipt": true`。由于 `holdedcli` 会根据 Holded 的模式进行验证（该模式不包含 `isReceipt` 字段），因此必须使用 `--skip-validation` 标志。
...
1. 使用 `holded actions list --json`（可结合 `--filter`）找到所需的操作。
2. 使用 `holded actions describe <action> --json` 验证路径/查询/请求体参数是否正确。
3. 使用 `holded actions run <action> ... --json` 运行操作。
4. 返回清晰的摘要和相关操作 ID，以便后续处理。

### 写入操作

1. 查找并验证所需的操作。
2. 使用 `holded actions describe <action> --json` 验证所需的/可选参数。
3. 准备最终的请求数据。
4. 如果是创建购买收据/票据，请确保 `docType=purchase` 且 `"isReceipt": true`，并使用 `--skip-validation` 标志。
5. 获取用户的确认。
6. 在确认后执行命令。
7. 报告结果（`status_code`、受影响的 ID 和 API 响应）。
...
**重要提示：**
- 由于 `holdedcli` 会根据 Holded 的模式进行验证（该模式不包含 `isReceipt` 字段），因此请使用 `--skip-validation` 标志。
- 在项目中使用 `subtotal` 字段（而非 `price`）——这是 Holded API 需要的字段名称。
- 时间戳必须以秒为单位（Unix 时间戳），并且采用 **Europe/Madrid** 时区。
...
**⚠️ 在创建任何费用文档之前，请务必检查这些规则：**

| 费用类型 | IVA 可抵扣性 | 费用可抵扣金额 | 账户 |
|--------------|----------------|---------------------|---------|
| 餐厅/餐饮 | ❌ 不可抵扣 | ✅ 可抵扣（需提供理由） | 629 |
| 交通费用 | ❌ 不可抵扣 | ✅ 可抵扣 | 629 |
| 燃油费用 | ⚠️ 可抵扣情况复杂 | ✅ 可抵扣 | 625/622 |
| 办公用品 | ✅ 可抵扣 | ✅ 可抵扣 | 600/602 |
| 保险费用 | ⚠️ 可抵扣情况复杂 | ✅ 可抵扣 | 625 |

**在创建任何文档之前，请务必核实：**
1. 该费用是否可抵扣？
2. IVA 是否可抵扣？（餐厅和交通费用通常不可抵扣）
- 如果有疑问，请在创建文档之前咨询相关人员。

**常见错误：** 不要为餐厅费用设置 `tax: 10` 或 `tax: 21`——除非是具有正当理由的商业活动，否则餐厅费用的 IVA 是不可抵扣的。

## 错误处理

- 如果出现 `MISSING_API_KEY`，请通过 `--api-key`、`HOLDED_API_KEY` 或 `holded auth set` 配置 API 密钥。
- 如果出现 `ACTION_NOT_FOUND`，请使用 `--filter` 列出可用操作并重新搜索。
- 如果出现 `INVALID_BODY`，请在执行前验证 JSON 数据的有效性。
- 如果出现 `API_ERROR`，请报告 `status_code` 和 API 响应内容。

## 参考资料

- 阅读 `{baseDir}/references/holdedcli-reference.md` 以获取快速命令和参考标准。
- 使用以下命令进行动态操作查找和参数验证：
  - `holded actions list --json`
