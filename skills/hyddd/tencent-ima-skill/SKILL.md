...
description: 控制 IMA (ima.copilot) 桌面应用程序，用于 AI 搜索和私人知识检索。
---
# IMA 技能
...
### ima_search
用于启动 IMA 并执行搜索操作。支持通过特殊标签使用“私有知识库”模式。

- **query**（必填）：搜索查询。使用 `@个人知识库` 或 `@knowledge` 作为前缀，可搜索您的私有知识库（需要 `config.json` 配置文件）。
...
- **公共搜索：** `clawdbot ima_search query="DeepSeek analysis"`
- **私有搜索：** `clawdbot ima_search query="@knowledge project update"`