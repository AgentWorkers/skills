...
## 特点  
- **令牌高效利用**：采用优化的API调用方式，降低大型语言模型（LLM）对上下文数据的需求。  
- **快速报告**：内置Shell脚本，可生成JSON和PDF格式的报告。  
- **直接API访问**：提供了使用`curl`与Toggl v3报告系统直接交互的示例。  
...
### CLI命令（Toggl-CLI）  
如果您安装了`toggl-cli`，可以执行以下命令：  
- `toggl te list --compact`：以最小的令牌使用量列出时间记录。  
- `toggl proj list --compact`：高效地列出项目信息。  

## 配置  
报告脚本依赖环境变量进行身份验证。请确保`TOGGL_API_TOKEN`和`TOGGL_WORKSPACE_ID`已正确设置。