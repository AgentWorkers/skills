---
name: twitterapi-io
description: 通过 TwitterAPI.io 与 Twitter/X 交互——可以搜索推文、获取用户信息、发布推文、点赞、转发、关注、发送私信等。涵盖了所有 59 个 API 端点。适用于用户需要读取或写入 Twitter 数据的场景。
metadata:
  version: 3.1.0
  updated: 2026-02-13
  author: dorukardahan
...
文档：https://docs.twitterapi.io | 控制台：https://twitterapi.io/dashboard

---

## 设置

1. 获取 API 密钥：https://twitterapi.io/dashboard（免费提供 0.10 美元的信用额度，无需信用卡）
2. 将密钥保存在 `.env` 文件中或 shell 的安全配置文件中（切勿在终端中使用 `export` 命令直接输出密钥，否则密钥会保存在终端历史记录中）。
3. 对于写入操作，您还需要来自 v2 登录的 `login_cookies` 以及专用的住宅代理（residential proxy）。

基础 URL：`https://api.twitterapi.io`
认证头：`X-API-Key: $TWITTERAPI_IO_KEY`（所有请求均需使用）
...
| 账户资料（每个返回的账户资料） | 18 | 0.18 美元 |
| 100 个以上账户资料批量查询（每个账户资料） | 10 | 0.10 美元 |
...
注意：即使 API 返回 0 或 1 条数据，仍需支付最低费用（15 信用额度）。
...
**V1 的 `login_cookies` 不能用于 V2 端点，反之亦然。请始终使用 V2 版本。**

### `login_cookie` 与 `login_cookies` 的命名不一致问题

API 在命名上存在不一致：
- `user_login_v2` 的 **响应** 中返回的字段名为 `login_cookie`（单数形式）
- 所有 V2 的 **操作** 端点要求该字段名为 `login_cookies`（复数形式）
...
- 写入操作（V2）端点文档（18 个端点）：`references/write-endpoints.md`
...
| `filter:media` | `filter:media` | 包含媒体文件 |
...
注意：`has_next_page` 有时即使没有更多数据也会返回 `true`（这是 Twitter API 的限制）。如果后续请求返回空结果，请停止分页。
...
| 错误 | 原因 | 解决方法 |
|-------|-------|-----|
| API 密钥无效 | `X-API-Key` 头部错误或缺失 | 在控制台检查密钥 |
| `login_cookie` 无效 | 证书过期或损坏 | 使用有效的 `totp_secret` 重新登录（通过 `user_login_v2`） |
| V2 操作失败 | 登录时使用的证书无效或 `totp_secret` 不正确 | 使用 16 位字符串 `totp_secret` 重新登录 |
| 代理错误 | 代理格式错误或代理无法使用 | 代理格式应为 `http://user:pass@host:port`，请使用专用的住宅代理 |
| 超过速率限制 | 超出了当前账户余额的 QPS 限制 | 增加余额以提高 QPS |
| 账户被暂停 | 账户被封禁 | 使用其他账户 |
| 端点错误 | 路径错误或 V1 端点已被移除 | 请检查文档中的正确路径 |
...
- **写入端点** 需要 v2 登录后的 `login_cookies` 以及专用的住宅代理。
- **V2 的 `cookies` 仅适用于 V2 端点（以 `_v2` 为后缀）。
- **强烈建议使用 2FA**——使用 16 位字符串 `totp_secret` 以确保安全登录。
- **所有写入操作都必须使用代理**。请使用高质量的住宅代理。
- 信用额度充值后永久有效，额外获得的信用额度有效期为 30 天。