---
name: skillboss
description: "多智能体（Multi-AI）网关：支持50多种模型，涵盖聊天、图像识别、视频处理、文本转语音（TTS）、音乐播放以及搜索功能。"
allowed-tools: Bash, Read
metadata: {"clawdbot":{"requires":{"bins":["node"],"env":["SKILLBOSS_API_KEY"]},"primaryEnv":"SKILLBOSS_API_KEY"}}
---
# SkillBoss

只需一个 API 密钥，即可使用来自多个提供商的 50 多个模型：聊天、图像处理、视频处理、文本转语音（TTS）、语音转文本（STT）、音乐服务以及网页搜索功能。
...
图像/视频/音频处理后的结果会生成一个 URL；可以使用 `curl` 命令进行保存：
...
**注意事项：**
- 请在 [https://www.skillboss.co](https://www.skillboss.co) 获取 SKILLBOSS_API_KEY。
- 使用 `models` 命令来查看可用的模型。