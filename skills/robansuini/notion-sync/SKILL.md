...
本工具支持 markdown 文件与 Notion 页面之间的双向同步，并提供数据库管理功能，用于研究内容跟踪和项目管理。

## 升级说明

**从 v2.0 开始：**
- 将 `--token "ntn_..."` 替换为 `--token-file`、`--token-stdin` 或环境变量 `NOTION_API_KEY`。单独使用 `--token` 的方式已不再被支持（凭证信息不应出现在进程输出中）。
...
- 复选框选中：`{"property": "Published", "checkbox": {"equals": true}}`  
...
```  
**功能：**  
- `--dry-run`：打印出将要更新的页面（包含当前属性值），但不会实际执行更新  
- `--limit <n>`：限制处理的页面数量（默认为 100 页）  
- 查询模式下支持分页（`has_more`/`next_cursor`）  
- 遵循速率限制（每次更新间隔 300 毫秒）  
- 进度信息输出到标准错误流（stderr），结果以 JSON 格式输出到标准输出（stdout）  

### 5. Markdown → Notion 同步  

将 markdown 内容推送至 Notion，并保留全部格式。  
```bash
...
**功能：**  
- 分批上传（每次请求最多 100 个代码块）  
- 自动实施速率限制（每次批量上传间隔 350 毫秒）  
- 富文本会自动分割以符合 Notion 的 2000 字符限制  
...
node scripts/watch-notion.js \
  ...
  "projects/newsletter-draft.md"
```  
**状态跟踪：** 默认情况下，状态信息存储在 `memory/notion-watch-state.json` 文件中（相对于当前工作目录）。你可以使用 `--state-file <path>` 进行自定义（支持通配符 `~`）。  
**默认状态结构：**  
```json
{
  "pages": {
...
```  
**自动监控：** 可通过 cron、CI 流程或任何任务调度器定期检查：  
```bash
...
```  
当 `hasChanges` 为 `true` 时，脚本会将结果输出到任何通知系统。  
...
**注意：** 创建后，需要手动在 Notion 界面设置额外的属性（类型、标签、状态等）。  
...
**注意：** 此操作仅将页面标记为“已归档”（`archived: true`），并不会永久删除页面。  
...
从 Notion URL（例如：`https://notion.so/Page-Title-abc123-example-page-id-456def`）中提取页面 ID：  
- 提取格式：`abc123-example-page-id-456def`（位于标题之后的部分）  
- 或使用 32 个字符的格式：`abc123examplepageid456def`（ hyphens 可选）  

## 限制与注意事项  

- **属性更新：** 数据库属性（类型、标签、状态）必须在创建页面后手动在 Notion 界面设置。  
- **文件大小限制：** 非常大的 markdown 文件（超过 1000 个代码块）可能因速率限制而需要较长时间才能同步完成。  
- **格式问题：** 部分复杂的 markdown 结构（如嵌套列表超过 3 层）可能无法完美转换。  
