...
description: "通过 Microsoft Graph API 保护 SharePoint 文件操作及 Office 文档的处理功能：支持证书认证（certificate auth），能够选择特定的 SharePoint 网站（Sites.Selected），以及读写 Word 文件（使用 mammoth 库）、Excel 文件（使用 exceljs 库）、PowerPoint 文件（使用 jszip 库）和 PDF 文件（使用 pdf-parse 库）。"
...
1. 创建一个具有 `Sites.Selected` 权限和证书认证的 Entra 应用程序。
