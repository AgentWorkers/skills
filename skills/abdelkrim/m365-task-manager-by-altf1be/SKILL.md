...
description: "使用 Microsoft To Do 和 Planner 来管理轻量级的 Microsoft 365 任务工作流。当用户需要在 Microsoft 365 中快速创建、分配、跟踪和跟进操作性任务时，这些工具非常实用——这些任务会明确指定负责人、截止日期、状态，并提供每日提醒。"
...
使用此技能可以对 Microsoft To-Do 任务执行实际的 Microsoft Graph CRUD（创建、读取、更新、删除）操作。
...
脚本在首次运行时会使用设备代码进行登录，并缓存令牌以供后续使用。
...
- 任务标题格式：`<项目>-<日期>-<负责人>-<操作>`
- 必填字段：标题、负责人、截止日期、状态
- 状态值：`Open`（未完成）、`In Progress`（进行中）、`Blocked`（已阻止）、`Done`（已完成）
...
- `scripts/m365-todo.mjs`：用于对 Microsoft To-Do 任务执行 CRUD 操作的脚本。
