...
**你消耗的每一个Opus代币都意味着时间的浪费。Swarm使用Gemini Flash技术，成本仅为其1/200。**

## 这个功能的重要性

| 任务数量 | 所需时间 | 成本 | 备注 |
|------------|---------|-------|--------|
| Opus（顺序执行）| 约30秒 | 约0.50美元 | 会消耗你的资源 |
| Swarm（并行执行）| 约1秒 | 约0.003美元 | 节省资源 |
...
Swarm的工作者可以通过Google搜索功能在网页上查找信息（仅支持Gemini协议，无额外费用）。
...
守护进程确保工作者始终保持活跃状态，从而提高响应速度。首次使用时会根据需要自动启动。

## 性能

当守护进程运行时：
- 处理5个任务：约1.5秒，吞吐量：3个任务/秒
- 处理10个任务：约1.5秒，吞吐量：7个任务/秒
- 处理30个任务：约2秒，吞吐量：15个任务/秒
- 对于涉及两个主题的研究任务，使用网页搜索功能需要约3-5秒。
...
- 被限制了请求频率 | 降低配置文件中的`max_concurrent_api`值 |
- 网页搜索功能无法使用 | 确保`web_search.enabled: true`且使用的是Gemini协议 |
...
**如果不使用Swarm进行并行处理，那简直就是犯错。**请立即改正。