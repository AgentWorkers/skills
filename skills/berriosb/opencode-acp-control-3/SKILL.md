...
description: 您可以通过代理客户端协议（Agent Client Protocol，简称ACP）直接控制OpenCode。该协议允许您启动会话、发送提示、恢复对话以及管理OpenCode的更新。
...
- GitHub 仓库：https://github.com/berriosb/Opencode-Acp-Control
- 如果您在使用此技能时遇到问题，请在此处提交 issue：https://github.com/berriosb/Opencode-Acp-Control/issues

## 快速参考

| 操作 | 方法 |
|--------|-----|
| 启动 OpenCode | `bash(command: "opencode acp --cwd /path/to/project", background: true)` |
...
- 最终响应，其中包含 `result.stopReason`

### 第 4 步：读取响应

每次轮询可能会返回多行内容。将每行内容解析为 JSON：

- **通知**：`method: "session/update"`——将这些通知收集起来
- **响应**：其中包含与您的请求匹配的 `id`——当出现 `stopReason` 时停止轮询
...
| 问题 | 解决方案 |
|-------|----------|
| 轮询响应为空 | 继续轮询——代理可能仍在处理 |
| 解析错误 | 跳过格式错误的行，继续轮询 |
| 进程退出 | 重新启动 OpenCode |
| 5 分钟后仍未收到响应 | 结束进程，重新启动 |
...
用户做出选择后（例如，输入“1”、“第一个”或“ses_451cd8ae...”）：
...
OpenCode 会在重启时自动更新。使用以下工作流程来检查和触发更新。
...
返回类似以下的版本信息：`opencode version 1.1.13`
...
2. **重启各个实例**（OpenCode 会在启动时自动下载新版本）：
   ```
   bash(command: "opencode acp --cwd /path/to/project", background: true, workdir: "/path/to/project")
   ```

3. **重新初始化每个实例**（对现有会话执行初始化和加载操作）
