...
description: 在代码库上运行由人工智能驱动的应用程序安全扫描。当需要扫描代码以查找安全漏洞、生成威胁模型、审查代码中的安全问题、执行增量安全扫描，或通过 cron 任务设置持续安全监控时，可以使用该功能。支持全面扫描（一次性完成）和增量扫描（由 cron 驱动，仅针对新的代码提交）。
---
# SecureVibes 扫描器

这是一个基于人工智能的安全平台，利用 Claude AI 来检测漏洞。该平台采用多子代理流程：评估 → 威胁建模 → 代码审查 → 报告生成 → 可选的动态应用安全测试（DAST）。支持增量扫描以实现持续监控。

## 先决条件

1. 安装 CLI：`pip install securevibes`
2. 使用 Anthropic 进行身份验证（选择以下方式之一）：
   - **Max/Pro 订阅（推荐）**：如果您通过 Claude Code 或 Claude CLI OAuth 进行身份验证，则无需 API 密钥。Claude Agent SDK 会自动获取您的 OAuth 会话。在 OpenClaw 中运行时，可以将 `ANTHROPIC_API_KEY` 设置为未定义或空值——SDK 会处理身份验证。
   - **API 密钥**：`export ANTHROPIC_API_KEY=your-key-here`（来自 console.anthropic.com）

## 安全注意事项

- 始终使用 `scripts/scan.sh` 包装脚本进行完整扫描——该脚本会在调用 `securevibes` 之前验证路径并拒绝 shell 元字符。
- **切勿将未经清洗的用户输入插入到 shell 命令中。**
- 该脚本使用 `realpath` 安全地解析路径，并拒绝包含 `;`, `|`, `&`, `$`, 单引号或其他元字符的路径。
- **扫描目标必须是本地目录。** 首先将远程仓库克隆到已知的安全位置，然后将解析后的路径传递给包装脚本。
- **DAST 扫描会向您提供的 `--target-url` 发送网络请求。** 仅应用于您拥有权限进行测试的应用程序。

## 执行模型

**完整扫描需要 10-30 分钟，分为 4 个阶段。** 建议将其作为后台作业（cron 或子代理）运行，而不是直接在命令行中执行。

**增量扫描需要 2-10 分钟**——仅扫描自上次运行以来的新提交。
...
1. 将目标仓库克隆到本地目录
2. 运行包装脚本：`bash scripts/scan.sh /path/to/repo --force --debug`
3. 结果将显示在 `/path/to/repo/.securevibes/` 中
...
2. 清除之前的 `.securevibes/` 文件
3. 通过包装脚本运行 `securevibes scan . --force`
4. 读取并汇总 `.securevibes/scan_report.md` 中的结果
...
运行一次包装脚本以设置基准提交（不执行扫描，仅记录当前的 HEAD）：
...
将 `<skill-path>` 替换为已安装的技能路径，将 `<repo-path>` 替换为目标仓库路径。
...
| `--severity` | 最低严重程度：`critical`、`high`、`medium`、`low` |
| `--scan-timeout-seconds` | 每次扫描的超时时间（默认：`900`） |
| `--git-timeout-seconds` | git 操作的超时时间（默认：`60`） |
| `--rewrite-policy` | 历史记录重写策略：`resetWarn`、`strict_fail`、`since_date` |
| `--since` | 从指定日期开始扫描的提交（ISO 或 YYYY-MM-DD 格式） |

### 运行保证

- `.securevibes/.incremental_scan.lock` 文件用于文件锁定，防止重复扫描。
- 使用 `fsync` 和 `os.replace` 确保状态写入的原子性，防止数据损坏。
- 日志记录保存在 `.securevibes/incremental_scan.log` 中。
- 每次扫描的运行记录保存在 `.securevibes/incremental_runs/` 中（每个运行对应一个 JSON 文件）。

### 重写策略

当 `last_seen_sha` 不是新远程 HEAD 的祖先时（例如强制推送），采取以下策略：

| 策略 | 行为 |
|--------|----------|
| `resetWarn` | 将基准提交重置为新 HEAD，继续扫描 |
| `strict_fail` | 失败并保持当前的基准提交 |
| `since_date` | 运行 `--since <today>` 的扫描以获取最新结果，同时保留之前的基准提交 |

## 完整扫描命令参考

### 扫描

`securevibes scan <path> [选项]`
...
| `--subagent` | 运行某个阶段：`assessment`、`threat-modeling`、`code-review`、`report-generator`、`dast` |
| `--resume-from` | 从特定阶段开始继续扫描 |
...
| 用户操作 | 对应操作 |
|-----------|--------|
| "扫描此仓库以查找安全问题" | 运行完整扫描：`bash scripts/scan.sh <path> --force` |
| "快速安全检查" | 运行完整扫描：`bash scripts/scan.sh <path> -m haiku --force` |
| "对该项目进行威胁建模" | 运行威胁建模：`bash scripts/scan.sh <path> --subagent threat-modeling --force` |
| "仅审查代码" | 运行代码审查：`bash scripts/scan.sh <path> --subagent code-review --force` |
| "仅显示严重/高风险的发现" | 运行扫描：`bash scripts/scan.sh <path> -s high --force` |
| "进行全面审计并使用 DAST" | 运行包含 DAST 的扫描：`bash scripts/scan.sh <path> --dast --target-url <url> --force` |
| "设置持续扫描" | 设置增量扫描：执行上述步骤 1-4 |
| "监控此仓库的安全问题" | 设置增量扫描：执行上述步骤 1-4 |
| "显示上次扫描结果" | 查看扫描报告：`securevibes report <path>` |

## 子代理流程

各阶段按顺序执行，每个阶段都基于前一个阶段的成果：

1. **评估** → 架构与攻击面分析 → 生成 `.securevibes/SECURITY.md`
2. **威胁建模** → 基于 STRIDE 的分析 → 生成 `.securevibes/THREAT_MODEL.json`
3. **代码审查** → 检测漏洞 → 生成 `.securevibes/VULNERABILITIES.json`
4. **报告生成** → 生成综合报告 → 生成 `.securevibes/scan_report.md`
...
2. 按严重程度汇总发现的问题（从高到低：Critical > High > Medium > Low）
3. 突出显示最严重的 3 个问题及其文件位置和修复建议
4. 提供后续步骤：运行 DAST、修复具体问题、在更改后重新扫描
