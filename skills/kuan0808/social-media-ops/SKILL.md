...
description: 在 OpenClaw 上搭建一个完整的多品牌社交媒体管理团队。该团队包含 7 个专门化的 AI 代理（负责人、研究员、内容策略师、视觉设计师、操作员、工程师和审核员），它们采用星型拓扑结构进行协作。系统支持持续的点对点通信（A2A）、三层记忆机制、共享知识库、审批工作流程以及品牌隔离功能。此方案适用于新社交媒体运营团队的组建、现有 OpenClaw 实例中多代理框架的添加，或当用户需要处理社交媒体管理、多品牌运营或内容团队设置相关任务时。
metadata:
  {
    "openclaw": {
      "emoji": "📱",
...
- **定时任务自动化**，用于每日记忆整理和每周知识库审查

## 可选依赖项

- **设计师代理的图像生成工具**：设计师代理需要在 `workspace-designer/skills/` 目录中安装图像生成工具，以便生成图像。推荐使用 `nano-banana-pro`（基于 Gemini，免费 tier）。如果没有此工具，设计师只能生成文本形式的视觉简报，无法生成图像。
...
> **高级提示：** 如果 later 需要精简团队规模，可以重新运行 `scaffold.sh --agents leader,content,designer,engineer` 命令来创建所需的代理组合。
...
配置文件 `patch-config.js` 会合并到 `openclaw.json` 中，内容包括：
- 代理定义（包括模型分配和工具限制）
- 点对点会话配置
- QMD（问答记忆系统）的路径（**仅当 QMD 安装时生效**，否则跳过此步骤）
- 内部钩子
...
2. **DM+Topics**——私有替代方案，无需群组
   - 每个品牌在机器人的私信中拥有自己的主题线程
   - 需要在机器人上启用线程模式（具体操作如下）
...
3. 自动创建 **Operations** 主题：
   ...
   node scripts/telegram-topics.js \
     ...
     --name "Operations"
   ```
4. 将生成的线程 ID 保存到 `shared/operations/channel-map.md` 中
...
4. 自动创建 **Operations** 主题：
   ...
   node scripts/telegram-topics.js \
     ...
     --name "Operations"
   ```
5. 将生成的线程 ID 保存到 `shared/operations/channel-map.md` 中
...
1. **实例设置**（`instance-setup` 技能）
   ...
   - 默认内容语言
   - 机器人身份（名称、表情符号、个性设定
   - 更新文件：`shared/INSTANCE.md`、`workspace/IDENTITY.md`

2. **首个品牌**（`brand-manager add`）
   - 设置品牌 ID、显示名称、域名
   - 目标市场和内容语言
   - **主题创建**（对于支持主题的模式）：
     - 代理调用 `scripts/telegram-topics.js` 创建以品牌名称命名的主题
     - 脚本返回线程 ID
     - 代理将线程 ID 保存到 `shared/operations/channel-map.md` 和品牌配置中
   - 对于简单模式：无需创建主题，跳过此步骤
...
2. **运行诊断工具**：
   ...
   openclaw doctor
   ```
   此步骤用于验证代理配置、私信允许列表的继承情况、会话状态、模型可用性以及工作空间的完整性。
...
如果在第 3 步中 `patch-config.js` 报告 “qmd binary not found”，代理将使用基于文件的记忆系统（这也是可行的）。若要启用增强型语义搜索功能：
- 输入 “Set up QMD” 以运行 `qmd-setup` 子技能，该技能会指导你完成安装和配置。

**建议的安装后首次任务：**
1. 填写品牌资料：`shared/brands/{brand_id}/profile.md`
2. 测试内容创建：`为 {brand} 发布一条 Facebook 帖子`
3. 添加更多品牌：`添加一个新的品牌`
...
负责人使用 **完全异步的调度机制**（`sessions_send` 的 `timeoutSeconds` 参数设置为 0）来处理所有代理之间的通信。这意味着：
- 负责人 **永远不会被阻塞**，随时可以接收代理的反馈
- 代理在完成任务后通过 `sessions_send` 回调给负责人（基于事件驱动，而非轮询）。负责人根据 `AGENTS.md` 中的 “Agent Callback Protocol” 处理回调
- 每个任务都会记录在单独的文件 `tasks/T-{YYYYMMDD}-{HHMM}.md` 中。已完成的任务会被归档到 `tasks/archive/` 目录
- **过期任务检测** 由定时任务 `stale-task-check` 每 5 分钟执行一次，该任务会检查 `tasks/` 目录中处于 `[⏳]` 状态的未完成任务。此任务由负责人执行（而非操作员）
- 默认情况下 `HEARTBEAT.md` 文件为空，定期检查由定时任务完成

### 秘密管理（可选）

为了集中管理 API 密钥，可以不使用分散的环境变量，而是使用专门的配置文件：
```
...
每个代理的行为通过两个文件进行配置：
- **SOUL.md** — 定义代理的个性、工作原则和行为规范
- **AGENTS.md** — 规定代理的操作流程、数据处理方式和职责范围
...
详细文档请参考 `references/memory-system.md`。

### 通信信号

代理使用标准化的信号来传递状态信息。完整信号字典请参见 `references/signals-protocol.md`。
...
| `references/memory-system.md` | 三层记忆系统及知识存储方式 | 了解记忆系统的运作原理 |
| `references/approval-workflow.md` | 审批流程及负责人操作快捷方式 | 内容发布流程 |
| `references/troubleshooting.md | 常见问题及解决方法 | 解决故障时参考 |
...
| `scripts/telegram-topics.js` | 在 Telegram 私信或超群组中创建主题 | 设置阶段或添加品牌时 |
