...
- 通过标准的 Alibaba Cloud 环境变量提供凭据：
  - `ALICLOUD_ACCESS_KEY_ID`
  - `ALICLOUD_ACCESS_KEY_SECRET`
  - `ALICLOUD_REGION_ID`（可选，默认值；如果未设置，请选择最适合任务的区域或询问用户）
...
## 关于 Claude Code/Codex 的说明：

- DocMind 是异步操作的：提交任务后，需要持续轮询直到任务状态变为 `completed=true`。
- 每约 10 秒轮询一次；最大处理时间为 120 分钟。
- 使用 URL 提交任务时，请确保文件可以公开访问。

## 错误处理

- `UrlNotLegal`：提供的 URL 无法公开访问或格式不正确。
...
成功标准：命令执行成功（退出代码为 0），并且生成了 `output/alicloud-ai-text-document-mind/validate.txt` 文件。

## 输出结果与证据

- 将处理结果、命令输出以及 API 响应摘要保存在 `output/alicloud-ai-text-document-mind/` 目录下。
