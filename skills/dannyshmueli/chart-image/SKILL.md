...
description: >
  **生成高质量图表图像的工具**  
  该工具能够从数据中生成适用于出版物的图表图像，支持多种图表类型，包括折线图、柱状图、面积图、点状图、蜡烛图、饼图/甜甜圈图、热力图、多系列图以及堆叠图。适用于数据可视化、图表制作、时间序列分析，或为报告/警报生成图表图像。专为 Fly.io/VPS 环境设计：无需进行本地编译，也不依赖 Puppeteer 或浏览器；完全基于 Node.js 构建，并提供了预编译的二进制文件。
provides:
...
- ✅ **轻量级**：总依赖项约为15MB，而基于Puppeteer的解决方案则需要400MB以上
...
| 选项 | 描述 | 默认值 |
|--------|-------------|---------|
| `--type` | 图表类型：折线图、条形图、面积图、点图、饼图、甜甜圈图、蜡烛图、热力图 | 折线图 |
| `--data` | 数据点的JSON数组 | - |
...
| `--y-domain` | Y轴刻度范围（最小值/最大值） | 自动 |

### 视觉效果选项
| 选项 | 描述 | 默认值 |
|--------|-------------|---------|
| `--color` | 折线/条形图的颜色 | #e63946 |
| `--dark` | 深色模式 | false |
| `--svg` | 生成SVG格式的图表 | false |
| `--color-scheme` | Vega的颜色方案（例如：category10, viridis等） | - |

### 警报/监控选项
| 选项 | 描述 | 默认值 |
|--------|-------------|---------|
| `--show-change` | 在最后一个数据点显示变化百分比注释 | false |
...
| `--y-category-field` | Y轴的分类字段 | y |

### 双Y轴选项
| 选项 | 描述 | 默认值 |
|--------|-------------|---------|
| `--y2-field` | 第二个Y轴的字段名（独立轴） | - |
| `--y2-title` | 第二个Y轴的标题 | 字段名 |
...
| 选项 | 描述 | 默认值 |
|--------|-------------|---------|
| `--volume-field` | 用于体积条形图的字段名（启用双Y轴） | - |
...
当X值为ISO日期且需要根据实际时间间隔显示数据时，使用`--x-type temporal`：
```bash
...
## ⚠️ 重要提示：** 生成图表后，请务必将其发送给用户！**
生成图表后，**务必将其发送到用户的指定渠道**。不要只是将其保存到文件中并简单描述——图表的最终目的是为了可视化展示。
...
**使用建议：**
- 将图表保存到`/data/clawd/tmp/`（永久存储目录），而非`/tmp/`（该目录可能会被清理）
