...
这是一个TypeScript命令行工具（CLI），它允许AI代理和人类通过Notion的官方API与Notion工作区进行交互。该工具支持页面、数据库、区块、用户、评论以及搜索功能。
...
| 身份验证 | 以用户身份操作 | 以机器人身份操作 |
| 设置 | 无需设置——凭证自动提取 | 需要手动在[notion.so/my-integrations]创建集成 |
...
**切勿编写任何脚本（Python、TypeScript、Bash等）来自动化Notion操作。** `batch`命令已经可以处理任意规模的大量操作。编写脚本来循环调用API是错误的做法——请使用`batch`命令并配合`--file`参数。

以下情况同样适用：
- 需要创建100多个页面或区块
- 需要在新创建的项目之间建立关联（请参考[批量操作策略](#bulk-operations-strategy)
- 操作量太大，无法通过单个命令完成

如果您有“应该编写脚本”的想法，请立即停止，并使用`batch`命令。
...
您可以在一次CLI调用中执行多个写入操作。这样可以在一次操作中创建、更新或删除多个项目，从而节省令牌并减少网络请求次数。
...
**快速失败处理**：操作按顺序执行。如果任何操作失败，整个流程会立即停止。输出将包含所有成功操作的结果以及失败操作的信息。失败时，程序的退出代码为1；成功时为0。
...
对于大量操作（数十到数百个项目），请使用`--file`参数来避免超出Shell命令行的参数限制，使操作更加可控。
...
**多步骤处理**（当新创建的项目需要相互引用时，例如一个页面属性需要引用另一个新创建的页面）：

1. **第一步——创建所有项目**：编写一个包含所有创建操作的JSON文件，但省略指向其他新项目的属性。执行该文件，收集返回的ID。
...
Notion对其API设置了速率限制。批量操作会按顺序执行，因此大量操作（超过30个）可能会触发“429 Too Many Requests”错误。为了避免这种情况：
- 将大量操作分成每批25-30个的操作
...
Notion API可能返回以下常见错误：
- `object_not_found`：ID不正确或集成权限不足。
