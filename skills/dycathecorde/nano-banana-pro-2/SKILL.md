...
- 或者在 `~/.clawdbot/moltbot.json` 文件中设置 `skills."nano-banana-pro".apiKey` 或 `skills."nano-banana-pro".env.GEMINI_API_KEY`。

**注意事项：**
- 分辨率选项：`1K`（默认）、`2K`、`4K`。
- 文件名格式：`yyyy-mm-dd-hh-mm-ss-name.png`。
- 脚本会输出一条 `MEDIA:` 信息，以便 Moltbot 在支持的聊天平台上自动插入该图像。
- 请勿读取已保存的图像文件；只需返回其保存路径即可。