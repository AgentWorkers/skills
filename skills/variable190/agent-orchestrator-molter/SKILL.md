...
version: 1.0.3
author: molter-white
description: 多代理编排的五种成熟模式：工作小组（Work Crew）、监督者（Supervisor）、流水线（Pipeline）、决策委员会（Council）和自动路由（Auto-Routing）
license: MIT
...
OpenClaw的多智能体协同系统。实现了5种经过验证的智能体协调模式：工作小组（Work Crew）、监督者（Supervisor）、流水线（Pipeline）、专家委员会（Expert Council）和自动路由（Auto-Routing）。

**适用场景：**
- 需要并行处理任务以提高效率或实现冗余（工作小组模式）
- 复杂任务需要动态规划和任务分配（监督者模式）
- 任务遵循可预测的阶段顺序（流水线模式）
- 需要来自多个专家的跨领域输入（专家委员会模式）
- 混合类型的任务需要自动路由到相应的专家（自动路由模式）
- 研究任务需要从多个角度进行广度优先的探索
- 高风险决策需要通过多角度的分析来提高决策的可靠性

**不适用场景：**
- 适合单个智能体处理的简单任务（请使用主会话模式）
- 无法并行化的顺序任务（请使用常规工具调用）
- 一次性、确定性的任务（请使用单个智能体）
- 需要实时智能体间协作的任务（该系统使用异步任务调度）
- 任务处理成本过高（超过15倍的token成本）的任务
- 协调开销大于收益的快速/简单任务
...
| **监督者**     | 需要动态任务分解、复杂规划                | 固定的工作流程、简单的任务分配                |
| **流水线**     | 有明确顺序的阶段、内容创作                | 路径需要实时调整                        |
| **专家委员会**   | 需要跨领域专业知识、风险评估、政策审查          | 单一领域的任务、需要快速达成共识                |
| **自动路由**     | 混合类型的工作负载、自动任务分类              | 任务类型已经明确                        |

## 自动路由模式

`route`命令会分析任务并自动对其进行分类，然后将其路由到相应的专家：
...
- **高信心（>0.85）**：立即自动路由
- **良好信心（0.7-0.85）**：提供选项供确认后执行
- **中等信心（0.5-0.7）**：显示最佳选项供选择
- **低信心（<0.5）**：请求进一步澄清

可用专家类型：编码员（coder）、研究员（researcher）、写手（writer）、分析师（analyst）、规划师（planner）、审核员（reviewer）、创意人员（creative）、数据分析师（data）、DevOps工程师（devops）、支持人员（support）
...
**错误1：** 对于简单的问题使用“工作小组”模式
**错误2：** 当“流水线”模式足够时使用“监督者”模式
**错误3：** 任务类型明确时使用自动路由模式
**错误4：** 对于非常简单的任务使用多智能体协同模式

## 注意事项：**

- **注意token成本**：多智能体模式使用的token数量大约是单智能体模式的15倍。仅适用于那些通过提高质量能够带来显著收益的高价值任务。参考Anthropic的研究：token使用情况可以解释复杂任务中80%的性能差异。
...
## 相关文件：
- `__main__.py`：命令行接口（CLI）入口点
- `crew.py`：工作小组模式实现代码
- `supervise.py`：监督者模式实现代码（第二阶段）
- `council.py`：专家委员会模式实现代码（第二阶段）
- `pipeline.py`：流水线模式实现代码（第二阶段）
- `route.py`：自动路由模式实现代码（第二阶段）
...
- **第二阶段：100%完成**
  ...
  - [x] 流水线模式实现 - 有序的阶段处理及验证机制
  - [x] 专家委员会模式实现 - 多专家讨论及共识形成机制
  - [x] 自动路由模式实现 - 智能任务分类和专家路由功能
