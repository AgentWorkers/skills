...
name: x-apify
version: 1.0.5
description: 通过 Apify 的代理服务，可以获取 X/Twitter 的数据。支持搜索推文、获取用户资料以及检索包含回复的特定推文。系统具备本地缓存功能，以降低 API 调用的成本。无论使用何种 IP 地址，都可以通过 Apify 的代理基础设施正常访问相关服务。
...
缓存位置：`skill` 目录下的 `.cache/` 文件夹（可通过 `X_APIFY_CACHE_DIR` 环境变量进行覆盖）
