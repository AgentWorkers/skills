...
description: "从档案中提取、清洗并整理旧的施工数据。将历史项目数据、成本记录和进度表迁移到现代格式中。"
...
本工具用于管理来自档案、旧系统和历史记录的遗留建筑数据。它能够提取、清洗、规范这些数据，并将其转换为现代格式，以便进行分析和基准测试。
...
1. **成本基准测试**：对历史成本进行标准化，以便进行比较
2. **效率分析**：追踪随时间变化的劳动力生产效率
3. **风险识别**：分析历史项目中的问题模式
4. **估算优化**：利用历史数据来改进估算方法
...
- **RSMeans历史成本指数**：用于成本估算的参考指标
- **ENR建筑成本指数**：另一种成本估算参考来源
