---
name: reflect
description: 通过对话分析实现自我提升：从错误修正和成功案例中提取经验教训，并将这些经验永久性地编码到代理程序的定义中。我们的理念是——“纠正一次，再也不会犯同样的错误。”
version: "2.0.0"
user-invocable: true
triggers:
  - reflect
...
将您的人工智能助手转变为一个持续改进的伙伴。每一次的纠正都将成为一项永久性的改进，这些改进会在所有未来的会话中持续生效。
...
| **低** | 观察到的有效行为模式 | 虽然有效但未得到明确确认 |

有关完整的检测规则，请参阅 [data/signal_patterns.md](data/signal_patterns.md)。
...
| 领域 | 领域特定的代理文件, `CLAUDE.md` |
| 工具 | `CLAUDE.md`, 相关专家文件 |
| 新技能 | 创建新的技能文件 |

有关映射规则，请参阅 [data/agent_mappings.md](data/agent_mappings.md)。

### 第三步：判断是否值得形成新技能

某些学习内容应该被转化为新的技能，而不仅仅是代理的更新：
...
状态信息存储在 `~/.reflect/` 目录中（可通过 `REFLECT_STATE_DIR` 配置）：
...
- `~/.reflect/reflect-metrics.yaml` – 综合指标报告 |

## 示例

### 示例1：代码风格纠正

**用户反馈**：“在TypeScript中永远不要使用 `var`，而应该使用 `const` 或 `let`”
