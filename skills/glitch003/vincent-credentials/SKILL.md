...
description: >
  **代理的安全凭证管理**  
  当用户需要存储 API 密钥、密码、OAuth 令牌或 SSH 密钥，并将这些信息写入 `.env` 文件时，可以使用此功能，同时确保这些敏感信息不会被泄露。  
  该功能适用于以下场景：  
  - 存储凭证  
  - 管理 API 密钥  
  - 将数据写入环境变量文件（`.env`）  
...
当代理需要使用凭证来构建某些内容（例如第三方 API 密钥）时，这一功能非常有用。用户无需将凭证粘贴到聊天框中（这样凭证就会进入代理的上下文），而是通过 Vincent 仪表板进行设置，代理再通过 CLI 将其写入 `.env` 文件。

所有命令都依赖于 `@vincentai/cli` 包。API 密钥会自动存储和解析——您无需直接处理原始密钥或文件路径。

## 安全模型

此功能的设计目的是**确保凭证不会出现在代理的上下文中**。

**工作原理：`secret env` CLI 命令从 Vincent 服务器获取凭证，并将其直接写入磁盘上的 `.env` 文件。该值永远不会被打印到标准输出中，也不会出现在代理的对话上下文中。**许多代理框架会禁止读取 `.env` 文件，因此即使文件存在于磁盘上，代理也无法读取其中的内容。应用程序在运行时可以正常读取 `.env` 文件。

**无需环境变量**，因为此功能采用“代理优先”的认证方式：代理在运行时通过调用 Vincent API 生成自己的凭证秘密值，该 API 会返回一个具有特定范围的 API 密钥。CLI 会在创建过程中自动存储返回的 API 密钥。密钥的存储路径（`${OPENCLAW_STATE_DIR:-$HOME/.openclaw}/credentials/credentials/` 或 `./credentials/`）在功能的元数据中进行了指定。

**覆盖保护机制：**一旦某个值被代理的 API 密钥设置，只有相同的 API 密钥才能覆盖它。这可以防止其他代理或密钥篡改凭证。这种保护机制在数据库层面是原子级执行的。

**所有 API 调用都通过 HTTPS/TLS 协议发送到 `heyvincent.ai`。**不会连接到其他端点、服务或外部主机。

**密钥生命周期：**

- **创建**：代理运行 `secret create` 命令，并指定 `--type CREDENTIALS` 参数——CLI 会自动存储 API 密钥，并返回一个 `keyId` 和 `claimUrl`。
- **设置值**：用户通过仪表板设置凭证值，或者代理通过 CLI 设置。
- **写入 `.env` 文件**：代理运行 `secret env` 命令将值写入 `.env` 文件，同时不会暴露该值。
- **声明所有权**：操作员可以通过仪表板使用声明 URL 来获取凭证的所有权并进行管理。
...
这四种类型的凭证都支持相同的创建、设置和写入 `.env` 文件的流程。
...
该命令会返回 `keyId`（用于后续所有命令）、`claimUrl`（与用户共享）和 `secretId`。

创建完成后，告知用户：

> “这是您的凭证声明 URL：`<claimUrl>`。请使用该 URL 在 https://heyvincent.ai 上声明所有权并设置凭证值。”

### 3. 设置凭证值

**选项 A：用户通过仪表板设置（推荐）**

用户通过声明 URL 声明对凭证的所有权，然后从仪表板设置凭证值。这样，凭证值完全不会落入代理的控制范围。

**选项 B：代理通过 CLI 设置**

对于那些由代理自行管理凭证的工作流程（例如代理从服务获取 API 密钥的情况）：
...
设置好值后，使用 CLI 将其写入 `.env` 文件。**该值永远不会被打印到标准输出中。**
...
命令会输出一个确认 JSON（不包含具体值），以便代理知道操作成功：
...
`secret env` 命令会输出一个确认 JSON（不包含凭证值）：
...
| 错误代码 | 原因 | 解决方案 |
|-------|-------|------------|
| `401 Unauthorized` | API 密钥无效或缺失 | 确保 `keyId` 正确；如有需要，请重新链接 |
| `403 Overwrite Rejected` | 其他 API 密钥设置了该凭证的值 | 必须由凭证所有者通过仪表板进行管理 |
| `404 Value Not Set` | 凭证值尚未设置 | 用户必须通过仪表板设置值，或由代理通过 CLI 设置 |
| Key not found` | API 密钥已被撤销或从未创建 | 请向凭证所有者请求新的令牌 |

## 重新链接（恢复 API 访问权限）

如果代理丢失了 API 密钥，凭证所有者可以从前端生成一个新的 **重新链接令牌**。代理可以使用这个令牌获取新的 API 密钥。

```bash
npx @vincentai/cli@latest secret relink --token <TOKEN_FROM_USER>
```

CLI 会使用这个令牌获取新的 API 密钥，并自动将其存储起来，同时返回新的 `keyId`。重新链接的令牌是一次性使用的，有效期为 10 分钟。

## 重要注意事项

- **凭证值永远不会进入代理的上下文中。** `secret env` 命令直接将凭证值写入文件，不会将其打印到标准输出。
- 许多代理框架（如 OpenClaw、Claude Code 等）会禁止读取 `.env` 文件，从而提供额外的安全保护。
- 创建凭证后，务必将声明 URL 分享给用户。
- 所有值的长度限制为 16KB。
- 如果出现 `403` 错误，表示有其他 API 密钥设置了该凭证的值。此时必须由凭证所有者通过仪表板进行管理。