...
description: >
  **Polymarket 预测市场交易功能**  
  该功能专为代理（agents）设计，适用于用户创建 Polymarket 钱包、浏览市场、下注、管理持仓或提取资金等操作。  
...
使用此工具为代理创建一个 Polymarket 钱包，并在预测市场中进行交易。您可以浏览市场、下注、跟踪持仓情况以及管理订单——所有这些操作都不会将私钥暴露给代理。该钱包使用 Polygon 上的 Gnosis Safe 技术，并通过 Polymarket 的中继器实现无 gas 的交易。
...
**无需环境变量**，因为此工具采用“代理优先”的接入方式：代理在运行时通过调用 Vincent API 自动创建其 Polymarket 钱包，API 会返回一个受限范围的 API 密钥。无需预先配置任何凭证。CLI 会在创建钱包时自动存储返回的 API 密钥。密钥的存储路径（`${OPENCLAW_STATE_DIR:-$HOME/.openclaw}/credentials/agentwallet/` 或 `./agentwallet/`）在工具的元数据中已明确指定。

**代理的 API 密钥并非私钥**。它是一个受限范围的持有者令牌，仅能执行钱包所有者设定的策略允许的操作。Vincent 服务器会在服务器端强制执行所有策略——无论代理发送什么请求，都无法绕过这些策略。如果交易违反策略，服务器会拒绝该请求。如果交易需要批准，服务器会暂缓处理并通过 Telegram 通知钱包所有者进行人工审批。

**有意启用模型调用**。此工具的目的是让 AI 代理具备自主的 Polymarket 交易能力。代理应在人类操作员定义的范围内自行执行交易操作（如浏览市场、下注、管理持仓）。人类操作员通过策略（如支出限制、审批阈值）来控制代理的行为，而不是通过限制单个请求来实现控制。存储的密钥具有范围限制和策略约束——即使其他进程读取了该密钥，也只能执行钱包所有者允许的操作，且所有者可以随时撤销该密钥。

**所有 API 调用都通过 HTTPS/TLS 发送到 `heyvincent.ai`**。不会访问其他端点、服务或外部主机。代理不会读取、收集或传输超出 Polymarket 钱包操作所需的数据。
...
- **重新链接**：如果代理丢失了 API 密钥，钱包所有者会生成一个一次性重新链接令牌，代理可以通过 `secret relink` 命令用该令牌获取新的 API 密钥。
...
返回 `keyId`（用于后续命令）、`claimUrl`（与用户共享）和 `walletAddress`（EOA 地址；Safe 会在首次使用时懒加载部署）。

创建完成后，告知用户：

> “这是您的钱包认领链接：`<claimUrl>`。使用此链接来认领所有权、设置支出策略，并在 `https://heyvincent.ai` 上监控代理的钱包活动。”
...
**请勿发送原始的 USDC**（`0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359`）。Polymarket 仅接受桥接后的 USDC.e。

### 5. 从 Vincent EVM 钱包转移资金（备用充值方式）

如果您拥有 Vincent EVM 钱包且其中有钱款，可以使用 `wallet transfer-between` 命令将资金转移到 Polymarket 钱包（详见钱包相关工具）。Vincent 会验证您是否拥有这两个钱包的密钥，并自动处理代币转换和跨链桥接，以便在 Polygon 上获取 USDC.e。
...
**关键点：**
- 使用您的 **EVM 钱包的密钥-id**（而非 Polymarket 的密钥-id）进行这些操作。
- `--to-secret-id` 必须是您的 Polymarket 钱包的密钥 ID。
- 对于 Polymarket 目标地址，只能使用 `--to-chain 137`（Polygon）和 `--token-out 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174`（USDC.e）。
- 服务器会验证您是否拥有这两个钱包的密钥——向其他用户钱包的转账会被拒绝。
...
**重要提示：** 始终使用市场响应中的 `tokenIds` 数组。对于“是/否”类型的市场：
- `tokenIds[0]` 对应“是”的代币 ID
- `tokenIds[1]` 对应“否”的代币 ID
...
- `--side`：`BUY` 或 `SELL`
- `--amount`：BUY 订单的金额（以 USD 计）；SELL 订单的卖出股份数
- `--price`：可选的限价（0.01 至 0.99）。如果没有指定限价，则使用市价订单。
- **BUY 订单**：
  - `amount` 表示您愿意支付的金额（例如，`5` 表示 5 美元）
  - 您将获得 `amount / price` 的股份（例如，5 美元对应 10 股）
- **SELL 订单**：
  - `amount` 表示要卖出的股份数
  - 您将收到 `amount * price` 的金额（以 USD 计）
- **注意时机**：在买入成交后，请等待几秒钟再卖出。股份需要在链上完成结算。

如果交易违反策略，服务器会返回错误信息，说明违反了哪项策略。如果交易需要人工审批（基于审批阈值策略），服务器会返回 `status: "pending_approval"`，并通知钱包所有者进行审批或拒绝。
...
**Holdings** 返回所有持仓的股份数、平均买入价格、当前价格和未实现的盈亏。此端点可用于：
- 在下卖单前查看当前持仓
- 设置止损或止盈规则
- 计算投资组合的总价值和表现
- 向用户显示其活跃的投注记录
...
市场结算后，可以回收盈利持仓并将其转换为 USDC.e。使用 `holdings` 命令查看哪些持仓满足 `redeemable: true` 条件，然后调用 `redeem` 命令进行回收。
...
如果没有可回收的持仓，`redeemed` 数组将为空，且不会执行任何交易。

**工作原理：** 回收操作是无 gas 的（通过 Polymarket 的中继器和 Gnosis Safe 实现）。对于标准市场，会调用 CTF 合同上的 `redeemPositions` 函数；对于高风险市场，则会调用 NegRiskAdapter 上的 `redeemPositions` 函数。这两种情况都会自动处理。

**何时回收？** 定期检查持仓情况。市场结算后，可能需要一段时间才能使持仓可回收。请查看持仓响应中的 `redeemable: true` 字段。
...
- `--amount`：转账金额（以 USDC 为单位，例如 “100” 表示 100 USDC）
...
如果转账金额超过钱包的 USDC 余额，服务器会返回 `INSUFFICIENT_BALANCE` 错误。提款同样受到支出限制和审批阈值的约束。
...
| 错误 | 原因 | 解决方案 |
|-------|-------|------------|
| `401 Unauthorized` | API 密钥无效或缺失 | 确认密钥 ID 是否正确；如有需要，请重新链接 |
| `403 Policy Violation` | 交易被服务器端策略阻止 | 用户需在 heyvincent.ai 上调整策略 |
| `INSUFFICIENT_BALANCE` | 资金不足 | 请向钱包中充值 USDC.e |
| `429 Rate Limited` | 请求过多 | 请稍后重试 |
| `pending_approval` | 交易超出审批阈值 | 系统会发送 Telegram 通知请求用户审批或拒绝 |
| `No orderbook exists` | 市场已关闭或代币 ID 错误 | 确认 `acceptingOrders: true` 并使用 `tokenIds[]`，而非 `conditionId` |
| `Key not found` | API 密钥已被撤销或从未创建 | 请从钱包所有者处获取新的重新链接令牌 |

## 策略（服务器端执行）

钱包所有者可以通过 `https://heyvincent.ai` 上的认领链接设置策略，从而控制代理的行为。所有策略都由 Vincent API 在服务器端强制执行——代理无法绕过或修改这些策略。如果交易违反策略，API 会拒绝该交易。如果交易触发审批阈值，系统会暂缓处理并通过 Telegram 通知钱包所有者进行人工审批。

| 策略                        | 功能                                                                                          |
| --------------------------- | --------------------------------------------------------------------------- |
| **Spending limit (per tx)** | 每笔交易的最高金额上限                                                                                   |
| **Spending limit (daily)** | 每 24 小时的最高金额上限                                                                                   |
| **Spending limit (weekly)** | 每 7 天的最高金额上限                                                                                   |
| **Require approval**        | 所有交易都需要通过 Telegram 进行人工审批                                                                                   |
| **Approval threshold**      | 金额超过指定阈值的交易需要人工审批                                                                                   |

在钱包被认领之前，代理可以无策略限制地操作。这是有意设计的：代理优先的接入方式允许代理立即开始交易。一旦人类操作员通过认领链接认领了钱包，他们可以添加任何策略来限制代理的行为。钱包所有者也可以随时完全撤销代理的 API 密钥。

## 重新链接（恢复 API 访问权限）

如果代理丢失了 API 密钥，钱包所有者可以通过前端生成一个 **重新链接令牌**。代理可以使用该令牌获取新的受限范围的 API 密钥。
...
CLI 会使用该令牌获取新的 API 密钥，并自动存储它，同时返回新的 `keyId`。请使用这个 `keyId` 进行后续操作。
...
## 重要说明：
- **每次下注或交易后**，请分享用户的 Polymarket 个人资料链接，以便他们可以查看自己的持仓情况：`https://polymarket.com/profile/<polymarketWalletAddress>`（使用钱包的 Safe 地址）。
- **所有交易均为无 gas 的**，通过 Polymarket 的中继器完成。
- **切勿尝试访问原始的密钥值**。私钥始终保留在服务器端——这就是设计初衷。
- 创建钱包后，请务必将认领链接分享给用户。
- 如果交易被拒绝，可能是由于服务器端策略的限制。请让用户查看 `https://heyvincent.ai` 上的策略设置。
- 如果交易需要审批，系统会返回 `status: "pending_approval"`。钱包所有者会收到 Telegram 通知以进行审批或拒绝。

有关常见错误及其解决方法，请参阅上述的 **错误处理** 部分。