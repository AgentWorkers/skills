...
description: 敏捷产品管理方法，用于任务列表（backlog）的管理和冲刺（sprint）的执行。内容包括用户故事的编写、验收标准的制定、冲刺计划的制定以及工作进度的跟踪。该方法适用于编写用户故事、创建验收标准、规划冲刺任务、估算任务的工作量（故事点数）、分解大型项目任务（epics），以及确定任务列表的优先级。
triggers:
  - write user story
...
这是一套专为产品经理设计的工具包，用于管理待办事项列表（Backlog）和执行冲刺（Sprint）任务，包括用户故事的创建、验收标准的制定、冲刺计划的制定以及工作量的跟踪。
...
根据INVEST原则创建符合要求的标准用户故事：

1. 确定目标用户群体（谁会从该功能中受益）
2. 定义所需的功能或能力
3. 阐述该功能带来的好处或价值
4. 使用“Given-When-Then”格式编写验收标准
5. 使用斐波那契比例估算故事点数
6. 根据INVEST原则进行验证
7. 为故事分配优先级并添加到待办事项列表中
8. **验证：**故事满足所有INVEST原则；验收标准可测试
...
| 类型 | 模板 | 示例 |
|------|----------|---------|
| 新功能 | 作为[目标用户群体]，我希望[执行某个操作]，以便[获得某种好处] | 作为用户，我希望过滤搜索结果，以便更快地找到所需内容 |
| 改进 | 作为[目标用户群体]，我需要[某种能力]来实现[目标] | 作为用户，我希望页面加载速度更快，以便更高效地完成任务 |
| 错误修复 | 作为[目标用户群体]，在[特定条件下]，我期望[某种行为] | 作为用户，我希望在刷新页面后购物车中的内容能够保留 |
| 支持性任务 | 作为开发人员，我需要完成[技术任务]来支持[所需功能] | 作为开发人员，我需要实现缓存功能以支持即时搜索 |
...
| 管理员 | 控制权、透明度、安全性 | 系统管理 |
| 高级用户 | 自动化、定制化、快捷操作 | 需要复杂的工作流程 |
| 新用户 | 指导、学习支持、安全性 | 产品入门阶段 |
...
| 类型 | 示例 |
|----------|---------|
| 正常流程 | 给定有效输入后，执行操作时，应显示成功消息 |
| 验证 | 如果必填字段为空，应拒绝输入 |
| 错误处理 | 当API失败时，必须显示用户友好的错误信息 |
| 性能 | 操作应在2秒内完成 |
| 可访问性 | 仅通过键盘即可导航 |
...
6. 确定故事之间的依赖关系
7. 按顺序安排故事的交付
8. **验证：**每个故事都能独立带来价值；所有故事共同覆盖历史级任务的范围

### 分解技巧

| 技巧 | 适用场景 | 示例 |
|-----------|-------------|---------|
| 按工作流程步骤分解 | 线性流程 | “结账” → “加入购物车” → “输入付款” → “确认订单” |
| 按目标用户群体分解 | 多种用户类型 | “管理员仪表盘” → “普通用户仪表盘” |
| 按数据类型分解 | 多种输入方式 | “导入” → “导入CSV” → “导入Excel” |
| 按操作类型分解 | 基本CRUD操作 | “管理用户” → “创建” → “编辑” → “删除” |
| 先分解核心流程 | 降低风险 | “新功能” → “基本流程” → “错误处理” → “边缘情况” |
...
1. 计算团队的工作量（冲刺速度 × 团队可用时间）
2. 与利益相关者讨论冲刺目标
3. 从优先级较高的待办事项列表中选择故事
4. 选择的工作量占总工作量的80-85%
5. 设定额外目标（占工作量的10-15%）
6. 识别故事之间的依赖关系和风险
7. 将复杂的故事分解为具体的任务
8. **验证：**已承诺的工作量不超过85%的工作量；所有故事都有明确的验收标准
...
| 高 | 核心功能、关键用户需求 | 本次冲刺需要完成 |
| 中等 | 改进措施、增强功能 | 下2-3次冲刺完成 |
| 低 | 可选功能、小改进 | 可延迟到后续冲刺 |
...
| 风险/依赖关系 | 15% | 是否存在技术风险？是否有外部依赖？ |
| 工作量 | 15% | 任务规模如何？复杂性如何？完成难度如何？ |

### 在添加故事到冲刺计划前需验证的INVEST原则

在将故事添加到冲刺计划之前，需验证每个故事是否符合以下标准：

| 标准 | 需要验证的内容 | 符合标准的情况 |
|-----------|----------|------------|
| **独立性** | 该任务能否在其他未承诺的故事完成之前独立开发？ | 无阻碍其他任务的依赖关系 |
| **灵活性** | 实现方案是否灵活？ | 是否有多种实现方式？ |
| **价值** | 该任务能否为用户或业务带来价值？ | 是否有明确的好处描述？ |
| **可估算性** | 团队能否准确估算完成时间？ | 需求是否足够明确？ |
| **规模** | 该任务能否在一次冲刺中完成？ | 工作量是否不超过8点？ |
| **可验证性** | 我们能否验证任务是否已完成？ | 验收标准是否明确？ |
...
- 各类型故事的标准模板（新功能、改进、错误修复、支持性任务）
- 验收标准模板（Given-When-Then、Should/Must/Can）
- INVEST原则验证检查表
- 故事点数估算指南（斐波那契比例）
- 常见的故事编写错误及解决方法
- 故事分解技巧
...
| 指标 | 计算公式 | 目标值 |
|--------|---------|--------|
| 工作量 | 完成的故事点数 / 总冲刺点数 | 稳定在±10%范围内 |
| 任务完成率 | 完成的任务数 / 承诺的任务数 | >85% |
| 范围变化 | 冲刺期间新增或删除的故事点数 | <10% |
| 未完成的任务 | 未完成的故事点数 | <15% |
