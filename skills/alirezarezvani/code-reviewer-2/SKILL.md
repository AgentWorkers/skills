---
name: code-reviewer
description: TypeScript、JavaScript、Python、Go、Swift、Kotlin 的代码审查自动化工具。该工具能够分析 Pull Request 的复杂性和风险，检查代码质量是否存在 SOLID 设计原则的违反情况以及代码中的不良实践（即“代码异味”），并生成相应的审查报告。适用于在审查 Pull Request、分析代码质量、识别问题以及生成审查任务清单时使用。
---
# 代码审查工具
...
反模式目录，包含示例及解决方法：
- 结构相关问题（“上帝类”、过长的方法、过深的嵌套结构）
- 逻辑相关问题（逻辑错误、硬编码的类型判断）
- 安全相关问题（SQL 注入、硬编码的凭据）
- 性能相关问题（重复的查询、无界的数据结构）
- 测试相关问题（代码重复、测试实现问题）
- 异步编程相关问题（未处理的 Promise、回调地狱）
