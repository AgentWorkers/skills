---
name: "risk-management-specialist"
description: 医疗设备风险管理专家负责在整个产品生命周期中实施ISO 14971标准。该专家提供风险分析、风险评估、风险控制以及产品上市后的信息分析服务。当用户提及风险管理、ISO 14971、风险分析、失效模式与效应分析（FMEA）、故障树分析、危害识别、风险控制、风险矩阵、收益-风险分析、剩余风险、风险可接受性或上市后风险等相关内容时，可参考该专家的专业服务。
---
# 风险管理专家

在整个医疗器械生命周期中，遵循ISO 14971:2019标准实施风险管理。

---

## 目录

- [风险管理规划工作流程](#risk-management-planning-workflow)
- [风险分析工作流程](#risk-analysis-workflow)
- [风险评估工作流程](#risk-evaluation-workflow)
- [风险控制工作流程](#risk-control-workflow)
- [生产后风险管理](#post-production-risk-management)
- [风险评估模板](#risk-assessment-templates)
- [决策框架](#decision-frameworks)
- [工具和参考资料](#tools-and-references)

---

## 风险管理规划工作流程

根据ISO 14971标准建立风险管理流程。

### 工作流程：创建风险管理计划

1. 明确风险管理活动的范围：
   - 医疗器械的识别
   - 覆盖的生命周期阶段
   - 适用的标准和法规
2. 制定风险可接受性标准：
   - 定义概率等级（P1-P5）
   - 定义严重程度等级（S1-S5）
   - 创建带有接受阈值的风险矩阵
3. 分配责任：
   - 风险管理负责人
   - 专家
   - 批准机构
4. 定义验证活动：
   - 控制验证的方法
   - 接受标准
5. 规划生产和生产后的活动：
   - 信息来源
   - 审查触发条件
   - 更新程序
6. 获得计划批准
7. 建立风险管理文件
8. **验证：** 计划获得批准；可接受性标准明确；责任分配完成；文件建立

### 风险管理计划内容

| 部分 | 内容 | 证据 |
|---------|---------|----------|
| 范围 | 医疗器械及其生命周期的覆盖范围 | 范围声明 |
| 标准 | 风险可接受性矩阵 | 风险矩阵文档 |
| 责任 | 角色和权限 | RACI图表 |
| 验证 | 方法和接受标准 | 验证计划 |
| 生产/生产后 | 监控活动 | 监控计划 |

### 风险可接受性矩阵（5x5）

| 概率 \ 严重程度 | 可忽略 | 轻微 | 严重 | 关键 | 灾难性 |
|------------------------|------------|-------|---------|----------|--------------|
| **频繁（P5）** | 中等 | 高 | 高 | 不可接受 | 不可接受 |
| **可能（P4）** | 中等 | 中等 | 高 | 高 | 不可接受 |
| **偶尔（P3）** | 低 | 中等 | 中等 | 高 | 高 |
| **罕见（P2）** | 低 | 低 | 中等 | 中等 | 高 |
| **极不可能（P1）** | 低 | 低 | 低 | 中等 | 中等 |

### 风险等级应对措施

| 等级 | 可接受 | 需要采取的行动 |
|-------|------------|-----------------|
| 低 | 是 | 记录并接受 |
| 中等 | ALARP（尽可能降低风险） | 如果可行，记录理由 |
| 高 | ALARP | 需要降低风险；证明风险已降至可接受水平 |
| 不可接受 | 否 | 必须进行设计变更 |

---

## 风险分析工作流程

系统地识别危险并评估风险。

### 工作流程：进行风险分析

1. 明确预期用途和合理可预见的误用情况：
   - 医疗用途
   - 患者群体
   - 使用者群体
   - 使用环境
2. 选择分析方法：
   - 对组件/功能进行分析的FMEA
   - 对系统级别进行分析的FTA
   - 对过程偏差进行分析的HAZOP
   - 对用户交互进行分析的错误分析
3. 按类别识别危险：
   - 能源危险（电气、机械、热能）
   - 生物危险（生物负荷、生物相容性）
   - 化学危险（残留物、可浸出物）
   - 操作危险（软件、使用错误）
4. 确定危险情况：
   - 事件序列
   - 可预见的误用场景
   - 单个故障条件
5. 估计危害的概率（P1-P5）
6. 估计危害的严重程度（S1-S5）
7. 在危险分析工作表中记录
8. **验证：** 所有危险类别都得到处理；所有危险都记录在案；概率和严重程度已确定

### 危险类别检查表

| 类别 | 例子 | 是否已分析 |
|----------|----------|----------|
| 电气 | 电击、烧伤、干扰 | ☐ |
| 机械 | 压伤、切割、夹伤 | ☐ |
| 热能 | 烧伤、组织损伤 | ☐ |
| 辐射 | 电离辐射、非电离辐射 | ☐ |
| 生物 | 感染、生物相容性 | ☐ |
| 化学 | 毒性、刺激 | ☐ |
| 软件 | 输出错误、时间错误 | ☐ |
| 使用错误 | 误用、感知错误、认知错误 | ☐ |
| 环境 | 电磁干扰、机械应力 | ☐ |

### 分析方法选择

| 情况 | 推荐方法 |
|-----------|-------------------|
| 组件故障 | FMEA |
| 系统级故障 | FTA |
| 过程偏差 | HAZOP |
| 用户交互 | 使用错误分析 |
| 软件行为 | 软件FMEA |
| 早期设计阶段 | PHA（故障模式与影响分析） |

### 概率标准

| 等级 | 名称 | 描述 | 发生频率 |
|-------|------|-------------|-----------|
| P5 | 频繁 | 预计会发生 | >10⁻³ |
| P4 | 可能 | 很可能发生 | 10⁻³ 至 10⁻⁴ |
| P3 | 偶尔 | 可能发生 | 10⁻⁴ 至 10⁻⁵ |
| P2 | 罕见 | 不太可能 | 10⁻⁵ 至 10⁻⁶ |
| P1 | 极不可能 | 非常不可能 | <10⁻⁶ |

### 严重程度标准

| 等级 | 名称 | 描述 | 危害 |
|-------|------|-------------|------|
| S5 | 灾难性 | 导致死亡 | 死亡 |
| S4 | 严重 | 造成永久性损伤 | 不可逆的伤害 |
| S3 | 严重 | 需要干预的伤害 | 可逆的伤害 |
| S2 | 轻微 | 造成暂时不适 | 不需要治疗 |
| S1 | 可忽略 | 造成不便 | 无伤害 |

参见：[references/risk-analysis-methods.md](references/risk-analysis-methods.md)

---

## 风险评估工作流程

根据可接受性标准评估风险。

### 工作流程：评估已识别的风险

1. 通过概率 × 严重程度计算初始风险等级
2. 与风险可接受性标准进行比较
3. 对每个风险做出判断：
   - 可接受：记录并接受
   - ALARP（尽可能降低风险）：继续进行风险控制
   - 不可接受：必须进行风险控制
4. 记录评估理由
5. 识别需要进行收益-风险分析的风险
6. 如适用，完成收益-风险分析
7. 编制风险评估总结
8. **验证：** 所有风险均已评估；可接受性已确定；理由已记录

### 风险评估决策树

```
Risk Estimated
      │
      ▼
Apply Acceptability Criteria
      │
      ├── Low Risk ──────────► Accept and document
      │
      ├── Medium Risk ───────► Consider risk reduction
      │   │                    Document ALARP if not reduced
      │   ▼
      │   Practicable to reduce?
      │   │
      │   Yes──► Implement control
      │   No───► Document ALARP rationale
      │
      ├── High Risk ─────────► Risk reduction required
      │   │                    Must demonstrate ALARP
      │   ▼
      │   Implement control
      │   Verify residual risk
      │
      └── Unacceptable ──────► Design change mandatory
                               Cannot proceed without control
```

### ALARP（尽可能降低风险）证明要求

| 标准 | 需要的证据 |
|-----------|-------------------|
| 技术可行性 | 替代控制措施的分析 |
| 成本效益 | 进一步降低风险的成本效益 |
| 最新技术水平 | 与类似设备的比较 |
| 利益相关者的意见 | 临床/用户的观点 |

### 收益-风险分析触发条件

| 情况 | 需要进行收益-风险分析 |
|-----------|----------------------|
| 残余风险仍然很高 | 是 |
| 无法降低风险 | 是 |
| 新型设备 | 是 |
| 存在临床效益但风险不可接受 | 是 |
| 所有风险都很低 | 否 |

---

## 风险控制工作流程

实施并验证风险控制措施。

### 工作流程：实施风险控制

1. 识别风险控制选项：
   - 通过设计实现的内在安全性（优先级1）
   - 设备中的保护措施（优先级2）
   - 安全信息（优先级3）
2. 根据优先级选择最佳控制措施
3. 分析新引入的危险
4. 将控制措施记录在设计要求中
5. 在设计中实施控制措施
6. 制定验证协议
7. 执行验证并记录结果
8. 在实施控制措施后评估剩余风险
9. **验证：** 控制措施已实施；验证通过；剩余风险可接受；没有未解决的新危险

### 风险控制优先级

| 优先级 | 控制类型 | 例子 | 有效性 |
|----------|--------------|----------|---------------|
| 1 | 内在安全性 | 消除危险，采用故障安全设计 | 最高优先级 |
| 2 | 保护措施 | 保护装置、警报、自动关闭 | 高优先级 |
| 3 | 安全信息 | 警告、培训、使用说明书 | 低优先级 |

### 风险控制选项分析模板

```
RISK CONTROL OPTION ANALYSIS

Hazard ID: H-[XXX]
Hazard: [Description]
Initial Risk: P[X] × S[X] = [Level]

OPTIONS CONSIDERED:
| Option | Control Type | New Hazards | Feasibility | Selected |
|--------|--------------|-------------|-------------|----------|
| 1 | [Type] | [Yes/No] | [H/M/L] | [Yes/No] |
| 2 | [Type] | [Yes/No] | [H/M/L] | [Yes/No] |

SELECTED CONTROL: Option [X]
Rationale: [Justification for selection]

IMPLEMENTATION:
- Requirement: [REQ-XXX]
- Design Document: [Reference]

VERIFICATION:
- Method: [Test/Analysis/Review]
- Protocol: [Reference]
- Acceptance Criteria: [Criteria]
```

### 风险控制验证方法

| 方法 | 适用情况 | 需要的证据 |
|--------|-------------|----------|
| 测试 | 可量化的性能 | 测试报告 |
| 检查 | 实际存在 | 检查记录 |
| 分析 | 设计计算 | 分析报告 |
| 审查 | 文档检查 | 审查记录 |

### 剩余风险评估

| 控制措施实施后 | 接下来的行动 |
|---------------|--------|
| 可接受 | 记录并继续 |
| 达到ALARP | 记录理由并继续 |
| 仍然不可接受 | 需要额外的控制或设计变更 |
| 引入新的危险 | 分析并控制新的危险 |

---

## 生产后风险管理

在整个产品生命周期中监控和更新风险管理。

### 工作流程：生产后风险监控

1. 识别信息来源：
   - 客户投诉
   - 服务报告
   - 监控/不良事件
   | 文献监控
   - 临床研究
2. 建立信息收集程序
3. 定义审查触发条件：
   - 识别新的危险
   - 已知危险的频率增加
   | 严重事件
   | 监管机构的反馈
4. 分析收到的信息以确定风险相关性
5. 根据需要更新风险管理文件
6. 传达重要发现
7. 定期进行风险管理审查
8. **验证：** 监控信息来源；文件保持最新；按计划完成审查

### 信息来源

| 来源 | 信息类型 | 审查频率 |
|--------|------------------|------------------|
| 投诉 | 使用问题、故障 | 持续 |
| 服务 | 现场故障、维修 | 每月 |
| 监控 | 严重事件 | 立即 |
| 文献 | 类似设备的问题 | 每季度 |
| 监管机构 | 监管机构的反馈 | 收到后立即处理 |
| 临床 | PMCF数据 | 按计划 |

### 风险管理文件更新触发条件

| 触发条件 | 响应时间 | 行动 |
|---------|---------------|--------|
| 严重事件 | 立即 | 全面风险审查 |
| 识别新的危险 | 30天 | 更新风险分析 |
| 风险趋势增加 | 60天 | 趋势分析 |
| 设计变更 | 实施前 | 影响评估 |
| 标准更新 | 在过渡期间 | 差异分析 |

### 定期审查要求

| 审查内容 | 审查频率 |
|----------------|-----------|
| 风险管理文件的完整性 | 每年 |
| 风险控制的有效性 | 每年 |
| 市场后信息分析 | 每季度 |
| 收益-风险结论 | 每年或根据新数据 |

---

## 风险评估模板
→ 详情请参见 references/risk-assessment-templates.md

## 决策框架

### 风险控制选择

```
What is the risk level?
        │
        ├── Unacceptable ──► Can hazard be eliminated?
        │                    │
        │                Yes─┴─No
        │                 │     │
        │                 ▼     ▼
        │            Eliminate  Can protective
        │            hazard     measure reduce?
        │                           │
        │                       Yes─┴─No
        │                        │     │
        │                        ▼     ▼
        │                   Add       Add warning
        │                   protection + training
        │
        └── High/Medium ──► Apply hierarchy
                            starting at Level 1
```

### 新危险分析

| 问题 | 如果是 | 如果不是 |
|----------|--------|-------|
| 控制措施是否引入新的危险？ | 分析新的危险 | 继续 |
| 新风险是否高于原有风险？ | 拒绝该控制措施 | 可接受的权衡 |
| 新危险是否可以控制？ | 添加控制措施 | 拒绝该控制措施 |

### 风险可接受性决策

| 条件 | 决策 |
|-----------|----------|
| 所有风险都很低 | 可接受 |
| 中等风险且风险已降至可接受水平 | 可接受 |
| 高风险且风险已降低到可接受水平 | 如果收益大于风险，则可接受 |
| 任何不可接受的风险 | 不可接受 - 需要重新设计 |

---

## 工具和参考资料

### 脚本

| 工具 | 用途 | 使用方法 |
|------|---------|-------|
| [risk_matrix_calculator.py](scripts/risk_matrix_calculator.py) | 计算风险等级和FMEA RPN | `python risk_matrix_calculator.py --help` |

**风险矩阵计算器功能：**
- 计算ISO 14971 5x5风险矩阵
- 计算FMEA RPN（风险优先数）
- 提供交互式评估模式
- 显示风险标准定义
- 输出JSON格式以便集成

### 参考资料

| 文档 | 内容 |
|----------|---------|
| [iso14971-implementation-guide.md](references/iso14971-implementation-guide.md) | 完整的ISO 14971:2019实施指南及模板 |
| [risk-analysis-methods.md](references/risk-analysis-methods.md) | FMEA、FTA、HAZOP、使用错误分析方法 |

### ISO 14971流程快速参考

| 阶段 | 关键活动 | 输出 |
|-------|----------------|--------|
| 规划 | 明确范围、标准、责任 | 风险管理计划 |
| 分析 | 识别危险、评估风险 | 危险分析 |
| 评估 | 与标准比较、进行ALARP评估 | 风险评估 |
| 控制 | 实施控制措施、进行验证 | 风险控制记录 |
| 剩余风险 | 总体评估、收益-风险分析 | 风险管理报告 |
| 生产 | 监控、审查、更新 | 更新后的风险管理文件 |

---

## 相关技能

| 技能 | 相关领域 | 集成点 |
|-------|-------------------|
| [quality-manager-qms-iso13485](../quality-manager-qms-iso13485/) | 质量管理体系（QMS）集成 |
| [capa-officer](../capa-officer/) | 基于风险的CAPA（纠正措施计划） |
| [regulatory-affairs-head](../regulatory-affairs-head/) | 监管事务管理 |
| [quality-documentation-manager](../quality-documentation-manager/) | 风险文件管理 |