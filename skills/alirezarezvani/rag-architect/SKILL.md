---
name: "rag-architect"
description: "**RAG Architect - 强大功能**  
RAG Architect 是一款功能强大的工具，专为软件架构设计和开发人员设计。它提供了丰富的工具集，帮助用户高效地管理软件系统的架构、组件和依赖关系。以下是 RAG Architect 的一些主要特点和优势：  
1. **直观的可视化界面**：RAG Architect 采用直观的图形化界面，让用户能够轻松地查看和编辑软件系统的架构结构。用户可以通过拖拽和缩放操作来调整组件之间的关系，从而更直观地理解系统的整体设计。  
2. **强大的组件管理**：该工具支持用户添加、删除和修改软件组件，并支持多种组件类型（如类、模块、服务等）。用户还可以为组件设置属性和依赖关系，以便更好地管理系统的复杂性。  
3. **详细的依赖关系管理**：RAG Architect 能够自动检测组件之间的依赖关系，并以图表的形式展示出来。这有助于开发者快速了解系统的依赖结构，避免潜在的依赖问题。  
4. **代码生成支持**：RAG Architect 支持根据用户定义的规则自动生成代码片段，如类定义、方法实现等。这大大提高了开发效率，减少了手动编写代码的工作量。  
5. **版本控制集成**：RAG Architect 可以与版本控制系统（如 Git）集成，确保架构设计和代码的一致性。用户可以轻松地跟踪架构的变化历史，便于团队协作和版本控制。  
6. **可扩展性**：RAG Architect 具有良好的扩展性，用户可以根据实际需求自定义工具的功能和界面。此外，它还支持插件机制，允许开发者添加新的插件以满足特定的需求。  
7. **文档生成**：RAG Architect 支持自动生成软件架构的文档，包括类图、组件图等。这些文档可以用于项目文档、技术文档或培训材料，提高项目的可维护性和可读性。  
8. **团队协作**：RAG Architect 支持多人协作，支持实时共享和编辑架构信息。团队成员可以同时查看和修改架构，提高沟通效率。  
9. **开源项目**：RAG Architect 是一个开源项目，用户可以自由地使用和贡献代码。这意味着用户可以根据自己的需求对其进行定制和优化，以满足特定的项目需求。  
总之，RAG Architect 是一款功能强大、易于使用的软件架构设计工具，能够帮助开发者更高效地管理和开发软件系统。"
---
# RAG 架构师 - 强大功能

## 概述

RAG（Retrieval-Augmented Generation，检索增强生成）架构师技能提供了设计、实现和优化生产级 RAG 管道所需的全面工具和知识。该技能涵盖了从文档分块策略到评估框架的整个 RAG 生态系统，使您能够构建可扩展、高效且准确的检索系统。
...
#### 固定大小分块
- **基于字符的分块**：根据字符数量进行简单分割（例如，512、1024、2048 个字符）
- **基于令牌的分块**：根据令牌数量进行分割以符合模型限制
...
- **性能**：ivfflat 和 HNSW 索引，支持并行查询处理
- **适用场景**：已经使用 PostgreSQL 且需要符合 ACID 原则时
...
#### 重新排序
- **两阶段方法**：先进行初步检索，再进行重新排序
- **重新排序模型**：交叉编码器、专用重新排序Transformer
...
#### 一致性指标
- **定义**：生成答案与检索上下文的匹配程度
- **测量方法**：通过 NLI 模型检查答案和上下文之间的蕴含关系
- **阈值**：生产系统要求 >90%

#### 相关性指标
- **上下文相关性**：检索到的部分与查询的相关性
- **答案相关性**：答案对原始问题的回答程度
- **测量方法**：嵌入相似性、人工评估、LLM 作为评判标准
- **目标**：上下文相关性 >0.8，答案相关性 >0.85

#### 上下文精确度和召回率
- **Precision@K**：前 K 个相关结果中的百分比
