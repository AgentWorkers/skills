...
description: 此技能适用于以下场景：用户需要“设计系统架构”、“评估微服务与单体架构的优劣”、“创建架构图”、“分析系统依赖关系”、“选择合适的数据库”、“规划系统的可扩展性”、“做出技术决策”或“审查系统设计”时。该技能可用于记录架构决策（Architecture Decision Records, ADRs）、技术栈评估、系统设计审查、依赖关系分析，以及生成 Mermaid、PlantUML 或 ASCII 格式的架构图。
---
# 高级架构师

提供架构设计和分析工具，以帮助您做出明智的技术决策。
...
分析项目中的依赖关系，识别耦合问题、循环依赖和过时的包。

**解决的问题：**“我需要了解依赖关系树并找出潜在问题”
...
**步骤 2：评估扩展需求**
- 数据量 < 100 万条，单地区部署 → PostgreSQL 或 MySQL
- 数据量 100 万至 1000 万条，读操作为主 → 带有读副本的 PostgreSQL
- 数据量 > 1000 万条，全球分布 → CockroachDB、Spanner 或 DynamoDB
- 写操作吞吐量高（>10K/秒） → Cassandra 或 ScyllaDB
