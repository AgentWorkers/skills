...
name: "capa-officer"
description: >
  **CAPA系统管理（用于医疗器械质量管理体系）**  
  该系统涵盖了根本原因分析、纠正措施规划、有效性验证以及CAPA（Corrective Action Program）相关指标的跟踪与管理。适用于CAPA调查、5-Why分析（连续追问法）、鱼骨图（Fishbone Diagram）的绘制、根本原因的确定、纠正措施的跟踪执行、有效性验证，以及CAPA项目的持续优化。
triggers:
  - CAPA investigation
  - root cause analysis
  - 5 Why analysis
...
在质量管理体系中，纠正性和预防性措施（CAPA）的管理重点在于系统性的根本原因分析、行动的实施以及效果验证。
...
1. 用客观证据记录触发事件
2. 评估问题的严重性并确定是否需要启动CAPA
3. 组建具有相关专业知识的调查团队
4. 系统地收集数据和证据
5. 选择并应用适当的根本原因分析（RCA）方法
6. 识别根本原因并附上支持证据
7. 制定纠正和预防措施
8. **验证：** 根本原因能够解释所有症状；如果消除了根本原因，问题将不会再次发生
...
| 内部审计发现（轻微问题） | 建议启动 | 孤立的失误或部分实施 |
| 不符合要求（重复发生） | 是 | 同一类问题发生3次以上 |
...
适用于：具有线性因果关系的单一原因问题，或流程偏差有明确故障点的情况。
...
| 方法（流程） | 流程、工作指示 | 步骤不明确、缺少控制措施 |
| 材料 | 规格、供应商、存储条件 | 不符合规格、质量下降、污染 |
| 测量工具 | 校准、方法、解读 | 仪器误差、使用错误的方法 |
| 环境因素 | 温度、湿度、清洁度 | 环境异常 |
...
| 类型 | 目的 | 时间节点 | 示例 |
|------|---------|----------|---------|
| 遏制措施 | 阻止问题的立即影响 | 24-72小时 | 将受影响的产品隔离 |
| 纠正措施 | 解决具体问题 | 1-2周 | 重新制作或更换受影响的产品 |
| 预防措施 | 防止类似问题再次发生 | 60-120天 | 将解决方案扩展到类似流程 |
...
| 指标 | 目标 | 计算方法 |
|--------|--------|-------------|
| CAPA周期时间 | <60天（平均） | （关闭日期 - 开始日期）/ CAPA总数 |
| 过期率 | <10% | 过期的CAPA数量 / 总开放CAPA数量 |
| 首次验证的有效性 | >90% | 首次验证即有效的CAPA数量 / 总验证CAPA数量 |
| 问题复发率 | <5% | 问题再次发生的次数 / 总关闭的CAPA数量 |
...
计算并报告：
- 总结指标（开放中的CAPA数量、已关闭的CAPA数量、过期的CAPA数量、周期时间、有效性）
- 状态分布 |
- 严重程度和问题来源分析 |
- 按时间段划分的老化报告 |
- 过期的CAPA列表 |
...
| 子条款 | 要求 | 关键活动 |
|------------|-------------|----------------|
| 8.5.2 纠正措施 | 消除不符合要求的原因 | 不符合要求情况的审查、原因确定、行动评估、实施、效果验证 |
| 8.5.3 预防措施 | 消除潜在的不符合要求的情况 | 趋势分析、原因确定、行动评估、实施、效果验证 |

### FDA 21 CFR 820.100

要求的CAPA要素包括：
- 实施纠正和预防措施的程序 |
- 分析质量数据来源（投诉、不符合要求的情况、审计结果、服务记录） |
- 调查不符合要求的原因 |
- 确定必要的纠正和预防措施 |
- 验证措施的有效性，确保不会对设备产生负面影响 |
- 提交相关信息供管理层审查
...
| 子条款 | 要求 | 关键活动 |
|------------|-------------|----------------|
| 8.5.2 纠正措施 | 消除不符合要求的原因 | 审查不符合要求的情况、确定原因、评估行动、实施、验证效果 |
| 8.5.3 预防措施 | 消除潜在的不符合要求的情况 | 进行趋势分析、确定原因、评估行动、实施、验证效果 |
