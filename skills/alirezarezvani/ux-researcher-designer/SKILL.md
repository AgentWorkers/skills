---
name: "ux-researcher-designer"
description: 面向高级用户体验设计师/研究员的UX研究与设计工具包，包含数据驱动的用户角色创建、用户旅程映射、可用性测试框架以及研究结果整合功能。适用于用户研究、用户角色建模、用户旅程分析以及设计验证等工作。
---
# 用户体验研究员与设计师

根据研究数据生成用户角色模型（user personas），创建用户旅程图（journey maps），规划可用性测试（usability tests），并将研究结果整合为可操作的设计建议。

---

## 目录

- [相关术语](#trigger-terms)
- [工作流程](#workflows)
  - [工作流程1：生成用户角色模型](#workflow-1-generate-user-persona)
  - [工作流程2：创建用户旅程图](#workflow-2-create-journey-map)
  - [工作流程3：规划可用性测试](#workflow-3-plan-usability-test)
  - [工作流程4：整合研究结果](#workflow-4-synthesize-research)
- [工具参考](#tool-reference)
- [快速参考表](#quick-reference-tables)
- [知识库](#knowledge-base)

---

## 相关术语

在需要执行以下操作时，请使用此技能：
- “创建用户角色模型”
- “根据数据生成角色模型”
- “构建客户旅程图”
- “绘制用户旅程”
- “规划可用性测试”
- “设计可用性研究”
- “分析用户研究”
- “整合访谈结果”
- “识别用户痛点”
- “定义用户原型”
- “计算研究样本量”
- “创建同理心地图”
- “识别用户需求”

---

## 工作流程

### 工作流程1：生成用户角色模型

**情境：**您拥有用户数据（分析数据、调查结果、访谈记录），需要根据这些数据创建一个有依据的角色模型。

**步骤：**

1. **准备用户数据**
   所需格式（JSON）：
   ```json
   [
     {
       "user_id": "user_1",
       "age": 32,
       "usage_frequency": "daily",
       "features_used": ["dashboard", "reports", "export"],
       "primary_device": "desktop",
       "usage_context": "work",
       "tech_proficiency": 7,
       "pain_points": ["slow loading", "confusing UI"]
     }
   ]
   ```

2. **运行角色模型生成工具**
   ```bash
   # Human-readable output
   python scripts/persona_generator.py

   # JSON output for integration
   python scripts/persona_generator.py json
   ```

3. **审查生成的组件**
   | 组件 | 需要检查的内容 |
|-----------|---------------|
| 原型 | 是否与数据模式匹配？ |
| 人口统计信息 | 是否来源于实际数据？ |
| 目标 | 是否具体且可操作？ |
| 挫折点 | 是否包含出现频率？ |
| 设计影响 | 设计师能否据此采取行动？ |

4. **验证角色模型**
   - 向3-5位真实用户展示： “这个描述符合你的情况吗？”
   - 与支持工单进行交叉核对
   - 根据分析数据进行验证

5. **参考资料：**请参阅 `references/persona-methodology.md` 以了解验证标准

---

### 工作流程2：创建用户旅程图

**情境：**您需要为特定目标可视化用户的完整使用体验。

**步骤：**

1. **明确范围**
   | 元素 | 描述 |
|---------|-------------|
| 用户角色 | 适用的用户类型 |
| 目标 | 他们试图实现的目标 |
| 起点 | 触发旅程的起点 |
| 结点 | 成功的标准 |
| 时间框架 | 小时/天/周 |

2. **收集旅程数据**
   数据来源：
   - 用户访谈（询问“请跟我演示一下……”）
   - 会话记录
   - 分析数据（用户流失情况）
   - 支持工单

3. **绘制各阶段**
   典型的B2B SaaS应用阶段：
   ```
   Awareness → Evaluation → Onboarding → Adoption → Advocacy
   ```

4. **填写各阶段的细节**
   ```
   Stage: [Name]
   ├── Actions: What does user do?
   ├── Touchpoints: Where do they interact?
   ├── Emotions: How do they feel? (1-5)
   ├── Pain Points: What frustrates them?
   └── Opportunities: Where can we improve?
   ```

5. **识别改进机会**
   优先级评分 = 出现频率 × 严重程度 × 可解决程度

6. **参考资料：**请参阅 `references/journey-mapping-guide.md` 以获取模板

---

### 工作流程3：规划可用性测试

**情境：**您需要通过真实用户来验证设计。

**步骤：**

1. **明确研究问题**
   将模糊的目标转化为可测试的问题：
   | 模糊问题 | 可测试问题 |
|-------|----------|
| “使用起来容易吗？” | “用户能在3分钟内完成结账吗？” |
| “用户喜欢这个设计吗？” | “用户会选择设计A还是设计B？” |
| “这个设计合理吗？” | “用户能在没有提示的情况下找到设置吗？” |

2. **选择测试方法**
   | 方法 | 参与者 | 时间 | 适用场景 |
|--------|--------------|----------|----------|
| 有指导的远程测试 | 5-8人 | 45-60分钟 | 深入了解用户需求 |
| 无指导的远程测试 | 10-20人 | 15-20分钟 | 快速验证 |
| 随机测试 | 3-5人 | 5-10分钟 | 快速获取反馈 |

3. **设计测试任务**
   优秀的任务设计格式：
   ```
   SCENARIO: "Imagine you're planning a trip to Paris..."
   GOAL: "Book a hotel for 3 nights in your budget."
   SUCCESS: "You see the confirmation page."
   ```

   任务流程：热身 → 核心任务 → 辅助任务 → 边缘情况 → 自由探索

4. **定义评估指标**
   | 指标 | 目标 |
|--------|--------|
| 完成率 | >80% |
| 任务耗时 | <预期时间的2倍 |
| 错误率 | <15% |
| 满意度 | >4/5 |

5. **准备测试引导手册**
   - 思维导语
   - 中立引导语
   - 测试后的问题

6. **参考资料：**请参阅 `references/usability-testing-frameworks.md` 以获取完整指南

---

### 工作流程4：整合研究结果

**情境：**您拥有原始的研究数据（访谈记录、调查结果、观察记录），需要从中提取可操作的见解。

**步骤：**

1. **对数据进行编码**
   为每个数据点添加标签：
   - `[目标]` - 用户想要实现的目标 |
   - `[痛点]` - 用户遇到的困难 |
   - `[行为] | 用户的实际操作 |
   - `[情境] | 用户使用产品的时间和地点 |
   - `[引语] | 用户的原话 |

2. **归纳相似的模式**
   ```
   User A: Uses daily, advanced features, shortcuts
   User B: Uses daily, complex workflows, automation
   User C: Uses weekly, basic needs, occasional

   Cluster 1: A, B (Power Users)
   Cluster 2: C (Casual User)
   ```

3. **计算各用户群体的规模**
   | 组别 | 用户数量 | 比例 | 可实施性 |
|---------|-------|---|-----------|
| 高频用户 | 18人 | 36% | 主要用户群体 |
| 商业用户 | 15人 | 30% | 主要用户群体 |
| 休闲用户 | 12人 | 24% | 辅助用户群体 |

4. **提取关键发现**
   对于每个主题：
   - 发现内容 |
   - 支持证据（引语、数据） |
   - 出现频率（X/Y用户） |
   - 对业务的影响 |
   - 建议措施

5. **确定优先级**
   | 评估因素 | 评分（1-5分） |
|--------|-----------|
| 出现频率 | 这种情况发生的频率？ |
| 严重程度 | 造成的困扰程度？ |
| 影响范围 | 受影响的用户数量？ |
| 可解决程度 | 我们能解决这个问题吗？ |

6. **参考资料：**请参阅 `references/persona-methodology.md` 以了解分析框架

---

## 工具参考

### persona_generator.py

根据用户研究数据生成基于数据驱动的角色模型。

| 参数 | 默认值 | 描述 |
|----------|--------|---------|-------------|
| 格式 | （无） | （无） | 输出格式 |

**示例输出：**

```
============================================================
PERSONA: Alex the Power User
============================================================

📝 A daily user who primarily uses the product for work purposes

Archetype: Power User
Quote: "I need tools that can keep up with my workflow"

👤 Demographics:
  • Age Range: 25-34
  • Location Type: Urban
  • Tech Proficiency: Advanced

🎯 Goals & Needs:
  • Complete tasks efficiently
  • Automate workflows
  • Access advanced features

😤 Frustrations:
  • Slow loading times (14/20 users)
  • No keyboard shortcuts
  • Limited API access

💡 Design Implications:
  → Optimize for speed and efficiency
  → Provide keyboard shortcuts and power features
  → Expose API and automation capabilities

📈 Data: Based on 45 users
    Confidence: High
```

**生成的原型：**

| 原型 | 特征 | 设计重点 |
|-----------|---------|--------------|
| 高频用户 | 每日使用，使用10个以上功能 | 效率、定制化 |
| 休闲用户 | 每周使用3-5次，使用3-5个功能 | 简单性、引导 |
| 商业用户 | 在工作环境中使用，团队协作 | 协作、报告功能 |
| 以移动设备为主 | 主要使用移动设备 | 触控操作、离线使用、快速响应 |

**输出内容：**
| 组件 | 描述 |
|-----------|-------------|
| 人口统计信息 | 年龄范围、地理位置、职业、技术水平 |
| 心理特征 | 动机、价值观、态度、生活方式 |
| 行为特征 | 使用习惯、功能偏好 |
| 需求与目标 | 主要需求、次要需求、功能需求、情感需求 |
| 挫折点 | 遇到的困难及证据 |
| 使用场景 | 具体使用场景 |
| 设计建议 | 可操作的设计建议 |
| 数据点 | 样本数量、置信度 |

---

## 快速参考表

### 研究方法选择

| 问题类型 | 最适合的方法 | 样本数量 |
|---------------|-------------|-------------|
| “用户具体做了什么？” | 分析数据、观察法 | 100次以上事件 |
| “他们为什么这样做？” | 访谈法 | 8-15名用户 |
| “他们的完成情况如何？” | 可用性测试 | 5-8名用户 |
| “他们更喜欢什么？” | 调查问卷、A/B测试 | 50名以上用户 |
| “他们的感受如何？” | 日记记录法、访谈法 | 10-15名用户 |

### 角色模型的置信度

| 样本数量 | 置信度 | 适用场景 |
|-------------|------------|----------|
| 5-10名用户 | 低 | 探索性研究 |
| 11-30名用户 | 中等 | 定向研究 |
| 31名以上用户 | 高 | 生成可推广的结论 |

### 可用性问题严重程度

| 严重程度 | 定义 | 应对措施 |
|----------|------------|--------|
| 4 - 致命性 | 阻碍任务完成 | 立即修复 |
| 3 - 重大 | 造成显著困难 | 在发布前修复 |
| 2 - 轻微 | 造成犹豫 | 尽可能修复 |
| 1 - 仅是外观问题 | 注意到但无关紧要 | 优先级较低 |

### 访谈问题类型

| 问题类型 | 例子 | 适用场景 |
|------|---------|---------|
| 环境相关 | “请描述你的一天” | 了解使用环境 |
| 行为相关 | “请演示一下你是如何做某事的” | 观察用户的实际操作 |
| 目标相关 | “你试图实现什么？” | 了解用户的动机 |
| 挫折点相关 | “最困难的部分是什么？” | 识别用户遇到的困难 |
| 反思相关 | “你会做出哪些改变？” | 生成改进建议 |

---

## 知识库

详细参考指南请参见 `references/` 目录下的文件：

| 文件名 | 内容 |
|------|---------|
| `persona-methodology.md` | 角色模型的有效性标准、数据收集方法、分析框架 |
| `journey-mapping-guide.md | 旅程图绘制流程、模板、机会识别方法 |
| `example-personas.md | 3个带有数据支持的完整角色模型示例 |
| `usability-testing-frameworks.md | 测试规划、任务设计、数据分析方法 |

---

## 验证检查表

### 角色模型质量
- [ ] 基于20名以上用户的数据（最低要求）
- [ ] 至少使用两种数据来源（定量数据 + 定性数据）
- [ ] 目标具体且可操作 |
- [ ] 挫折点包含出现频率 |
- [ ] 设计建议具体明确 |
- [ ] 明确说明了置信度

### 用户旅程图质量
- [ ] 范围明确（用户角色、目标、时间框架）
- [ ] 基于真实用户数据，而非假设 |
- [ ] 所有环节都填写完整（动作、触点、情感体验）
- [ ] 每个阶段都识别了用户的痛点 |
- [ ] 优先级明确 |

### 可用性测试质量
- [ ] 研究问题具有可测试性 |
- [] 测试任务符合实际使用场景 |
- [] 每个设计任务都有5名以上参与者 |
- [ ] 明确了评估指标 |
- [ | 测试结果包含严重程度评级 |

### 研究结果整合质量
- [ ] 数据编码一致 |
- [ ] 模式基于多个数据点得出 |
- [ | 发现结果有依据 |
- [ | 建议措施具有可操作性 |
- [ | 优先级合理 |

---