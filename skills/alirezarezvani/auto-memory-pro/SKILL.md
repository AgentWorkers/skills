...
description: "将 Claude Code 自动记录的记忆内容整理为可长期使用的项目知识。分析 MEMORY.md 文件中的模式，将经过验证的学习成果整合到 CLAUDE.md 和 .claude/rules/ 文件中，并将重复出现的解决方案提取为可复用的技能。适用场景包括：(1) 查看 Claude 对您项目的学习成果；(2) 将某些模式从笔记提升为强制执行的规则；(3) 将调试解决方案转化为可复用的技能；(4) 检查内存的使用状况和容量。"
---
# 自我提升代理（Self-Improving Agent）

> 该插件负责自动捕获并整理 Claude Code 的学习内容。

Claude Code 的自动记忆功能（版本 2.1.32 及以上）会自动将项目中的模式、调试心得以及用户设置记录到 `MEMORY.md` 文件中。该插件进一步增强了这一功能：它分析 Claude 学到的内容，将经过验证的模式提升为项目规则，并将重复出现的解决方案提取为可复用的技能。
...
| `/si:promote` | 将某个模式从 `MEMORY.md` 提升到 `CLAUDE.md` 或 `.claude/rules/` 目录中 |
| `/si:extract` | 将经过验证的模式转换为独立的技能文件 |
| `/si:status` | 显示记忆系统的健康状况（包括文件行数、主题文件列表及建议） |
...
| `~/.claude/CLAUDE.md` | 用户 | 全局设置 | 每次会话都会加载完整文件 |
| `~/.claude/projects/<path>/memory/MEMORY.md` | Claude 自动生成 | 项目学习内容（仅加载前 200 行） |
...
### 自动记忆系统仅负责数据捕获，不负责内容筛选

自动记忆系统擅长记录 Claude 的学习内容，但它无法判断：
- 哪些学习内容是临时的，哪些是永久性的；
- 哪些模式应该被提升为项目规则；
- 当文件超过 200 行时，哪些内容属于过时的条目；
- 哪些解决方案足够成熟，可以成为可复用的技能。

而这正是该插件所负责的任务。

### 提升机制（Promotion Process）

当你将某个学习内容提升为规则时，它就会从 Claude 的临时存储区（`MEMORY.md`）转移到项目的规则系统中（`CLAUDE.md` 或 `.claude/rules/`）。两者之间的区别很重要：
- **`MEMORY.md`**：记录的是“我发现这个项目使用了 pnpm”（背景信息）；
- **`CLAUDE.md`**：则明确指示“使用 pnpm，而非 npm”（强制性的规则）。

被提升的规则具有更高的优先级，并且会完整加载（不会被截断为 200 行）。
...
该目录仅在 Claude 处理 API 测试文件时才会被加载；在其他情况下不会产生任何额外开销。

## 相关组件

### `memory-analyst`  
分析 `MEMORY.md` 文件和主题文件，以识别：
- 在多次会话中重复出现的条目（适合提升的内容）；
- 引用已删除文件或过时模式的条目；
- 需要整合的相关条目；
- `MEMORY.md` 与 `CLAUDE.md` 之间的差异。
...
- 参考文档；
- 示例代码及边缘情况；
- 可以通过 `plugin install` 命令安装或通过 ClawHub 发布。

## 各组件的交互方式

### `error-capture`（在 `PostToolUse` 后执行，通过 Bash 脚本）
...
**性能开销**：成功时没有额外开销；仅在检测到错误时需要约 30 个系统令牌。
...
- [pskoett/self-improving-agent](https://clawhub.ai/pskoett/self-improving-agent) — 灵感来源  
- [playwright-pro](../playwright-pro/) — 该仓库中的另一个相关插件