---
name: status
description: "内存健康仪表板显示行数、主题文件数量、存储容量、过时条目以及相关建议。"
command: /si:status
---
# /si:status — 内存健康状况仪表盘

本工具可快速查看您项目中所有内存系统的内存使用情况。
...
- **黄色（60-90%）**：内存即将满载。建议运行 `/si:review` 命令来清理或优化内存使用。
- **红色（> 90%）**：内存接近满载状态。自动内存管理功能可能会开始删除旧条目。请立即运行 `/si:review`。

## 使用提示：
- 可随时使用 `/si:status --brief` 进行快速检查。
- 如果内存使用率为黄色或更高，请运行 `/si:review` 以确定需要清理的文件。
