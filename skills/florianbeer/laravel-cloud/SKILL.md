...
请在 **cloud.laravel.com → 设置 → API Tokens** 页面生成您的 API 令牌。
...
|---------------|-------------------|
| `apps`         | 列出、获取、创建、更新、删除       |
| `envs`         | 列出、获取、创建、更新、删除、启动、停止、查看指标、日志记录、添加变量、替换变量 |
| `commands`      | 列出、获取、运行命令           |
| `deployments`     | 列出、获取、启动部署          |
| `domains`       | 列出、获取、创建、更新、删除、验证域名     |
| `instances`      | 列出、获取实例信息、调整实例大小、创建、更新、删除   |
| `bg-processes`    | 列出、获取、创建、更新、删除后台进程     |
| `databases`     | 管理数据库集群（包括创建、更新、删除等操作）  |
| `caches`       | 管理缓存（包括列表、获取、创建、更新、删除等操作）  |
| `buckets`       | 管理存储桶（包括列表、获取、创建、删除等操作）  |
| `bucket-keys`     | 管理桶键（包括列表、获取、创建、更新、删除等操作）  |
| `websockets`     | 管理 WebSocket 服务（包括列表、获取、创建、更新、删除等操作） |
| `ws-apps`      | 管理 WebSocket 应用程序（包括列表、获取、创建、更新、删除等操作） |
| `ips`          | 获取 IP 地址列表           |
| `org`          | 获取组织信息           |
| `regions`       | 列出可用区域           |
