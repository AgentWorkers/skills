...
name: agent-passport
version: 2.1.5
description: "**面向代理时代的OAuth机制**——为所有敏感代理操作（包括购买、发送邮件、文件操作、系统命令以及API调用）实施基于用户同意的权限控制。该机制支持设置消费限额、速率限制、允许访问的列表、操作的有效期（TTL），同时提供审计追踪功能以及用户身份验证（KYA，即“了解你的代理”）相关的元数据。"
...
> 运行 `./mandate-ledger.sh templates` 可查看所有选项。

---

## ⚡ 代理行为规范

**如果你是正在阅读此规范的代理，请遵守以下规则。**
...
**无需检查的情况**：读取文件、进行网络搜索、内部逻辑处理或任何没有副作用的操作。
...
- 对于类似的请求，可以批量处理（例如：“我需要执行多个 git 命令——需要创建一个 `dev-tools` 指令吗？”）
- 操作完成后记录所有操作
- 在相关情况下提醒用户剩余的权限使用限制（例如：“每天最多可以使用3封电子邮件”）

### 首次设置

如果 `check-action` 返回 `"hint": "templates"`，则表示权限账本为空。此时需要指导用户完成设置：
...
- 对于财务操作：记录花费的金额
- 对于其他操作：记录为1
- 描述应便于人类阅读（例如：“已向 bob@company.com 发送关于 Q1 报告的电子邮件”）

### 杀死开关机制

如果用户激活了杀死开关，所有操作将被冻结，直到解除冻结。
...
Agent Passport 为代理提供了自主性控制机制。用户不是授予全权或无权的权限，而是根据具体需求授予 **带有约束条件的操作指令**：
...
| 类别 | 示例 | 典型约束条件 |
|----------|----------|---------------------|
| `financial` | 购买、转账、订阅 | 花费限额、允许的交易商列表 |
| `communication` | 发送电子邮件、消息、推文 | 收件人列表、发送频率限制 |
| `data` | 删除文件、编辑文档、写入数据库 | 文件路径限制、必须备份 |
...
- **预览模式**：不进行数据存储，不使用网络连接。仅生成验证后的数据包和 curl 模板。
- **实时模式（未来计划）**：未来将连接到 Agent Bridge 后端以实现多代理同步和合规性管理。目前此功能尚未实现——当前版本完全为离线模式。
...
随时可以导出本地权限账本：`./mandate-ledger.sh export > backup.json`
