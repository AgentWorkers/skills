...
name: agent-passport
version: 2.1.0
description: "面向“代理时代”的OAuth机制：为所有敏感的代理操作（包括购买、发送邮件、文件操作、系统命令以及API调用）实施基于用户同意的权限控制。该机制支持设置消费上限、速率限制、允许操作列表、操作有效期（TTL）、审计追踪功能，并提供“了解你的代理”（KYA，Know Your Agent）相关的元数据。"
...
**无需检查的情况**：读取文件、网络搜索、内部逻辑处理或没有任何副作用的操作。
...
- 对于相似的请求，可以批量处理（例如：“我需要运行多个 git 命令——需要一个 `dev-tools` 指令吗？”）
- 操作完成后记录每一项操作
- 在相关情况下提醒用户剩余的权限使用限制（例如：“每天已使用3封电子邮件”）
...
- 不得尝试执行敏感操作
- 不得循环调用 `check-action`
- 通知用户操作已被阻止，并要求用户明确解除锁定。

---

## 概述

Agent Passport 为代理提供了自主性的权限控制机制。用户不是授予全权或无权的权限，而是根据具体需求授予**带有约束条件的操作指令**：
...
| `external_api` | 第三方 API 调用 | 允许使用的服务列表、发送频率限制 |
...
| 模式 | 匹配条件 | 示例 |
|---------|---------|---------|
| `prefix *` | 以该前缀开头的所有内容 | `git *` → `git pull`、`git status` |
| `*.suffix` | 以该后缀结尾的所有内容 | `*.env` → `config.env`、`.env` |
| `*middle*` | 包含该中间部分的任何内容 | `*/.git/*` → `repo/.git/config` |
| `*@domain` | 与指定域名匹配的电子邮件 | `*@company.com` → `bob@company.com` |
| `exact` | 完全匹配 | `api.github.com` |

## 模式

- **本地模式**（默认）：完全离线操作。操作指令存储在 `~/.openclaw/agent-passport/` 中。
- **预览模式**：不进行数据存储，也不使用网络。仅生成验证后的数据包和 curl 模板。
- **实时模式（未来计划）**：未来将连接到 Agent Bridge 后端以实现多代理同步和合规性检查。当前版本仍为完全离线模式。
...
> **注意：** 此功能完全基于本地操作，不进行任何网络请求。Agent Bridge 是未来的计划功能——当前版本不包含相关网络代码，也不需要 API 密钥。

本地模式适用于单用户、单代理的场景。未来的 Agent Bridge 服务将提供以下功能：
- **多代理协调**：防止操作指令冲突
- **跨设备同步**：确保所有代理使用相同的规定
- **组织政策**：提供 IT 约束和用户自定义选项
- **合规性报告**：生成符合监管要求的审计报告
- **商家/服务注册**：验证商家和服务提供商的信誉评分
...
所有数据均存储在本地 `~/.openclaw/agent-passport/` 目录下：
