...
通过MrScraper API，您可以执行基于人工智能的、无法被屏蔽的网络爬取和数据提取操作，这些操作支持自然语言指令。

## 功能支持

- 使用屏蔽器（隐秘浏览器 + IP轮换）打开被屏蔽的页面
- 根据自然语言指令启动AI爬虫
- 重新运行一个或多个URL的现有爬虫配置
- 执行基于手动工作流程的重新爬取
- 获取分页结果以及按ID获取详细结果
...
- 屏蔽器API：`https://api.mrscraper.com`
...
5. 点击**Create**。
6. 复制新令牌，并将其安全地保存为`MRSCRAPER_API_TOKEN`。
...
- 该API密钥适用于所有V3平台端点。
- 同一密钥也可用于`sync.scraper.mrscraper.com`上的端点。
...
通过隐秘浏览和IP轮换打开目标URL，然后返回HTML。当直接访问因验证码或反爬虫保护而被阻止时，可以使用此方法。

#### 查询参数：

| 字段            | 类型      | 是否必填 | 默认值 | 描述                             |
| ---------------- | --------- | -------- | ------- | --------------------------------------- |
| `token`          | `string`  | 是      | —       | 屏蔽器令牌 (`MRSCRAPER_API_TOKEN`) |
| `url`            | `string`  | 是      | —       | URL编码的目标URL                  |
| `timeout`        | `number`  | 否       | 60      | 最大等待时间（例如 `120` 秒）     |
| `geoCode`        | `string`  | 否       | 无    | 地理路由代码（例如 `SG`）  |
| `blockResources` | `boolean` | 否       | false   | 阻止非必要资源的访问           |
...
- 为了获得可重复的结果，请使用明确的`geoCode`和合适的超时设置。
- 仅在需要会话特定内容时传递cookies。
...
#### 请求体参数（对于`agent`：`general`或`agent`：`listing`）：
...
| ----------------- | -------- | -------- | --------- | ------------------------------------------------------------------------------------------------------------- |
| `url`             | `string` | 是      | —         | 目标URL                                           |
| `agent`           | `string` | 否       | map       | 用于爬取的AI代理类型（在此情况下为`map`）                                             |
| `maxDepth`        | `number` | 否       | 2         | 从起始URL开始爬取链接的最大深度。<br>0 = 仅起始URL，1 = 包括直接链接 |
| `maxPages`        | `number` | 否       | 50        | 爬取过程中最多抓取的页面数。                                                |
...
- 请正确选择代理类型，因为每种代理类型都适用于特定的使用场景。对于大多数标准的网络爬取任务，使用`general`代理。如果用户未指定代理类型，或者连接的LLM无法确定页面类型，也可以使用`general`代理。`map`代理通常用于爬取产品页面，但也能很好地处理任何类型的页面。如果用户确定给定的URL是列表页面，可以选择`listing`代理；如果用户指定URL是一个网站而不是特定页面，则选择`map`代理。对于`map`代理，可以使用特殊参数来配置爬取过程：
- `maxDepth`（较低值1-2适用于针对性爬取，推荐最大值为3），
- `maxPages`（限制总爬取的页面数），
- `limit`（限制提取的总记录数），
- `includePatterns`/`excludePatterns`（用`||`分隔的正则表达式模式，用于指定要爬取或跳过的URL，例如`*/products/*||*/blog/*`或`*/cart/*||*.pdf`）。
- 如果`includePatterns`为空字符串，则包含所有URL；如果`excludePatterns`为空字符串，则不排除任何URL。
...
| 字段        | 类型     | 是否必填 | 默认值 | 描述                                  |
| ------------ | -------- | -------- | ------- | -------------------------------------------- |
| `scraperId`  | `string` | 是      | —       | 从创建的AI爬虫中获取的爬虫ID |
| `url`        | `string` | 是      | —       | 目标URL                                   |
...
- 方法：`POST`
- 主机：`https://api.app.mrscraper.com`
- 路径：`/api/v1/scrapers-ai-run/bulk`
- 认证：`x-api-token`
...
| 字段       | 类型            | 是否必填 | 默认值 | 描述                       |
| ----------- | --------------- | -------- | ------- | --------------------------------- |
| `scraperId` | `string`        | 是      | —       | 现有爬虫配置ID |
| `urls`      | `array[string]` | 是      | —       | 要运行的目标URL                |
...
1. 打开`MrScraper`控制台并进入`Scraper`。
2. 点击`New Manual Scraper +`。
3. 输入目标URL。
4. 添加与您的网站行为匹配的工作流程步骤（例如`Input`、`Click`、`Delay`、`Extract`、`Inject JavaScript`）。
5. 如有需要，配置分页（使用`QueryPagination`、`DirectoryPagination`或`Next Page Link`等选项）。
6. 测试并保存爬虫，然后复制其`scraperId`以在API中重新运行。
...
| `workflow`   | `array<object>`| 否       | 无    | 允许覆盖保存的工作流程步骤。默认使用创建时保存的工作流程。|
...
| 字段             | 类型   | 是否必填 | 默认值     | 描述               |
| -------------------|--------|----------|-------------|---------------------------|
| `sortField`       | string | 是      | `updatedAt` | 排序列               |
| `sortOrder`       | string | 是      | `DESC`      | 排序方向            |
| `page`            | number | 是      | 1           | 页码               |
| `pageSize`        | number | 是      | 10          | 每页显示的项数            |
| `search`          | string | 否       | 无        | 搜索关键词            |
| `dateRangeColumn` | string | 否       | `createdAt` | 过滤日期字段      |
| `startAt`         | string | 否       | 无        | 日期范围开始（ISO格式）    |
| `endAt`           | string | 否       | 日期范围结束（ISO格式）      |
