...
通过MrScraper API，利用人工智能技术进行无法被屏蔽的网页抓取和数据提取，支持自然语言指令。
...
API令牌允许您的应用程序安全地与MrScraper API进行交互，并重新运行在控制台中创建的抓取任务。
...
通过隐身浏览和IP轮换打开目标URL，然后返回HTML。当直接访问因验证码或反机器人保护而被阻止时，可以使用此方法。
...
- 为了确保行为的可重复性，请使用明确的`geoCode`和合适的超时设置。
...
| -------------- | -------- | -------- | -------- | ---------------------------------------------------------- |
| `url`          | string   | 是      | —        | 目标URL                                      |
| `message`      | string   | 是      | —        | 抓取指令                                      |
| `agent`        | string   | 否       | general  | 使用的AI代理类型：`general`、`listing` 或 `map`           |
| `proxyCountry` | string   | 否       | 无        | 用于基于代理抓取的ISO国家代码                         |
...
| ----------------- | -------- | -------- | --------- | ----------------------------------------------------------------------------- |
| `url`             | string   | 是      | —         | 目标URL                                      |
| `agent`           | string   | 否       | map       | 用于抓取的AI代理类型（本例中为`map`）                         |
| `maxDepth`        | number | 否       | 2         | 从起始URL开始爬取链接的最大深度。<br>0 = 仅起始URL，1 = 包含直接链接         |
| `maxPages`        | number | 否       | 50        | 爬取过程中允许抓取的最大页面数                          |
| `limit`           | number | 否       | 1000      | 每页提取的最大数据记录数。达到此限制时停止抓取                   |
...
- 请正确选择代理类型，因为每种代理类型适用于特定的使用场景。对于大多数标准网页抓取任务，使用`general`代理。如果用户未指定代理类型，或者连接的LLM无法确定页面类型，也可以使用`general`代理。`agent`类型`map`适用于抓取产品列表页面等列表页面；如果用户确认给定URL是列表页面，也可以使用`agent`类型`map`。对于`agent`类型`map`，可以使用特殊参数来配置抓取过程：
- `maxDepth`（较低值1-2适用于针对性抓取，推荐最大值3），
- `maxPages`（限制总抓取页面数），
- `limit`（限制提取的总记录数），
- `includePatterns`/`excludePatterns`（用`||`分隔的正则表达式模式，用于指定要抓取或跳过的URL）。
...
| 参数        | 类型     | 是否必填 | 默认值 | 描述                                      |
| ------------ | -------- | -------- | ------- | -------------------------------------------- |
| `scraperId`  | `string` | 是      | —       | 从创建的AI抓取器中获取的ID                         |
| `url`        | `string` | 是      | 目标URL                                      |
...
在调用手动重新运行端点之前，您需要从控制台中创建并保存一个手动抓取器。请按照以下步骤操作：

1. 打开`MrScraper`控制台，然后进入`Scraper`。
2. 点击`New Manual Scraper +`。
3. 输入目标URL。
4. 添加与网站行为匹配的工作流程步骤（例如`Input`、`Click`、`Delay`、`Extract`、`Inject JavaScript`）。
5. 如有需要，配置分页（使用`QueryPagination`、`DirectoryPagination`或`Next Page Link`等选项）。
6. 测试并保存抓取器，然后复制其`scraperId`以在API中重新运行。

#### 请求体参数：

| 参数        | 类型           | 是否必填 | 默认值 | 描述                                                                                                      |
|--------------|----------------|----------|---------|------------------------------------------------------------------------------------------------------------------|
| `scraperId`  | `string`       | 是      | —       | 要重新运行的手动抓取器的ID                         |
