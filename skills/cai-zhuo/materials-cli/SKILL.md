...
当用户需要将 JSON 模式渲染为图像（PNG/JPG 格式）、验证渲染数据模式，或根据自然语言提示生成模式并对其进行渲染时，可以使用此工具。
...
- 用户请求将模式渲染为图像、将 JSON 数据转换为图片，或根据模式生成图像。
- 用户希望根据描述生成图像，或根据提示创建模式并对其进行渲染。
- 用户需要验证 JSON 文件是否符合渲染数据模式的要求。

## 使用方法

通过 Node.js 运行（在项目目录中直接运行，或先使用 `npm install -g materials-cli` 安装后使用）：
...
- 选项：`-o, --output`（指定输出图像文件路径），`-f, --format`（指定输出图像格式），`-w, --width`（指定图像宽度），`-h, --height`（指定图像高度），`--output-schema`（指定 JSON 模式文件路径），`--model`（指定使用的 AI 模型，默认为 OpenAI），`--api-key`（指定 OpenAI API 密钥），`--base-url`（指定 OpenAI API 基本地址），`--i, --interactive`（交互式模式）。
- 如果未通过参数传递这些选项，系统将使用默认的 `OPENAI_API_KEY`、`OPENAI_MODEL` 和 `OPENAI_BASE_URL`。
