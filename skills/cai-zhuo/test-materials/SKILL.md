...
- 用户请求将模式渲染为图像、将 JSON 数据转换为图片，或根据描述生成图像。
- 用户希望根据提示生成图像或创建模式并对其进行渲染。
...
- 选项：`-s, --schema <path>`（指定 JSON 模式文件路径），`-o, --output <path>`（指定输出图像文件路径，默认为 `./output.png`），`-f, --format <png|jpg>`（指定输出图像格式），`-w, --width`（指定图像宽度），`-h, --height`（指定图像高度），`--output-schema <path>`（指定输出 JSON 模式文件路径），`-i, --interactive`（交互式模式）。

### Generate（生成）

- `materials generate "A red circle with text Hello" -o out.png`
- 选项：`-o, --output`（指定输出图像文件路径），`-f, --format`（指定输出图像格式），`-w, --width`（指定图像宽度），`-h, --height`（指定图像高度），`--output-schema`（指定输入 JSON 模式文件路径），`--model`（指定使用的 AI 模型，默认为 OpenAI），`--api-key`（指定 OpenAI API 密钥），`--base-url`（指定 OpenAI API 基础 URL，默认为空），`--interactive`（交互式模式）。
- 如果未通过参数传递 `OPENAI_API_KEY`，则会使用系统默认的 API 密钥。
...
JSON 模式遵循 `declare-render` 格式：根节点包含 `id`、`width`、`height` 和 `layers`。层类型包括 `text`（文本）、`image`（图像）、`container`（容器）等。在渲染之前，可以使用 `materials validate <file>` 命令检查 JSON 模式的有效性。