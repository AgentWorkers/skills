---
name: activecampaign
description: ActiveCampaign CRM集成用于客户线索管理、交易跟踪以及电子邮件自动化。该集成支持同步演示线索、管理诊所的销售流程，并触发后续沟通环节。
metadata: {"clawdbot":{"emoji":"📧","requires":{"bins":["activecampaign"],"env":["ACTIVECAMPAIGN_URL","ACTIVECAMPAIGN_API_KEY"]},"primaryEnv":["ACTIVECAMPAIGN_URL","ACTIVECAMPAIGN_API_KEY"]}}
---

# ActiveCampaign 技能 📧  
ActiveCampaign 集成用于 CRM 自动化和销售管道管理。  

## 目的  
该技能用于管理销售相关的潜在客户、交易以及电子邮件自动化流程：  
- **联系人**：同步演示会参与者、潜在客户和潜在销售对象；  
- **交易**：跟踪销售管道的各个阶段；  
- **标签**：对潜在客户进行分类（如“请求演示”、“需要培养”、“准备成交”）；  
- **自动化**：根据用户操作触发电子邮件序列；  
- **自定义字段**：映射订单、运输、账单和订阅数据。  

## 设置  

### 1. 凭据  
从 ActiveCampaign 获取 API 凭据：  
- **网址**：设置 → 开发者 → API 访问  
- **API 密钥**：设置 → 开发者 → API 访问  

### 2. 自定义字段配置（可选）  
该技能支持对订单、运输、账单和订阅数据的自定义字段映射。  
（配置文件为 **gitignored**，无需提交到代码仓库。）  

## 使用方法  
（使用方法的具体步骤在此处说明。）  

## 自定义字段配置  
该技能提供了全面的字段配置系统，包括以下类别：  
| 类别 | 字段            |  
|----------|----------------|  
| **订单** | 订单 ID、编号、日期、总价、税费、状态、小计、折扣、货币、支付详情 |  
| **运输** | 名称、地址 1/2、城市、州、邮政编码、国家、运输方式、费用 |  
| **账单** | 地址 1/2、城市、州、邮政编码、国家 |  
| **订阅** | 订阅 ID、状态、订阅计划、金额、货币、订阅周期、开始时间、试用期结束时间 |  
| **其他** | 公司名称、产品信息、潜在客户活动、备注、生日等 |  

### 设置字段值  
（具体字段值的设置方法在此处说明。）  

## 速率限制  
- 每秒最多 5 次请求；  
- 该工具会自动处理速率限制。  

## 相关技能  
- `shapescale-crm`：Attio CRM 集成  
- `shapescale-sales`：销售工作流程和客户资格评估  
- `campaign-orchestrator`：多渠道跟进活动