---
name: openbroker
description: Hyperliquid交易插件具备背景头寸监控功能：可执行市场订单和限价订单，管理头寸，查看资金费率，并运行交易策略。该插件还提供自动警报功能，用于通知盈亏变化和清算风险。
license: MIT
...
这是一个用于在Hyperliquid去中心化交易所（DEX）执行交易操作的命令行工具，支持交易手续费（builder fee）的自动计算和支付。
...
`setup`命令提供了三种模式：
1. **生成新钱包**（推荐给代理）：创建一个专用的交易钱包，交易手续费会自动被批准。无需浏览器操作，只需用USDC充值即可开始交易。
2. **导入现有密钥**：使用您已有的私钥。
3. **生成API钱包**：创建一个只能交易但不能取款的受限钱包，需要从主钱包（master wallet）获得浏览器授权。

对于第1和第2种模式，`setup`命令会自动保存配置并批准交易手续费。
关于第3种模式（API钱包），请参阅下面的“API钱包设置”部分。

### 新钱包设置（推荐给代理）

这是最适合代理的设置方式。系统会生成一个新钱包，交易手续费会自动被批准，代理在充值后即可立即开始交易。

**操作流程：**
1. 运行`openbroker setup`并选择选项1（“生成新钱包”）。
2. 命令行工具会生成钱包，保存配置并自动批准交易手续费。
3. 在Arbitrum平台上用USDC充值钱包，然后登录https://app.hyperliquid.xyz/进行交易。

### API钱包设置（备用方案）

API钱包可以代表主账户进行交易，但**不能取款**。如果您希望将资金保留在现有钱包中，并仅委托交易权限，可以选择此模式。

**操作流程：**
1. 运行`openbroker setup`并选择选项3（“生成API钱包”）。
2. 命令行工具会生成一对密钥，并打印出一个授权URL（例如：`https://openbroker.dev/approve?agent=0xABC...`）。
3. 代理所有者需要在浏览器中打开该URL，并连接他们的主钱包（如MetaMask）。
4. 主钱包需要执行两个交易：`ApproveAgent`（授权API钱包）和`ApproveBuilderFee`（批准1 bps的交易手续费）。
5. 命令行工具会自动检测到授权结果并保存配置。
...
**重要提示（针对代理）：** 使用API钱包时，需将授权URL提供给代理所有者（即控制主钱包的人）。代理在交易前必须获得授权。命令行工具会等待最多10分钟以获取授权。如果超时，请重新运行`openbroker setup`。
...
所有交易命令都支持使用`dex:COIN`语法来操作HIP-3资产：
```bash
...
| 设定获利点（价格达到目标时退出） | `openbroker trigger --type tp` |
...
| `--trigger` | 触发价格（针对触发订单） |
| `--type` | 触发类型：`tp` 或 `sl` |
| `--slippage` | 订单滑点（以bps为单位） |
| `--tif` | 订单有效期：GTC, IOC, ALO |
| `--reduce` | 仅用于减少订单数量的命令 |
...
|--------|---------|-------------|
| 绝对值 | `--tp 40` | 40美元的价格 |
| 相对于入场价的百分比 | `--tp +10%` | 入场价上浮10% |
| 相对于入场价的百分比 | `--sl -5%` | 入场价下浮5% |
| 入场价 | `--sl entry` | 破盘止损 |
...
交易手续费（1 bps / 0.01%）是硬编码的，无法更改。

## OpenClaw插件（可选）

该工具可以通过Bash独立使用——上述所有命令都可以通过`openbroker` CLI执行。为了获得更多功能，`openbroker` npm包还提供了一个**OpenClaw插件**，您可以将其与本工具一起使用。

### 插件功能：

- **结构化的代理工具**（`ob_account`, `ob_buy`, `ob_limit`等）：使用结构化的数据输入格式进行命令调用，而非简单的Bash字符串。代理会收到结构化的JSON响应。
- **后台持仓监控**：每30秒检查一次您的Hyperliquid账户，并在持仓开仓/平仓、盈亏显著变化或保证金使用率达到危险水平时发送Webhook警报。
...
插件会从`~/.openbroker/.env`文件中读取钱包凭据（由`openbroker setup`设置），因此除非您需要覆盖配置，否则无需在插件中重复输入`privateKey`。

### 设置Webhook以接收警报

为了使代理能够收到持仓警报，请在您的网关配置中启用相应的钩子：
...
即使没有启用钩子，监控功能仍然会运行并跟踪持仓状态（可通过`ob_watcher_status`访问），但无法主动通知代理。

### 是否使用插件

- **仅使用命令行工具（不使用插件）**：直接使用Bash命令（例如：`openbroker buy --coin ETH --size 0.1`）。此时没有后台监控功能。
- **使用插件**：如果可用，代理会优先使用`ob_*`工具（因为它们提供结构化数据）；对于未涵盖的功能（如交易策略或批量操作），则使用Bash命令。后台监控功能会自动发送警报。

## 风险提示

- 使用前请务必先使用`--dry`参数进行预览操作。
