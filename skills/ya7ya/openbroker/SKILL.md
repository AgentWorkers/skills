---
name: openbroker
description: Hyperliquid交易插件具备后台持仓监控功能：可执行市场订单和限价订单，管理持仓情况，查看资金费率，并运行交易策略。同时提供自动警报功能，用于通知盈亏变化和清算风险。
license: MIT
...
---
# Open Broker - Hyperliquid 交易 CLI

通过此 CLI，您可以在 Hyperliquid DEX 上执行交易操作，并支持构建者费用（builder fee）的收取。
...
2. **创建新钱包** — 创建一个新的主钱包
3. **创建 API 钱包**（推荐给代理） — 创建一个只能交易但不能取款的受限钱包

对于选项 1 和 2，设置会自动保存配置并批准构建者费用。
对于选项 3（API 钱包），请参阅下面的 API 钱包设置部分。

### API 钱包设置（推荐给代理）

API 钱包可以代表主账户进行交易，但**不能取款**。这是自动化代理最安全的选择。

**操作流程：**
1. 运行 `openbroker setup` 并选择选项 3（“创建 API 钱包”）
2. CLI 会生成一对密钥并对一个批准 URL 进行打印（例如：`https://openbroker.dev/approve?agent=0xABC...`）
3. 代理所有者需要在浏览器中打开该 URL 并连接他们的主钱包（如 MetaMask 等）
4. 主钱包需要签署两个交易：`ApproveAgent`（授权 API 钱包）和 `ApproveBuilderFee`（批准 1 bps 的费用）
5. CLI 会自动检测到批准并保存配置
...
**重要提示（针对代理）：** 使用 API 钱包时，需要将批准 URL 传递给代理所有者（控制主钱包的人）。代理在交易前必须获得所有者的批准。CLI 会等待最多 10 分钟以获取批准。如果超时，请重新运行 `openbroker setup`。
...
| 情况 | 命令 |
|----------|---------|
| 在市场价下方以特定价格买入 | `openbroker limit` |
| 在市场价上方以特定价格卖出 | `openbroker limit` |
| 设置止损（价格下跌时退出） | `openbroker trigger --type sl` |
| 设定获利目标（价格达到目标时退出） | `openbroker trigger --type tp` |
| 为现有头寸添加止盈/止损 | `openbroker tpsl` |
...
|--------|---------|-------------|
| 绝对值 | `--tp 40` | 40 美元的价格 |
| 相对值（上涨） | `--tp +10%` | 达到入场价的 10% 以上 |
| 相对值（下跌） | `--sl -5%` | 达到入场价的 5% 以下 |
| 入场价格 | `--sl entry` | 平衡止损 |
...
| 参数 | 是否必填 | 描述 |
|----------|----------|-------------|
| `HYPERLIQUID_PRIVATE_KEY` | 是 | 钱包私钥（格式为 0x...） |
| `HYPERLIQUID_NETWORK` | 否 | `mainnet`（默认）或 `testnet` |
| `HYPERLIQUID_ACCOUNT_ADDRESS` | 否 | 主账户地址（API 钱包所需） |

构建者费用（1 bps / 0.01%）是硬编码的，不可配置。

## OpenClaw 插件（可选）

此工具可以通过 Bash 单独使用——上述所有命令都可以通过 `openbroker` CLI 来执行。为了获得更多功能，`openbroker` npm 包还提供了一个 **OpenClaw 插件**，您可以将其与本工具一起使用。

### 插件提供的功能：

- **结构化的代理工具**（`ob_account`、`ob_buy`、`ob_limit` 等）——使用结构化输入格式进行命令调用，而非简单的 Bash 字符串。代理会收到结构化的 JSON 响应。
- **后台订单监控**——每 30 秒检查一次您的 Hyperliquid 账户，并在头寸开仓/平仓、盈亏显著变化或保证金使用情况危险时发送 Webhook 警报。
- **CLI 命令**——`openclaw ob status` 和 `openclaw ob watch` 可用于查看监控状态。

### 启用插件

插件包含在同一个 `openbroker` npm 包中。要在 OpenClaw 配置中启用它，请执行以下操作：
...
为了使代理能够收到订单状态警报，请在您的网关配置中启用相应的钩子：
...
即使没有启用钩子，监控功能仍然会运行并跟踪订单状态（可通过 `ob_watcher_status` 查看），但它无法主动通知代理。

### 是否使用插件

- **仅使用工具（不使用插件）：** 可以直接使用 Bash 命令（例如：`openbroker buy --coin ETH --size 0.1`）。此时没有后台监控功能。
- **使用插件：** 当有 `ob_*` 工具可用时，代理会优先使用这些工具（因为它们提供结构化数据）；对于工具未覆盖的命令（如策略或批量操作），则使用 Bash 命令。后台监控功能会自动发送警报。

## 风险提示

- 在实际操作前，务必先使用 `--dry` 参数进行预览。
