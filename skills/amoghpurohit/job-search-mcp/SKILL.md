---
name: Job Search MCP
description: 使用 JobSpy MCP 服务器，在 LinkedIn、Indeed、Glassdoor、ZipRecruiter、Google Jobs、Bayt、Naukri 和 BDJobs 等平台上搜索职位。
slug: job-search-mcp
tags:
  - job-search
  - career
...
将以下内容添加到您的Claude桌面配置文件中（macOS上位于`~/Library/Application Support/Claude/claude_desktop_config.json`）：
...
**原因：**同时从太多网站请求过多结果会触发使用频率限制并导致超时。
...
**原因：**LinkedIn的使用频率限制最严格。启用`linkedin_fetch_description: true`会增加请求次数。
...
4. **Glassdoor** - 提供公司评价信息，可靠性较高
5. **LinkedIn** - 使用频率限制最严格，应谨慎使用
...
3. **具体明确：**使用精确的搜索关键词
4. **谨慎筛选：**在Indeed/LinkedIn上一次只使用一个筛选条件组
5. **分页：**使用`offset`获取更多结果，而不是设置较高的`results_wanted`值
...
运行：`playwright install chromium`（某些爬虫工具需要Chromium）
