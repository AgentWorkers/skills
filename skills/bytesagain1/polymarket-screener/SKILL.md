...
version: 1.0.0
description: 按类别、概率和流动性筛选 Polymarket 的预测市场，并跟踪概率变化，以发现高价值的投资机会。
---
# Polymarket 筛选器 🎯

使用 Polymarket 的公共 API 对预测市场进行筛选和分析。找出被低估的投注机会，跟踪概率变化，并发现高潜力的市场。

## Polymarket 筛选器与手动浏览的对比

| 功能 | 手动浏览 | Polymarket 筛选器 |
|-----------|----------------|-------------------|
| 按概率范围筛选 | ❌ 有限 | ✅ 可以精确指定范围（例如：20%-40%） |
| 按流动性筛选 | ❌ 不支持 | ✅ 支持最小/最大流动性筛选 |
| 跟踪概率变化 | ❌ 需手动检查 | ✅ 自动跟踪概率变化（使用 delta 算法） |
| 多类别扫描 | ❌ 一次只能扫描一个类别 | ✅ 可同时扫描所有类别 |
| 概率变化警报 | ❌ 无 | ✅ 可配置警报阈值 |
| 历史概率数据 | ❌ 图表显示有限 | ✅ 可导出时间序列数据 |
| 批量机会评分 | ❌ 无 | ✅ 自动评分 |
| 自定义观察列表 | ❌ 无 | ✅ 支持 JSON 格式的观察列表及跟踪功能 |
...
2. **流动性 > 50,000 美元** — 可以进行有效的投资
3. **近期价格变动 > 5%** — 市场正在积极调整价格
4. **事件在 30 天内发生** — 时间价值较为明确
5. **你拥有独特的知识优势** — 你知道市场尚未了解的信息

## 输出格式

| 命令 | 输出文件 | 内容 |
|---------|------------|----------|
| `list` | 控制台 / JSON | 市场列表及关键数据 |
| `opportunities` | 控制台 / JSON | 评分后的机会列表 |
| `track` | `tracking-SLUG.json` | 概率变化时间序列 |
| `report` | `polymarket-report.html` | 完整的可视化报告 |
| `watchlist` | `watchlist.json` | 保存的观察列表及警报信息 |

## API 使用限制

Polymarket 的公共 API 有使用频率限制。该筛选器会自动遵守这些限制：
- 列表端点：每分钟 60 次请求
- 市场详情端点：每分钟 120 次请求
- 支持指数级退避的重试机制
...
⚠️ 预测市场涉及真实资金和财务风险。本工具仅提供分析功能，不负责下注或管理投资组合。请始终自行进行充分研究。