...
description: "监控 DEX（去中心化交易所）的流动性池深度，追踪流动性变化，计算 LP（锁定仓）的收益，并估算非永久性损失。适用于需要流动性监控功能的场景。触发条件：流动性监控触发。"
---
# 流动性监控器 💧  
实时监控去中心化交易所（DEX）的流动性池，提供永久性损失（Impermanent Loss, IL）计算及流动性提供者（LP）收益估算功能。  
...
- 计算任何价格变动情况下的永久性损失  
- 比较永久性损失与持有代币的收益  
- 对不同价格区间内的永久性损失进行建模（适用于流动性集中的情况）  
- 提供现有头寸的历史永久性损失分析  
...
## 永久性损失参考表  
| 价格变动百分比 | 永久性损失（50/50分配池） | 相当损失金额（每1,000美元） |
|-------------|-----------------|-----------------|
| ±25% | 0.6% | 6美元 |
| ±50% | 2.0% | 20美元 |
| ±75% | 3.8% | 38美元 |
...
| 流动性池年龄 | 15% | 年龄越长的池通常更安全 |
| 最大流动性提供者占比 | 15% | 流动性提供者分布越分散，风险越低 |
| 智能合约审计情况 | 10% | 经过审计的DEX协议评分更高 |
