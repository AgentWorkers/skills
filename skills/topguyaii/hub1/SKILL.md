...
为你的代理提供持久化的内存，使其能够在会话结束后仍然保留数据。存储事实、偏好设置、决策以及学习内容，并在需要时以语义方式检索它们。高级功能包括声誉跟踪、跨代理内存池以及时间旅行快照功能。
...
OpenClawdy 使用基于钱包的认证机制。你的代理的钱包地址作为其唯一标识——无需 API 密钥。

在使用内存工具之前，请确保你的代理已配置好钱包。每个钱包都有一个独立的内存存储空间。
...
通过语义搜索来检索相关记忆。根据信息的含义而非关键词进行查找。
...
**跟踪哪些记忆带来了良好的结果**。为内存记录分配声誉分数，并根据成功或失败的情况更新分数；按有效性对记忆进行排序。
...
**跨代理内存池** - 允许多个代理之间共享知识。创建内存池，存储共享的内存记录，并从集体智慧中检索信息。非常适合代理团队和群体使用。
...
**内存时间旅行** - 创建内存状态的快照并恢复。通过查看过去的状态来调试决策，比较内存变化，或恢复到之前的检查点。对于高风险代理来说非常有用。
...
| `history` | 历史事件 | “2.1 版本于 1 月 15 日发布” |
| `context` | 一般上下文 | “正在开发电子商务项目” |
...
- 在回顾策略时 → 使用 `recall_ranked` 来选择已被验证有效的方法
...
| 等级 | 内存记录数 | 每天检索次数 | 共享池数量 | 快照数量 | 价格 |
|------|----------|-------------|-------|-----------|-------|
| 免费 | 1,000 | 100 | 1 | 3 | $0 |
| 专业版 | 50,000 | 无限制 | 10 | 50 | $10/月 |
...
| 方法 | 端点 | 描述 |
|--------|----------|-------------|
| POST | `/memory/store` | 存储内存记录 |
| POST | `/memory/recall` | 语义搜索 |
| GET | `/memory/list` | 列出内存记录 |
| GET | `/memory/{id}` | 获取特定内存记录 |
| DELETE | `/memory/{id}` | 删除内存记录 |
| GET | `/memory/vault` | 导出所有记录 |
| DELETE | `/memory/vault` | 清空所有记录 |
| GET | `/agent/stats` | 使用统计信息 |

### 声誉端点
| 方法 | 端点 | 描述 |
|--------|----------|-------------|
| POST | `/memory/reputation/store` | 带有声誉分数存储内存记录 |
| POST | `/memory/reputation/recall` | 按声誉检索记录 |
| POST | `/memory/reputation/update` | 更新内存记录的声誉 |

### 共享池端点
| 方法 | 端点 | 描述 |
|--------|----------|-------------|
| POST | `/memory/pool/create` | 创建共享池 |
| POST | `/memory/pool/store` | 将记录存储到共享池 |
| POST | `/memory/pool/recall` | 从共享池中检索记录 |
