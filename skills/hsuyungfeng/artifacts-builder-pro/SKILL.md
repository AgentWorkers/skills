...
要构建工件，请编辑生成的文件。有关开发指导，请参阅下方的 **常见开发任务**。
...
这会生成 `bundle.html` 文件——一个包含所有 JavaScript、CSS 以及依赖项的独立工件。该文件可以直接在 Claude 对话中分享给用户。
...
注意：这一步完全是可选的。只有在必要时或用户要求时才进行。

要测试或可视化工件，可以使用现有的工具（包括其他 Skills 或内置工具，如 Playwright 或 Puppeteer）。一般来说，建议在展示工件后再进行测试，因为提前测试会增加请求与最终结果显示之间的延迟。如果需要或出现问题时，可以在展示工件后进行测试。
