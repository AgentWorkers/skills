---
name: amazon
description: 使用浏览器自动化技术在亚马逊上购买和退货商品。该技术可用于完成购买、重新下单、查看订单历史以及处理退货等操作。
...
- Chrome 浏览器已开启，并配置为使用 `--remote-debugging-port=9222`（请参阅 [如果浏览器未运行，请启动浏览器](#starting-the-browser-if-not-running)）。
...
### 退货流程步骤
1. 进入订单页面 → 查找需要退货的商品 → 选择“退货或更换商品”
2. 选择“改变主意” → “我的需求发生了变化” → 继续操作
3. 使用上述默认答案回答相关问题
4. 继续操作，跳过“获取产品支持”的提示
5. 选择“退款至原支付方式”
6. 选择退货地点
7. 确认退货操作
8. 退货完成后，系统会通过电子邮件发送二维码。

### 通信方式
- **无需逐一说明每一步操作**，只需默默完成整个退货流程。
- **只有在退货操作确认完成后**，才向用户发送简短的消息，内容包括：
  - 商品名称
  - 退款金额
  - 退货地点及截止日期
- 如果出现任何问题或需要进一步说明，请及时联系客服。
...
**请始终使用新标签页**——多个会话共享同一个 Chrome 浏览器实例。每次使用 `open` 命令时，请加上 `--new-tab` 选项。
...
- 在无头模式的 Linux 系统中，VNC 显示通常使用端口 5999（noVNC 使用端口 6080）。
- 订单确认信息会发送到用户的亚马逊账户邮箱。
- 如果遇到 CAPTCHA 或需要二次验证（2FA），请让用户在 Chrome 窗口中完成相关操作（适用于 macOS 或 Linux 桌面环境）。