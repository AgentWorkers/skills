---
name: ogment
description: 通过 Ogment CLI 调用 MCP 工具——通过 Ogment 的治理层实现安全访问 Linear、Notion、Gmail 以及 100 多个 SaaS 服务。
version: 1.0.2
metadata:
  openclaw:
    requires:
...
等待批准后，使用 `ogment auth status` 验证登录状态。
...
⚠️ **重要提示（对于代理）：** 不要让相关人员直接运行 `ogment auth login` — 请您自己运行该命令，然后将生成的代码发送给他们！
...
ogment auth login
```
此操作会返回包含设备代码的 JSON 数据。提取该代码并发送给相关人员：
...
### 第 4 步：等待批准
批准后，`ogment auth login` 命令将自动完成。然后再次验证登录状态：
```bash
ogment auth status
```
...
- **代理权限：** 每个代理只能看到您明确授予的权限

此技能不会存储任何凭证 — 所有认证操作均由 Ogment CLI 管理。
...
返回认证状态、连接状态及可用服务器列表。通过 `summary.status` 了解系统运行状况。
...
| 错误代码 | 含义 | 处理方式 |
|------------|---------|--------|
| `TOOL_NOT_FOUND` | 服务器/工具名称不存在 | 重新运行 `ogment catalog` 进行查找 |
| `VALIDATION_INVALID_INPUT` | JSON 格式错误 | 检查 JSON 语法 |
| `TRANSPORT_REQUEST_FAILED` | 服务器拒绝请求 | 添加 `--debug` 标志并检查工具架构 |
| `AUTH_INVALID_CREDENTIALS` | API 密钥无效/过期 | 重新运行 `ogment auth login` |
| `HTTP_401` | 服务连接已过期 | 告知相关人员重新连接（详见下方说明） |
...
|------|--------|
| `--debug` | 显示详细的错误诊断信息 |
| `--quiet` | 仅输出最基本的 JSON 数据（包括成功与否） |
| `--api-key <key>` | 替换 API 密钥 |
| `--example` | 生成示例输入数据（仅用于工具列表显示） |
